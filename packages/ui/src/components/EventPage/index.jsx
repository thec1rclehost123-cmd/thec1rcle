...
const fallbackGuestPool = [
    { name: "Ari", gender: "female" }, { name: "Ira", gender: "female" }, { name: "Nia", gender: "female" },
    { name: "Reva", gender: "female" }, { name: "Luna", gender: "female" }, { name: "Mira", gender: "female" },
    { name: "Sara", gender: "female" }, { name: "Zara", gender: "female" }, { name: "Anaya", gender: "female" },
    { name: "Isha", gender: "female" }, { name: "Priya", gender: "female" }, { name: "Meera", gender: "female" },
    { name: "Diya", gender: "female" }, { name: "Kavya", gender: "female" }, { name: "Anika", gender: "female" },
    { name: "Neha", gender: "female" }, { name: "Tara", gender: "female" }, { name: "Sia", gender: "female" },
    { name: "Rhea", gender: "female" }, { name: "Noor", gender: "female" }, { name: "Leila", gender: "female" },
    { name: "Alina", gender: "female" }, { name: "Zoe", gender: "female" }, { name: "Anya", gender: "female" },
    { name: "Mahi", gender: "female" }, { name: "Riya", gender: "female" }, { name: "Shreya", gender: "female" },
    { name: "Pooja", gender: "female" }, { name: "Natasha", gender: "female" }, { name: "Kamya", gender: "female" },
    { name: "Vanya", gender: "female" }, { name: "Aisha", gender: "female" }, { name: "Sana", gender: "female" },
    { name: "Raina", gender: "female" }, { name: "Urvi", gender: "female" },
    { name: "Dev", gender: "male" }, { name: "Vik", gender: "male" }, { name: "Taj", gender: "male" },
    { name: "Noah", gender: "male" }, { name: "Kian", gender: "male" }, { name: "Aryan", gender: "male" },
    { name: "Rohan", gender: "male" }, { name: "Jai", gender: "male" }, { name: "Sai", gender: "male" },
    { name: "Zaid", gender: "male" }, { name: "Nikhil", gender: "male" }, { name: "Veer", gender: "male" },
    { name: "Dhruv", gender: "male" }, { name: "Aadi", gender: "male" }, { name: "Rehan", gender: "male" },
];
...
const getCuratedAvatars = (users = [], targetCount = 10) => {
    ...
    const femalePlaceholders = femaleSlots.length < femaleTarget
        ? fallbackGuestPool
            .filter(g => g.gender === "female" && !realFemales.some(u => u.name === g.name))
            .slice(0, femaleTarget - femaleSlots.length)
            .map((g, i) => ({ id: `ph-f-${i}`, name: g.name, gender: "female", color: avatarPalette[i % avatarPalette.length], initials: g.name.slice(0, 2).toUpperCase(), photoURL: null }))
        : [];
    const maleSlots = realMales.slice(0, maleTarget);
    const malePlaceholders = maleSlots.length < maleTarget
        ? fallbackGuestPool
            .filter(g => g.gender === "male" && !realMales.some(u => u.name === g.name))
            .slice(0, maleTarget - maleSlots.length)
            .map((g, i) => ({ id: `ph-m-${i}`, name: g.name, gender: "male", color: avatarPalette[(i + 3) % avatarPalette.length], initials: g.name.slice(0, 2).toUpperCase(), photoURL: null }))
        : [];
...
const createGuestDirectory = (guests = []) => {
    if (guests?.length) {
        return guests.map((name, index) => ({
            id: `${name}-${index}`, name, gender: null,
            handle: buildHandle(name, index), stats: guestStats(index),
            color: avatarPalette[index % avatarPalette.length], initials: initials(name),
        }));
    }
    return fallbackGuestPool.map((guest, index) => ({
        id: `fp-${guest.name}-${index}`, name: guest.name, gender: guest.gender,
        handle: buildHandle(guest.name, index), stats: guestStats(index),
        color: avatarPalette[index % avatarPalette.length], initials: initials(guest.name),
    }));
};
...
const getTierStyle = (ticket) => {
    ...
    if (qty <= 0) return {
        border: "border-red-500/20 opacity-60", shadow: "", badge: null,
        priceColor: "text-red-400 line-through", avail,
    };
    if (key.includes("vip") || key.includes("table")) return {
        border: "border-yellow-500/40", shadow: "shadow-[0_0_30px_rgba(234,179,8,0.08)]",
        badge: "VIP", badgeIcon: "crown", badgeStyle: "text-yellow-400 bg-yellow-500/10 border-yellow-500/20",
        priceColor: "text-yellow-400", avail,
    };
    if (key.includes("crew") || key.includes("artist")) return {
        border: "border-[#5D5FEF]/40", shadow: "shadow-[0_0_30px_rgba(93,95,239,0.08)]",
        badge: "Creator", badgeIcon: "sparkles", badgeStyle: "text-[#8B8DFF] bg-[#5D5FEF]/10 border-[#5D5FEF]/20",
        priceColor: "text-[#8B8DFF]", avail,
    };
    if (key.includes("female") || key.includes("ladies")) return {
        border: "border-rose-400/40", shadow: "shadow-[0_0_20px_rgba(251,113,133,0.06)]",
        badge: "Ladies", badgeIcon: "heart", badgeStyle: "text-rose-300 bg-rose-500/10 border-rose-400/20",
        priceColor: "text-rose-300", avail,
    };
    if (key.includes("couple") || key.includes("pair")) return {
        border: "border-pink-400/30", shadow: "",
        badge: "Couple", badgeIcon: "pair", badgeStyle: "text-pink-300 bg-pink-500/10 border-pink-400/20",
        priceColor: "text-pink-300", avail,
    };
    return { border: "border-white/10", shadow: "", badge: null, priceColor: "text-white", avail };
};

const SectionLabel = ({ children, className }) => (
    <p className={`mb-6 text-[10px] font-black uppercase tracking-[0.4em] ${className ?? "text-white/40"}`}>{children}</p>
);
...
export default function EventDetailPage({
    ...
    const interestedUsers = useMemo(() => {
        ...
        return createGuestDirectory().map(u => ({ ...u, photoURL: isPreview ? "placeholder" : null }));
    ...
    ].filter(Boolean), [event?.genres, event?.tags, event?.category, event?.dressCode, event?.ageLimit, event?.ageRestriction]);

    // Schedule timeline from real event data
    const scheduleEntries = useMemo(() => [
        event?.doorsOpen ? { label: "Doors Open", time: event.doorsOpen } : null,
        (event?.startTime || event?.time) ? { label: "Event Starts", time: event.startTime || event.time } : null,
        event?.lastEntry ? { label: "Last Entry", time: event.lastEntry } : null,
        event?.endTime ? { label: "Event Ends", time: event.endTime } : null,
    ].filter(Boolean), [event?.doorsOpen, event?.startTime, event?.time, event?.lastEntry, event?.endTime]);
    ...
    return (
        <div className={`relative isolate min-h-screen pb-32 text-white bg-black overflow-x-hidden ${isPreview ? "preview-mode" : ""}`}>
            {/* Film grain overlay */}
            <div className="pointer-events-none fixed inset-0 z-50 opacity-[0.025] mix-blend-overlay" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")` }} />
            ...
            <section className="relative h-[100dvh] min-h-[620px] overflow-hidden flex flex-col">
                ...
                {eventImage && (
                    <div
                        ...
                        style={{
                            maskImage: "linear-gradient(to bottom, transparent 5%, black 35%, black 65%, transparent 100%)",
                            WebkitMaskImage: "linear-gradient(to bottom, transparent 5%, black 35%, black 65%, transparent 100%)",
                        }}
                    >
                        <ShimmerImage
                            src={eventImage} alt="" fill sizes="100vw"
                ...
                {eventImage && (
                    ...
                    >
                        ...
                        >
                            <ShimmerImage
                                src={eventImage} alt={event?.title || ""} fill priority sizes="100vw"
                    ...
                    <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 rounded-full border border-white/10 bg-black/60 px-5 py-3 shadow-2xl backdrop-blur-3xl">
                        <div className="flex items-center gap-4">
                            <Link href="/" className="flex items-center gap-3">
                                <div className="h-8 w-8 flex items-center justify-center rounded-full bg-white text-black font-black text-sm shrink-0">C</div>
                            ...
                            {host && (
                                <>
                                    ...
                                    <div className="hidden sm:flex items-center gap-2.5">
                                        {host.avatar && (
                                            <Image src={host.avatar} alt={host.name || ""} width={24} height={24} className="h-6 w-6 rounded-full object-cover" />
                                        )}
                                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/50">{host.name}</p>
                                    </div>
                                </>
                            )}
                        </div>
                        <div className="flex items-center gap-2">
                            <LikeButton
                                eventId={event?.id} isLiked={isLiked} isPreview={isPreview}
                                onLike={(val) => { setIsLiked(val); handleAction("LIKE", { val }); }}
                            />
                            <button
                                type="button" disabled={isPreview}
                    ...
                    <div className="mx-auto max-w-7xl">
                        ...
                        >
                            ...
                            <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/40 mb-10">
                                ...
                                {(event?.venue || event?.location) && <> &middot; {event?.venue || event?.location}</>}
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex -space-x-3">
                                            {previewInterested.map((guest, i) => (
                                                <div key={i} className="h-11 w-11 overflow-hidden rounded-full border-[2.5px] border-black bg-zinc-900 shrink-0">
                                                    {guest.photoURL && guest.photoURL !== "placeholder" ? (
                                                        <Image src={guest.photoURL} alt={guest.name} width={44} height={44} className="object-cover" />
                                                    ) : (
                                                        <div className="flex h-full w-full items-center justify-center text-[8px] font-black" style={{ backgroundColor: guest.color }}>
                                                            {guest.initials}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <div>
                                            <p className="text-xl font-display text-white tracking-tighter">{(interestedCount + 80).toLocaleString()}+</p>
                                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40">People Going</p>
                                        </div>
                                    </div>
                                )}
                                <button
                ...
                >
                    ...
                    <span className="text-[8px] font-black uppercase tracking-[0.3em] text-white/30">Scroll</span>
                </motion.div>
            </section>
            ...
            <main className="mx-auto max-w-7xl px-4 pt-20 pb-8 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-10">
                    ...
                    <div className="space-y-8">
                            ...
                            >
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {event.artists.map((name, i) => (
                                        <div key={i} className="flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2.5 hover:border-white/20 transition-colors">
                                            ...
                                            <span className="text-sm font-bold uppercase tracking-wider text-white/70">{name}</span>
                                        </div>
                                    ))}
                                </div>
                            </motion.section>
                            ...
                            >
                                ...
                                <div className="space-y-0">
                                    {scheduleEntries.map((entry, i) => (
                                        <div key={i} className="flex items-center gap-5 py-4 border-b border-white/5 last:border-0">
                                            <div className="h-2 w-2 rounded-full bg-orange shrink-0" />
                                            <p className="text-[10px] font-black uppercase tracking-[0.25em] text-white/30 w-28 shrink-0">{entry.label}</p>
                        ...
                        >
                            <p className="mb-6 text-[10px] font-black uppercase tracking-[0.4em] text-black/40">Vibe</p>
                            <div className="flex flex-wrap gap-2">
                                {(vibeTags.length > 0 ? vibeTags : ["Intimate", "Underground", "High-Energy"]).map(tag => (
                                    <span key={tag} className="px-4 py-2 rounded-xl bg-black/5 border border-black/10 text-[10px] font-black uppercase tracking-widest">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                            ...
                            >
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        ...
                                        <h3 className="text-2xl font-display uppercase text-white -mt-4">Who's Going</h3>
                                    </div>
                                    <button
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                    {interestedUsers.slice(0, 6).map((guest) => (
                                        <div key={guest.id} className="flex items-center gap-3 p-4 rounded-3xl border border-white/[0.05] bg-white/[0.02]">
                                            <div
                                            ...
                                            >
                                                {guest.photoURL && guest.photoURL !== "placeholder" ? (
                                                    <Image src={guest.photoURL} alt={guest.name} width={40} height={40} className="object-cover w-full h-full" />
                                                ) : guest.initials}
                                            </div>
                        ...
                        >
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-6">
                                {event?.city && <span className="text-[10px] font-bold text-orange uppercase tracking-[0.15em]">{event.city}</span>}
                                {event?.ageLimit && <span className="text-[9px] font-black border border-white/10 px-2.5 py-1 rounded-md bg-white/5 text-white/50">{event.ageLimit}</span>}
                                {!event?.ageLimit && event?.ageRestriction && <span className="text-[9px] font-black border border-white/10 px-2.5 py-1 rounded-md bg-white/5 text-white/50">{event.ageRestriction}</span>}
                                {event?.dressCode && <span className="text-[9px] font-black border border-white/10 px-2.5 py-1 rounded-md bg-white/5 text-white/50">DRESS: {event.dressCode.toUpperCase()}</span>}
                            </div>
                            ...
                            <div className="flex items-start justify-between mt-5 gap-4">
                                ...
                                {event?.arrivalInstructions && (
                                    <p className="text-[11px] text-white/30 max-w-xs text-right leading-relaxed">{event.arrivalInstructions}</p>
                                )}
                            </div>
                        </motion.section>
                    </div>
                    ...
                    <aside>
                        <div className="sticky top-6 rounded-[48px] border border-white/10 bg-black/70 p-8 shadow-2xl backdrop-blur-3xl">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/25 mb-8">Select your access tier</p>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-start justify-between gap-3 mb-3">
                                                <div className="flex flex-wrap items-center gap-1.5">
                                                    {style.badge && (
                                                        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[8px] font-black uppercase tracking-widest ${style.badgeStyle}`}>
                                                            <BadgeIcon type={style.badgeIcon} />
                                                            {style.badge}
                                                        </span>
                                                    )}
                                                    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[8px] font-black uppercase tracking-widest ${style.avail.chip}`}>
                                                        {style.avail.label}
                                                    </span>
                                                </div>
                                                <p className={`text-2xl font-display tracking-tighter shrink-0 group-hover:scale-105 transition-transform ${style.priceColor}`}>
                                                    {Number(ticket.price) === 0 ? "Free" : `₹${Number(ticket.price).toLocaleString("en-IN")}`}
                                                </p>
                                            </div>
                                            ...
                                            {/* Description snippet */}
                                            {ticket.description && (
                                                <p className="mt-1 text-[11px] text-white/25 line-clamp-1">{ticket.description}</p>
                                            )}
                            ...
                            {host && (
                                <div className="mt-8 pt-6 border-t border-white/5 flex items-center gap-3">
                                    {host.avatar && (
                                        <Image src={host.avatar} alt={host.name || ""} width={36} height={36} className="h-9 w-9 rounded-full object-cover shrink-0" />
                                    )}
                                    <div>
                                        <p className="text-[8px] font-black uppercase tracking-widest text-white/20">Hosted by</p>
            ...
            {/* Modals */}
            <GuestlistModal
                open={guestModalOpen} guests={interestedUsers}
                onClose={() => setGuestModalOpen(false)} title="Guestlist" isPreview={isPreview}
            />
            <TicketModal
                open={ticketModalOpen} onClose={() => setTicketModalOpen(false)}
                tickets={tickets} eventId={event?.id} isPreview={isPreview}
            ...
            >
                <div className="flex w-full max-w-2xl items-center justify-between gap-4 rounded-full border border-white/10 bg-black/80 px-6 py-4 shadow-2xl backdrop-blur-3xl">
                    <div className="hidden sm:block">
                        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/25 mb-0.5">From</p>
