...
const DashboardEventCard = ({
  ...
  return (
    <div
      className="group relative flex h-full w-full flex-col overflow-hidden rounded-[28px] bg-[#0d0d0f] transition-all duration-500 hover:-translate-y-1"
      style={{
        border: '1px solid rgba(255,255,255,0.12)',
        boxShadow:
          '0 2px 20px rgba(0,0,0,0.4), 0 20px 60px -20px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.04)',
      }}
    >
      {/* ── Poster Section ── */}
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16 / 10' }}>
        ...
        <div className="absolute left-4 top-4 z-10 flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-black/50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.16em] backdrop-blur-xl ${status.class}`}
          >
            <span
              className={`inline-block h-[5px] w-[5px] rounded-full ${status.dot} ${status.pulse ? 'animate-pulse' : ''}`}
            />
            {status.label}
          </span>
          {priceDisplay && (
            <span
              className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.16em] backdrop-blur-xl ${
                isFree
      ...
      <div className="relative flex flex-1 flex-col px-5 pb-4 pt-3">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span className="text-[34px] font-black leading-none tracking-[-0.04em] text-white">
              {dateParts.day}
            </span>
            <span className="mt-1 text-[11px] font-black tracking-[0.26em] text-[#F97316]">
              {dateParts.month}
            </span>
          </div>
            ...
            <div className="mt-2 flex items-center gap-2.5">
              ...
              >
                <div className="flex h-full w-full items-center justify-center rounded-full bg-[#0d0d0f]">
                  <span className="text-[8px] font-black text-white/85">
                    {getMonogram(hostName)}
                  </span>
                </div>
              </div>
              <span className="truncate text-[13px] font-bold tracking-[0.02em] text-white">
                {hostName}
              </span>
            </div>
          </div>
