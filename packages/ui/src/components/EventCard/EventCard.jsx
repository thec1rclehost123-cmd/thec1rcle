...
const EventCard = ({
  ...
  const isDefaultImage =
    !poster || poster.includes('placeholder.svg') || poster.includes('holi-edit.svg');
  ...
  return (
    ...
    >
      <Wrapper
        {...wrapperProps}
        className={`group relative block h-full w-full ${isPreview ? 'cursor-default' : ''}`}
      >
        <div
          className={`gradient-border relative ${height} w-full overflow-hidden rounded-[20px] sm:rounded-[32px] bg-white dark:bg-surface transition-all duration-500 shadow-sm dark:shadow-none ${!isPreview ? 'btn-lift' : ''} ${showDemoHover ? 'translate-y-[-4px] shadow-lg shadow-orange/40' : ''}`}
        >
          {/* Image */}
          <div
            className={`absolute inset-0 transition-transform duration-700 ${!isPreview || showDemoHover ? 'group-hover:scale-110' : ''}`}
          >
            <ShimmerImage
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-3 sm:p-6">
            ...
            <div className="flex items-start justify-between relative z-30">
              <div className="flex flex-col gap-2">
                ...
                {priceDisplay && (
                  <span
                    className={`inline-flex items-center rounded-full border px-2 py-0.5 sm:px-3 sm:py-1 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest backdrop-blur-md ${
                      isFree
            ...
            {/* Bottom Info */}
            <div
              className={`absolute bottom-0 left-0 right-0 p-3 sm:p-6 transform transition-transform duration-500 z-20 ${!isPreview || showDemoHover ? 'group-hover:translate-y-[-4px]' : ''}`}
            >
              {/* Glass Background on Hover */}
              <div
                className={`absolute inset-0 -z-10 bg-black/60 dark:bg-black/40 backdrop-blur-xl transition-all duration-500 rounded-t-[16px] sm:rounded-t-[24px] translate-y-full ${!isPreview || showDemoHover ? 'group-hover:translate-y-0 opacity-100' : 'opacity-0'}`}
              />
              ...
              <div className="relative z-10">
                ...
                {/* CTA Button */}
                <div
                  className={`mt-4 sm:mt-6 flex items-center gap-3 transition-all duration-500 delay-75 ${!isPreview || showDemoHover ? 'opacity-100 translate-y-0' : 'opacity-100 translate-y-0'}`}
                >
                  <span
