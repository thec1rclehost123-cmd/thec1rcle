...
function reservationItemsMatch(left = [], right = []) {
  return (
    JSON.stringify(normalizeReservationItems(left)) ===
    JSON.stringify(normalizeReservationItems(right))
  );
}
  ...
  if (queueId && isFirebaseConfigured()) {
    ...
    if (!snapshot.empty) {
      ...
      if (new Date(data.expiresAt) > new Date() && reservationItemsMatch(data.items, items)) {
        console.log(
          `[CheckoutService] Reusing existing reservation ${doc.id} for queueId ${queueId}`,
        );
        return {
          success: true,
      ...
      await releaseReservation(doc.id).catch((error) => {
        console.warn(
          `[CheckoutService] Failed to release stale queue reservation ${doc.id}:`,
          error?.message || error,
        );
      });
    }
...
export async function validateAndCalculatePromoDiscount(eventId, code, items, userId = null) {
  ...
  if (promoCode) {
    ...
    return {
      ...
      amount,
      label:
        promoCode.discountType === 'percent' ? `${promoCode.discountValue}% off` : `₹${amount} off`,
    };
  }
  ...
  try {
    ...
    if (!acquired) {
      return {
        success: false,
        error:
          'Checkout is already in progress for this reservation. Please wait a moment and try again.',
      };
    }
  } catch (redisErr) {
    // Redis unavailable — fall through without lock (best effort)
    console.warn(
      '[CheckoutService] Redis lock unavailable, proceeding without lock:',
      redisErr.message,
    );
    redisClient = null;
  }
...
async function _initiateCheckoutInner(reservationId, userId, userDetails, options, redisClient) {
  ...
  const pricingResult = await calculatePricing(reservation.eventId, reservation.items, {
    promoCode,
    promoterCode: eligiblePromoterCode,
    userId,
  });
  ...
  if (event.isRSVP) {
    ...
    if (pricing.isFree) {
      ...
      return await processFreePaidOrder(
        reservation,
        userId,
        userDetails,
        pricing,
  ...
  for (const eventId of Object.keys(eventGroups)) {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (!eventDoc.exists) {
          group.forEach((r) =>
            transaction.update(db.collection(RESERVATIONS_COLLECTION).doc(r.id), {
              status: 'expired',
            }),
          );
          return;
        ...
        for (const reservation of group) {
          ...
          for (const item of reservation.items) {
            ...
            if (idx !== -1) {
              updatedTiers[idx] = {
                ...updatedTiers[idx],
                lockedQuantity: Math.max(
                  0,
                  (updatedTiers[idx].lockedQuantity || 0) - item.quantity,
                ),
              };
            }
          }
