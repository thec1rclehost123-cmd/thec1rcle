...
function requireRuntimeSecret(name, developmentFallback) {
    ...
    if (env === "development" || env === "test") {
        console.warn(`[SECURITY WARNING] Missing ${name} - using development fallback. DO NOT USE IN PRODUCTION.`);
        return developmentFallback;
    }

    // Hard-fail in production/staging
    throw new Error(`CRITICAL SECURITY FAILURE: ${name} environment variable is required in ${env} mode.`);
}
