...
function makeWallet(overrides: Record<string, any> = {}) {
    return {
        ...
        rules: {
            ...
            allowedPresetItems: [
                { id: 'ITEM-WHISKEY', name: 'Whiskey Soda', amountPaise: 35000, isAvailable: true, sortOrder: 0 },
                { id: 'ITEM-BEER', name: 'Beer Pint', amountPaise: 20000, isAvailable: true, sortOrder: 1 },
                { id: 'ITEM-UNAVAILABLE', name: 'Off Menu', amountPaise: 10000, isAvailable: false, sortOrder: 99 },
            ],
            currency: 'INR',
            maxTxnsPerWallet: 10,
...
describe('transaction limits', () => {
    it('rejects debit when txnCount >= maxTxnsPerWallet', () => {
        const wallet = makeWallet({ txnCount: 10, rules: { ...makeWallet().rules, maxTxnsPerWallet: 10 } });
