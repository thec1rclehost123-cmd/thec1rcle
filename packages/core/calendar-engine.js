...
export async function getVenueAvailability(venueId, startDate, endDate) {
  const db = getAdminDb();
  const snapshot = await db.collection(CALENDAR_COLLECTION).where('venueId', '==', venueId).get();
...
export async function respondToSlotRequest(id, action, responseData, actor) {
  ...
  const updates = {
    status:
      action === 'approve' ? 'approved' : action === 'counter' ? 'counter_proposed' : 'rejected',
    respondedAt: now,
    updatedAt: now,
...
export async function getOperatingCalendar(db, partnerId, role, startDate, endDate) {
  ...
  if (role === 'venue') {
    ...
    blocks = calSnap.docs
      ...
      .filter(
        (entry) => entry.date >= startDate && entry.date <= endDate && entry.status === 'blocked',
      );
  }
