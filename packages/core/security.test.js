...
function makePipeline() {
    const cmds = [];
    const pipe = {
        incr: (key) => { cmds.push(async () => { const v = (_store.get(key) || 0) + 1; _store.set(key, v); return [null, v]; }); return pipe; },
        ttl:  (key) => { cmds.push(async () => [null, _store.has(key) ? 60 : -1]); return pipe; },
...
vi.mock("./redis.js", () => ({
    getRedisClient: () => ({
        incr:           vi.fn(async key => { const v = (_store.get(key) || 0) + 1; _store.set(key, v); return v; }),
        expire:         vi.fn(async () => 1),
        get:            vi.fn(async key => _store.has(key) ? String(_store.get(key)) : null),
        set:            vi.fn(async (key, val) => { _store.set(key, val); return "OK"; }),
        del:            vi.fn(async key => { _store.delete(key); return 1; }),
        hset:           vi.fn(async (key, ...args) => { const m = _store.get(key) || {}; for (let i = 0; i < args.length; i += 2) m[args[i]] = args[i+1]; _store.set(key, m); return 1; }),
        hget:           vi.fn(async (key, field) => { const m = _store.get(key); return m?.[field] ?? null; }),
        ...
        hincrby:        vi.fn(async (key, field, by) => { const m = _store.get(key) || {}; m[field] = (Number(m[field]) || 0) + by; _store.set(key, m); return m[field]; }),
        zrange:         vi.fn(async () => []),
...
vi.mock("./admin.js", () => ({
    getAdminDb:             vi.fn(() => ({ collection: vi.fn(() => ({ add: vi.fn(async () => ({ id: "mock-id" })), doc: vi.fn(() => ({ update: vi.fn(async () => {}), get: vi.fn(async () => ({ exists: false })) })), orderBy: vi.fn(function() { return this; }), where: vi.fn(function() { return this; }), limit: vi.fn(function() { return this; }), get: vi.fn(async () => ({ docs: [] })) })) })),
