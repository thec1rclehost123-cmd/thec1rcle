...
function getEventDate(order, event) {
    return getDateValue(event?.startDate || event?.startAt || order?.eventDate || order?.eventStartAt);
}
...
export function buildAppleWalletPassPreview(order, event = {}, env = process.env) {
    ...
    const hasAppleCerts = Boolean(env.APPLE_PASS_TYPE_ID && env.APPLE_PASS_TEAM_ID && env.APPLE_PASS_CERT_PATH);
    ...
    return {
        ...
        pass: {
            ...
            eventTicket: {
                headerFields: [
                    { key: "event", label: "EVENT", value: event.title || order.eventTitle },
                ],
                primaryFields: [
                    {
                        key: "date",
...
export function buildGoogleWalletPassPreview(order, event = {}, env = process.env) {
    const startDate = getEventDate(order, event);
    const hasGoogleCreds = Boolean(env.GOOGLE_WALLET_ISSUER_ID && env.GOOGLE_WALLET_SERVICE_ACCOUNT_KEY);
