...
import { buildGuestAuthProfile, filterGuestProfileUpdates, normalizeGuestEmail } from './guest-auth-engine.js';
import {
...
describe('guest core surface', () => {
    ...
    it('keeps guest auth profile normalization and safe update filtering in core', () => {
        ...
        expect(filterGuestProfileUpdates({ displayName: 'Guest', role: 'admin', gender: 'female' })).toEqual({
            displayName: 'Guest',
            gender: 'female',
        });
        expect(buildGuestAuthProfile({ uid: 'user_1', displayName: 'Guest', gender: 'female' })).toMatchObject({
            uid: 'user_1',
    ...
    it('centralizes guest discovery visibility and envelope helpers', () => {
        ...
        expect(isGuestDiscoveryVisible({
            visibility: 'public',
            lifecycle: 'scheduled',
            statusKey: 'upcoming',
        })).toBe(true);
    ...
    it('owns guest discovery projection, filtering, and search ranking in core', () => {
        ...
        const ranked = rankGuestSearchGroups({
            events: [card],
            hosts: [{ id: 'host_1', visibility: 'public', searchText: 'C1RCLE after dark', followersCount: 50 }],
            venues: [
                { id: 'venue_1', visibility: 'public', searchText: 'High Spirits pune', followersCount: 20 },
                { id: 'venue_private', visibility: 'private', searchText: 'High Spirits', followersCount: 999 },
            ],
        }, 'high', 5);
    ...
    it('parses scan payloads and builds the legacy scan decision shape', () => {
        expect(parseGuestTicketPayload(JSON.stringify({ eid: 'ent_1' }))).toMatchObject({ kind: 'entitlement' });
