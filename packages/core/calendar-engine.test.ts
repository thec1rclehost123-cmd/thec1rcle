                ...
                doc(id: string) {
                    return {
                        async set(data: any, options?: { merge?: boolean }) {
                            ...
                            collectionStore.set(id, options?.merge ? { ...previous, ...clone(data) } : clone(data));
                        },
                            ...
                            return {
                                async get() {
                                    const docs = Array.from(collectionStore.entries())
                                        .filter(([, data]) => nextFilters.every((filter) => data?.[filter.field] === filter.value))
                                        .slice(0, count)
...
describe("calendar-engine", () => {
    ...
    it("approves a host slot request into a booked venue date and schedules the event", async () => {
        ...
        await respondToSlotRequest(request.id, "approve", {}, { uid: "venue-user", name: "Venue User" });
    ...
    it("rejects a host slot request and clears the tentative calendar hold", async () => {
        ...
        await respondToSlotRequest(request.id, "reject", { reason: "Not available" }, { uid: "venue-user", name: "Venue User" });
