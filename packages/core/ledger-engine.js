 ...
 */
export async function transitionMoneyState(
  entityId,
  fromState,
  toState,
  amount,
...
export async function recordOrderCaptured(order, paymentId, transaction = null) {
  ...
  if (!transaction) {
    ...
    if (!existing.empty) {
      console.log(
        `[Ledger] Payment ${paymentId} already captured for order ${order.id}. Skipping.`,
      );
      return;
    }
  }
  ...
  try {
    await transitionMoneyState(
      order.id,
      MONEY_STATES.AUTHORIZED,
      MONEY_STATES.CAPTURED,
      amount,
      {
        entityType: 'order',
...
export async function holdOrderRevenue(order, transaction = null) {
  return await transitionMoneyState(
    order.id,
    MONEY_STATES.CAPTURED,
    MONEY_STATES.HELD,
    order.totalAmount,
...
export async function initiateRefund(
  ...
  amount,
  reason,
  currentState = MONEY_STATES.CAPTURED,
  transaction = null,
) {
  ...
  if (currentBalance < amount) {
    throw new Error(
      `Insufficient funds in ${currentState} for refund. Available: ${currentBalance}, Requested: ${amount}`,
    );
  }
