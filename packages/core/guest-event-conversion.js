...
export function normalizeInterestedUserGender(value) {
    ...
    if (["female", "woman", "women", "girl", "girls", "f", "cis female", "cis woman"].includes(normalized)) return "female";
    if (["male", "man", "men", "boy", "boys", "m", "cis male", "cis man"].includes(normalized)) return "male";
    return "other";
}
...
export async function getEventInterested(db, eventId, limit = 20) {
    ...
    const likesSnapshot = await db.collection(LIKES_COLLECTION)
    ...
    const userDocs = await Promise.all(userIds.map((uid) => db.collection(USER_COLLECTION).doc(uid).get()));
    const users = userDocs
        ...
        .map((doc) => {
            ...
            return {
                ...
                initials: displayName.split(" ").map((part) => part[0]).join("").toUpperCase().slice(0, 2),
...
export async function trackGuestEventInteraction(db, { eventId, type, ref }) {
    ...
    try {
        ...
        if (ref) {
            await db.collection("event_conversion_events").add({
                eventId,
