...
export async function syncGalleryToVenue(db, venueId) {
    const snap = await db.collection(CMS_COLLECTIONS.GALLERY)
...
export async function syncMenuToVenue(db, venueId) {
    const snap = await db.collection(CMS_COLLECTIONS.MENU)
        .where("venueId", "==", venueId)
        .orderBy("order", "asc")
        .get();
...
export async function createHighlight(db, venueId, data, actorId) {
    const id = randomUUID();
    const existingSnap = await db.collection(CMS_COLLECTIONS.HIGHLIGHTS)
...
export async function addGalleryPhoto(db, venueId, imageUrl, caption = "") {
    const existingSnap = await db.collection(CMS_COLLECTIONS.GALLERY)
        .where("venueId", "==", venueId)
        .get();
