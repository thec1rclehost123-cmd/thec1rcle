...
export async function upsertPromoCode(eventId, codeData, actor) {
  ...
  const promoData = {
    ...
    maxPerUser:
      codeData.maxPerUser !== undefined ? codeData.maxPerUser : existing?.maxPerUser || null,
  ...
  const finalDoc = {
    ...
    createdBy:
      existing?.createdBy ||
      (actor ? { uid: actor.uid, name: actor.name, role: actor.role } : null),
  };
...
export async function getEventPromoCodes(eventId, options = {}) {
  ...
  if (!isFirebaseConfigured()) {
    let codes = Array.from(fallbackPromoCodes.values()).filter((pc) => pc.eventId === eventId);
...
export async function validatePromoCode(eventId, code, userId, items) {
  ...
  if (promoCode.maxRedemptions) {
    ...
    const currentCount =
      currentRedisUses !== null ? parseInt(currentRedisUses) : promoCode.redemptionCount || 0;
