...
async function getCoverWalletsByOrder(db, wallet = {}) {
    ...
    const groups = {};
    await Promise.all(chunk(orderIds, 10).map(async (batchOrderIds) => {
...
export function findGuestWalletTicket(wallet = {}, ticketId) {
    ...
    return allTickets.find((ticket) => ticket.ticketId === ticketId || ticket.id === ticketId) || null;
}

export async function invalidateGuestWallet(users = []) {
    await Promise.all(users.filter(Boolean).flatMap((userId) => ([
...
export async function getGuestProfileSummary(dbOrProfileUserId, authOrViewerUserId, maybeProfileUserId, maybeViewerUserId) {
    const profileUserId = typeof dbOrProfileUserId === "string" ? dbOrProfileUserId : maybeProfileUserId;
    const viewerUserId = typeof dbOrProfileUserId === "string" ? authOrViewerUserId : maybeViewerUserId;
...
export async function markGuestNotificationRead(dbOrUserId, maybeUserIdOrNotificationId, maybeNotificationId) {
    ...
    const notificationId = typeof dbOrUserId === "string" ? maybeUserIdOrNotificationId : maybeNotificationId;
