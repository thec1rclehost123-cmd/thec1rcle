...
function calculateMatchScore(event, { preferredTags, preferredCities, preferredHosts, pastEventIds }) {
    let score = 0;
    ...
    if (orders.length > 0) {
        ...
        if (eventIds.length > 0 && isFirebaseConfigured()) {
            ...
            const eventDocs = (await Promise.all(
                chunks.map(chunk => db.collection(EVENT_COLLECTION).where('__name__', 'in', chunk).get())
...
export async function getSimilarEvents(eventId, limit = 3) {
    ...
    return candidates
        ...
        .map(event => {
            let score = 0;
            const overlap = (event.tags || []).map(t => t.toLowerCase()).filter(t => sourceTags.has(t)).length;
            score += overlap * 10;
