export class FirebaseNotificationRepository {
    ...
    async getVenueNotifications(venueId) {
        const [connectionsSnap, slotRequestsSnap, eventsSnap, ordersSnap, payoutsSnap, reservationsSnap] = await Promise.all([
        ...
        slotRequestsSnap.docs.forEach((doc) => {
            const d = doc.data();
            notifications.push({
    ...
    async markAsRead(venueId, notificationId, type) {
    ...
    async markAllAsRead(venueId) {
        ...
        await Promise.all(Object.keys(collectionMap).map(async (coll) => {
            const snap = await this.db.collection(coll).where('venueId', '==', venueId).get();
            const batch = this.db.batch();
            snap.docs.forEach((d) => { if (!d.data().venueReadAt)
                batch.update(d.ref, { venueReadAt: now }); });
            await batch.commit();
        }));
    }
    ...
    async getFollowerTokens(venueId) {
        ...
        for (let i = 0; i < userIds.length; i += 30) {
            ...
            usersSnap.docs.forEach((doc) => {
                const ud = doc.data();
                if (Array.isArray(ud.pushTokens))
                    tokens.push(...ud.pushTokens);
            });
        }
        return tokens;
    }
