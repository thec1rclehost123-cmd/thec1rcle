...
export class MatchingService {
    ...
    profileRepo;
    eventRepo;
    constructor(matchingRepo, profileRepo, eventRepo) {
    ...
    async getAdaptiveBoosts(userId) {
        const redis = getRedisClient();
        const prefKey = `user_prefs_boost:${userId}`;
        try {
    ...
    calculateInterestScore(user, target, adaptiveBoosts = {}) {
        const userInterests = user.interests || user.genres || [];
        const targetTags = target.genres || target.tags || [];
        if (userInterests.length === 0 || targetTags.length === 0)
            return 0.5;
        let score = 0;
        const intersection = userInterests.filter((x) => targetTags.includes(x));
    ...
    calculateProximityScore(lat, lng, target) {
        if (!lat || !lng || !target.coordinates)
            return 0.5;
        // Simple linear falloff: 1.0 at 0km, 0.0 at 50km
        const distance = this.haversine(lat, lng, target.coordinates.latitude, target.coordinates.longitude);
        const maxDistance = 50;
        return Math.max(0, 1 - (distance / maxDistance));
    }
    calculateActivityScore(target) {
