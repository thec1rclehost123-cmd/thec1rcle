...
export async function issueWallet(
    { orderId, eventId, venueId, userId, tierConfig, eventStartIso, tzOffset = '+05:30', termsAcceptedAt },
    transaction = null
) {
    if (!Number.isInteger(tierConfig.walletAmountPaise) || tierConfig.walletAmountPaise <= 0) {
        throw new Error(`walletAmountPaise must be a positive integer, got ${tierConfig.walletAmountPaise}`);
    }

    const db = getAdminDb();

    // Idempotency: check for existing wallet for this order
    const existingQuery = await db.collection(WALLET_COLLECTION)
...
export async function debitWallet(req) {
    const { walletId, presetItemId, quantity, idempotencyKey,
        operatorId, operatorName, operatorRole, deviceId, eventCodeId } = req;

    if (!idempotencyKey) {
        return { success: false, code: 'MISSING_IDEMPOTENCY_KEY', message: 'idempotencyKey is required' };
    }
    ...
    return await db.runTransaction(async (tx) => {
        ...
        // --- Idempotency check ---
        const idempSnap = await db.collection(WALLET_COLLECTION).doc(walletId)
        ...
        if (wallet.txnCount >= wallet.rules.maxTxnsPerWallet) {
            return { success: false, code: 'TXN_LIMIT_REACHED', message: `Maximum ${wallet.rules.maxTxnsPerWallet} transactions per wallet` };
        }

        // --- Preset item lookup ---
        const item = wallet.rules.allowedPresetItems?.find(i => i.id === presetItemId);
        if (!item) {
            return { success: false, code: 'ITEM_NOT_ALLOWED', message: 'Preset item not found or not available for this wallet' };
        }
        if (!item.isAvailable) {
        ...
        if (!Number.isInteger(totalAmountPaise)) {
            return { success: false, code: 'AMOUNT_CALCULATION_ERROR', message: 'Amount calculation error' };
        }

        if (totalAmountPaise < wallet.rules.minChargeAmountPaise) {
            return { success: false, code: 'AMOUNT_BELOW_MIN', message: `Minimum charge is ${formatPaise(wallet.rules.minChargeAmountPaise)}` };
        }
        if (totalAmountPaise > wallet.rules.maxChargeAmountPaise) {
            return { success: false, code: 'AMOUNT_EXCEEDS_MAX', message: `Maximum single charge is ${formatPaise(wallet.rules.maxChargeAmountPaise)}` };
        }
        ...
        const txnRef = db.collection(WALLET_COLLECTION).doc(walletId)
            .collection(TXN_SUBCOLLECTION).doc(txnId);
...
export async function reverseTransaction(req) {
    const { walletId, transactionId, reason, supervisorPinHash,
        operatorId, operatorRole, deviceId, eventCodeId } = req;

    if (!['manager', 'admin', 'super'].includes(operatorRole)) {
        return { success: false, code: 'INSUFFICIENT_ROLE', message: 'Reversals require manager or admin role' };
    }
    if (!supervisorPinHash) {
        return { success: false, code: 'SUPERVISOR_PIN_REQUIRED', message: 'Supervisor PIN is required for reversals' };
    }
    ...
    if (!pinVerified) {
        return { success: false, code: 'INVALID_SUPERVISOR_PIN', message: 'Supervisor PIN is incorrect' };
    }
    ...
    return await db.runTransaction(async (tx) => {
        ...
        const txnRef = db.collection(WALLET_COLLECTION).doc(walletId)
            .collection(TXN_SUBCOLLECTION).doc(transactionId);
        ...
        if (originalTxn.type !== 'DEBIT') {
            return { success: false, code: 'NOT_REVERSIBLE', message: 'Only DEBIT transactions can be reversed' };
        }
        if (originalTxn.status === 'REVERSED') {
            return { success: false, code: 'ALREADY_REVERSED', message: 'Transaction has already been reversed' };
        }
        if (originalTxn.walletId !== walletId) {
            return { success: false, code: 'TRANSACTION_MISMATCH', message: 'Transaction does not belong to this wallet' };
        }
        ...
        if (newBalance > wallet.rules.maxTotalBalancePaise) {
            return { success: false, code: 'BALANCE_EXCEEDS_MAX', message: `Reversal would exceed max balance ${formatPaise(wallet.rules.maxTotalBalancePaise)}` };
        }
        ...
        const reversalRef = db.collection(WALLET_COLLECTION).doc(walletId)
            .collection(TXN_SUBCOLLECTION).doc(reversalId);
...
export async function topUpWallet(req) {
    ...
    if (!['host', 'admin', 'super'].includes(operatorRole)) {
        return { success: false, code: 'INSUFFICIENT_ROLE', message: 'Top-up requires host or admin role' };
    }
    if (!Number.isInteger(amountPaise) || amountPaise <= 0) {
        return { success: false, code: 'INVALID_AMOUNT', message: 'amountPaise must be a positive integer' };
    }
    if (!idempotencyKey) {
        return { success: false, code: 'MISSING_IDEMPOTENCY_KEY', message: 'idempotencyKey is required' };
    }
    ...
    return await db.runTransaction(async (tx) => {
        ...
        if (!idempSnap.empty) {
            ...
            return { success: true, transactionId: existing.id, code: 'IDEMPOTENCY_REPLAY', message: 'Already committed' };
        }

        if (!wallet.rules.topUpAllowed) {
            return { success: false, code: 'TOP_UP_DISABLED', message: 'Top-up is not enabled for this wallet' };
        }
        if (wallet.state !== 'ACTIVE') {
            return { success: false, code: 'WALLET_INACTIVE', message: `Cannot top-up wallet in state: ${wallet.state}` };
        }
        if (amountPaise > wallet.rules.maxTopUpAmountPaise) {
            return { success: false, code: 'AMOUNT_EXCEEDS_MAX', message: `Maximum single top-up is ${formatPaise(wallet.rules.maxTopUpAmountPaise)}` };
        }

        const newBalance = wallet.currentBalancePaise + amountPaise;
        if (newBalance > wallet.rules.maxTotalBalancePaise) {
            return { success: false, code: 'BALANCE_EXCEEDS_MAX', message: `Top-up would exceed maximum balance ${formatPaise(wallet.rules.maxTotalBalancePaise)}` };
        }
...
export async function terminateExpiredWallets(eventId) {
    ...
    const activeWallets = await db.collection(WALLET_COLLECTION)
    ...
    for (const walletDoc of activeWallets.docs) {
        ...
        try {
            ...
            await db.runTransaction(async (tx) => {
                ...
                const txnType = w.rules.terminationPolicy === 'partial_refund' && expiredBalancePaise > 0
                    ? 'EXPIRY_REFUND'
                    : 'EXPIRY_FORFEIT';
...
export async function generateReconciliation(eventId, venueId) {
    ...
    const walletsSnap = await db.collection(WALLET_COLLECTION)
        .where('eventId', '==', eventId)
        .get();
    ...
    for (const walletDoc of walletsSnap.docs) {
        ...
        const txnsSnap = await db.collection(WALLET_COLLECTION).doc(wallet.id)
            .collection(TXN_SUBCOLLECTION).get();
        ...
        for (const txnDoc of txnsSnap.docs) {
            ...
            switch (txn.type) {
                case 'DEBIT': computedDebited += txn.amountPaise; break;
                case 'REVERSAL': computedReversed += txn.amountPaise; break;
                case 'TOP_UP':
                case 'CREDIT': computedCredited += txn.amountPaise; break;
                case 'EXPIRY_FORFEIT':
                case 'EXPIRY_REFUND': computedExpired += txn.amountPaise; break;
            }
        }

        // Verify balance consistency
        const expectedBalance = wallet.openingBalancePaise + computedCredited - computedDebited + computedReversed - computedExpired;
