import { EVENT_LIFECYCLE, PUBLIC_LIFECYCLE_STATES, mapEventForClient, normalizeCity } from "./events.js";
...
export const isCurrentOrUpcomingGuestEvent = isCurrentOrUpcomingEvent;
...
export function deriveGuestPublicLifecycle(event = {}, statusKey = normalizeStatusKey(event)) {
    ...
    if (rawLifecycle === EVENT_LIFECYCLE.CANCELLED || statusKey === "canceled") return EVENT_LIFECYCLE.CANCELLED;
...
export function buildHostSummaryReadModel(host = {}, eventCards = [], { readModelVersion = 1 } = {}) {
    const slug = host.slug || slugify(host.displayName || host.name || host.handle || host.id);
    const cards = eventCards.filter((event) => event.hostId === host.id && event.visibility === "public");
    const upcoming = cards.filter((event) => event.statusKey === "upcoming").sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
...
export function buildVenueSummaryReadModel(venue = {}, eventCards = [], {
    readModelVersion = 1,
    menuAvailable = false,
    highlightsCount = 0,
} = {}) {
    const slug = venue.slug || slugify(venue.name || venue.id);
    const cards = eventCards.filter((event) => event.venueId === venue.id && event.visibility === "public");
    const upcoming = cards.filter((event) => event.statusKey === "upcoming").sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
...
export function projectHostDetail(rawHost = {}, summary = {}) {
    return {
        ...pickAllowed(rawHost, [
            "id", "slug", "handle", "name", "displayName", "avatar", "photoURL", "image", "cover", "coverURL",
            "bio", "description", "tagline", "role", "genres", "styleTags", "vibes", "videos", "socialLinks",
            "website", "neighborhood", "city", "location", "verified", "trending", "followers", "followersCount",
...
export function projectVenueDetail(rawVenue = {}, summary = {}, menuDoc = null) {
    ...
    return {
        ...pickAllowed(rawVenue, [
            "id", "slug", "name", "displayName", "photoURL", "logo", "image", "coverURL", "coverImage",
            "bannerImage", "venueType", "category", "type", "area", "neighborhood", "city", "address",
            "contact", "phone", "email", "whatsapp", "website", "socialLinks", "coordinates", "location",
            "timings", "openingHours", "costForTwo", "averageCost", "priceBand", "genres", "musicGenres",
            "dressCode", "entryRules", "rules", "ageLimit", "minimumAge", "facilities", "amenities",
            "photos", "gallery", "menuImages", "menu", "presenceConfig", "tablesAvailable", "hasReservation",
            "hasTickets", "primaryCta", "followers", "followersCount", "verified", "isVerified", "tags",
            "vibes", "bio", "description", "publicProfileEnabled",
        ]),
        ...summary,
        id: summary.id,
...
export function buildGuestDiscoveryEnvelope(items = [], { nextCursor = null, hasMore = false, appliedFilters = {} } = {}) {
    return {
        items,
        nextCursor,
...
export function filterGuestEventCards(rawItems = [], query = {}) {
    ...
    const datePreset = String(query.datePreset || "").trim().toLowerCase();
    const startDate = query.startDate ? new Date(query.startDate) : null;
    const endDate = query.endDate
        ? new Date(/^\d{4}-\d{2}-\d{2}$/.test(String(query.endDate))
...
export function rankGuestSearchGroups({ events = [], hosts = [], venues = [] } = {}, query = "", limit = 6) {
