...
export const handleEventCancellation = inngest.createFunction(
    ...
    async ({ event, step }) => {
            ...
            for (const order of paidOrders) {
                try {
                    ...
                    if (!RAZORPAY_KEY_ID || !RAZORPAY_KEY_SECRET) {
                        console.log(`[Refund] Razorpay not configured, marking order ${order.id} for manual refund`);
                        details.push({
                            orderId: order.id,
                    ...
                    if (refundResponse.ok) {
                        ...
                        console.log(`✅ Refund processed for order ${order.id}: ₹${refundAmount} (${refundData.id})`);
                        details.push({
                            orderId: order.id,
                            status: "refunded",
                            amount: refundAmount,
        ...
        const statusUpdateResult = await step.run("update-order-statuses", async () => {
            ...
            const entitlementSnapshot = await db.collection("entitlements")
                .where("eventId", "==", eventId)
                .where("state", "!=", "REVOKED")
                .get();
        ...
        await step.run("notify-ticket-holders", async () => {
            ...
            for (let i = 0; i < userIds.length; i += batchSize) {
                ...
                for (const userId of chunk) {
                    ...
                    const refundMsg = refundPolicy === "full"
        ...
        await step.run("record-ledger-entries", async () => {
            ...
            try {
                const { initiateRefund, finalizeRefund, MONEY_STATES } = await import("../ledger-engine.js");
