...
export const onOrderPaid_createGuestRecord = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const displayName = orderSnap.displayName
        ...
        const results = await step.run("create-guest-records", async () => {
            ...
            batch.set(summaryRef, {
                totalExpected: FieldValue.increment(ticketList.reduce((a, t) => a + (t.quantity ?? 1), 0)),
                ticketedGuests: FieldValue.increment(ticketList.reduce((a, t) => a + (t.quantity ?? 1), 0)),
...
export const onTableConfirmed_createGuestRecords = inngest.createFunction(
    ...
    async ({ event, step }) => {
        const { bookingId, eventId, venueId, tableId, tableName, hostId, hostName, guestNames, addedByUid, addedByName } = event.data;
        if (!bookingId || !eventId || !venueId) return { skipped: true };

        const db = getAdminDb();
        const names: string[] = Array.isArray(guestNames) ? guestNames : [];
...
export const copyVenueDefaultsToEventRules = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await step.run("copy-defaults", async () => {
            ...
            const venueDefaults = await db
                .collection("venue_guest_rule_defaults")
                .doc(venueId)
                .get();

            const defaults = venueDefaults.exists ? venueDefaults.data() : {};

            await db.collection("event_guest_rules").doc(eventId).set({
                eventId,
                venueId,
