...
function getPreferredProfilePhoto(data = {}) {
    ...
    if (typeof data.selfiePhoto === "string" && data.selfiePhoto.trim()) return data.selfiePhoto.trim();
    return null;
}
...
export async function getUserTickets(userId) {
    ...
    const { getUserClaimedTickets, getPendingTransfers, getOrderShareBundles, getOrderAssignments } = await import("@c1rcle/core/ticket-share-engine");
    const [
        claimedTickets,
        transfers,
...
export async function getProfilePosts(profileId, type, limit = 20) {
    ...
    const db = getAdminDb();
    const snapshot = await db.collection(PROFILE_POSTS_COLLECTION).where("profileId", "==", profileId).where("profileType", "==", type).limit(limit).get();
    return snapshot.docs.map(serializeDoc).sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
}

export async function getProfileHighlights(profileId, type) {
    if (!isFirebaseConfigured()) return [];
    const db = getAdminDb();
    const snapshot = await db.collection(PROFILE_HIGHLIGHTS_COLLECTION).where("profileId", "==", profileId).where("profileType", "==", type).get();
    const now = new Date().toISOString();
    return snapshot.docs.map(serializeDoc).sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()).filter(h => h.permanent || !h.expiresAt || h.expiresAt > now);
}

export async function getProfileStats(profileId, type) {
    if (!isFirebaseConfigured()) return { followersCount: 1200, postsCount: 5, totalLikes: 450, totalViews: 2800 };
