...
export async function getQueueStatus(db, queueId) {
  ...
  const timeout =
    data.status === 'admitted' ? ADMISSION_GRACE_PERIOD_SECONDS : INACTIVITY_TIMEOUT_SECONDS;
...
export async function admitUsers(db, eventId, totalCount = 10, source = 'system') {
  ...
  const counts = {
    ...
    [QUEUE_TIERS.AUTHENTICATED]: Math.max(
      1,
      Math.floor(totalCount * ADMISSION_RATIO[QUEUE_TIERS.AUTHENTICATED]),
    ),
    [QUEUE_TIERS.ANONYMOUS]: Math.max(
      0,
      totalCount -
        Math.max(1, Math.floor(totalCount * ADMISSION_RATIO[QUEUE_TIERS.LOYAL])) -
        Math.max(1, Math.floor(totalCount * ADMISSION_RATIO[QUEUE_TIERS.AUTHENTICATED])),
    ),
  };
...
export async function getSurgeAnalytics(db, eventId) {
  ...
  const metricsDocs = await db
    .collection(SURGE_METRICS_COLLECTION)
    .where('eventId', '==', eventId)
    .get();
  ...
  const queueStats = await db.collection(QUEUE_COLLECTION).where('eventId', '==', eventId).get();
...
export async function validateAdmission(db, eventId, userId, token) {
  ...
  const retryUntil = data.retryUntil
    ? data.retryUntil.toDate
      ? data.retryUntil.toDate()
      : new Date(data.retryUntil)
    : null;
...
export async function recordSurgeMetric(db, eventId, type) {
  ...
  try {
    ...
    if (newValue % 50 === 0) {
      ...
      db.collection(SURGE_METRICS_COLLECTION)
        .doc(`${eventId}_${minuteKey}`)
        .set(
          {
            eventId,
            minute: minuteKey,
        ...
        .catch((err) => console.error('Firestore Metric Sync Error:', err));
    }
