...
function sortReadModels(items: Record<string, any>[], orderByField: string, direction: 'asc' | 'desc') {
...
function buildDiscoveryListCursor(item: Record<string, any>, orderByField: string): DiscoveryListCursor | null {
...
function matchesHostDiscoveryFilters(item: Record<string, any>, query: ListParams = {}) {
    ...
    if (query.role && String(item.role || '').toLowerCase() !== String(query.role).toLowerCase()) return false;
    if (query.vibe) {
        const vibe = String(query.vibe).toLowerCase();
        const hasVibe = Array.isArray(item.vibes) && item.vibes.some((value) => String(value).toLowerCase() === vibe);
    ...
    if (status === 'popular' && !(item.popular || Number(item.followersCount || item.followers || 0) > 0)) return false;
    return true;
}
...
function matchesVenueDiscoveryFilters(item: Record<string, any>, query: ListParams = {}) {
    ...
    if (query.vibe) {
        const vibe = String(query.vibe).toLowerCase();
        const hasVibe = [item.tags, item.vibes, item.genres]
            .some((values) => Array.isArray(values) && values.some((value) => String(value).toLowerCase() === vibe));
        if (!hasVibe) return false;
    }
    if (search && !String(item.searchText || '').toLowerCase().includes(search)) return false;
    if ((normalizeBoolean(query.tablesAvailable) || normalizeBoolean(query.tablesOnly)) && !item.tablesAvailable) return false;
    if (normalizeBoolean(query.verified) && !item.verified) return false;
    return true;
}

async function processInBatches<T>(items: T[], batchSize: number, handler: (item: T) => Promise<void>) {
...
class EventCardIndexRepository {
    ...
    async querySearchPrefix(needle: string, limit = 24) {
        ...
        try {
            const snapshot = await this.db.collection(EVENT_CARD_INDEX)
...
class HostSummaryRepository {
    ...
    async querySearchPrefix(needle: string, limit = 24) {
        ...
        if (!normalized) return [];
        try {
            const snapshot = await this.db.collection(HOST_SUMMARY)
                .where('visibility', '==', 'public')
                .orderBy('searchText')
                .startAt(normalized)
                .endAt(`${normalized}\uf8ff`)
...
class VenueSummaryRepository {
    ...
    async querySearchPrefix(needle: string, limit = 24) {
        ...
        if (!normalized) return [];
        try {
            const snapshot = await this.db.collection(VENUE_SUMMARY)
                .where('visibility', '==', 'public')
                .orderBy('searchText')
                .startAt(normalized)
                .endAt(`${normalized}\uf8ff`)
...
export class PublicDiscoveryService {
    ...
    private resolveEventQueryShape(query: ListParams = {}) {
        ...
        return {
            ...
            orderByField: normalizedSort === 'heat' || normalizedSort === 'heatscore' || normalizedSort === 'trending' || normalizedSort === 'popular'
                ? 'heatScore'
                : normalizedSort === 'new' || normalizedSort === 'newest' || normalizedSort === 'publishedat' || normalizedSort === 'createdat'
                    ? 'publishedAt'
                    : normalizedSort === 'price' || normalizedSort === 'price low to high' || normalizedSort === 'pricemin'
                        ? 'priceMin'
                        : 'startAt',
            direction: normalizedSort === 'heat' || normalizedSort === 'heatscore' || normalizedSort === 'trending' || normalizedSort === 'popular' || normalizedSort === 'new' || normalizedSort === 'newest' || normalizedSort === 'publishedat' || normalizedSort === 'createdat'
                ? 'desc'
                : 'asc' as 'asc' | 'desc',
        };
    }
    ...
    private async getBootstrapState(): Promise<BootstrapState | null> {
        const doc = await this.db.collection(SYSTEM_META).doc(PUBLIC_DISCOVERY_BOOTSTRAP_DOC).get().catch(() => null);
    ...
    private isBootstrapStateCurrent(state: BootstrapState | null) {
        ...
        return state.eventCardIndexVersion === EVENT_CARD_INDEX_VERSION
    ...
    private async ensureHostSummarySeeded(state?: BootstrapState | null, eventCards?: Record<string, any>[]) {
        ...
        const needsBackfill = snapshot.empty || snapshot.docs.some((doc) => doc.data()?.readModelVersion !== HOST_SUMMARY_VERSION);
        if (!needsBackfill) {
            this.hostSummaryChecked = true;
            return;
        }
    ...
    private async ensureVenueSummarySeeded(state?: BootstrapState | null, eventCards?: Record<string, any>[]) {
        ...
        const needsBackfill = snapshot.empty || snapshot.docs.some((doc) => doc.data()?.readModelVersion !== VENUE_SUMMARY_VERSION);
        if (!needsBackfill) {
            this.venueSummaryChecked = true;
            return;
        }
    ...
    private async syncHostReadModelsFromSnapshot(doc: any, eventCardsOverride?: Record<string, any>[]) {
    ...
    private async syncVenueReadModelsFromSnapshot(doc: any, eventCardsOverride?: Record<string, any>[]) {
        if (!doc.exists) return;
        const venue = serializeDoc(doc);
        const eventCards = (eventCardsOverride || await this.events.listAll()).filter((event) => event.venueId === venue.id && event.visibility === 'public');
    ...
    async listEvents(query: ListParams) {
        try {
            ...
            const searchNeedle = String(query.search || query.q || '').trim().toLowerCase();
            ...
            const items = searchNeedle
                ...
                : await this.events.queryList({
                    ...this.resolveEventQueryShape({ ...query, hostId, venueId }),
                    hostId,
                    venueId,
                    limit: Math.min(Math.max(limit * 2, 24), 48),
                });
            const normalizedItems = items.map((item: any) => buildEventCardReadModel(item, {
    ...
    async getEventDetail(idOrSlug: string) {
        ...
        const normalizedLifecycle = derivePublicLifecycleForDetail(eventSource, normalizedCard.statusKey);
        const event = {
            ...(details || {}),
            ...(indexed || {}),
            ...normalizedCard,
    ...
    async getHostPublicProfile(slug: string) {
        ...
        const [rawDoc, postsSnap, highlightsSnap, statsSnap, allEvents] = await Promise.all([
            ...
            this.events.queryList({ hostId: host.id, limit: 48, orderByField: 'startAt', direction: 'asc' }).catch(() => []),
        ]);
        ...
        try {
            ...
            while (!exhausted) {
                const chunk = await this.venues.queryList({
                    cityKey: normalizeCityKey(query.cityKey || query.city || null),
                    areaKey: normalizeFilterKey(query.areaKey || query.area || null),
                    tablesAvailable: String(query.tablesOnly || query.tablesAvailable || '').toLowerCase() === 'true' ? true : null,
                    orderByField,
                    direction,
                    limit: chunkSize,
                    cursor,
    ...
    async getVenuePublicProfile(slug: string) {
        ...
        const [rawDoc, highlightsSnap, statsSnap, menuSnap, allEvents, allVenues] = await Promise.all([
            ...
            this.events.queryList({ venueId: venue.id, limit: 96, orderByField: 'startAt', direction: 'asc' }).catch(() => []),
        ...
        const similarVenues = allVenues
            .filter((item: any) => item.id !== venue.id && (item.cityKey === venue.cityKey || item.areaKey === venue.areaKey))
            .slice(0, 6);
        ...
        return {
            ...
            stats: statsSnap?.exists ? serializeDoc(statsSnap) : { followersCount: venue.followersCount, upcomingEventsCount: venue.upcomingEventsCount },
            highlights: highlightsSnap?.docs?.map(serializeDoc) || [],
            upcomingEvents: venueEvents.filter((event: any) => event.statusKey === 'upcoming').slice(0, 6),
