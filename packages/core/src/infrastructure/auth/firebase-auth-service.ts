...
export class FirebaseAuthService implements IAuthService {
    ...
    async verifyTokenDetailed(
        ...
        const attempts = preferredSource === 'session_cookie'
            ? ['session_cookie', 'id_token']
            : preferredSource === 'id_token'
                ? ['id_token', 'session_cookie']
                : ['id_token', 'session_cookie'];
        const failures: Array<{ source: TokenVerificationSource; code: string | null; message: string | null }> = [];

        for (const source of attempts as TokenVerificationSource[]) {
            try {
                const decodedToken = source === 'session_cookie'
        ...
        if (preferredSource === 'session_cookie' && codes.some((code) => code.includes('invalid-session-cookie'))) {
            return { status: 'session_cookie_mismatch', user: null, source: null, errorCode, errorMessage };
        }
