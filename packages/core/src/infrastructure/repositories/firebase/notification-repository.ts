  ...
  async getVenueNotifications(venueId: string): Promise<Notification[]> {
    ...
    connectionsSnap.docs.forEach((doc: any) => {
      ...
      notifications.push({
        ...
        createdAt: d.requestedAt || d.createdAt,
        readAt: d.venueReadAt || null,
      });
    });
    ...
    ordersSnap.docs.forEach((doc: any) => {
      ...
      if (d.status === 'confirmed' || d.status === 'paid') {
        notifications.push({
          ...
          createdAt: d.confirmedAt || d.createdAt,
          readAt: d.venueReadAt || null,
        });
      }
    });
    ...
    await Promise.all(
      Object.keys(collectionMap).map(async (coll) => {
        ...
        snap.docs.forEach((d: any) => {
          if (!d.data().venueReadAt) batch.update(d.ref, { venueReadAt: now });
        });
        await batch.commit();
      }),
    );
  ...
  async performAction(notificationId: string, type: string, action: string): Promise<string> {
    ...
    if (type === 'connection_request') {
      ...
      await this.db
        .collection('promoter_connections')
        .doc(notificationId)
        .update({ status: newStatus, resolvedAt: now, venueReadAt: now });
      return newStatus;
    }
