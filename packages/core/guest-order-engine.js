...
import {
  validateOrder as coreValidateOrder,
  executeOrderCreation as coreExecuteOrderCreation,
  generateOrderId,
...
import { buildStoredOrderTicket } from './ticket-share-engine.js';
...
export async function createRSVPOrder(payload) {
  ...
  sendEvent(
    ...
    { idempotencyKey: `ticket-purchased-${persistedOrder.id}` },
  ).catch((err) =>
  ...
  if (reservationId) {
    (async () => {
      ...
      if (res?.queueId) {
        const { consumeAdmission } = await import('./guest-queue-engine.js');
  ...
  if (promoterLinkId) {
    recordConversion(
      ...
      persistedOrder.id,
      0,
      persistedOrder.tickets[0]?.ticketId,
...
export async function createOrder(payload) {
  ...
  const existingTicketCount = await getUserTicketCountForEvent(eventId, {
    userId,
    email: userEmail,
  });
  ...
  const totalAmount =
    payload.totalAmount !== undefined ? payload.totalAmount : subtotal - discountAmount;
  ...
  const order = {
    ...
    currency: 'INR',
    paymentMethod,
    status: totalAmount === 0 ? 'confirmed' : 'payment_pending', // Auto-confirm free tickets IN PAID FLOW
  ...
  try {
    ...
    if (persistedOrder.status === 'confirmed') {
      ...
      sendEvent(
        ...
        { idempotencyKey: `ticket-purchased-${orderId}` },
      ).catch((err) =>
      ...
      (async () => {
        const { createShareBundle } = await import('@c1rcle/core/ticket-share-engine');
...
export async function getOrderById(orderId) {
  ...
  if (!isFirebaseConfigured()) {
    return (
      ...
      fallbackRSVPs.find((o) => o.id === orderId) ||
      null
    );
  }
...
export async function getUserTicketCountForEvent(eventId, { userId, email }) {
  ...
  if (!isFirebaseConfigured()) {
    confirmedOrders = fallbackOrders.filter(
      (o) =>
        o.eventId === eventId &&
        o.status === 'confirmed' &&
        ((userId && o.userId === userId) || (email && o.userEmail === email)),
    ...
    const userIdSnapshot = userId
      ? await query.where('userId', '==', userId).get()
      : { empty: true, docs: [] };
    const emailSnapshot = email
      ? await query.where('userEmail', '==', email).get()
      : { empty: true, docs: [] };
...
export async function getOrderByReservationId(reservationId) {
  ...
  if (!isFirebaseConfigured()) {
    return (
      (fallbackOrders || []).find((o) => o.reservationId === reservationId) ||
      (fallbackRSVPs || []).find((o) => o.reservationId === reservationId) ||
      null
    );
  }
...
export async function confirmOrder(orderId, paymentDetails = {}) {
  ...
  try {
    const { notifyTicketPurchase } = await import('@c1rcle/core/guest-notification-engine');
