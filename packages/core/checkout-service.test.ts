...
class FakeOrderRepository {
    ...
    async checkExistingRSVP(eventId: string, lookup: { userId?: string | null; email?: string | null }) {
    ...
    async getUserTicketCountForEvent(eventId: string, lookup: { userId?: string | null; email?: string | null }) {
        let total = 0;
        for (const order of this.orders.values()) {
            if (order.eventId !== eventId || order.status !== 'confirmed') continue;
            if ((lookup.userId && order.userId === lookup.userId) || (lookup.email && order.userEmail === lookup.email)) {
                total += (order.tickets || []).reduce((sum: number, ticket: any) => sum + (Number(ticket.quantity) || 0), 0);
            }
        }
        return total;
    }
...
function buildEvent({ id, price = 500, isRSVP = false, lifecycle = 'scheduled' }: { id: string; price?: number; isRSVP?: boolean; lifecycle?: string }) {
    return {
        id,
        workspaceId: 'ws_1',
        title: `Event ${id}`,
...
function buildReservation({ id, eventId, status = 'active' }: { id: string; eventId: string; status?: string }) {
    return {
        id,
        eventId,
        workspaceId: 'ws_1',
...
describe('CheckoutService parity', () => {
    ...
    it('returns the existing confirmed order after a free reservation is converted', async () => {
        ...
        orderRepo.reservations.set('res-free', buildReservation({ id: 'res-free', eventId: 'evt-free' }));
        const service = new CheckoutService(orderRepo as any, eventRepo as any);
    ...
    it('blocks duplicate RSVP purchases for the same user identity', async () => {
        ...
        orderRepo.reservations.set('res-rsvp', buildReservation({ id: 'res-rsvp', eventId: 'evt-rsvp' }));
    ...
    it('rejects reservations for non-public lifecycle events before inventory work runs', async () => {
        ...
        await expect(service.reserveItems(
            'evt-paused',
            'user_1',
            'device_1',
    ...
    it('computes cancellation policy using event timing and policy snapshot rules', () => {
        const service = new CheckoutService(new FakeOrderRepository() as any, new FakeEventRepository({}) as any);
        const order = {
            id: 'ord-cancel',
            status: 'confirmed',
            totalAmount: 2000,
