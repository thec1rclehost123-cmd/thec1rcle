...
export async function getSlotCalendar(venueId, startDate, endDate) {
    ...
    while (cur <= end) {
        ...
        const fullyBlocked = slots.some(s => (s.status === "blocked" || s.status === "booked") && s.startTime === null);
        const partiallyBlocked = !fullyBlocked && slots.some(s => (s.status === "blocked" || s.status === "booked") && s.startTime !== null);
