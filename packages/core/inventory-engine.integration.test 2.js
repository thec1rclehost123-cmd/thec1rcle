...
const redisMock = {
    ...
    async smembers(key) { return Array.from(this._sets[key] ?? []); },
    ...
    multi() {
        const ops = [];
        const chain = {
            set: (...a) => { ops.push(['set', a]); return chain; },
            del: (...a) => { ops.push(['del', a]); return chain; },
            sadd: (...a) => { ops.push(['sadd', a]); return chain; },
            srem: (...a) => { ops.push(['srem', a]); return chain; },
            expire: (...a) => { ops.push(['expire', a]); return chain; },
            exec: async () => {
                const results = [];
...
describe('calculateEffectiveInventory', () => {
    ...
    it('subtracts active Redis cart reservations from effective inventory', async () => {
        ...
        redisMock._store[`res:data:${resId}`] = JSON.stringify({
            id: resId, eventId: event.id, items: [{ tierId: tier.id, quantity: 3 }], status: 'active',
        });
    ...
    it('throws InventoryReadError when Redis is not ready (fail-closed)', async () => {
        ...
        const event = makeEvent('tier-a', 10, 0);
        const tier = event.tickets[0];
        await expect(calculateEffectiveInventory(tier, event)).rejects.toBeInstanceOf(InventoryReadError);
    });
...
describe('createReservation — concurrent race protection', () => {
    ...
    it('throws InventoryUnavailableError when Redis is missing (fail-closed)', async () => {
        ...
        redisMock.status = 'end';
        await expect(createReservation(event, 'user-2', 'dev-2', makeItems('tier-a', 1)))
    ...
    it('rejects duplicate concurrent lock (system busy error)', async () => {
        ...
        await expect(createReservation(event, 'user-3', 'dev-3', makeItems('tier-a', 1)))
    ...
    it('does NOT return a reservation ID when multi-exec fails', async () => {
        ...
        redisMock.multi = () => {
            ...
            chain.exec = async () => { throw new Error('pipeline error'); };
            return chain;
        };

        await expect(createReservation(event, 'user-4', 'dev-4', makeItems('tier-a', 1)))
...
describe('circuit breaker', () => {
    ...
    it('opens after repeated InventoryReadErrors and blocks subsequent calls', async () => {
        ...
        for (let i = 0; i < 3; i++) {
            try { await calculateEffectiveInventory(tier, event); } catch {}
        }
        ...
        await expect(calculateEffectiveInventory(tier, event))
            .rejects.toMatchObject({ name: 'InventoryReadError', message: /circuit is open/ });
    });
});
