...
export const RISK_TIERS = [
    { minScore: 50, multiplier: 0.10, label: "critical"  },
    { minScore: 25, multiplier: 0.25, label: "high"      },
    { minScore: 10, multiplier: 0.50, label: "elevated"  },
    { minScore: 0,  multiplier: 1.00, label: "normal"    },
];
...
function scoreKey(type, id) { return `rep:score:${type}:${id}`; }
function topKey(type)        { return `rep:top:${type}s`; }
...
export async function addReputation(type, id, event) {
    ...
    return safeExec(async (redis) => {
        ...
        const decayedBase = data?.score
            ? applyDecay(parseFloat(data.score), data.updatedAt)
            : 0;
