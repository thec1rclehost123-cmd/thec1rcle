...
expectIncludes(
  ...
  'Inngest dev target points at guest-portal',
);
expectIncludes(
  'apps/partner-dashboard/package.json',
  'next dev --turbo -p 3001',
  'Partner dashboard dev port is 3001',
);
expectIncludes(
  'apps/admin-console/package.json',
  'next dev --turbo -p 3002',
  'Admin console dev port is 3002',
);
expectIncludes(
  'apps/partner-dashboard/Dockerfile',
  'ENV PORT 3001',
  'Partner dashboard Docker port is 3001',
);
expectIncludes(
  'docker-compose.yml',
  '- "3001:3001"',
  'Docker compose exposes partner dashboard on 3001',
);
expectIncludes(
  'docker-compose.yml',
  '- "3002:3002"',
  'Docker compose exposes admin console on 3002',
);
expectIncludes(
  'apps/api-gateway/src/config/index.ts',
  "default('4000')",
  'API gateway defaults to port 4000',
);
expectIncludes(
  'apps/api-gateway/.env.example',
  'FRONTEND_URLS=http://localhost:3000,http://localhost:3001,http://localhost:3002',
  'API gateway example CORS origins match local web ports',
);
expectIncludes(
  'packages/core/workflows/maintenance.js',
  'http://localhost:4000/api/v1',
  'Maintenance workflow pings the API gateway',
);
expectIncludes('scripts/patchExpoRouter.js', 'major !== 5', 'Expo Router patch is version-gated');
expectIncludes(
  'packages/ui/package.json',
  '"react": "^18.0.0 || ^19.0.0"',
  'UI peers allow React 18 and 19',
);
