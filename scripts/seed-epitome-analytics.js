...
const INDIAN_FIRST_NAMES_M = [
    "Aarav", "Vihaan", "Arjun", "Reyansh", "Aditya", "Sai", "Rohan", "Kabir",
    "Arnav", "Dhruv", "Harsh", "Ishaan", "Karan", "Yash", "Kunal", "Rahul",
    "Pranav", "Vivek", "Nikhil", "Siddharth", "Akash", "Dev", "Manish", "Rajesh",
    "Vikram", "Ankit", "Deepak", "Gaurav", "Sahil", "Mohit"
];
const INDIAN_FIRST_NAMES_F = [
    "Ananya", "Aadhya", "Myra", "Sara", "Aanya", "Diya", "Isha", "Kiara",
    "Riya", "Priya", "Shreya", "Neha", "Pooja", "Kavya", "Tanvi", "Sakshi",
    "Bhavna", "Nikita", "Aditi", "Sanya", "Meera", "Radhika", "Anjali", "Nisha",
    "Swati", "Deepika", "Komal", "Jhanvi", "Aisha", "Simran"
];
const LAST_NAMES = [
    "Sharma", "Patel", "Gupta", "Singh", "Kumar", "Joshi", "Verma", "Reddy",
    "Nair", "Mehta", "Deshmukh", "Pawar", "Kulkarni", "Jadhav", "Chavan",
    "Malhotra", "Kapoor", "Bhat", "Rao", "Thakur", "Chauhan", "Yadav",
    "Pandey", "Srivastava", "Banerjee", "Ghosh", "Das", "Chakraborty",
    "Mishra", "Saxena"
];
const CITIES = [
    "Pune", "Pune", "Pune", "Pune", "Pune", "Mumbai", "Mumbai", "Mumbai",
    "Nashik", "Nagpur", "Aurangabad", "Bangalore", "Delhi", "Hyderabad"
];
...
const makeEvent = (id, title, subtitle, startDate, lifecycle, status, ticketsSold, viewsCount, poster) => {
    ...
    return {
        ...
        rejectionReason: "",
        auditTrail: [
            { action: "created", actor: { uid: OWNER_UID, role: "venue" }, timestamp: new Date(new Date(startDate).getTime() - 14 * 24 * 60 * 60 * 1000).toISOString(), notes: "" },
            { action: "transitioned_to_scheduled", actor: { uid: OWNER_UID, role: "venue" }, timestamp: new Date(new Date(startDate).getTime() - 10 * 24 * 60 * 60 * 1000).toISOString(), notes: "Event published" }
        ],
        status,
        heatScore: Math.floor(200 + ticketsSold * 2 + viewsCount * 0.1),
        keywords: ["epitome", "neon", "noir", "friday", "nightclub", "pune", "party", "edm", "bollywood"],
...
function generateEntitlements(eventId, orders, rsvps, checkInRate) {
    ...
    for (const item of allItems) {
        ...
        const checkInTime = new Date(
            ...
            (Math.random() * 4 * 60 * 60 * 1000) // random checkin within first 4h
        );
        ...
        entitlements.push({
            ...
            requiredGender: item.tickets[0].name.includes("Ladies") ? "female" : (item.tickets[0].name.includes("Stag") ? "male" : "any")
        });
        idx++;
    }
    return entitlements;
...
function generateLedgerEntries(eventId, orders) {
    ...
    for (let i = 0; i < refundCount; i++) {
        ...
        entries.push({
            ...
            timestamp: new Date(new Date(refundedOrder.createdAt).getTime() + 24 * 60 * 60 * 1000).toISOString(),
...
function generateShareBundles(eventId, orders) {
    ...
    const eligibleOrders = orders.filter(o =>
        o.tickets[0].name.includes("Couple") || o.tickets[0].name.includes("VIP")
    );
