    ...
    for (const e of entries) {
      ...
      try {
        ...
        if (idx === parts.length - 1) {
          if (parts[idx] === '**' || parts[idx] === '*' || e.endsWith(parts[idx].replace('*', '').replace('.*', ''))) {
            if (parts[idx] === '**' || parts[idx] === '*' || e.includes(parts[idx].replace('*', '').replace('*', ''))) {
              if (!parts[idx].includes('*') || e.match(new RegExp('^' + parts[idx].replace(/\*/g, '.*') + '$'))) {
                results.push(rel);
              }
            }
          }
...
if (rootEntries.length) {
  error(`Found ${rootEntries.length} debug files in root (should not be committed):\n  ${rootEntries.join('\n  ')}`);
}
...
function findBackupFiles(dir) {
  ...
  for (const e of entries) {
    ...
    try {
      ...
      else if (s.isFile() && /\s2\.[^.]+$/.test(e)) results.push(relative(ROOT, full).replace(/\\/g, '/'));
    } catch { }
  }
  return results;
}
...
for (const dir of srcDirs) {
  const base = join(ROOT, dir);
  if (statSync(base, { throwIfNoEntry: false })?.isDirectory()) {
    const testFiles = findFiles(base, '.test.js').concat(findFiles(base, '.test.ts')).concat(findFiles(base, '.test.tsx'));
    for (const f of testFiles) {
      const content = readFileSync(f, 'utf-8');
      const lines = content.split('\n');
      for (let i = 0; i < lines.length; i++) {
        const ln = lines[i];
        if (ln.includes('.only(') || ln.includes('test.only(') || ln.includes('it.only(') || ln.includes('describe.only(')) {
  ...
  if (statSync(base, { throwIfNoEntry: false })?.isDirectory()) {
    for (const ext of sourceExts) {
      ...
      for (const f of files) {
        ...
        if (rel.includes('/__tests__/') || rel.includes('/tests/') || rel.includes('.test.')) continue;
        try {
          const content = readFileSync(f, 'utf-8');
          const lines = content.split('\n');
          for (let i = 0; i < lines.length; i++) {
            if (/\bconsole\.(log|warn|error|info|debug)\s*\(/.test(lines[i]) && !lines[i].trim().startsWith('//')) {
...
if (sourcesWithLogs.length > 0) {
  ...
  warn(`Found ${total} console.* calls across ${sourcesWithLogs.length} non-test source files (first 10):\n  ${sourcesWithLogs.slice(0, 10).map(f => `  ${f} (${consoleCounts[f]})`).join('\n  ')}`);
}
