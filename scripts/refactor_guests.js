...
const filePath = path.join(__dirname, '../apps/partner-dashboard/app/promoter/guests/PageClient.tsx');
...
`;
content = content.replace('export default function GuestStreamPage() {', debounceHook + '\nexport default function GuestStreamPage() {');
...
const originalFetcherRegex = /const fetcher = async \([\s\S]*?const loading = !guests\.length && !error && promoterId;/;
...
const filterRegex = /const filteredGuests = guests\.filter\(\(g\) => \{[\s\S]*?const paginatedGuests = filteredGuests\.slice\(\(currentPage - 1\) \* itemsPerPage, currentPage \* itemsPerPage\);/;
