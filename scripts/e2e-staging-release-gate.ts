...
import { createTicketQrJwt, verifyTicketQrJwt } from '../packages/core/ticket-checkout-wallet-service.js';
...
async function runStagingReleaseGate() {
  ...
  assert(isPublicLifecycle(EVENT_LIFECYCLE.DRAFT) === false, 'DRAFT is hidden from public discovery');
  assert(isPublicLifecycle(EVENT_LIFECYCLE.CANCELLED) === false, 'CANCELLED is hidden from public discovery feed');
  ...
  assert(nextStatus === 'partially_checked_in', 'Next status for confirmed + CHECK_IN_PARTIAL is partially_checked_in');
