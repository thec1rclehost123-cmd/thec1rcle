...
function collectStoreInventory() {
  const sourceFiles = findSourceFiles(partnerDashboardRoot).filter(
    (filePath) => !filePath.endsWith('.test.ts') && !filePath.endsWith('.test.js'),
  );
...
export function buildPartnerDashboardHardeningBaseline() {
  return {
    ...
    routeDirectories: {
      host: {
        discover: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'host', 'discover', 'page.tsx'),
        ),
        network: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'host', 'network', 'page.tsx'),
        ),
        partnerships: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'host', 'partnerships', 'page.tsx'),
        ),
      },
      venue: {
        connections: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'venue', 'connections', 'page.tsx'),
        ),
        partners: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'venue', 'partners', 'page.tsx'),
        ),
        partnerships: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'venue', 'partnerships', 'page.tsx'),
        ),
      },
      promoter: {
        connections: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'promoter', 'connections', 'page.tsx'),
        ),
        partners: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'promoter', 'partners', 'page.tsx'),
        ),
        partnerships: fs.existsSync(
          path.join(partnerDashboardRoot, 'app', 'promoter', 'partnerships', 'page.tsx'),
        ),
      },
    },
