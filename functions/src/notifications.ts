...
export const notifyOrderConfirmed = onDocumentUpdated('orders/{orderId}', async (event) => {
...
export const notifyEventUpdated = onDocumentUpdated('events/{eventId}', async (event) => {
  const before = event.data?.before.data();
  const after = event.data?.after.data();
...
export const notifyDmRequest = onDocumentCreated('dmRequests/{requestId}', async (event) => {
