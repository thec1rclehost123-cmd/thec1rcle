 ...
 * On ticket transfer
 */
export const onTicketTransferred = onDocumentCreated("transfers/{transferId}", async (event) => {
 ...
 */
export const onEventUpdated = onDocumentUpdated("events/{eventId}", async (event) => {
  const before = event.data?.before.data();
  const after = event.data?.after.data();
 ...
 * On DM request
 */
export const onDMRequest = onDocumentCreated("dmRequests/{requestId}", async (event) => {
...
export { sendPushNotification, saveNotification, getUserPushTokens, isNotificationEnabled };
