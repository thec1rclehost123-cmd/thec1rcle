...
export const onOrderWrite = functions.firestore.document('orders/{orderId}').onWrite(async (change, context) => {
    ...
    if (!wasConfirmed && isConfirmed) {
        ...
        if (after.promoterId) {
            const promoterStatsRef = admin.firestore().collection('promoter_stats').doc(after.promoterId);
            const commission = after.promoterAttribution?.commissionAmount || 0;
            const revenue = after.totalAmount || 0;
            
            promoterStatsRef.set({
                ...
                updatedAt: admin.firestore.FieldValue.serverTimestamp()
            }, { merge: true }).catch(err => console.error("Failed to update promoter stats", err));
        }
