 ...
 */
export const postChatMessageInternal = async (
  data: any,
  ...
  if (!isAllowed) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'You need a ticket to join this conversation',
    );
  }
  ...
  const sanitized = stripHtml(content).substring(0, 1000).trim();
  if (!sanitized) {
    throw new functions.https.HttpsError(
      'invalid-argument',
