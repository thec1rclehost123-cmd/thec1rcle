...
function getEffectivePrice(tier, timestamp = new Date()) {
    ...
    return {
        price: Number((_b = (_a = tier.basePrice) !== null && _a !== void 0 ? _a : tier.price) !== null && _b !== void 0 ? _b : 0),
        label: null,
        isScheduled: false
    };
}
...
async function calculatePricingInternal(event, items, options = {}) {
    ...
    if (promoterCode && ((_b = event.promoterSettings) === null || _b === void 0 ? void 0 : _b.enabled) && ((_c = event.promoterSettings) === null || _c === void 0 ? void 0 : _c.buyerDiscountsEnabled)) {
        let totalPromoterDiscount = 0;
        for (const item of result.items) {
            const tier = tiers.find((t) => t.id === item.tierId);
            if (!tier || tier.promoterEnabled === false)
                continue;
    ...
    if (promoCode) {
        const promoRes = await (0, promos_1.validatePromoCode)(event.id, promoCode, userId, result.items);
