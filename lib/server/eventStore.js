...
let fallbackEvents = seedEvents.map(duplicateEvent);
const findFallbackEvent = (identifier) =>
  fallbackEvents.find((event) => event.id === identifier || event.slug === identifier);
...
const listFallbackEvents = ({ city, limit = 12, sort = "heat" } = {}) => {
...
const createFallbackEvent = (payload) => {
  ...
  fallbackEvents = [event, ...fallbackEvents];
  return event;
};
...
export async function listEvents({ city, limit = 12, sort = "heat", search } = {}) {
  if (!isFirebaseConfigured()) {
    // Update fallback filtering to include search
    let results = listFallbackEvents({ city, limit: 1000, sort }); // Get all for filtering
    if (search) {
  ...
  // We prioritize search over sort if search is present because we can't easily do both
  if (search) {
    const searchTerms = search.toLowerCase().split(" ").filter(t => t.length > 0);
    if (searchTerms.length > 0) {
      // Use array-contains for the first term (most relevant)
      query = query.where("keywords", "array-contains", searchTerms[0]);
    }
  } else {
    // Only apply sort if not searching (or if we have a composite index, but let's be safe)
    const ordering = {
  ...
  // Post-filter for city (since we can't easily do double inequality/array-contains in all cases)
  if (city) {
    const normalized = normalizeCityLabel(city);
