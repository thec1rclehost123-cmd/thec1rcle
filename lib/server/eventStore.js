...
export async function listEvents({ city, limit = 12, sort = "heat" } = {}) {
  if (!isFirebaseConfigured()) {
    return listFallbackEvents({ city, limit, sort });
  }
  const db = getAdminDb();
  ...
  const snapshot = await db.collection(EVENT_COLLECTION).orderBy(order.field, order.direction).limit(baseLimit).get();
