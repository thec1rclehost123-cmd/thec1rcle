...
import { Search, Filter, ShieldCheck, Mail, Phone, Users, History, Activity, AlertCircle, TrendingUp, BadgeCheck, ShieldAlert, Ban, RotateCcw, ChevronRight } from "lucide-react";
...
export default function AdminHosts() {
    ...
    const fetchApps = async () => {
        try {
            ...
            const res = await fetch('/api/admin/list?collection=host_applications', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const json = await res.json();
            setApps(json.data || []);
        } catch (err) {
            console.error("Failed to fetch host apps", err);
        } finally {
            setLoading(false);
        }
    };
        ...
        try {
            ...
            const res = await fetch('/api/admin/actions', {
                ...
                body: JSON.stringify({
                    ...
                    targetId: selectedApp.uid, // Targeted identity is the user/host
                    reason,
                    evidence,
                    params: {
                        type: 'user', // Hosts are users
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 border-b border-slate-200 pb-12">
                <div>
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Onboarding & Growth</span>
                    </div>
                    <h1 className="text-5xl font-black tracking-tighter text-slate-900">Host Pipeline</h1>
                    <p className="text-base text-slate-500 mt-3 font-medium max-w-2xl leading-relaxed">
                        Real-time visibility into the verification status of venues and event organizers. <span className="text-slate-900">Audit applications, verify identities, and manage capital flow.</span>
                    </p>
                </div>
            </div>
                ...
                <div className="lg:col-span-3 space-y-8">
                    <div className="flex gap-4 p-3 rounded-[2.5rem] bg-white border border-slate-200 shadow-sm">
                        <div className="relative flex-1">
                            ...
                            <input
                                type="text"
                                placeholder="Search by name, organization or status..."
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {loading ? (
                            ...
                            <div
                                ...
                                className={`flex items-center gap-8 p-8 rounded-[3rem] border transition-all cursor-pointer group shadow-sm ${selectedApp?.id === app.id ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-200 hover:border-slate-300'
                                    }`}
                            >
                                <div className="h-16 w-16 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center font-black text-xl text-slate-300 shadow-inner group-hover:scale-105 transition-transform">
                                    {app.organizationName?.[0] || app.contactName?.[0] || 'H'}
                                </div>
                                <div className="flex-1">
                                    <h3 className="font-black text-xl tracking-tight text-slate-900">{app.organizationName || app.contactName}</h3>
                                    <div className="flex items-center gap-4 mt-1.5">
                                        <span className="text-[10px] text-indigo-600 font-black uppercase tracking-widest">{app.type || 'Standard'}</span>
                                        <span className="h-1 w-1 rounded-full bg-slate-200"></span>
                                        <span className="text-[10px] text-slate-400 font-bold italic tracking-wide">{app.email}</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className={`inline-flex items-center gap-2.5 px-5 py-2 rounded-full border text-[10px] font-black uppercase tracking-widest ${app.status === 'approved' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' :
                                        app.status === 'pending' ? 'bg-amber-50 border-amber-100 text-amber-600' :
                                            'bg-red-50 border-red-100 text-red-600'
                                        }`}>
                                        <span className={`h-2 w-2 rounded-full ${app.status === 'approved' ? 'bg-emerald-500' :
                                            app.status === 'pending' ? 'bg-amber-500' : 'bg-red-500'
                                            }`}></span>
                                        {app.status}
                                    </div>
                                    <p className="text-[10px] text-slate-300 mt-3 font-black uppercase tracking-widest">REF: {app.id.slice(0, 8)}</p>
                                </div>
                            </div>
                        )) : (
                            <div className="p-24 text-center rounded-[4rem] border-2 border-slate-200 border-dashed bg-slate-50/50">
                                ...
                                <p className="text-sm text-slate-400 font-black uppercase tracking-widest leading-relaxed">The onboarding pipeline<br />is currently empty.</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {selectedApp ? (
                        <div className="sticky top-24 p-12 rounded-[3.5rem] border border-slate-200 bg-white shadow-2xl space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
                            <div className="space-y-4">
                                <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-indigo-600">Application Insight</h2>
                                <h3 className="text-3xl font-black tracking-tighter text-slate-900 leading-none">{selectedApp.organizationName || selectedApp.contactName}</h3>
                            </div>

                            <div className="space-y-10">
                                <div className="space-y-4">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Primary Contact Gateway</p>
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-4 p-5 rounded-2xl bg-slate-50 border border-slate-100 shadow-inner">
                                            <Mail className="h-5 w-5 text-indigo-600" />
                                            <span className="text-sm font-bold text-slate-900">{selectedApp.email}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="pt-10 border-t border-slate-100">
                                <p className="text-[10px] text-slate-400 font-bold italic leading-relaxed text-center">
                                    Capital actions require dual-signature authorization.<br />120m cooling protocol applies.
                                </p>
                            </div>
                        </div>
                    ) : (
                        <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-16 rounded-[4rem] border-2 border-slate-200 border-dashed text-center bg-slate-50/50">
                            ...
                            <p className="text-sm text-slate-400 font-black uppercase tracking-widest leading-relaxed">Select an application<br />for deeper registry<br />inspection.</p>
                        </div>
                    )}
                </aside>
            </div>
