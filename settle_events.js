...
async function runSettlement() {
  ...
  const eventsSnapshot = await db.collection('events').where('lifecycle', '==', 'completed').get();
  ...
  for (const eventDoc of eventsSnapshot.docs) {
    ...
    try {
      ...
      console.log(
        `   ✅ Success: Processed ${result.processedCount} of ${result.totalOrders} total orders.`,
      );
    } catch (err) {
