...
async function runSimulation() {
  ...
  try {
    ...
    const systemAuthBalance = await getBalance({
      entityId: order.id,
      actorId: 'SYSTEM',
      state: MONEY_STATES.AUTHORIZED,
    });
    const userAuthBalance = await getBalance({
      entityId: order.id,
    ...
    console.log(
      `   Internal Balance Check: System(${systemAuthBalance}) + User(${userAuthBalance}) = ${systemAuthBalance + userAuthBalance}`,
    );
    if (systemAuthBalance + userAuthBalance !== 0)
      throw new Error('Double-entry violation in Authorization');
    ...
    const splits = [
      ...
      {
        actorId: 'PROMOTER-ABC',
        actorType: 'promoter',
        amount: 100,
        description: 'Promoter 10% Share',
      },
      {
        actorId: ACCOUNTS.PLATFORM_OPERATIONS,
        actorType: 'system',
        amount: 200,
        description: 'Platform 20% Fee',
      },
    ];
    ...
    const promoterBalance = await getBalance({
      actorId: 'PROMOTER-ABC',
      state: MONEY_STATES.PAYABLE,
    });
    const platformBalance = await getBalance({
      actorId: ACCOUNTS.PLATFORM_OPERATIONS,
      state: MONEY_STATES.PAYABLE,
    });

    console.log(
      `   Verified Club(${clubBalance}) + Promoter(${promoterBalance}) + Platform(${platformBalance}) = ${clubBalance + promoterBalance + platformBalance}`,
    );
    if (clubBalance + promoterBalance + platformBalance !== 1000)
      throw new Error('Split distribution mismatch');
    ...
    console.log(
      "   ✅ Logic verified: transitionMoneyState requires explicit 'from' state balance.",
    );
