...
function formatChatTime(iso: string): string {
    ...
    if (diffMinutes < 1440) return date.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
...
function OrangeRequestPanel() {
    ...
    const hasRequests = DEMO_NEW_MATCHES.length > 0;
        ...
        <Animated.View style={[styles.orangePanel, panelStyle]}>
            <View style={[styles.searchRow, { marginTop: insets.top + spacing.sm }]}>
                ...
                <Pressable style={styles.collapseButton} onPress={toggle}>
                    <Heart size={24} color={colors.gold} strokeWidth={2.2} fill={open ? colors.gold : "transparent"} />
                    {hasRequests && <View style={styles.requestDot} />}
                </Pressable>
            </View>
            ...
            >
                ...
                <View style={rowStyles.copy}>
                    <View style={rowStyles.titleRow}>
                        <Text style={rowStyles.title} numberOfLines={1}>{chat.otherUserName}</Text>
        ...
        <Animated.View entering={FadeInDown.delay(index * 40).duration(260)}>
            ...
            >
                <View style={rowStyles.eventCoverWrap}>
                    <Image source={{ uri: chat.eventCover }} style={rowStyles.eventCover} resizeMode="cover" />
                ...
                <View style={rowStyles.copy}>
                    <View style={rowStyles.titleRow}>
                        <Text style={rowStyles.title} numberOfLines={1}>{chat.eventTitle}</Text>
                        <Text style={rowStyles.time}>{formatChatTime(chat.lastMessageTime)}</Text>
                    </View>
                    <View style={rowStyles.previewRow}>
                        <Text style={[rowStyles.preview, chat.unreadCount > 0 && rowStyles.previewUnread]} numberOfLines={1}>
    ...
    return (
        <View style={styles.tabs}>
            <Pressable style={styles.tab} onPress={() => onChange("direct")}>
                <MessageCircle size={15} color={activeTab === "direct" ? colors.gold : colors.goldMetallic} />
            ...
            <Pressable style={styles.tab} onPress={() => onChange("groups")}>
                ...
                <Text style={[styles.tabText, activeTab === "groups" && styles.tabTextActive]}>Event groups</Text>
                            ...
                            <Animated.View entering={FadeIn.duration(180)}>
                                {DEMO_MODE && DEMO_PRIVATE_CHATS.length > 0 ? (
                                    ...
                                    <View style={styles.empty}>
                                        ...
                                        <Text style={styles.emptyBody}>Your one-on-one conversations will appear here.</Text>
                                    </View>
                                )}
                            </Animated.View>
                        ) : (
