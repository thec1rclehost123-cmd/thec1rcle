        ...
        try {
            ...
            if (active && response.typers) {
                ...
                const users = response.typers
                    .filter(t => t.userId !== currentUserId && (now - new Date(t.timestamp).getTime()) < TYPING_TIMEOUT)
...
export function subscribeToDMTyping(
    ...
    async function poll() {
        ...
        try {
            ...
            if (active && response.typers) {
                const now = new Date().getTime();
                const otherTyper = response.typers.find(t => t.userId !== currentUserId && (now - new Date(t.timestamp).getTime()) < TYPING_TIMEOUT);
