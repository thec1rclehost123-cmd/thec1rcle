  ...
  return (
    ...
    >
      <View className="items-center px-8">
          ...
          <View className="mt-6 items-center">
            ...
            <View className="flex-row items-center mt-2">
              <View className="bg-white/20 px-4 py-2 rounded-full mr-2">
                <Text className="text-white font-bold">{result.guest.ticketType}</Text>
              </View>
              {result.guest.quantity > 1 && (
                <View className="bg-white/20 px-3 py-2 rounded-full">
                  <Text className="text-white font-bold">×{result.guest.quantity}</Text>
                </View>
              )}
            </View>
        ...
        {result.type === 'already_scanned' && result.previousScan && (
          <View className="mt-6 items-center">
            ...
            {result.previousScan.by && (
              <Text className="text-white/70 mt-1">by {result.previousScan.by}</Text>
            )}
          </View>
        )}
        ...
        <Text className="text-white/50 text-sm mt-8">Tap anywhere to continue</Text>
      </View>
    </TouchableOpacity>
  );
}
