    ...
    <SafeAreaView className="flex-1 bg-success items-center justify-center px-6">
      <View className="items-center">
        ...
        <View className="bg-white/20 rounded-2xl px-8 py-6 mt-6 items-center">
          <Text className="text-white text-2xl font-bold">{data.guestName}</Text>
          <View className="flex-row items-center mt-2">
            <Text className="text-white/80 text-lg">{data.tierName}</Text>
            {data.quantity > 1 && <Text className="text-white/80 text-lg"> × {data.quantity}</Text>}
          </View>
          <Text className="text-white text-3xl font-bold mt-3">₹{data.total.toLocaleString()}</Text>
        </View>

        {/* QR Code (if available) */}
        {data.qrData && (
          <View className="bg-white rounded-2xl p-4 mt-6">
            <QRCode value={data.qrData} size={150} backgroundColor="#FFFFFF" color="#000000" />
          </View>
        )}

        {/* Order ID */}
        <Text className="text-white/60 text-sm mt-6">Order: {data.orderId}</Text>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.9}
        >
          <Text className="text-success font-bold text-lg">Next Guest</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
