...
export default function GuestListScreen() {
  ...
  const filteredGuests = guests.filter((guest) => {
    // Search filter
    if (search && !guest.name.toLowerCase().includes(search.toLowerCase())) {
      return false;
    }
    ...
    <View className="bg-background-secondary rounded-xl p-4 mb-3 flex-row items-center">
      ...
      <View
        className={`w-10 h-10 rounded-full items-center justify-center mr-4 ${
          item.status === "entered" ? "bg-success/20" : "bg-border"
        }`}
      >
        <Ionicons
      ...
      <View className="flex-1">
        <View className="flex-row items-center">
          <Text className="text-text-primary text-lg font-semibold">{item.name}</Text>
          {item.source === "door" && (
        ...
        <View className="flex-row items-center mt-1">
          ...
          {item.quantity > 1 && <Text className="text-text-muted ml-2">×{item.quantity}</Text>}
        </View>
      </View>
            ...
            >
              ...
              <Text
                className={`text-xs mt-0.5 ${
                  filter === f.key ? "text-white/70" : "text-text-muted"
                }`}
              >
                {counts[f.key]}
              </Text>
