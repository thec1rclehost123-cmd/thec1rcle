...
export default function StatsScreen() {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      const data = await refreshEventStats(eventData?.event.id || '', eventData?.code);
      if (data) {
        setStats({
    ...
    <SafeAreaView className="flex-1 bg-background-primary" edges={['bottom']}>
      ...
      >
        ...
        <View className="bg-background-secondary rounded-3xl p-6 mb-4 items-center">
          ...
          <Text className="text-text-primary text-display">{stats.totalEntered}</Text>
          <Text className="text-text-secondary text-lg mt-1">of {stats.capacity} capacity</Text>
          ...
          <Text className="text-text-muted text-sm mt-2">{fillPercentage.toFixed(0)}% full</Text>
        </View>
        ...
        <View className="flex-row gap-4 mb-4">
          <View className="flex-1 bg-background-secondary rounded-2xl p-4">
            ...
            <Text className="text-text-primary text-3xl font-bold">{stats.prebooked}</Text>
          </View>
          ...
          <View className="flex-1 bg-background-secondary rounded-2xl p-4">
            ...
            <Text className="text-text-primary text-3xl font-bold">{stats.doorEntries}</Text>
          </View>
        </View>
          ...
          {eventData?.tiers.map((tier) => {
            ...
            return (
              ...
              >
                ...
                <Text className="text-text-primary text-xl font-bold">{count}</Text>
              </View>
            );
          })}
        </View>
