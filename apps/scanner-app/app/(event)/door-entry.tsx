...
export default function DoorEntryScreen() {
  ...
  // Show success screen
  if (successData) {
    return <DoorEntrySuccess data={successData} onDone={resetForm} />;
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-primary" edges={['bottom']}>
      <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled">
        {/* Guest Info Section */}
        <View className="mb-6">
          <Text className="text-text-secondary text-sm font-medium mb-2">GUEST INFORMATION</Text>
        ...
        {/* Ticket Type Section */}
        <View className="mb-6">
          <Text className="text-text-secondary text-sm font-medium mb-2">TICKET TYPE</Text>
          <View className="flex-row flex-wrap gap-3">
            {tiers.map((tier) => (
              ...
              >
                ...
                {!tier.available && (
                  <Text className="text-text-muted text-xs text-center mt-1">Sold Out</Text>
                )}
              </TouchableOpacity>
            ))}
          </View>
        ...
        {/* Quantity Section */}
        <View className="mb-6">
          <Text className="text-text-secondary text-sm font-medium mb-2">QUANTITY</Text>
        ...
        >
          {isSubmitting ? (
            ...
            <>
              ...
              <Text className="text-white font-bold text-xl ml-2">Admit Guest</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
