...
export async function fetchGuestList(eventId: string, eventCode: string): Promise<Guest[]> {
  try {
    const response = await fetch(`${API_BASE}/guestlist?eventId=${eventId}&eventCode=${eventCode}`);
...
export async function searchGuests(
  ...
  return guests.filter((guest) => guest.name.toLowerCase().includes(lowerQuery));
}
