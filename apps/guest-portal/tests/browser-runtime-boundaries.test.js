...
test('Guest portal runtime files stay free of server bridge imports and unsafe request helpers', () => {
  for (const relativePath of runtimeFiles) {
    ...
    assert.equal(
      ...
      false,
      `${relativePath} must not import lib/server/* runtime helpers`,
    );
    assert.equal(
      source.includes('next/headers'),
      false,
      `${relativePath} must not import next/headers`,
    );
    assert.equal(
    ...
    if (!seoRenderingPages.has(relativePath)) {
      assert.equal(
        ...
        false,
        `${relativePath} must not own request-time metadata`,
      );
    } else {
      assert.equal(
        source.includes('guestServerJson'),
        true,
        `${relativePath} metadata must use the server API seam`,
      );
    }
  }
});
...
test('Guest portal owns only rendering-safe sitemap and robots metadata routes', () => {
  ...
  assert.equal(
    sitemapSource.includes('guestServerJson'),
    true,
    'sitemap must use the server API seam',
  );
  assert.equal(
    sitemapSource.includes('lib/server'),
    false,
    'sitemap must not restore app-local server business modules',
  );
  assert.equal(
    robotsSource.includes('getSiteUrl'),
    true,
    'robots must derive canonical host from SEO config',
  );
  assert.equal(
    robotsSource.includes('lib/server'),
    false,
    'robots must not restore app-local server business modules',
  );
});
...
test('Guest portal uses the Next 16 proxy convention for API tracing', () => {
  ...
  assert.equal(
    source.includes('export function proxy'),
    true,
    'proxy.js must export the proxy function',
  );
