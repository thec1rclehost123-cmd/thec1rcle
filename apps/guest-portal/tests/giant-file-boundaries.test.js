...
test("guest hot-path orchestration files stay below the 1,000 line ceiling", () => {
  for (const relativePath of hotPathFiles) {
    ...
    assert.ok(lineCount < 1000, `${relativePath} has ${lineCount} lines and should stay decomposed`);
  }
});
...
test("checkout inventory refresh is event-driven instead of interval polling", () => {
  ...
  const inventoryHook = readFileSync(join(root, "features/checkout/hooks/useCheckoutSession.js"), "utf8");

  assert.equal(checkout.includes("setInterval("), false, "CheckoutContainer must not own an inventory polling loop");
  assert.equal(inventoryHook.includes("setInterval("), false, "checkout session must not poll inventory on an interval");
  assert.equal(inventoryHook.includes("calculateCheckout("), true, "checkout session should refresh inventory through the quote seam");
  assert.equal(inventoryHook.includes("selectedTicketSignature"), true, "inventory should refresh when ticket selections change");
  assert.equal(inventoryHook.includes("cartReservation"), true, "inventory should refresh when reservation state changes");
});
