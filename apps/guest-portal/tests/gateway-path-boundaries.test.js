...
test('generated guest API client operations stay fresh against the gateway OpenAPI artifact', () => {
  const openApi = JSON.parse(
    readFileSync(join(root, '../api-gateway/openapi/guest-v1.json'), 'utf8'),
  );
...
test('generated guest API helper builds parameterized and queried paths', () => {
  assert.equal(
    buildGuestOperationPath('getPublicEvent', { idOrSlug: 'event 1' }),
    '/public/events/event%201',
  );
  assert.equal(
    buildGuestOperationPath('getCoverChargeWallet', { walletId: 'wallet/1' }),
    '/cover-charge/wallet/wallet%2F1',
  );
  assert.equal(
    buildGuestOperationPath('getTicketShareBundle', {}, { token: 'abc 123' }),
    '/tickets/claim?token=abc+123',
  );
});
...
test('production gateway rewrite and type checking are not pinned to localhost-only shortcuts', () => {
  ...
  assert.equal(
    ...
    false,
    'production builds must not ignore TypeScript errors',
  );
  assert.equal(
    nextConfig.includes('resolveGuestApiOrigin'),
    true,
    'gateway rewrite should use the shared guest API origin resolver',
  );
  assert.equal(
    nextConfig.includes('destination: `${gatewayOrigin}/api/v1/:path*`'),
    true,
    'rewrite destination should be derived from gateway env',
  );
});
