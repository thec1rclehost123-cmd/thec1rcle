...
test('SEO-critical public pages fetch initial data through the server API seam', () => {
  ...
  for (const relativePath of files) {
    ...
    assert.equal(
      source.includes('guestServerJson'),
      true,
      `${relativePath} must fetch initial public data server-side`,
    );
    assert.equal(
      source.includes('lib/server'),
      false,
      `${relativePath} must not restore app-local server business modules`,
    );
    assert.equal(
      /fetch\(\s*[`"']\/api\/v1/.test(source),
      false,
      `${relativePath} must not raw-fetch /api/v1`,
    );
  }
});
...
test('Public detail pages expose metadata and canonical SEO helpers', () => {
  ...
  for (const relativePath of files) {
    ...
    assert.equal(
      ...
      true,
      `${relativePath} must export generateMetadata`,
    );
    assert.equal(
      source.includes('alternates'),
      true,
      `${relativePath} must define canonical metadata`,
    );
  }
...
test('Public directory pages expose canonical metadata', () => {
  const files = ['app/page.js', 'app/explore/page.js', 'app/hosts/page.js'];
...
test('Promoter vanity links resolve server-side before falling back to client resolution', () => {
  ...
  assert.equal(
    source.includes('loadVanityLink'),
    true,
    'vanity route must compose the feature resolver',
  );
  assert.equal(
    source.includes('guestServerJson'),
    false,
    'vanity route must not own gateway fetching',
  );
  assert.equal(
    resolver.includes('guestServerJson'),
    true,
    'vanity feature resolver must call the gateway from the server',
  );
  assert.equal(
    resolver.includes('/public/promoters/'),
    true,
    'vanity feature resolver must use the canonical promoter link API',
  );
  assert.equal(
    source.includes('redirect('),
    true,
    'vanity route must server-redirect when the gateway resolves the link',
  );
  assert.equal(
    source.includes('<PageClient />'),
    true,
    'vanity route must keep the client fallback for unresolved links during migration',
  );
});
...
test('Sitemap and robots are rendering-owned and gateway-backed', () => {
  ...
  assert.equal(
    sitemap.includes('guestServerJson'),
    true,
    'sitemap must fetch public index data through the server API seam',
  );
  assert.equal(
    sitemap.includes('/public/events?limit=500'),
    true,
  ...
  assert.equal(
    ...
    true,
    'robots metadata must point crawlers at sitemap.xml',
  );
});
...
test('Venue menu page renders gateway menu data instead of hard-coded sample inventory', () => {
  ...
  assert.equal(
    pageClient.includes('menu={data?.menu || venue.menuDoc || venue.menu || null}'),
    true,
  );
  assert.equal(
    menuClient.includes('normalizeMenuSections'),
    true,
    'menu client should normalize returned venue/menu payloads',
  );
  assert.equal(
    menuClient.includes('Neapolitan Pizza'),
    false,
    'menu client must not ship hard-coded sample sections',
  );
  assert.equal(
    menuClient.includes('Mutton Kheema'),
    false,
    'menu client must not ship hard-coded sample items',
  );
});
