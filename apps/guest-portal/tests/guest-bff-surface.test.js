...
test("Guest Portal BFF routes validate their response DTOs before replying", () => {
  ...
  for (const relativePath of routeFiles) {
    ...
    assert.equal(source.includes("dataSchema:"), true, `${relativePath} should pass a response dataSchema`);
    assert.equal(source.includes("guestBffJsonResponse"), true, `${relativePath} should reply through the shared BFF envelope helper`);
  }
});
...
test("Guest Portal migrated surfaces are wired to BFF helpers behind flags", () => {
  ...
  for (const [relativePath, token] of expectedReferences) {
    ...
    assert.equal(source.includes("isGuestBffEnabled"), true, `${relativePath} should stay flag-gated`);
  }
});
