...
const legacyHostProfileClientPath = path.resolve(process.cwd(), "app/host/[slug]/HostProfileClient.jsx");
const legacyVenueProfileClientPath = path.resolve(process.cwd(), "app/venue/[slug]/VenueProfileClient.jsx");
...
test("logout redirects reuse the canonical login entry after awaiting sign-out", () => {
  const profileSource = readFileSync(path.resolve(process.cwd(), "app/profile/[userId]/PageClient.jsx"), "utf8");
...
test("auth flows standardize callback redirects on the next query param", () => {
  ...
  assert.equal(loginSource.includes("await loginWithGoogle(buildAuthCallbackUrl(redirectUrl));"), true);
...
test("login client no longer stores the password in session persistence or treats invalid credentials as signup", () => {
  ...
  assert.equal(modelSource.includes('return "Google sign-in is not enabled for this Firebase project."'), true);
  ...
  assert.equal(modelSource.includes('if (step === 1 && form.email && form.password && !isLoginMode)'), true);
  assert.equal(modelSource.includes('if (step === 1 && form.email && form.password && isLoginMode)'), true);
...
test("protected auth redirects use next across guest conversion surfaces", () => {
  const checkoutSource = readFileSync(path.resolve(process.cwd(), "features/checkout/hooks/useCheckoutSession.js"), "utf8");
  const checkoutModelSource = readFileSync(path.resolve(process.cwd(), "features/checkout/utils/checkoutSessionModel.js"), "utf8");
  const eventSource = readFileSync(path.resolve(process.cwd(), "features/events/hooks/useEventTicketSelection.js"), "utf8");
...
test("auth intent replay uses real RSVP and follow gateway mutations", () => {
  ...
  assert.equal(manager.includes("setEventRsvp"), true, "GlobalAuthManager must replay RSVP through the event feature seam");
  assert.equal(manager.includes("followHost"), true, "GlobalAuthManager must replay host follow through the social feature seam");
  assert.equal(manager.includes("followVenue"), true, "GlobalAuthManager must replay venue follow through the social feature seam");
  assert.equal(manager.includes('intent.type === "RESERVE_VENUE"'), true, "GlobalAuthManager must replay venue reservation intents");
  assert.equal(manager.includes("OPEN_VENUE_RESERVATION"), true, "GlobalAuthManager must reopen the venue reservation flow after auth");
  assert.equal(manager.includes("updateEventList("), false, "GlobalAuthManager must not replay RSVP by mutating profile arrays");

  assert.equal(hostFollow.includes('saveIntent("FOLLOW_HOST"'), true, "Host follow CTA should persist host follow intents");
  assert.equal(hostFollow.includes("getHostFollowStatus"), true, "Host follow CTA should use the social feature follow-status seam");
  assert.equal(hostFollow.includes("user.uid"), false, "Host follow CTA should not send client user ids to follow-status");

  assert.equal(venueFollow.includes('saveIntent("FOLLOW_VENUE"'), true, "Venue page should persist venue follow intents");
  assert.equal(venueFollow.includes('saveIntent("RESERVE_VENUE"'), true, "Venue page should persist venue reservation intents");
  assert.equal(venueFollow.includes("OPEN_VENUE_RESERVATION"), true, "Venue page should be able to reopen its reservation modal from auth replay");
  assert.equal(venueFollow.includes("getVenueFollowStatus"), true, "Venue page should use the social feature venue follow seam");

  assert.equal(legacyHostFollow.includes("getHostFollowStatus"), true, "Legacy host profile should use the host follow-status seam");
  assert.equal(legacyHostFollow.includes("followHost"), true, "Legacy host profile should persist host follow actions");
  assert.equal(legacyHostFollow.includes("unfollowHost"), true, "Legacy host profile should persist host unfollow actions");
  assert.equal(legacyHostFollow.includes('saveIntent("FOLLOW_HOST"'), true, "Legacy host profile should persist follow intents for auth replay");

  assert.equal(legacyVenueFollow.includes("getVenueFollowStatus"), true, "Legacy venue profile should use the venue follow-status seam");
  assert.equal(legacyVenueFollow.includes("followVenue"), true, "Legacy venue profile should persist venue follow actions");
  assert.equal(legacyVenueFollow.includes("unfollowVenue"), true, "Legacy venue profile should persist venue unfollow actions");
  assert.equal(legacyVenueFollow.includes('saveIntent("FOLLOW_VENUE"'), true, "Legacy venue profile should persist follow intents for auth replay");
});
...
test("auth and profile surfaces use typed guest API helpers", () => {
  ...
  assert.equal(authProvider.includes("gatewayJson("), false, "AuthProvider should stay on the typed guest API seam");
  ...
  assert.equal(authService.includes("gatewayJson("), false, "authService should stay on the typed guest API seam");

  assert.equal(passwordReset.includes("guestApi.auth.passwordReset"), true);
  assert.equal(passwordReset.includes("gatewayJson("), false, "password reset should stay on the typed guest API seam");

  assert.equal(editProfile.includes("useEditProfileFlow"), true, "edit profile modal should delegate profile orchestration to its feature hook");
  assert.equal(profileApi.includes("guestApi.profiles.avatar"), true);
  assert.equal(editProfile.includes("gatewayJson("), false, "avatar uploads should stay on the typed guest API seam");
});
