...
test("Guest Portal does not restore deleted legacy server directories", () => {
  ...
  for (const relativePath of staleDirs) {
    assert.equal(existsSync(join(root, relativePath)), false, `${relativePath} should stay deleted`);
  }
});
...
test("Guest Portal no longer keeps the temporary client compatibility shim or root-level runtime fixtures", () => {
  ...
  for (const relativePath of staleFiles) {
    assert.equal(existsSync(join(root, relativePath)), false, `${relativePath} should be removed or quarantined from runtime`);
  }
});
...
test("Guest Portal debug scripts do not call deleted local bridges or legacy app API routes", () => {
  ...
  for (const relativePath of scripts) {
    ...
    assert.equal(source.includes("../lib/server/"), false, `${relativePath} must not import deleted lib/server modules`);
    assert.equal(source.includes("/api/events"), false, `${relativePath} must not call legacy /api/events`);
    assert.equal(source.includes("app/tickets/actions"), false, `${relativePath} must not require deleted server actions`);
  }
});
...
test("Auth and profile client runtime no longer imports Firebase SDK modules", () => {
  ...
  for (const relativePath of files) {
    const source = readFileSync(join(root, relativePath), "utf8");
    assert.equal(source.includes("firebase/auth"), false, `${relativePath} must not import firebase/auth`);
    assert.equal(source.includes("firebase/storage"), false, `${relativePath} must not import firebase/storage`);
    assert.equal(source.includes("getFirebaseAuth"), false, `${relativePath} must not call getFirebaseAuth`);
    assert.equal(source.includes("getFirebaseStorage"), false, `${relativePath} must not call getFirebaseStorage`);
  }
});
...
test("about route no longer keeps dead host modal scaffolding", () => {
  ...
  assert.equal(source.includes("showHostModal"), false, "about page should not keep dead host modal state");
  assert.equal(source.includes("function HostModal"), false, "about page should not keep the removed host modal component");
  assert.equal(source.includes("useAuth"), false, "about page should not import auth state it no longer uses");
  assert.equal(source.includes("useRouter"), false, "about page should not import router state it no longer uses");
  assert.equal(source.includes('window.open("https://thec1rclehost.com/host", "_blank")'), true, "about page should keep the dedicated host site handoff");
});
...
test("Guest Portal runtime keeps backend-only and raw API access out of app code", () => {
  ...
  const visit = (dir) => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      const checks = [
        ...
        [source.includes("lib/server/") || /from\s+["'][^"']*\/server\//.test(source) || /import\([^)]*\/server\//.test(source), "server module import"],
  ...
  assert.deepEqual(offenders, [], "Guest Portal runtime must stay on the typed API/client boundary");
});
