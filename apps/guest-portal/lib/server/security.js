...
export function validateConfig() {
  ...
  if (missing.length > 0 && process.env.NODE_ENV === 'production') {
    ...
    if (
      ...
      console.warn(
        `⚠️  BUILD WARNING: Missing environment variables: ${missing.join(', ')}. Build will continue.`,
      );
      return;
    }

    throw new Error(
      `🛑 SECURITY BREACH: Missing critical environment variables: ${missing.join(', ')}`,
    );
  }
  ...
  if (pk && !pk.includes('BEGIN PRIVATE KEY') && process.env.NODE_ENV === 'production') {
    console.warn(
      '⚠️  WARNING: FIREBASE_PRIVATE_KEY format seems invalid. Ensure it includes header/footer.',
    );
  }
}
