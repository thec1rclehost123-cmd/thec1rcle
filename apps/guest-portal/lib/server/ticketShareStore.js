...
export async function createShareBundle(
  ...
  if (hasOwnerClaimed && tier) {
    const isClaimBasedFreeTier =
      (tier.price === 0 || tier.isFree) && (tier.genderRequirement || order.isRSVP);
    if (isClaimBasedFreeTier) {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        if (eDoc.exists) {
          ...
          if (tIdx !== -1) {
            const currentRem =
              Number(updatedTiers[tIdx].remaining ?? updatedTiers[tIdx].quantity) || 0;
            if (currentRem > 0) {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    if (eligibleSlots.length === 0) {
      ...
      if (unclaimedSlots.length > 0) {
        ...
        throw new Error(
          `Restricted: This slot is for ${required === "female" ? "Females" : "Males"} only.`,
        );
      } else {
...
async function _handleOrderScan(transaction, ticketId, eventId, scannerId, now) {
  ...
  const requiredGender =
    tier?.requiredGender ||
    (tier?.genderRequirement === "couple" ? "male" : tier?.genderRequirement || "any");
...
export async function initiateTransfer(ticketId, senderId, recipientEmail = null) {
  ...
  if (event) {
    ...
    if (hoursUntilEvent < (SECURITY_CONFIG.TRANSFER_BLOCK_HOURS_BEFORE_EVENT || 2)) {
      throw new Error(
        `Transfers are locked ${SECURITY_CONFIG.TRANSFER_BLOCK_HOURS_BEFORE_EVENT || 2} hours before the event.`,
      );
    }
  }
