...
export const adminStore = {
  ...
  async logAction({
    ...
    if (!reason || reason.trim().length < minReason) {
      throw new Error(
        `Audit reason is mandatory (min ${minReason} chars for this priority level).`,
      );
    }

    if ((isTier2 || isTier3) && !evidence) {
      throw new Error(
        `Protocol Error: Reference evidence (link) is required for Tier ${isTier3 ? 3 : 2} actions.`,
      );
    }
  ...
  async proposeAction(adminId, adminRole, actionData) {
    ...
    const now = new Date();
    const coolingPeriodEnd = isTier3
