...
export async function buildHomeOverviewView({ city = "" } = {}) {
  ...
  const [featuredResult, eventsResult, selectsResult, interviewsResult] = await Promise.all([
    guestBffUpstreamJson(`/public/events/featured?limit=6&city=${encodeURIComponent(selectedCity)}`, {
      cacheMode: GUEST_BFF_CACHE.PUBLIC_REVALIDATED,
      forwardCookies: false,
      next: { revalidate: 60 },
    }),
    guestBffUpstreamJson(`/public/events?city=${encodeURIComponent(selectedCity)}&limit=30&sort=heat`, {
      cacheMode: GUEST_BFF_CACHE.PUBLIC_REVALIDATED,
      forwardCookies: false,
      next: { revalidate: 60 },
    }),
  ...
  const featuredEvents = featuredResult.response.ok ? extractItems(featuredResult.data, "events") : [];
  const events = eventsResult.response.ok ? extractItems(eventsResult.data, "events") : [];
  const selects = selectsResult.response.ok ? selectsResult.data?.data || selectsResult.data || [] : [];
  const interviews = interviewsResult.response.ok ? interviewsResult.data?.data || interviewsResult.data || [] : [];
  ...
  return buildGuestBffResult({
    ...
    error: eventsResult.response.ok
      ? null
      : buildGuestBffError(
          getGuestBffUpstreamError(eventsResult.data, "The live event feed is temporarily unavailable."),
          {
