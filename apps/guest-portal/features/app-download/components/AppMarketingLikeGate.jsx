...
export default function AppMarketingLikeGate() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        <button onClick={closeModal} className="absolute right-8 top-8 text-white/40 transition-colors hover:text-white">
          <X size={24} />
        </button>
        <div className="flex flex-col items-center text-center">
          ...
          <button onClick={closeModal} className="mt-8 text-xs font-bold uppercase tracking-[0.3em] text-white/30 transition-colors hover:text-white">
            Continue Browsing
          </button>
        </div>
      </motion.div>
