          ...
          <div className="space-y-3 overflow-y-auto custom-scrollbar">
            {selectedTickets.length > 0 ? (
              selectedTickets.map((ticket) => (
                <div key={ticket.id} className="flex items-start justify-between gap-4 border-b border-white/4 pb-3 last:border-b-0 last:pb-0">
                  <div className="min-w-0 pr-4">
                    <p className="truncate text-[13px] font-black uppercase tracking-[0.08em] text-white">{ticket.name}</p>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <span className="text-[13px] font-semibold text-white/72">
                    +₹{displayFees.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                  </span>
                </button>
                        ...
                        {feeBreakdown.map((item) => (
                          <div key={item.label} className="flex items-center justify-between gap-4">
                            ...
                            <span className="text-[12px] font-semibold text-white/70">
                              ₹{item.value.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                            </span>
                          </div>
                        ))}
                      </div>
