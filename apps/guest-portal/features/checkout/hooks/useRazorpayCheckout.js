...
export function useRazorpayCheckout({
  ...
  return useCallback(async (initiateData, checkoutOptions = {}) => {
    const paymentVerifyKey =
      checkoutOptions.paymentVerifyKey ||
      checkoutOptions.paymentAttemptId ||
      null;
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        finishWithError(new Error("Could not launch payment window. Please disable ad-blockers."));
      }
    });
  }, [attendeeDetails.email, attendeeDetails.name, attendeeDetails.phone, eventTitle, isPaymentPending, onPaymentCancelled, onPaymentError, onPaymentStateChange, onPaymentVerified]);
}
