...
export function persistReservationSnapshot(reservation) {
  ...
  try {
    ...
    window.localStorage.setItem(RESERVATION_SYNC_EVENT_KEY, JSON.stringify({
...
export function useReservationStorage({
  ...
  const persistReservation = useCallback((reservation, itemsOverride = null) => {
  ...
  const applyPersistedReservation = useCallback((saved) => {
    if (!saved) {
      setCartReservation(null);
      setOtherEventReservation(null);
      return;
  ...
  useEffect(() => {
    ...
    const handleStorage = (eventSnapshot) => {
      if (![RESERVATION_STORAGE_KEY, RESERVATION_SYNC_EVENT_KEY].includes(eventSnapshot.key)) return;
