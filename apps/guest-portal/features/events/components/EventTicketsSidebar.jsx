  ...
  return (
    <aside className="relative order-1 flex flex-col gap-2.5 sm:gap-3 lg:order-2">
      <div
        ...
        style={{ background: `radial-gradient(circle, rgba(${dominantColor}, 0.5) 0%, transparent 60%)` }}
      />
      ...
      <GlassCard className="p-3 sm:p-4" glowColor={dominantColor}>
        <div className="relative aspect-[0.78] overflow-hidden rounded-[24px] border border-white/10">
          ...
          {eventImage ? <Image src={eventImage} alt={event?.title || ""} fill priority unoptimized className="object-cover" /> : null}
          ...
          <div className="absolute inset-x-0 bottom-0 p-4">
            <div className="rounded-[20px] border border-white/10 bg-black/35 p-4 backdrop-blur-md">
              <div className="mt-1 text-[18px] font-black uppercase leading-[0.96] tracking-[-0.05em] text-white">{displayTitle}</div>
              <div className="mt-3 text-[11px] uppercase tracking-[0.18em] text-white/46">{hostLabel}</div>
            </div>
          </div>
        </div>
      </GlassCard>
      ...
      <GlassCard glowColor={dominantColor}>
        <div className="border-b border-white/[0.08] px-5 py-5">
          ...
          <div className="mt-3 text-[28px] font-black uppercase tracking-[-0.04em] text-white">{entryLabel}</div>
          <div className="mt-2 max-w-[28ch] text-[13px] leading-6 text-white/48">{ticketLeadCopy}</div>
        </div>
        ...
        <div className="flex flex-col gap-3 px-5 py-5">
          ...
          {tickets.map((ticket) => {
            ...
            const freeTierLimitLabel =
              quantity >= 1 || (totalFreeSelected >= 1 && quantity === 0) ? "Limit Reached" : "Limit 1";

            return (
              <div
                key={ticket.id}
                className={`relative overflow-hidden rounded-[20px] border px-5 py-5 transition-all duration-300 ${
                  isSelected ? "border-white/20 bg-white/[0.08] shadow-lg" : "border-white/[0.06] bg-black/40 hover:bg-black/60"
              ...
              >
                <div className="relative z-10 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className={`${badge ? "mt-2" : ""} text-[16px] font-bold tracking-wide text-white`}>{ticket.name || "Entry Tier"}</div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="mt-1 text-[9px] font-bold uppercase tracking-[0.2em] text-white/30">
                      {Number(ticket.price || 0) === 0 ? freeTierLimitLabel : limit > 0 ? `Limit ${limit}` : "Closed"}
                    </div>
                  </div>
                </div>

                <div className="relative z-10 mt-4 flex items-center justify-between border-t border-white/[0.06] pt-4">
                  <div className="text-[11px] font-semibold text-white/40">{isSelected ? "Selected" : "Select Quantity"}</div>
                  ...
                  {availability.isSoldOut ? (
                    ...
                    <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-black/40 px-1 py-1 backdrop-blur-md">
                      ...
                      <span className="w-4 text-center text-[14px] font-bold text-white">{quantity}</span>
                      <button
                        type="button"
                        onClick={() => setQuantity(ticket, quantity + 1)}
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-0.5 text-[13px] font-semibold text-white/70">
                {selectedTickets.length > 0 ? `${totalSelected} selected` : tickets.length > 0 ? "Pick your tickets" : "Join the waitlist"}
              </div>
            </div>
