...
export async function fetchPublicEvents(params = {}, options = {}) {
  return unwrap(() => guestApi.public.events(toQueryString(params), options), "Unable to load public events");
}

export async function fetchFeaturedEvents(params = {}, options = {}) {
  return unwrap(() => guestApiOperationJson("listFeaturedEvents", { query: params, ...options }), "Unable to load featured events");
}

export async function fetchHomepageSelects(options = {}) {
  const data = await unwrap(() => guestApiOperationJson("listHomepageSelects", options), "Unable to load homepage selects");
  return Array.isArray(data) ? data : [];
}

export async function fetchHomepageInterviews(options = {}) {
  const data = await unwrap(() => guestApiOperationJson("listHomepageInterviews", options), "Unable to load homepage interviews");
  return Array.isArray(data) ? data : [];
}
...
export async function fetchPublicHosts(params = {}, options = {}) {
  return unwrap(() => guestApi.public.hosts(toQueryString(params), options), "Unable to load public hosts");
}

export async function fetchPublicHost(slug, options = {}) {
  return unwrap(() => guestApiOperationJson("getPublicHost", { params: { slug }, ...options }), "Unable to load public host");
}

export async function fetchPublicVenues(params = {}, options = {}) {
  return unwrap(() => guestApi.public.venues(toQueryString(params), options), "Unable to load public venues");
}

export async function searchPublicDiscovery(params = {}, options = {}) {
  return unwrap(() => guestApi.public.search(toQueryString(params), options), "Unable to search discovery");
}

export async function fetchSearchSuggestions(query, options = {}) {
  if (!query || String(query).trim().length < 2) return [];
  const data = await searchPublicDiscovery({ q: String(query).trim(), type: "suggestions" }, options);
  return Array.isArray(data?.suggestions) ? data.suggestions : [];
}

export async function fetchPublicVenue(slug, options = {}) {
  const data = await unwrap(() => guestApiOperationJson("getPublicVenue", { params: { slug }, ...options }), "Unable to load public venue");
  return normalizeVenueIdentity(data);
}

export async function fetchPromoterByUsername(username, options = {}) {
  return unwrap(() => guestApiOperationJson("getPublicPromoter", { params: { username }, ...options }), "Unable to load promoter");
}

export async function fetchPromoterVanityLink(username, alias, options = {}) {
  const data = await unwrap(() => guestApiOperationJson("resolvePromoterVanityLink", { params: { username, alias }, ...options }), "Unable to resolve promoter link");
