...
export async function unfollowHost(targetId) {
  return requireOk(() => guestApi.social.unfollow(targetId, 'host'), 'Unable to unfollow host');
}
...
export async function followVenue(venueId) {
  return requireOk(() => guestApi.venues.follow(venueId), 'Unable to follow venue');
}

export async function unfollowVenue(venueId) {
  return requireOk(() => guestApi.venues.unfollow(venueId), 'Unable to unfollow venue');
}
