...
import { useTicketDetailQuery } from './ticketsQueries';
...
const AuroraBackground = ({ rgb = '244, 74, 34' }) => (
  <div className="fixed inset-0 -z-10 overflow-hidden bg-[var(--bg-color)]">
    ...
    {/* Right orb */}
    <div
      className="absolute top-[10%] right-[-10%] h-[1400px] w-[1400px] rounded-full blur-[120px] opacity-[0.8] mix-blend-multiply dark:mix-blend-screen transition-all duration-1000"
      style={{
    ...
    <div
      className="absolute bottom-[5%] left-[-15%] h-[1000px] w-[1000px] rounded-full blur-[100px] opacity-[0.8] mix-blend-multiply dark:mix-blend-screen transition-all duration-1000"
      style={{
        backgroundColor: `rgba(${rgb}, 0.15)`,
      }}
    />
...
const TicketCard = ({ ticket, onShare, onClick, onPartner, onTransfer }) => {
  ...
  // Tier-based theming
  const tierKey = (
    ticket.tickets?.[0]?.tierId ||
    ticket.tickets?.[0]?.ticketType ||
    ''
  ).toLowerCase();
  ...
  return (
    <div className="relative group overflow-visible">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-visible flex items-center justify-center">
        <div
          className="absolute w-[120%] h-[120%] opacity-60 blur-[120px] transition-all duration-1000 group-hover:opacity-[0.9] group-hover:scale-110"
          style={{
            background: `radial-gradient(circle at center, rgba(${rgb}, 0.8) 0%, rgba(${rgb}, 0.2) 50%, transparent 80%)`,
          }}
        />
      </div>
      ...
      <motion.div
        ...
        className={`relative z-10 flex flex-col cursor-pointer overflow-hidden rounded-[32px] border transition-all duration-500 h-full min-h-[200px] ${
          isPast
            ? 'border-white/5 bg-white/5 opacity-40 grayscale'
            : 'border-white/10 bg-zinc-900/60 backdrop-blur-3xl shadow-[0_32px_80px_-20px_rgba(0,0,0,0.6)]'
        }`}
      >
...
const QRModal = ({ isOpen, ticket, onClose, onPartner, onTransfer }) => {
  ...
  const tickets = ticket?.tickets || (ticket ? [ticket] : []);
  const [currentIndex, setCurrentIndex] = useState(0);
  const currentTicketBase = tickets[currentIndex];

  // On-Demand Handshake for the active ticket
  const { data: detailTicket, isLoading: isDetailLoading } = useTicketDetailQuery(
    profile?.uid,
    currentTicketBase?.ticketId,
    { enabled: !!currentTicketBase?.ticketId && !currentTicketBase?.isTransferPending },
  );
    ...
    <AnimatePresence>
      {isOpen && ticket && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 flex items-center justify-center p-0 md:p-8"
          style={{ zIndex: 10000 }}
          onClick={onClose}
        >
          <motion.div
            ...
            <div className="rounded-none md:rounded-[44px] h-full w-full p-4 md:p-6 pt-8 sm:pt-10 md:pt-6 flex flex-col items-center bg-white/40 dark:bg-black/40 backdrop-blur-3xl border-none md:border md:border-white/60 dark:md:border-white/20 overflow-y-auto">
              ...
              <div className="relative w-full flex items-center justify-center py-4">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={currentTicket.ticketId || `ticket-${currentIndex}`}
                              ...
                              {currentTicket.assignment?.avatar ? (
                                ...
                                <span className="text-xl font-black text-iris">
                                  {currentTicket.assignment?.userName?.charAt(0) ||
                                    currentTicket.assignment?.redeemerId
                                      ?.slice(0, 1)
                                      .toUpperCase() ||
                                    'G'}
                                </span>
                        ...
                        <>
                          {isDetailLoading ? (
                            <div className="flex flex-col items-center justify-center gap-4 py-8">
                              <div className="animate-pulse w-[160px] h-[160px] rounded-2xl bg-black/5" />
                              <p className="text-[8px] font-black text-black/20 uppercase tracking-widest">
                                Verifying Identity...
                              </p>
                            </div>
                            ...
                            <>
                              ...
                              >
                                <IdentityQR
                                  entitlementId={
                                    currentTicket.qrPayload || currentTicket.entitlementId
                                  }
                  ...
                  {currentTicket.isClaimedByOther ? (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-iris/5 border border-iris/10">
                      <div className="w-5 h-5 rounded-full overflow-hidden border border-iris/20 bg-iris/5 relative">
                        <ShimmerImage
                          src={
                            currentTicket.assignment?.avatar || currentTicket.assignment?.photoURL
                          }
