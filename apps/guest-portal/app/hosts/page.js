...
export default function DiscoveryPage() {
  ...
  const { results, status: fetchStatus, error, fetchData, hasMore } = useHostsStore();
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#030303] transition-colors duration-300">
      {/* Hero Section */}
          ...
          <div className="flex p-1 bg-black/5 dark:bg-white/5 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-full w-fit mx-auto relative z-10 transition-colors">
            {["explore", "hosts"].map((tab) => (
              <motion.button
                ...
                className={`relative px-10 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${
                  (activeTab === "venues" && tab === "explore") ||
                  (activeTab === "hosts" && tab === "hosts")
      ...
      {/* Search Row - Sticky. Adjusted top for Navbar (approx 80-100px) */}
      <div className="sticky top-[72px] sm:top-[88px] z-40 bg-white/80 dark:bg-[#030303]/80 backdrop-blur-xl border-y border-black/5 dark:border-white/5 py-4 shadow-sm transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative group">
            <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none">
              <Search
                size={20}
                className="text-black/20 dark:text-white/20 group-focus-within:text-orange transition-colors"
              />
            </div>
            <input
              type="text"
      ...
      {/* Filters Row - Sticky. Adjusted top for Search Row height (approx 80px) */}
      <div className="sticky top-[164px] sm:top-[180px] z-30 bg-white/50 dark:bg-[#030303]/50 backdrop-blur-md border-b border-black/5 dark:border-white/5 py-3 shadow-sm transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 overflow-x-auto pb-1 scrollbar-hide snap-x">
            <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[10px] font-black uppercase tracking-widest text-black/40 dark:text-white/40">
            ...
            {activeTab === "venues" ? (
              <>
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all snap-center flex-shrink-0 border ${
                    tablesOnly
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading && results.length === 0 ? (
          ...
          <div className="py-20 text-center space-y-6">
            ...
            <button
              onClick={fetchData}
              className="px-8 py-3 rounded-full bg-black dark:bg-white text-white dark:text-black text-xs font-bold uppercase tracking-widest"
            >
              Retry
            </button>
          </div>
          ...
          <div className="py-32 text-center space-y-6 max-w-md mx-auto">
            ...
            <button
              onClick={clearAll}
              className="px-10 py-4 rounded-full bg-black dark:bg-white text-white dark:text-black text-[10px] font-bold uppercase tracking-widest shadow-glow"
            >
              Reset {activeTab === "venues" ? "Explore" : "Hosts"}
            </button>
          </div>
        ) : (
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all snap-center flex-shrink-0 border ${
            active === item
              ...
              : "bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-black/50 dark:text-white/50 hover:bg-black/10 dark:hover:bg-white/10 hover:text-black dark:hover:text-white"
          }`}
        >
          {item}
        </button>
