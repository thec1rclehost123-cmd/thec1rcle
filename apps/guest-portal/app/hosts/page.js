...
export default function DiscoveryPage() {
    ...
    const hasActiveFilters = activeArea || activeVibe || activeRole || activeStatus || search || tablesOnly;
                ...
                <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 space-y-3">
                    ...
                    <div className="relative group">
                        <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none">
                            <Search size={18} className="text-black/20 dark:text-white/20 group-focus-within:text-[#F44A22] transition-colors" />
                        </div>
                        <input
                            type="text"
                            value={search}
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide snap-x">
                        ...
                        {isVenues ? (
                            <>
                                <FilterChip active={tablesOnly} onClick={() => setTablesOnly(!tablesOnly)} label="Tables Available" accent />
                    ...
                    {!loading && results.length > 0 && (
                        <div className="flex items-center justify-between">
                            ...
                            <div className="inline-flex gap-1.5 items-baseline px-5 py-2.5 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10">
                                <p className="text-xl font-black bg-gradient-to-r from-[#F44A22] to-[#FF6B4A] bg-clip-text text-transparent">{results.length}</p>
                                <p className="text-[9px] font-bold uppercase tracking-widest text-black/50 dark:text-white/50">Found</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="min-h-[400px]">
                        {loading && results.length === 0 ? (
                            ...
                            <div className="rounded-[32px] border border-black/10 dark:border-white/10 bg-gradient-to-br from-black/5 to-transparent dark:from-white/5 dark:to-transparent p-16 text-center space-y-5">
                                ...
                                <h3 className="text-3xl font-heading font-black text-black dark:text-white uppercase tracking-tighter">Nothing here</h3>
