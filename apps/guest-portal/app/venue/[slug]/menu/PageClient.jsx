...
export default function VenueMenuPageClient() {
  ...
  if (status !== "ready" || !venue) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-black px-6 text-center text-white">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-orange">Menu unavailable</p>
          <h1 className="mt-4 text-3xl font-black uppercase tracking-tight">We could not load this venue menu.</h1>
        </div>
      </div>
    );
  }
  ...
  return <MenuClient venue={venue} menu={data?.menu || venue.menuDoc || venue.menu || null} slug={slug} />;
}
