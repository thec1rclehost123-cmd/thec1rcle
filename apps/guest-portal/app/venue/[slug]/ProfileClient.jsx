              ...
              <div className="relative p-1 rounded-full border-2 border-emerald-500/30 group-hover:border-emerald-500 transition-all">
                ...
                >
                  {h.imageUrl ? (
                    ...
                    <Camera
                      className="h-6 w-6 text-[var(--text-muted)] group-hover:text-[var(--text-primary)] transition-colors"
        ...
        <div className="flex p-1 bg-[var(--surface-2)] rounded-2xl border border-[var(--border-secondary)] min-w-max mx-auto w-fit">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 py-2.5 px-3 sm:px-6 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all ${
                activeTab === tab.id
          ...
          {activeTab === 'events' && (
            <div className="space-y-12" id="events-section">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {upcomingEvents
                      ...
                      .map((event) => (
                        <div
                          key={event.id}
                          className="relative group cursor-pointer overflow-hidden rounded-[3rem] border border-white/10 aspect-[16/9]"
                        >
                          <Image
                          ...
                          <div className="absolute bottom-4 sm:bottom-8 left-4 sm:left-10 right-4 sm:right-10 flex items-end justify-between gap-3">
                            <div>
                              <span className="px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[9px] font-black uppercase tracking-widest text-white mb-3 inline-block">
                                Featured
                              </span>
                            ...
                            <button className="shrink-0 px-4 sm:px-6 py-2 sm:py-3 bg-white text-black rounded-full text-[10px] font-black uppercase tracking-widest">
                              Book Now
                            </button>
                          </div>
                        </div>
              ...
              <div className="flex gap-2">
                <div className="px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all border bg-[var(--text-primary)] text-[var(--bg-color)] border-[var(--text-primary)]">
          ...
          {activeTab === 'posts' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <div
                  key={post.id}
                  className="p-0 rounded-[2.5rem] bg-[var(--surface-1)] border border-[var(--border-primary)] overflow-hidden hover:border-[var(--text-muted)] transition-all group shadow-sm hover:shadow-md"
                >
                  {post.imageUrl && (
            ...
            <div className="space-y-12">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                  ...
                  <p className="text-[12px] text-[var(--text-muted)] font-bold uppercase tracking-widest mt-2">
                    Visual journey of {venue.displayName || venue.name}
                  </p>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-16">
                <section className="space-y-8">
                  ...
                  <p className="text-xl md:text-2xl text-[var(--text-primary)] leading-relaxed font-medium">
                    {venue.bio ||
                      venue.description ||
                      'A space curated for exceptional experiences.'}
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <section className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {venue.genres?.map((genre) => (
                        <span
                          key={genre}
                          className="px-4 py-1.5 rounded-full border border-[var(--border-primary)] text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]"
                        >
                          {genre}
                        </span>
                      ))}
                  ...
                  <section className="space-y-4">
                    ...
                    <p className="text-[13px] font-bold text-[var(--text-secondary)] uppercase tracking-wide">
                      Standard • 6:00 PM - 4:00 AM
                    </p>
                  </section>
                </div>
              </div>
                ...
                <div className="p-10 rounded-[2.5rem] bg-[var(--surface-2)] border border-[var(--border-secondary)] space-y-10">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-5">
                      ...
                      <a
                        href={`mailto:${venue.email || 'hello@c1rcle.in'}`}
                        className="flex items-center gap-4 text-[13px] font-bold text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors group"
                      >
