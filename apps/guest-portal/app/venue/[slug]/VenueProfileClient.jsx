            ...
            {genres.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {genres.slice(0, 4).map((g) => (
                  <span
                    key={g}
                    className="px-2.5 py-1 rounded-full bg-white/10 backdrop-blur-sm text-[9px] font-bold uppercase tracking-widest text-white/60"
                  >
                    {g}
                  </span>
                ))}
            ...
            <div className="flex flex-wrap gap-2">
              {facilities.map((f, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-black/[0.04] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 text-[10px] font-bold uppercase tracking-widest text-black/60 dark:text-white/50"
                  >
                    <FacIcon size={12} />
                    {f.name || f}
                  </div>
        ...
        {upcomingEvents.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-5">
              ...
              <Link
                href={`/explore?venue=${venue.slug || venue.id}`}
                className="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white transition-colors"
              >
                See all
                <svg
