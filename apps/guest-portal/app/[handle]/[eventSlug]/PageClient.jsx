...
import { fetchPromoterByUsername, fetchPromoterVanityLink, fetchPublicEvent } from "../../../features/discovery/publicDiscovery";
    ...
    async function resolveRoute() {
      ...
      try {
        ...
        if (vanityLink?.eventId && vanityLink?.code) {
          router.replace(`/event/${encodeURIComponent(vanityLink.eventSlug || vanityLink.eventId)}${buildRedirectQuery(searchParams, { ref: vanityLink.code })}`);
          return;
        }
        ...
        if (!promoterResult?.promoter || !event || !PUBLIC_LIFECYCLE_STATES.includes(event.lifecycle)) {
          setStatus("missing");
          return;
        }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#050508] px-6 text-center text-white">
      <div>
        ...
        <h1 className="mt-4 text-3xl font-black tracking-tight">
          {status === "missing" ? "We could not open this promoter link." : "Taking you to the event."}
        </h1>
      </div>
    </div>
  );
