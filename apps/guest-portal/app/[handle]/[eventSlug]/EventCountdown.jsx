...
function CountdownCell({ value, label, accent = false }) {
    return (
        <div className={`relative overflow-hidden rounded-[1.6rem] border px-4 py-4 sm:px-5 sm:py-5 ${accent ? "border-white/18 bg-white/[0.09]" : "border-white/10 bg-black/20"}`}>
            <div className="absolute inset-x-3 top-0 h-px bg-gradient-to-r from-transparent via-white/30 to-transparent" />
            <div className={`text-[2rem] sm:text-[2.8rem] font-black tracking-[-0.08em] leading-none ${accent ? "text-white" : "text-white/88"}`}>
                {pad(value)}
            </div>
...
export default function EventCountdown({ startDate, title }) {
    ...
    const eyebrow = state.status === "live" ? "In Motion" : state.status === "ended" ? "Night Archived" : "Countdown";
    ...
    return (
        <section
            ...
            style={{
                boxShadow: "0 20px 60px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.05), 0 0 80px rgba(139,92,246,0.12)",
        ...
        >
            ...
            <div className="relative">
                <div className="flex items-start justify-between gap-4">
                    ...
                    <div className={`mt-1 flex h-3 w-3 flex-shrink-0 rounded-full ${state.status === "live" ? "bg-emerald-400 shadow-[0_0_20px_rgba(52,211,153,0.8)]" : "bg-violet-400 shadow-[0_0_20px_rgba(167,139,250,0.7)]"}`} />
                </div>

                <div className="mt-3 text-[11px] uppercase tracking-[0.26em] text-white/45">
                    {detail}
                </div>
                ...
                {state.status === "upcoming" ? (
                    ...
                    <div className="mt-6 rounded-[1.4rem] border border-white/10 bg-black/20 px-4 py-5">
                        <div className="text-[0.9rem] font-semibold text-white/88">
                            {state.status === "live" ? "The event is likely happening right now." : "The countdown has closed for this event."}
                        </div>
