...
async function handler(request) {
  try {
    ...
    if (surgeStatus.status === 'surge') {
      ...
      if (!isValid) {
        return NextResponse.json(
          {
            error: 'Queue admission required during surge. Please join the waiting room.',
            code: 'SURGE_REQUIRED',
          },
          { status: 403 },
        );
    ...
    if (!payload.items || !Array.isArray(payload.items) || payload.items.length === 0) {
      return NextResponse.json({ error: 'At least one ticket item is required' }, { status: 400 });
    }
