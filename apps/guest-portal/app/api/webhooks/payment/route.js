  ...
  try {
    ...
    if (eventType === 'payment.captured' || eventType === 'payment_success') {
      ...
      try {
        ...
        console.error(
          `[Webhook] Inngest dispatch failed for order ${orderId}:`,
        ...
        try {
          ...
          console.error(
            `[Webhook] Fallback email failed for order ${orderId}:`,
            emailError.message,
          );
        }
      }
    ...
    if (
      ...
      console.log(
        `[Webhook] Refund event: ${eventType} | Refund: ${refundId} | Payment: ${paymentId} | Status: ${refundStatus} | Amount: ₹${refundAmount}`,
      );
