...
const Badge = ({ label, type = 'default' }) => {
  const styles = {
    default:
      'bg-black/[0.05] dark:bg-white/10 text-black/60 dark:text-white/60 border-black/5 dark:border-white/10',
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[9px] font-bold uppercase tracking-widest backdrop-blur-md transition-shadow duration-300 ${styles[type] || styles.default}`}
    >
      {type === 'pro' && (
        <span className="h-1.5 w-1.5 rounded-full bg-orange dark:bg-gold animate-pulse" />
      )}
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            {(badges.length ? badges : ['Member']).map((badge) =>
              badge === 'Host Pending' ? (
                <span
                  key={badge}
                  className="inline-flex items-center gap-1.5 rounded-full border border-orange-500/20 bg-orange-500/5 dark:bg-orange-500/10 px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-orange-600 dark:text-orange-400 backdrop-blur-md"
                >
                  {badge}
                </span>
              ) : (
        ...
        {cultureStats?.uniqueVenues > 0 && (
          <div className="text-center md:text-right">
            ...
            <p className="mt-1 font-mono text-xl text-black dark:text-white">
              {cultureStats.uniqueVenues}
            </p>
          </div>
        )}
        <div className="text-center md:text-right">
          <p className="text-[10px] font-bold uppercase tracking-widest text-black/40 dark:text-white/40">
            {cultureStats?.prestige || 'Member'}
          </p>
          <p
    ...
    >
      <Link href={eventLink} className="aspect-[4/5] relative overflow-hidden">
        ...
        <div className="absolute left-4 top-4">
          <span
            className={`rounded-full px-3 py-1 text-[8px] font-bold uppercase tracking-widest backdrop-blur-md border ${
              isUpcoming
                ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20'
                : 'bg-black/10 dark:bg-white/10 text-white dark:text-white/60 border-white/10'
            }`}
          >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex items-center justify-between pt-4 border-t border-black/5 dark:border-white/5">
          ...
          <Link
            href={
              isOwner ? (event.participationType === 'TICKET' ? ticketsLink : eventLink) : eventLink
            }
    ...
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-[var(--bg-color)]">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-heading font-black uppercase text-black dark:text-white">
            Profile Not Found
          </h1>
          <Link
            href="/explore"
            className="mt-8 inline-block rounded-full bg-black dark:bg-white px-8 py-4 text-xs font-bold uppercase tracking-widest text-white dark:text-black shadow-md"
          >
            Back to Explore
          </Link>
        </div>
            ...
            >
              {events[activeTab]?.length > 0 ? (
                ...
                <div className="py-24 text-center rounded-[40px] border border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-white/[0.02]">
                  ...
                  <Link
                    href="/explore"
                    className="mt-8 inline-block text-[10px] font-bold uppercase tracking-widest transition-opacity text-orange dark:text-white/60 hover:opacity-100"
                  >
                    Explore the C1rcle
                  </Link>
                </div>
