...
export default async function HostPublicPage({ params }) {
  ...
  const pastEvents = hostEvents
    .filter((e) => new Date(e.startDate || e.startAt) <= new Date())
    .slice(0, 6);
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {hostProfile.genres?.slice(0, 3).map((genre, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-orange/20 rounded-full text-[10px] font-bold uppercase tracking-wider text-orange"
                    >
                      {genre}
                    </span>
                  ))}
                ...
                <div className="flex gap-10 pt-4">
                  <div className="text-center">
                    <p className="text-3xl md:text-4xl font-black">
                      {(stats.followersCount || 0).toLocaleString("en-IN")}
                    </p>
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-24">
            <div className="flex flex-wrap gap-3">
              {hostProfile.styleTags.map((tag, idx) => (
                <span
                  key={idx}
                  className="px-5 py-2 bg-white/5 border border-white/10 rounded-full text-sm font-medium text-white/60"
                >
                  {tag}
                </span>
              ))}
