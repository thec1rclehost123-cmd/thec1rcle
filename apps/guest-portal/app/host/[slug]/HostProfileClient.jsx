          ...
          <div className="flex gap-8">
            ...
            {nextEventLabel && (
              <div className="hidden sm:block">
                <p className="text-sm font-black bg-gradient-to-r from-[#F44A22] to-[#FF6B4A] bg-clip-text text-transparent">
                  {nextEventLabel}
                </p>
                <p className="text-[9px] font-bold uppercase tracking-widest text-black/40 dark:text-white/40 mt-0.5">
                  Next Event
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-2 space-y-10">
            {(host.bio || host.tagline) && (
              ...
              >
                {host.tagline && (
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#F44A22] mb-3">
                    {host.tagline}
                  </p>
                )}
                {host.bio && (
                  <p className="text-base leading-relaxed text-black/65 dark:text-white/55 font-medium">
                    {host.bio}
                  </p>
                )}
              </motion.div>
            ...
            {(host.genres || host.vibes || []).length > 0 && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-[0.35em] text-black/30 dark:text-white/30">
                    Sound
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {(host.genres || host.vibes || []).map((g) => (
                    <span
                      key={g}
                      className="px-4 py-2 rounded-full bg-[#F44A22]/8 border border-[#F44A22]/20 text-[10px] font-black uppercase tracking-widest text-[#F44A22]"
                    >
                      {g}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(host.styleTags || []).map((t) => (
                    <span
                      key={t}
                      className="px-4 py-2 rounded-full bg-black/[0.04] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 text-[10px] font-bold uppercase tracking-widest text-black/50 dark:text-white/50"
                    >
                      {t}
                    </span>
                  ))}
          ...
          <div className="space-y-5">
            {nextEventLabel && (
              <div className="p-5 rounded-[24px] bg-[#F44A22]/8 border border-[#F44A22]/20">
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#F44A22] mb-2">
                  Next Event
                </p>
              ...
              {activeContentTab === 'posts' && hasPosts && (
                ...
                >
                  {(host.posts || []).map((post) => (
                    <div
                      key={post.id}
                      className="rounded-[24px] overflow-hidden border border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02]"
                    >
                      {post.imageUrl && (
                      ...
                      {post.content && (
                        <div className="p-4">
                          <p className="text-sm text-black/65 dark:text-white/55 leading-relaxed line-clamp-3 font-medium">
                            {post.content}
                          </p>
                          {post.createdAt && (
              ...
              {activeContentTab === 'highlights' && hasHighlights && (
                ...
                >
                  {(host.highlights || []).map((hl) => (
                    <div
                      key={hl.id}
                      className="rounded-[20px] overflow-hidden border border-black/10 dark:border-white/10"
                    >
                      {hl.images?.[0] && (
                        <div className="relative aspect-square">
                          ...
                          <p className="absolute bottom-3 left-3 right-3 text-[10px] font-black uppercase tracking-widest text-white truncate">
                            {hl.title}
                          </p>
                        </div>
                      )}
