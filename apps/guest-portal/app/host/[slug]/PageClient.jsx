...
import { CheckCircle2, MapPin, ExternalLink, Instagram, Music, Play, ChevronRight } from "lucide-react";
...
export default function HostPublicPageClient({ initialData = null, initialSlug = "" }) {
  ...
  const upcomingEvents = useMemo(() => (data?.upcomingEvents || []).map(normalizeEventCard), [data]);
  const pastEvents = useMemo(() => (data?.pastEvents || []).map(normalizeEventCard).slice(0, 6), [data]);
  ...
  if (status !== "ready" || !hostProfile) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0A0A0A] px-6 text-center text-white">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-orange">Host unavailable</p>
          <h1 className="mt-4 text-3xl font-black uppercase tracking-tight">We could not load this host profile.</h1>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-white selection:bg-white selection:text-black">
            ...
            <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:gap-12">
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {hostProfile.tagline ? (
                    <p className="max-w-2xl text-xl font-medium text-white/50 md:text-2xl">{hostProfile.tagline}</p>
                  ) : null}
                </div>
        ...
        <section className="border-t border-white/5 py-16">
          <div className="mx-auto max-w-7xl px-6 sm:px-12 lg:px-24">
            <div className="mb-10 flex items-center justify-between">
              ...
              <Link href="/explore" className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-white/40 transition-colors hover:text-white">
                ...
                <Link key={event.id} href={`/event/${event.id}`} className="group">
                  <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-white/10 bg-white/5 transition-all hover:border-white/20">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-6">
                      ...
                      <h3 className="mb-1 line-clamp-2 text-xl font-bold text-white">{event.name}</h3>
                      <p className="text-sm font-medium text-white/50">{event.venueName || "Venue TBA"}</p>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <ProfileClient upcomingEvents={upcomingEvents} posts={posts} highlights={highlights} venue={hostProfile} />
