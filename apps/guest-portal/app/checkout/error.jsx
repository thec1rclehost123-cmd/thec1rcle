...
export default function CheckoutError({ error, reset }) {
    ...
    // Payment-specific: do NOT offer "Try Again" — could cause double-charge confusion.
    // Direct user to check their tickets first, then contact support.
    return (
        <motion.div
        ...
        >
            <div className="max-w-md w-full space-y-6">
                ...
                <div className="space-y-2">
                    <h2 className="text-2xl font-black text-white tracking-tight">Checkout interrupted</h2>
                    <p className="text-sm text-white/50 font-medium leading-relaxed">
                        Something went wrong during checkout. Before trying again, check your tickets — your order may have already been confirmed.
                    </p>
                    {error?.digest && (
                        <p className="text-[11px] text-white/20 font-mono mt-2">Ref: {error.digest}</p>
                    )}
                </div>
