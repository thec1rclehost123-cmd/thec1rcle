...
function LoginForm() {
    ...
    // cityOverride exists because setForm is async — we pass the freshly-selected city directly
    ...
    // ── City auto-transition: selecting a city immediately advances the flow ──
    const handleCitySelect = (city) => {
        setForm(prev => ({ ...prev, city }));
        if (isNewUser) {
            // handleStartRegistration uses cleanForm.phone (already set), not city
    ...
    // ── UI helpers ────────────────────────────────────────────────────────
    // Visible steps in progress dots: 6 for new/onboarding, 2 for returning users
    const totalSteps = (isNewUser || isOnboarding) ? 6 : 2;

    const stepHeading = () => {
        if (step === 1) return <>What's your <br /><span className="text-orange">Email?</span></>;
        if (step === 2) return <>And your <br /><span className="text-orange">Password?</span></>;
        ...
        if (step === 5) return <>How do you <br /><span className="text-orange">Identify?</span></>;
                                    ...
                                    <form onSubmit={(e) => { e.preventDefault(); nextStep(); }} className="space-y-8">
                                        <AnimatePresence mode="wait">
                                            ...
                                            >
                                                ...
                                                {/* Step 6 — City (auto-transitions on select) */}
                                                {(isNewUser || isOnboarding) && step === 6 && (
                                                    <div className="space-y-4">
