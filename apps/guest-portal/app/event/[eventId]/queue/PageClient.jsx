...
const WaitingRoom = () => {
  ...
  const { errorMessage, eventData, lowestPrice, queueData, refreshPage, status, waitTime } =
      ...
      <AnimatePresence mode="wait">
        {status === 'initializing' && (
          ...
          >
            ...
            <p className="text-white/40 mt-4 font-bold text-[10px] tracking-[0.3em] uppercase">
              Analyzing Behavioural Signals
            </p>
          </motion.div>
        )}
            ...
            <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-5 sm:p-8 mb-6 overflow-hidden relative">
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  <h2 className="text-xl font-black text-white uppercase tracking-tight line-clamp-1">
                    {eventData?.title}
                  </h2>
            ...
            <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[40px] p-6 sm:p-10 text-center shadow-2xl">
              ...
              <div className="mb-10 flex flex-col items-center">
                <div className="px-6 py-2 rounded-full bg-white/5 border border-white/10 text-iris font-black text-[10px] uppercase tracking-widest mb-6 flex items-center gap-3">
                  ...
                  {queueData?.tier === 'loyal'
                    ? 'Prioritized Access Lane'
                    : 'Fair Admission Active'}
                </div>
                <h1 className="text-3xl sm:text-5xl font-black uppercase tracking-tighter text-white mb-4">
                  You're in line
                </h1>
                <p className="text-white/40 font-medium text-sm max-w-sm">
                  We are processing attendees in cohorts to ensure server stability. Please keep
                  this tab active.
                </p>
              </div>

              <div className="grid grid-cols-2 gap-4 sm:gap-6 mb-10">
                <div className="bg-white/5 rounded-3xl p-5 sm:p-8 border border-white/5 group hover:border-white/10 transition-all">
                  <div className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3 text-left">
                    Lane Rank
                  </div>
                  <div className="text-3xl sm:text-5xl font-black text-white text-left tracking-tighter">
                    #{queueData?.lanePosition || '...'}
                  </div>
                </div>
                <div className="bg-white/5 rounded-3xl p-5 sm:p-8 border border-white/5 group hover:border-white/10 transition-all">
                  <div className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3 text-left">
                  ...
                  <div className="text-3xl sm:text-5xl font-black text-white text-left tracking-tighter">
                    {waitTime || '...'}
                    <span className="text-sm ml-1 opacity-20">MIN</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-iris/5 border border-iris/20 rounded-3xl p-6 mb-10 flex items-start gap-4 text-left">
                ...
                <div>
                  <div className="text-white font-black text-xs uppercase tracking-widest mb-1">
                    Fee Rules Locked
                  </div>
                  <p className="text-white/40 text-[10px] leading-relaxed">
                    Base starting price <span className="text-white font-bold">₹{lowestPrice}</span>
                    . Standard fees & taxes (+ ~9%) will be calculated at checkout based on chosen
                    tier. No bot scalp-markup.
                  </p>
                </div>
              </div>
        ...
        {status === 'admitted' && (
          ...
          >
            ...
            <p className="text-white/50 text-sm font-medium tracking-wide max-w-xs mx-auto">
              {queueData?.status === 'payment_failed'
                ? 'Payment Failure Detected. Retry window restored for 3 minutes.'
                : 'Your unique admission key is active. Redirecting to checkout...'}
            </p>
          </motion.div>
        )}
        ...
        {status === 'expired' && (
          ...
          >
            ...
            <h1 className="text-white text-3xl font-black uppercase tracking-tight mb-4">
              Session Expired
            </h1>
