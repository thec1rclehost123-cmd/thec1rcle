...
import {
  ...
  TicketsGuestView,
} from '../../features/tickets/TicketsGuestExperience';
...
function TicketsContent() {
  ...
  const ticketStatus = ticketsQuery.isSuccess
    ...
    : ticketsQuery.isError
      ...
      : ticketsQuery.isLoading
        ...
        : 'idle';
  const loading =
    Boolean(user) && (ticketsQuery.isLoading || (ticketsQuery.isFetching && !ticketsQuery.data));
  ...
  const activePoster =
    tickets.upcomingTickets?.[0]?.posterUrl || tickets.actionNeeded?.[0]?.posterUrl;
            ...
            >
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-orange">
                    Incomplete Payment
                  </p>
            ...
            <AnimatePresence mode="wait" key="state-presence">
              <motion.div
                key={
                  ...
                  Object.keys(tickets.coverWalletsByOrder || {}).length === 0
                    ? 'empty'
                    : 'content')
                }
                  ...
                  <div className="flex flex-col gap-1 w-full">
                    ...
                    <div className="flex flex-col gap-10 max-w-2xl mx-auto w-full">
                      {(activeTab === 'upcoming'
                        ...
                        return (
                          <div
                            key={ticket.ticketId || ticket.orderId || `ticket-${idx}`}
                            className="w-full"
                          >
                            <TicketCard
                              ticket={ticket}
                            ...
                            {activeTab === 'upcoming' &&
                              ...
                              ticket.tickets?.every((t) => !t.qrPayload && !t.entitlementId) && (
                                ...
                                >
                                  <RefreshCw
                                    className={`h-3 w-3 ${reissuingOrderId === ticket.orderId ? 'animate-spin' : ''}`}
                                  />
                                  {reissuingOrderId === ticket.orderId
                                    ? 'Sending...'
                                    : 'Re-send Tickets'}
                                </button>
                              )}
