    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 right-4 z-20">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange mb-0.5">
                {host.role || "Host"}
              </p>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {(host.genres || host.vibes)?.slice(0, 3).map((genre) => (
              <span
                key={genre}
                className="px-2 py-0.5 rounded-md bg-orange/20 backdrop-blur-md text-[9px] font-bold uppercase tracking-widest text-orange"
              >
                {genre}
              </span>
            ))}
            {host.styleTags?.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 rounded-md bg-white/10 backdrop-blur-md text-[9px] font-bold uppercase tracking-widest text-white/60"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="flex items-center justify-between p-6 border-b border-black/5 dark:border-white/5 bg-black/[0.03] dark:bg-black/20">
        <div className="flex flex-col">
          ...
          <span className="text-[9px] uppercase font-bold tracking-widest text-black/40 dark:text-white/40">
            Followers
          </span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-sm font-black text-black dark:text-white">
            {host.upcomingEventsCount}
          </span>
          <span className="text-[9px] uppercase font-bold tracking-widest text-black/40 dark:text-white/40">
            Events
          </span>
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[32px] border border-black/5 dark:border-white/10 bg-[#F2F2F7] dark:bg-[#121212]">
      ...
      <div className="p-6 border-b border-black/5 dark:border-white/5 flex justify-between">
        ...
        <div className="space-y-2">
          <div className="h-4 w-8 bg-black/10 dark:bg-white/10 rounded" />
          <div className="h-2 w-12 bg-black/10 dark:bg-white/10 rounded" />
        </div>
