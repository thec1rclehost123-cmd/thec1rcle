...
const HeroCarousel3D = dynamic(() => import('./HeroCarousel3D'), {
  ssr: false,
  loading: () => (
    <div className="h-[600px] w-full bg-black/20 animate-pulse flex items-center justify-center text-white/20 uppercase tracking-widest text-xs">
...
export default function HeroCarousel({ cards = [] }) {
  return (
    <section className="relative w-full min-h-[300px] sm:min-h-[420px] md:min-h-[620px] lg:min-h-[720px] bg-[#FAFAF9] dark:bg-[#050505] flex flex-col items-center pt-6 pb-8 md:pt-10 md:pb-12 overflow-hidden">
      ...
      {/* Heading Block */}
      <div className="flex flex-col items-center justify-center text-center z-20 mb-4 md:mb-6 relative px-4">
        <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-black/[0.08] dark:border-white/10 bg-black/[0.04] dark:bg-white/5 backdrop-blur-md text-[10px] tracking-[0.3em] text-[#ff4b1f] font-black uppercase mb-4 shadow-sm dark:shadow-2xl">
