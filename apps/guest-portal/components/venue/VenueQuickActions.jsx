        ...
        <div className="flex items-center justify-center gap-3 py-6 overflow-x-auto no-scrollbar">
            {visibleActions.map((action) => (
                <button
                    ...
                    className={`flex flex-col items-center gap-2 min-w-[72px] py-3 px-4 rounded-2xl border transition-all duration-300 hover:scale-105 ${action.active
                ...
                >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${action.active
                        ? action.activeBg
                        : action.activeBg || 'bg-black/5 dark:bg-white/5'
                        }`}>
                        <action.icon
                            className={`h-5 w-5 transition-colors ${action.active
                                ? `${action.activeColor} ${action.fillActive ? 'fill-current' : ''}`
                                : action.activeColor || 'text-black/60 dark:text-white/60'
                                }`}
                        />
                    </div>
                    <span className={`text-[9px] font-bold uppercase tracking-widest whitespace-nowrap ${action.active ? action.activeColor : 'text-black/50 dark:text-white/50'
                        }`}>
                        {action.label}
                    </span>
                </button>
