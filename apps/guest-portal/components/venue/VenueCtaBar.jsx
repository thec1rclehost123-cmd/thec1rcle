...
export default function VenueCtaBar({
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 z-40 md:hidden transition-all duration-500 ${visible ? "translate-y-0" : "translate-y-full"}`}
      >
        <div className="bg-white/90 dark:bg-[#0A0A0A]/90 backdrop-blur-2xl border-t border-black/10 dark:border-white/10 p-4 safe-area-bottom">
          <div className="flex items-center gap-3">
            ...
            >
              <ChevronDown
                className={`h-5 w-5 text-black/60 dark:text-white/60 transition-transform ${expanded ? "rotate-180" : ""}`}
              />
            </button>
          </div>
          ...
          {expanded && (
            <div className="grid grid-cols-4 gap-3 mt-4 pt-4 border-t border-black/5 dark:border-white/5">
              <button onClick={openCall} className="flex flex-col items-center gap-2 py-3">
                ...
                <span className="text-[9px] font-bold uppercase text-black/40 dark:text-white/40">
                  Call
                </span>
              </button>
      ...
      <div
        className={`hidden md:block fixed bottom-8 left-1/2 -translate-x-1/2 z-40 transition-all duration-500 ${visible ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0 pointer-events-none"}`}
      >
        <div className="flex items-center gap-4 p-3 bg-white/90 dark:bg-[#161616]/90 backdrop-blur-2xl rounded-full border border-black/10 dark:border-white/10 shadow-2xl">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-[#25D366]/10 flex items-center justify-center hover:bg-[#25D366]/20 transition-colors"
          >
          ...
          {instagram && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500/10 to-pink-500/10 flex items-center justify-center hover:from-purple-500/20 hover:to-pink-500/20 transition-colors"
            >
