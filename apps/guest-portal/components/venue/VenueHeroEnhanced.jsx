...
import { CheckCircle2, Sparkles, Zap, Flame, Star, Clock, Users, Crown } from "lucide-react";
...
function DynamicBadge({ type }) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-4 py-2.5 backdrop-blur-xl border rounded-full shadow-lg ${config.bgClass}`}
    >
      {config.animate && (
...
export default function VenueHeroEnhanced({ venue, video = null, badges = [], stats = null }) {
  ...
  if (venue.reservationOnly || badges.includes("reservationOnly"))
    activeBadges.push("reservationOnly");
