...
export default function VenueHighlights({ highlights = [], venueName = "Venue" }) {
  ...
  const validHighlights = highlights.filter(
    (h) => h && (h.stories?.length > 0 || h.coverImage || h.imageUrl),
  );
  ...
  const goToNextStory = () => {
    ...
    const stories = activeHighlight.stories || [
      { type: "image", url: activeHighlight.coverImage || activeHighlight.imageUrl },
    ];
  ...
  const goToPrevStory = () => {
    if (activeStoryIndex > 0) {
      ...
      if (currentHighlightIndex > 0) {
        ...
        const prevStories = prevHighlight.stories || [
          { type: "image", url: prevHighlight.coverImage || prevHighlight.imageUrl },
        ];
              ...
              >
                <Heart
                  className={`h-5 w-5 transition-colors ${
                    liked[`${activeHighlight.id}-${activeStoryIndex}`]
                      ? "text-red-500 fill-current"
                      : "text-white"
                  }`}
                />
                <span className="text-xs font-bold text-white">
                  {(activeHighlight.likes || 0) +
                    (liked[`${activeHighlight.id}-${activeStoryIndex}`] ? 1 : 0)}
                </span>
              </button>
