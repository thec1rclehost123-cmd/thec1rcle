...
export default function VenueRestaurantSection({
  ...
  const displayTags =
    cuisineTags.length > 0 ? cuisineTags : ['Contemporary', 'Bar Bites', 'Fusion'];
  ...
  return (
    <>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-20">
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-amber-600 dark:text-amber-400">
                  Culinary Experience
                </span>
              </div>
              <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-8xl font-heading font-black uppercase tracking-tighter leading-none text-black dark:text-white">
                Taste
                <br />
                <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-orange-500">
                  Excellence.
                </span>
              </h2>
            </div>
            ...
            <div className="flex flex-col items-start lg:items-end gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {displayTags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="px-4 py-2 bg-black/[0.02] dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded-full text-[10px] font-bold uppercase tracking-widest text-black/60 dark:text-white/60"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {displayDishes.map((dish, idx) => (
                  <div
                    key={idx}
                    className="group relative rounded-[2rem] overflow-hidden border border-black/5 dark:border-white/5 shadow-lg hover:shadow-xl transition-all"
                  >
      ...
      {showMenuPreview && (
        ...
        >
          <div
            className="relative max-w-2xl w-full max-h-[90vh] bg-white dark:bg-[#161616] rounded-[2rem] overflow-hidden"
