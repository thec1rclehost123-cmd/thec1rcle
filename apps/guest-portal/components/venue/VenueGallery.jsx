            ...
            <section className="px-4 sm:px-6 md:px-12 lg:px-24 py-8 sm:py-12">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-[#F44A22] block mb-2">Gallery</span>
                ...
                <div className="fixed inset-0 z-[100] bg-black flex flex-col">
                    ...
                    <div className="flex items-center justify-between p-4">
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${likedPhotos[selectedIndex]
                                    ? 'bg-red-500/20'
                                    : 'bg-white/10 hover:bg-white/20'
                                    }`}
                            >
                                <Heart className={`h-5 w-5 ${likedPhotos[selectedIndex] ? 'text-red-500 fill-current' : 'text-white'}`} />
                            </button>
                            <button
                                onClick={closeViewer}
                    ...
                    <div className="p-4 overflow-x-auto">
                        <div className="flex items-center justify-center gap-2">
                            {validPhotos.slice(Math.max(0, selectedIndex - 4), selectedIndex + 5).map((photo, idx) => {
                                const actualIndex = Math.max(0, selectedIndex - 4) + idx;
                                return (
                                    <button
                                        key={actualIndex}
                                        onClick={() => setSelectedIndex(actualIndex)}
                                        className={`relative w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-all ${actualIndex === selectedIndex ? 'border-white' : 'border-transparent opacity-50 hover:opacity-100'
                                            }`}
                                    >
                                        <Image
                                            src={photo}
