 ...
 */
export default function VenueActionCards({ venueId, upcomingEvents = [], menuImages = [] }) {
                ...
                >
                  ...
                  <div className="absolute top-4 left-4 flex flex-col items-center justify-center w-12 h-12 bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl">
                    <span className="text-[9px] font-black text-white/60 uppercase">
                      {new Date(event.startDate || event.startAt).toLocaleDateString(undefined, {
                        month: 'short',
                      })}
                    </span>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 space-y-2">
                    <div className="flex items-center gap-2 text-white/50">
                      ...
                      <span className="text-[10px] font-bold">
                        {new Date(event.startDate || event.startAt).toLocaleTimeString(undefined, {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </span>
          ...
          <div className="flex-1 relative flex items-center justify-center p-4">
            ...
            {menuImages.length > 1 && (
              <>
                ...
                <button
                  onClick={() =>
                    setCurrentMenuIndex((prev) => Math.min(menuImages.length - 1, prev + 1))
                  }
          ...
          {menuImages.length > 1 && (
            <div className="flex items-center justify-center gap-2 p-4 border-t border-white/10 overflow-x-auto">
              {menuImages.map((img, idx) => (
                ...
                >
                  <Image src={img} fill className="object-cover" alt={`Menu page ${idx + 1}`} />
                </button>
              ))}
            </div>
          )}
