...
export default function VerifyPanel({ phone, onVerify, onResend, onEdit, loading, error }) {
    ...
    return (
        <div className="space-y-6 w-full max-w-[500px]">
            <div className="flex justify-between items-center px-2">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-orange">Identity Verification</p>
                <button
                    onClick={onEdit}
            ...
            {(localError || error) && (
                ...
                >
                    ...
                    <p className="text-[10px] font-bold text-orange uppercase tracking-widest">{localError || error}</p>
                </motion.div>
            )}
            ...
            <button
                ...
                className={clsx(
                    "group relative w-full h-16 flex items-center justify-center rounded-full font-black uppercase tracking-[0.5em] text-xs transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 overflow-hidden shadow-[0_20px_40px_rgba(255,165,0,0.15)]",
                    phoneVerified ? "bg-white text-black" : "bg-white/5 text-white/20 border border-white/10"
                )}
            >
                {loading ? (
                    ...
                    <span className="relative z-10">
                        {phoneVerified ? "Finalize Ritual" : "Awaiting Verification"}
                    </span>
                )}
            </button>
        </div>
...
function VerificationCard({ title, identifier, icon, verified, loading, value, onChange, onComplete, onResend }) {
    return (
        <motion.div
