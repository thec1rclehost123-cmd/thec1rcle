    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="glass-panel card-hover mx-auto mt-40 w-full max-w-xs rounded-3xl p-6 text-center" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex justify-center space-x-2">
          {[1,2,3,4,5].map((item) => (
            <div key={item} className="h-10 w-10 overflow-hidden rounded-full bg-black/10 dark:bg-white/10" />
          ))}
        </div>
        <p className="mb-4 text-sm text-black/70 dark:text-white/70">Buy tickets to see who's going</p>
