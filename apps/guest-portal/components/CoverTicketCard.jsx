  ...
  return (
    <div className="mt-4 pt-4 border-t border-white/10">
      ...
      <div className="flex items-baseline gap-1.5 mt-2">
        ...
        <span className="text-xs text-white/35">/ {formatPaise(wallet.openingBalancePaise)}</span>
        {countdown && <span className="ml-auto text-[10px] text-white/30">{countdown}</span>}
        {!countdown && !isExpired && (
          <span className="ml-auto text-[10px] text-white/30">
            {formatExpiryLabel(wallet.rules?.terminationTime)}
          </span>
        )}
        {isExpired && <span className="ml-auto text-[10px] text-white/30">Expired</span>}
      </div>
      ...
      {showHistory && wallet.rules?.showTransactionHistory && <TxnHistory walletId={walletId} />}

      {/* Disclosure notice */}
      <p className="mt-3 text-[10px] text-white/25 leading-relaxed">
        Cover Wallet credit is valid only at this venue during this event. Unused balance expires at{' '}
        {formatExpiryLabel(wallet.rules?.terminationTime)} and is non-refundable.
      </p>
    </div>
  );
}
      ...
      {txns.map((txn) => (
        <div key={txn.id} className="flex justify-between items-center text-xs">
          <span className="text-white/60">
            ...
            {txn.operatorName && <span className="text-white/30 ml-1">· {txn.operatorName}</span>}
          </span>
