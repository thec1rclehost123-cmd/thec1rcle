    ...
    <section className="mx-auto mb-16 max-w-7xl px-4 pt-8 sm:mb-32 sm:px-6 sm:pt-12">
      ...
      <div className="flex flex-wrap justify-center gap-10">
        {items.map((item, index) => {
          ...
          return (
            <div key={item.title} className="w-full sm:max-w-md lg:max-w-[420px] flex-1">
              <Link
                href={item.href}
                className="group relative block aspect-[4/5] overflow-hidden rounded-[40px] border border-white/10 bg-[#0A0A0A] btn-lift shadow-2xl"
              >
                <Image
                  src={item.image}
                  alt={item.title}
