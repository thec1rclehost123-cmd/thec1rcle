...
const SHARE_OPTIONS = [
    ...
    { id: "instagram", icon: "logo-instagram", label: "Instagram", gradient: ["#833AB4", "#E1306C", "#FD1D1D"] },
        ...
        <View>
            {tiers.length > 0 ? (
                    ...
                    {tiers.map((tier) => (
                        ...
                        >
                            <View>
                                ...
                                {tier.gender ? <Text style={styles.tierMeta}>{tier.gender.toUpperCase()} only</Text> : null}
                            </View>
                ...
                <View style={styles.banner}>
                    ...
                    <Text style={styles.bannerText}>No unclaimed tickets are available to share from this order.</Text>
                </View>
            )}
            ...
            <View style={styles.shareGrid}>
                {SHARE_OPTIONS.map((option) => (
                    ...
                    >
                        <LinearGradient colors={option.gradient as unknown as [string, string, ...string[]]} style={styles.iconCircle}>
                            <Ionicons name={option.icon as any} size={26} color="#fff" />
                        </LinearGradient>
                        <Text style={styles.shareLabel}>{loading === option.id ? "Working..." : option.label}</Text>
                    </Pressable>
                ))}
            </View>
        </View>
...
export function TransferSheetContent({
    ...
    return (
        <View>
            ...
            <View style={styles.segmentRow}>
                <Pressable onPress={() => setMode("email")} style={[styles.segment, mode === "email" && styles.segmentActive]}>
                    <Text style={[styles.segmentText, mode === "email" && styles.segmentTextActive]}>Email</Text>
                </Pressable>
                <Pressable onPress={() => setMode("link")} style={[styles.segment, mode === "link" && styles.segmentActive]}>
                    <Text style={[styles.segmentText, mode === "link" && styles.segmentTextActive]}>Share Link</Text>
                </Pressable>
            </View>
            ...
            {mode === "email" ? (
                <View>
                    <View style={styles.banner}>
                        ...
                        <Text style={styles.bannerText}>Send a direct ownership transfer to a specific email address.</Text>
                    </View>
                    <TextInput
                        value={email}
                        ...
                        style={styles.input}
                    />
                    <Pressable onPress={() => void handleEmail()} disabled={!!loading} style={[styles.primaryButton, loading === "email" && styles.disabledButton]}>
                        <Text style={styles.primaryButtonText}>{loading === "email" ? "Sending..." : "Send Transfer"}</Text>
                    </Pressable>
                </View>
            ) : (
                <View>
                    <View style={styles.banner}>
                        ...
                        <Text style={styles.bannerText}>Generate a shareable transfer link for WhatsApp, SMS, or DM.</Text>
                    </View>
                    <Pressable onPress={() => void handleLink()} disabled={!!loading} style={[styles.primaryButton, loading === "link" && styles.disabledButton]}>
                        <Text style={styles.primaryButtonText}>{loading === "link" ? "Generating..." : "Generate Link"}</Text>
                    </Pressable>
                </View>
            )}
        </View>
