...
import { View, Text, ScrollView, StyleSheet, Pressable, Dimensions, FlatList, Image as RNImage } from "react-native";
...
export const MOCK_VENUES = [
    { id: "1", name: "Kiki", area: "Koregaon Park", type: "Premium Lounge", logo: "https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?w=200" },
    { id: "2", name: "Cobbler & Crew", area: "Baner", type: "Cocktail Bar", logo: "https://images.unsplash.com/photo-1566737236500-c8ac43014a67?w=200" },
    { id: "3", name: "Pune 14", area: "Viman Nagar", type: "Club", logo: "https://images.unsplash.com/photo-1541086095944-f4b5412d3666?w=200" },
    { id: "4", name: "House of Medici", area: "Mundhwa", type: "Luxury Club", logo: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=200" },
    { id: "5", name: "Toit", area: "Kalyani Nagar", type: "Brewery", logo: "https://images.unsplash.com/photo-1532635224-cf024e66d122?w=200" },
];
...
export function ScenesWorthIt({ events }: { events: Event[] }) {
    ...
    return (
        <View style={styles.section}>
            <SectionHeader title="Worth The Hype" onViewAll={() => router.push({ pathname: "/events/feed", params: { type: "free" } })} />
            <HypeCashPill />
            <FlatList
                bounces={false}
                ...
                renderItem={({ item, index }) => <PremiumEventCard event={item} index={index} variant="compact" />}
            />
        </View>
    );
}
    ...
    return (
        <View style={styles.section}>
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => <PremiumEventCard event={item} index={index} variant="large" />}
            />
        </View>
    );
}
...
export function TrendingRightNow({ events }: { events: Event[] }) {
    ...
    return (
        <View style={styles.section}>
            <SectionHeader title="Hottest Scenes" onViewAll={() => router.push({ pathname: "/events/feed", params: { type: "trending" } })} />
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 24 }}>
...
// ── 8. When Is The Plan? ──
export function WhenIsThePlan({ filter, setFilter }: { filter: string, setFilter: (f: string) => void }) {
    const filters = [
...
// ── 9. All Scenes ──
export function AllScenes({ events, onPageChange }: { events: Event[]; onPageChange?: () => void }) {
    ...
    return (
        <View style={styles.section}>
            ...
            <View style={{ paddingHorizontal: 16, flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", rowGap: 24 }}>
            ...
            {totalPages > 1 && (
                <View style={styles.paginationContainer}>
                    ...
                    <View style={styles.pageNumbersContainer}>
                        {Array.from({ length: totalPages }).map((_, i) => {
                            ...
                            return (
                                ...
                                >
                                    <Text style={[styles.pageNumberText, isSelected && styles.selectedPageNumberText]}>
                                        {pageNum}
                                    </Text>
                                </Pressable>
                            );
                    ...
                    >
                        <ChevronRight size={16} color={currentPage === totalPages ? "rgba(255,255,255,0.2)" : "#FFF"} />
                    </Pressable>
                </View>
            )}
        </View>
...
export function PremiumEventCard({ event, index, variant = "standard", hideGradient = false }: any) {
    const scale = useSharedValue(1);

    const img = event.coverImage || event.images?.[0] || "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600";
    const price = event.minPrice ? `₹${event.minPrice}` : "Free";
    const posterTransitionTag = `poster-${event.id}-${variant}-${index}`;
    
    let cardStyle: any = { width: FOR_YOU_CARD_WIDTH, height: FOR_YOU_CARD_WIDTH * 1.35, borderRadius: 16, overflow: "hidden" };
    if (variant === "featured") cardStyle = { width: SCREEN_WIDTH * 0.75, height: SCREEN_WIDTH * 0.75 * 1.35, borderRadius: 20, overflow: "hidden" };
    if (variant === "list") cardStyle = { width: "100%", height: 140, borderRadius: 16, overflow: "hidden", flexDirection: "row" };
        ...
        >
            ...
            <View style={[cardStyle, { position: "relative", backgroundColor: "transparent" }]}>
                ...
                <View style={{ flex: 1, padding: 1.2 }}>
                    ...
                    >
	                        <Animated.Image sharedTransitionTag={posterTransitionTag} source={{ uri: img }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />
                        
                        {/* Holographic / Glass Overlay */}
                        <LinearGradient colors={["rgba(244,74,34,0.15)", "rgba(254,248,232,0.05)", "transparent"]} start={{x: 0, y: 0}} end={{x: 1, y: 1}} style={StyleSheet.absoluteFillObject} />
...
const styles = StyleSheet.create({
    ...
    rankNumber: { position: "absolute", top: -30, left: -15, fontSize: 120, fontWeight: "900", color: "rgba(255,255,255,0.1)", zIndex: -1, fontStyle: "italic", letterSpacing: 0 },
    paginationContainer: {
        flexDirection: "row",
        justifyContent: "center",
        alignItems: "center",
