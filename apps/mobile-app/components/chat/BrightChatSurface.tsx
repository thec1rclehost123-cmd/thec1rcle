    ...
    const content = (
        <>
            ...
            <View style={brightChatStyles.headerCopy}>
                <Text style={brightChatStyles.headerTitle} numberOfLines={2}>{theme.title}</Text>
                <Text style={brightChatStyles.headerSubtitle} numberOfLines={1}>{theme.subtitle}</Text>
            </View>
        </>
    );
...
export const BrightMessage = memo(function BrightMessage({
    ...
    if (type === "announcement") {
        return (
            <Animated.View entering={animate ? FadeInDown.delay(Math.min(index * 18, 150)).duration(240) : undefined} style={brightChatStyles.announcement}>
    ...
    const bubble = (
        <Animated.View
            ...
            style={[brightChatStyles.messageWrap, isOwnMessage ? brightChatStyles.messageWrapOwn : brightChatStyles.messageWrapOther]}
        >
            {!isOwnMessage && senderName ? <Text style={brightChatStyles.senderName}>{senderName}</Text> : null}
            <View style={!isOwnMessage ? brightChatStyles.messageRowOther : brightChatStyles.messageRowOwn}>
                ...
                {type === "image" ? (
                    ...
                    <View style={[brightChatStyles.messageBubble, isOwnMessage ? brightChatStyles.ownBubble : brightChatStyles.otherBubble]}>
                        ...
                        {time ? <Text style={[brightChatStyles.messageTime, isOwnMessage && brightChatStyles.ownMessageTime]}>{time}</Text> : null}
                    </View>
                )}
            </View>
        </Animated.View>
...
function MessageAvatar({ senderAvatar, senderName }: { senderAvatar?: string; senderName?: string }) {
    return (
...
export function BrightComposerDock({ children, error }: { children: ReactNode; error?: string | null }) {
    return (
...
export function BrightToolButton({
    ...
    return (
        <Pressable style={[brightChatStyles.toolButton, disabled && brightChatStyles.sendButtonDisabled]} onPress={onPress} disabled={disabled}>
            {children}
        </Pressable>
    );
}
