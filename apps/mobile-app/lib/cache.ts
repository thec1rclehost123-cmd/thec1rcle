...
export async function clearAllCaches(): Promise<void> {
    try {
        await Promise.all(
            Object.values(CACHE_KEYS).map((key) => SecureStore.deleteItemAsync(key))
        );
    } catch (error) {
...
export async function getLastSyncTime(): Promise<Date | null> {
    try {
        const timestamp = await SecureStore.getItemAsync(CACHE_KEYS.lastSync);
        if (timestamp) {
...
export async function updateLastSyncTime(): Promise<void> {
    try {
        await SecureStore.setItemAsync(CACHE_KEYS.lastSync, Date.now().toString());
    } catch (error) {
