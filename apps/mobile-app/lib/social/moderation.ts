...
export async function isUserBlocked(userId: string, otherUserId: string): Promise<boolean> {
  try {
    ...
    const [snap1, snap2] = await Promise.all([getDocs(block1Query), getDocs(block2Query)]);
...
export async function getBlockedUsers(userId: string): Promise<string[]> {
  try {
    ...
    const blocksQuery = query(collection(db, "userBlocks"), where("blockerId", "==", userId));
...
export async function getPendingReports(eventId?: string): Promise<UserReport[]> {
  try {
    const db = getFirebaseDb();
    ...
    if (eventId) {
      ...
      reportsQuery = query(collection(db, "userReports"), where("status", "==", "pending"));
    }
