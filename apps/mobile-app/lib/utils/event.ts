...
export function getEventImage(event: any): string | null {
    ...
    if (!selected) {
        ...
        else if (event.image?.url && !isInternalPlaceholder(event.image.url)) selected = event.image.url;
        else if (event.cover?.url && !isInternalPlaceholder(event.cover.url)) selected = event.cover.url;
    }
 ...
 */
export const EVENT_PLACEHOLDER = "https://firebasestorage.googleapis.com/v0/b/c1rcle-staging.firebasestorage.app/o/brand%2Fplaceholder-dark.png?alt=media&token=placeholder-token";
