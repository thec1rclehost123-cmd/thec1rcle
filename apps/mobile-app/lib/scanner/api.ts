...
export function cacheCheck(key: string) {
...
export async function registerScannerDevice(
  ...
  try {
    ...
    return {
      success: false,
      error:
        err.name === 'AbortError' ? 'Device registration timed out' : 'Unable to register device',
    };
  }
}

export async function sendScannerHeartbeat(
  input: {
    eventId: string;
    eventCode: string;
    venueId?: string;
  ...
  if (cached?.result === 'valid') {
    return {
      ...
      previousScan: {
        ...
        scannedBy: { name: 'This device', role: 'door_staff' },
      },
    };
  }
...
export function recordStaffDeny(
  payload: {
    qrData: string;
    eventId: string;
    eventCode: string;
...
export async function submitDebit(
  ...
  try {
    ...
    return {
      success: false,
      error: err.name === 'AbortError' ? 'Timed out — safe to retry' : 'Network error',
    };
  }
}
...
export async function fetchWalkIns(
  ...
  try {
    const res = await fetch(
      `${SCAN_API}/walk-in?eventId=${eventId}&eventCode=${eventCode}&limit=50`,
      {
        headers: withScannerAuth(sessionToken),
        signal,
      },
...
function getMockWalkIns(): WalkInEntry[] {
  return [
    ...
    },
    {
      ...
      guestAge: 27,
      addedAt: new Date(Date.now() - 900000).toISOString(),
    },
  ];
}
