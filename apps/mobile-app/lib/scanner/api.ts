import { ScannerEventData, ScanRequest, ScanResponse, DoorEntryRequest, DoorEntryResponse, Guest } from "./types";
...
export async function fetchGuestList(eventId: string, eventCode: string): Promise<Guest[]> {
    try {
        const response = await fetch(
            `${SCAN_API}/guestlist?eventId=${eventId}&eventCode=${eventCode}`
        );
        const data = await response.json();
...
export async function searchGuests(eventId: string, eventCode: string, query: string): Promise<Guest[]> {
...
function simulateDoorEntry(request: DoorEntryRequest): DoorEntryResponse {
    ...
    return {
        ...
        qrData: JSON.stringify({
            ...
            q: request.quantity, et: request.entryType, ts: Date.now(), v: 1,
        }),
    };
}
