...
export const useEventsStore = create<EventsState>((set, get) => ({
  ...
  searchEvents: async (filters: SearchFilters) => {
    ...
    try {
      ...
      let constraints: any[] = [where("status", "in", ["published", "scheduled", "live"])];
      ...
      // Date range filter
      if (filters.dateFrom) {
        constraints.push(where("startDate", ">=", filters.dateFrom.toISOString()));
      }
      ...
      if (filters.priceMin !== undefined || filters.priceMax !== undefined) {
        results = results.filter((event) => {
          const minPrice = Math.min(...(event.tickets?.map((t) => t.price) || [Infinity]));
