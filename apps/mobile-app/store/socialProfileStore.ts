...
interface SocialProfileState {
    ...
    completeSetup: (
        userId: string,
        data: Pick<SocialProfile,
            "photos" | "primaryPhotoIndex" | "city" | "interestedIn" |
            "ageRangeMin" | "ageRangeMax" | "height" | "smoking" | "drinking" |
            "sexuality" | "lookingFor" | "isVisible"
        >
    ) => Promise<void>;

    submitVerification: (userId: string, selfieUri: string) => Promise<{ success: boolean; message: string }>;
...
export const useSocialProfileStore = create<SocialProfileState>((set, get) => ({
    ...
    submitVerification: async (userId, selfieUri) => {
        try {
            ...
            await addDoc(
                collection(getDb(), `verificationAttempts/${userId}/attempts`),
                {
                    userId,
                    selfieUrl,
