...
describe('processQRScan', () => {
    ...
    it('posts to SCAN_API with correct body', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({ success: true, result: 'valid', scanId: 'scan_1', ticket: { userName: 'Test', quantity: 1, entryType: 'stag' } }), { status: 200 });
    ...
    it('does NOT add to cache on invalid result', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({ result: 'invalid', error: 'Bad sig' }), { status: 400 });
        await processQRScan(MOCK_SCAN_REQ);
        const key = `${MOCK_SCAN_REQ.eventId}:${MOCK_SCAN_REQ.qrData}`;
...
const MOCK_WALLET = {
    id: 'wallet_1', orderId: 'order_1', currentBalancePaise: 50000,
    openingBalancePaise: 100000, totalDebitedPaise: 50000,
    guestFirstName: 'Arjun', state: 'ACTIVE', terminationTime: null,
    rules: { allowedPresetItems: [], showBalanceToGuest: true, maxChargeAmountPaise: 0, minChargeAmountPaise: 0 },
};
...
const MOCK_DEBIT_REQ = {
    walletId: 'wallet_1', presetItemId: 'item_1', quantity: 1,
    idempotencyKey: 'test-uuid-1234', operatorId: 'scanner_code',
    operatorName: 'Scanner', operatorRole: 'door_staff',
    deviceId: 'ios-test-device', eventCodeId: 'code_123',
    isOnline: true as const,
};

describe('submitDebit', () => {
    it('POSTs to /debit with idempotencyKey', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({ success: true, balanceAfterPaise: 40000 }), { status: 200 });
    ...
    it('returns { success, balanceAfterPaise } on 200', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({ success: true, balanceAfterPaise: 40000, receipt: { itemName: 'Beer', amountPaise: 10000 } }), { status: 200 });
    ...
    it('returns { success: false, error } on 400', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({ success: false, message: 'Insufficient balance' }), { status: 400 });
        const result = await submitDebit(MOCK_DEBIT_REQ, MOCK_SESSION_TOKEN);
