    ...
    it("reserves inventory, opens Razorpay, verifies payment, and clears recovery state", async () => {
        ...
        expect(reserveTickets).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                headers: expect.objectContaining({ "x-idempotency-key": expect.stringContaining(":reserve") }),
            })
        );
        expect(initiateCheckout).toHaveBeenCalledWith(
            expect.objectContaining({ reservationId: "res_1", userEmail: "aayush@example.com" }),
            expect.objectContaining({
                headers: expect.objectContaining({ "x-idempotency-key": expect.stringContaining(":initiate") }),
            })
        );
        ...
        expect(statuses).toEqual(["reserving", "initiating", "awaiting_payment", "verifying", "confirmed"]);
