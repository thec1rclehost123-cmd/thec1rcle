...
function PhotoSection({ photo }: { photo: DatingPhoto }) {
    return (
        <View style={styles.photoSection}>
            ...
            {photo.caption ? (
                <LinearGradient colors={["rgba(0,0,0,0)", "rgba(0,0,0,0.72)"]} style={styles.captionGradient}>
    ...
    if (!profile) {
        return (
            <View style={[styles.container, styles.centerContent]}>
                <Pressable onPress={() => router.back()} style={[styles.backButton, { top: insets.top + 10, left: 20, position: 'absolute' }]}>
        ...
        <View style={styles.container}>
            ...
            >
                ...
                <View style={styles.content}>
                    ...
                    {profile.prompts[0] && (
                        <PromptBlock prompt={profile.prompts[0]} onReply={() => handleOpenReply(profile.prompts[0])} />
                    )}
                    {profile.photos[1] && (
                        <PhotoSection photo={profile.photos[1]} />
                    )}
                    {profile.prompts[1] && (
                        <PromptBlock prompt={profile.prompts[1]} onReply={() => handleOpenReply(profile.prompts[1])} />
                    )}
                    {profile.photos[2] && (
                        <PhotoSection photo={profile.photos[2]} />
                    )}
                    {profile.prompts[2] && (
                        <PromptBlock prompt={profile.prompts[2]} onReply={() => handleOpenReply(profile.prompts[2])} />
                    )}
                </View>
            </ScrollView>
