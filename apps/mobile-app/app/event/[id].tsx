...
function TicketOriginEventView({
  ...
  const poster =
    getEventImage(event) ||
    (typeof params.eventCoverImage === 'string' ? params.eventCoverImage : undefined);
      ...
      <SafeAreaView style={styles.goingSafeArea}>
        ...
        <View style={styles.goingContent}>
          <Animated.View entering={FadeInDown.duration(420)} style={styles.goingHeader}>
            ...
            <Text style={styles.goingAddress} numberOfLines={2}>
              {address}
            </Text>
    ...
    >
      ...
      <View style={styles.tierHeader}>
        ...
        <View style={styles.tierPricing}>
          ...
          <Text style={styles.tierRemaining}>{tier.remaining} left</Text>
        </View>
      </View>
...
export default function EventDetailScreen() {
  ...
  const confirmedOrder = useMemo(() => {
    ...
    return orders.find(
      (o) => o.eventId === id && (o.status === 'confirmed' || o.status === 'checked_in'),
    );
  }, [orders, id]);
  ...
  const compactHeaderStyle = useAnimatedStyle(() => ({
    ...
    transform: [{ translateY: interpolate(scrollY.value, [150, 240], [-10, 0]) }],
  }));
  ...
  const handleGetTickets = () => {
    ...
    ticketTransition.value = 0;
    ticketButtonMorph.value = 0;
    ticketButtonScale.value = withTiming(
      0.96,
      { duration: 70, easing: Easing.out(Easing.quad) },
      () => {
        ticketButtonScale.value = withTiming(1, {
          duration: 110,
          easing: Easing.out(Easing.cubic),
        });
      },
    );
    ticketButtonMorph.value = withTiming(
      1,
      { duration: 170, easing: Easing.out(Easing.cubic) },
      (morphed) => {
        if (!morphed) return;
        ticketTransition.value = withTiming(
          1,
  ...
  if (!event) {
    return (
      <View style={[styles.container, styles.centerContent]}>
        ...
        >
          <LinearGradient colors={gradients.primary as [string, string]} style={styles.errorButton}>
  ...
  const handleOpenInstagram = () => {
    ...
    const instagramUrl = cleanHandle
      ? `instagram://user?username=${cleanHandle}`
      : 'instagram://app';
    const webUrl = cleanHandle
  ...
  const handleContactProfile = () => {
    ...
    const contactUrl =
      (event as any).contactUrl || (event as any).venueContactUrl || (event as any).hostContactUrl;
    if (contactUrl) {
    ...
    <View style={[styles.container, { backgroundColor: '#050505' }]}>
      ...
      >
        ...
        <Animated.View pointerEvents="none" style={[styles.compactTitle, compactHeaderStyle]}>
          ...
          <Text style={styles.compactVenueText} numberOfLines={1}>
            {addressLabel}
          </Text>
        </Animated.View>
        ...
        <View style={styles.detailContent}>
          ...
          <Animated.View entering={FadeInDown.delay(180).springify()} style={styles.detailFacts}>
            <View style={styles.detailFactRow}>
              ...
              <Text
                style={[styles.detailFactText, { color: 'rgba(255,255,255,0.78)' }]}
                numberOfLines={2}
              >
                {addressLabel}
              </Text>
            ...
            <View style={styles.detailFactRow}>
              ...
              <Text style={[styles.detailFactText, { color: 'rgba(255,255,255,0.78)' }]}>
                {timeLabel}
              </Text>
            </View>
          </Animated.View>
          ...
          >
            ...
            >
              <Text style={styles.venueProfileLogoText}>
                {venueProfileName.charAt(0).toUpperCase()}
              </Text>
            </Pressable>
            <View style={styles.venueProfileHeroNameRow}>
              <Text style={styles.venueProfileHeroName} numberOfLines={2}>
                {venueProfileName.toUpperCase()}
              </Text>
          ...
          <View style={[styles.ticketSheet, { paddingBottom: insets.bottom + 18 }]}>
            <View style={styles.ticketSheetHeader}>
              <View>
                ...
                <Text style={styles.ticketSheetSubtitle} numberOfLines={1}>
                  {event.title}
                </Text>
              </View>
            ...
            {cartCount > 0 && (
              <Pressable
                ...
                style={[styles.ticketSheetCheckout, { backgroundColor: accent }]}
              >
