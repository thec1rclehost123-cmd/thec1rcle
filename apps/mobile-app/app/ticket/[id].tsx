...
export default function TicketDetailScreen() {
    ...
    const qrCodes = order?.qrCodes?.length
        ? order.qrCodes
        : [{ ticketId: order?.id || "ticket", ticketIndex: 0, qrCode: order?.qrData || order?.id || "ticket", isUsed: false }];
    ...
    const handleShareChannel = async (channel: string, tierId?: string, expiresAt?: string) => {
        ...
        try {
            ...
            if (channel === "copy") {
                ...
                await Linking.openURL(`mailto:?subject=${encodeURIComponent(`Ticket for ${order.eventTitle || "The C1RCLE"}`)}&body=${encodeURIComponent(message)}`);
            } else {
    ...
    const handleTransferEmail = async (email: string) => {
        ...
        const response = await initiateFormalTransfer({ ticketId: claimedTicket.ticketId, recipientEmail: email });
        if (!response?.success) {
    ...
    const confirmCancelTransfer = (transferId: string) => {
        Alert.alert("Cancel transfer", "This restores your ticket access and invalidates the transfer.", [
    ...
    const confirmCancelBundle = (bundleId: string) => {
        Alert.alert("Cancel share link", "This invalidates the active share link and reclaims any unclaimed tickets.", [
            { text: "Keep", style: "cancel" },
            {
                text: "Cancel share",
                style: "destructive",
        ...
        <View style={styles.container}>
            <ScrollView bounces={false} overScrollMode="never" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
                <View style={styles.hero}>
                    ...
                    <LinearGradient colors={["rgba(0,0,0,0.25)", "rgba(0,0,0,0.72)", "#161616"]} style={StyleSheet.absoluteFill} />
                    ...
                    <View style={[styles.topBar, { paddingTop: insets.top + 8 }]}>
                        ...
                        <Pressable
                            onPress={() => order.eventId && shareEventLink(order.eventId, order.eventTitle || "Event")}
                    ...
                    <View style={styles.heroCopy}>
                        ...
                        {order.venueLocation ? <Text style={styles.heroMeta}>{order.venueLocation}</Text> : null}
                    </View>
                </View>
                ...
                <View style={styles.body}>
                    <Animated.View entering={FadeInDown.delay(60)} style={styles.card}>
                        ...
                        {showQr ? (
                            <View style={styles.qrBlock}>
                                ...
                                {qrCodes.length > 1 ? (
                                    <View style={styles.qrPager}>
                                        <Pressable
                                            onPress={() => setActiveQrIndex((current) => (current - 1 + qrCodes.length) % qrCodes.length)}
                            ...
                            <Text style={styles.cardBodyText}>Reveal the QR when you are at the door. Each active ticket can be scanned once.</Text>
                        )}
                    </Animated.View>

                    <Animated.View entering={FadeInDown.delay(90)} style={styles.actionGrid}>
                        <Pressable
                            onPress={() => order.eventId && router.push({ pathname: "/event/[id]", params: { id: order.eventId } })}
                    ...
                    <Animated.View entering={FadeInDown.delay(120)} style={styles.card}>
                        ...
                        <View style={styles.primaryActions}>
                            <Pressable onPress={() => setActiveSheet("share")} style={styles.primaryAction}>
                                <LinearGradient colors={gradients.primary as [string, string]} style={StyleSheet.absoluteFill} />
                                <Text style={styles.primaryActionText}>Share Unclaimed Ticket</Text>
                            </Pressable>
                            <Pressable onPress={() => setActiveSheet("transfer")} style={[styles.primaryAction, styles.primaryActionAlt]}>
                            ...
                            {pendingTransfers.map((transfer) => (
                                <View key={transfer.id} style={styles.statusRow}>
                                    <View style={styles.statusCopy}>
                                        <Text style={styles.statusTitle}>{transfer.ticketName || transfer.ticketId || "Ticket transfer"}</Text>
                                        <Text style={styles.statusMeta}>Waiting on {transfer.recipientEmail || "recipient"}</Text>
                                    </View>
                                    <Pressable onPress={() => confirmCancelTransfer(transfer.id)} style={styles.statusButton}>
                            ...
                            {ticketShares.map((bundle: any) => (
                                <View key={bundle.id} style={styles.bundleCard}>
                                    <View style={styles.bundleHeader}>
                                        <View>
                                            <Text style={styles.statusTitle}>{bundle.ticketName || bundle.tierName || "Shared ticket"}</Text>
                                            <Text style={styles.statusMeta}>{bundle.slots?.filter((slot: any) => slot.claimStatus === "unclaimed").length || 0} unclaimed left</Text>
                                        </View>
                                        <Pressable onPress={() => confirmCancelBundle(bundle.id)} style={styles.statusButton}>
                                    ...
                                    {(bundle.slots || [])
                                        ...
                                        .map((slot: any) => (
                                            <View key={`${bundle.id}-${slot.slotIndex}`} style={styles.slotRow}>
                                                ...
                                                <Pressable onPress={() => reclaimSlot(bundle.id, slot.slotIndex)} style={styles.slotButton}>
                    ...
                    <Animated.View entering={FadeInDown.delay(210)} style={styles.card}>
                        ...
                        {flattenedTickets.map((ticket) => {
                            const pendingTransfer = pendingTransfers.find((transfer) => transfer.ticketId === ticket.ticketId);
                            const activeBundle = ticketShares.find((bundle: any) => bundle.tierId === ticket.ticketId || bundle.tierId === ticket.tierId);
                            const claimName =
                            ...
                            return (
                                <View key={ticket.rowId} style={styles.rosterRow}>
                                    ...
                                    <View style={styles.rosterCopy}>
                                        ...
                                        {pendingTransfer ? <Text style={styles.pendingText}>Transfer pending</Text> : null}
                                        {!pendingTransfer && activeBundle ? <Text style={styles.pendingText}>Active share link</Text> : null}
                                    </View>
                                </View>
                            );
                        })}
                    ...
                    <Animated.View entering={FadeInDown.delay(240)} style={styles.card}>
                        ...
                        {order.tickets.map((ticket, index) => (
                            <View key={`${ticket.ticketId || ticket.tierId || index}`} style={styles.breakdownRow}>
                                <Text style={styles.breakdownLabel}>
                                    {ticket.tierName || "Ticket"} x{ticket.quantity}
                                </Text>
                                <Text style={styles.breakdownValue}>
                                    {ticket.price > 0 ? `INR ${(ticket.price * ticket.quantity).toLocaleString("en-IN")}` : "Free"}
                                </Text>
                            </View>
                        ))}
                        ...
                        <View style={styles.breakdownRow}>
                            ...
                            <Text style={styles.breakdownTotal}>
                                {order.totalAmount > 0 ? `INR ${order.totalAmount.toLocaleString("en-IN")}` : "Free"}
                            </Text>
                        </View>
                    </Animated.View>
                </View>
