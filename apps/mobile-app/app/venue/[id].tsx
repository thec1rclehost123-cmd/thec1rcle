    ...
    return (
        <View style={styles.segmentedContainer}>
            ...
            {TABS.map((tab) => (
                ...
                >
                    <Text style={[styles.segmentedTabText, active === tab.id && styles.segmentedTabTextActive]}>
                        {tab.label}
                    </Text>
                </Pressable>
            ))}
...
// ── Gallery Modal ───────────────────────────────────────────────────────────────
function GalleryModal({ visible, imageUrl, onClose }: { visible: boolean; imageUrl: string | null; onClose: () => void }) {
        ...
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <View style={styles.modalBackdrop}>
                ...
                <Animated.View entering={FadeIn} style={styles.storyCard}>
                    <Image source={{ uri: highlight.images[imageIndex] }} style={StyleSheet.absoluteFill} contentFit="cover" />
                    <LinearGradient
            ...
            >
                ...
                <Animated.View entering={FadeInDown.delay(200)} style={styles.statsRow}>
                    ...
                    <View style={styles.statChip}>
                        ...
                        <Text style={styles.statValue}>{venue.upcomingEventsCount || upcomingEvents.length || 0}</Text>
                        <Text style={styles.statLabel}>Events</Text>
                    </View>
                    {timingsEntries.length > 0 && (
                        <View style={styles.statChip}>
                            <Clock size={14} color={colors.iris} />
                            <Text style={styles.statValue} numberOfLines={1}>{timingsEntries[0][1]}</Text>
                                        ...
                                        {facilities.map((facility) => (
                                            <View key={facility.id} style={styles.facilityCard}>
                                                <Text style={styles.facilityEmoji}>
                                                    {getFacilityEmoji(facility.icon)}
                                                </Text>
                    ...
                    setStoryModal((c) =>
                        c
                            ? {
                                  ...c,
                                  imageIndex:
                                      (c.imageIndex - 1 + c.highlight.images.length) %
                                      c.highlight.images.length,
                              }
                            : c
                    )
                }
                ...
                <View style={styles.modalBackdrop}>
                    ...
                    {menuModalIndex !== null && menu[menuModalIndex] && (
                        <Animated.View entering={FadeIn} style={styles.galleryModalCard}>
                            ...
                            />
                            <Pressable
                                onPress={() => setMenuModalIndex(null)}
