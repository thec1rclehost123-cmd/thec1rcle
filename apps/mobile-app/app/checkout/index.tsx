        ...
        <View style={styles.itemRow}>
            ...
            <View style={styles.itemSide}>
                <View style={styles.quantityControl}>
                    <Pressable onPress={() => onQuantityChange(item.quantity - 1)} style={styles.quantityButton}>
                        <Ionicons name="remove" size={15} color="#fff" />
                    </Pressable>
                    <Text style={styles.quantityValue}>{item.quantity}</Text>
                    <Pressable onPress={() => onQuantityChange(Math.min((item.tier.remaining || 10), item.quantity + 1))} style={styles.quantityButton}>
...
export default function CheckoutScreen() {
    ...
    const discount = Number(pricing?.discountTotal ?? pricing?.discount ?? promo?.discountAmount ?? 0);
    const platformFee = Number(pricing?.fees?.platform ?? pricing?.fees?.platformFee ?? pricing?.platformFee ?? 0);
    const paymentFee = Number(pricing?.fees?.payment ?? pricing?.fees?.paymentFee ?? 0);
    const taxes = Number(pricing?.fees?.gst ?? 0);
    const total = Number(pricing?.grandTotal ?? Math.max(0, subtotal - discount + platformFee + paymentFee + taxes));
    ...
    const handlePay = async () => {
        ...
        if (quoteError) {
            Alert.alert("Live price unavailable", "Refresh pricing before checkout so the backend can confirm fees and inventory.");
            return;
        }
        ...
        if (!result.success || !result.orderId) {
            ...
            setProcessing(false);
            Alert.alert("Checkout failed", result.error || "We could not complete checkout. Please try again.");
            return;
        }
    ...
    return (
        <SafeAreaView style={styles.screen} edges={["top"]}>
            ...
            >
                ...
                <View style={styles.eventCard}>
                    {cartEventImage ? (
                        ...
                        <LinearGradient colors={gradients.primary as [string, string]} style={styles.eventImage} />
                    )}
                    <View style={styles.eventCopy}>
                        <Text style={styles.eventTitle} numberOfLines={2}>{cartEventTitle}</Text>
                        <Text style={styles.eventMeta}>
                            {cartEventDate ? `${formatEventDate(cartEventDate)} · ${formatEventTime(cartEventDate)}` : "Date TBA"}
                        </Text>
                        <Text style={styles.eventMeta} numberOfLines={1}>{cartEventVenue}</Text>
                    </View>
                </View>

                <View style={styles.sectionHeader}>
                    <Text style={styles.sectionTitle}>Tickets</Text>
                    <Pressable onPress={() => router.push({ pathname: "/checkout/[eventId]", params: { eventId: items[0]?.eventId || "" } })}>
                ...
                <View style={styles.panel}>
                    ...
                    <Text style={styles.panelCopy}>Codes are checked against live event pricing before payment.</Text>
                    {promo ? (
                        <View style={styles.appliedPromoRow}>
                            <View>
                                <Text style={styles.appliedPromoCode}>{promo.code}</Text>
                                <Text style={styles.appliedPromoLabel}>{promo.label || `${promo.discountPercent}% discount`}</Text>
                            </View>
                ...
                <View style={styles.panel}>
                    ...
                    <Text style={styles.panelCopy}>THE C1RCLE reserves inventory first, then Razorpay collects payment details in the native checkout.</Text>
                    <View style={styles.bankNotice}>
                        <Ionicons name={isFreeOrder ? "checkmark-circle-outline" : "shield-checkmark-outline"} size={20} color={colors.irisGlow} />
                        <Text style={styles.bankNoticeText}>
                            {isFreeOrder ? "This order will be confirmed by the backend with no payment required." : "Card, UPI, wallet, and bank options open inside Razorpay after live inventory is reserved."}
                        </Text>
                    </View>
                    <TextInput
                ...
                <View style={styles.panel}>
                    ...
                    {discount > 0 ? (
                        <View style={styles.summaryRow}>
                            <Text style={[styles.summaryLabel, styles.discountText]}>Discount {promo?.code ? `(${promo.code})` : ""}</Text>
                            <Text style={[styles.summaryValue, styles.discountText]}>-{formatMoney(discount)}</Text>
                        </View>
                    ) : null}
                    {platformFee > 0 ? (
                        <View style={styles.summaryRow}>
                ...
                <Text style={styles.termsText}>
                    By paying, you agree to ticket transfer, refund, and venue entry rules. Prices, inventory, and order creation are confirmed by THE C1RCLE servers.
                </Text>
            </ScrollView>
            ...
            <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 14 }]}>
                ...
                <Pressable onPress={handlePay} disabled={processing || quoteLoading || !!quoteError} style={[styles.payButton, (processing || quoteLoading || !!quoteError) && styles.payButtonDisabled]}>
                    {processing ? (
                        <ActivityIndicator color="#fff" />
                    ) : (
                        <>
                            <Text style={styles.payButtonText}>{isFreeOrder ? "Confirm" : getCheckoutStatusLabel(checkoutStatus)}</Text>
