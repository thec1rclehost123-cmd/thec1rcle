...
export default function OtpScreen() {
  ...
  const { confirmPhoneCode, linkPhoneCode, sendPhoneCode, signOut, loading, error, clearError } =
    useAuth();

  useEffect(() => {
    void readPhoneAuthTransaction().then((value) => {
      setTransaction(value);
      if (!value) setLocalError('This code has expired. Go back and request a new one.');
    });
  }, []);
  useEffect(() => {
    if (timer <= 0) return;
  ...
  const submit = async (value = code) => {
    ...
    if (!result.success) {
      ...
      trackFirstRun('first_run_otp_failed', {
        provider: 'phone',
    ...
    trackFirstRun('first_run_otp_verified', {
      provider: 'phone',
      mode: transaction.mode,
      outcome: 'success',
    });
  ...
  const resend = async () => {
    ...
    const refreshed = {
      ...
      expectedUid: result.expectedUid ?? transaction.expectedUid,
      startedAt: Date.now(),
    };
    trackFirstRun('first_run_otp_resent', {
      provider: 'phone',
      mode: transaction.mode,
      outcome: 'success',
    });
    ...
    <FirstRunShell
      ...
      action={
        <FirstRunButton
          ...
          onPress={() => submit()}
          loading={loading}
          disabled={code.length !== 6 || !transaction || success}
        />
      }
    >
      <Pressable
        ...
        style={[styles.codeBox, (localError || error) && styles.codeError]}
      >
        {success ? (
          <Check color={firstRunTokens.accent} size={36} strokeWidth={3} />
        ) : (
          Array.from({ length: 6 }).map((_, index) => (
            <View key={index} style={[styles.digit, code.length === index && styles.digitActive]}>
      ...
      <View style={styles.links}>
        ...
        <Pressable accessibilityRole="button" disabled={timer > 0} onPress={resend}>
          <Text style={[styles.link, timer > 0 && styles.muted]}>
...
const styles = StyleSheet.create({
  ...
  codeError: { borderWidth: 1, borderColor: firstRunTokens.error },
  digit: {
    width: 38,
    height: 48,
    borderBottomWidth: 2,
  ...
  digitActive: { borderBottomColor: firstRunTokens.accent },
  digitText: { color: firstRunTokens.text, fontSize: 26, fontWeight: '800' },
  hidden: { position: 'absolute', width: 1, height: 1, opacity: 0 },
  links: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  link: { color: firstRunTokens.text, fontSize: 14, fontWeight: '700', paddingVertical: 12 },
  muted: { color: firstRunTokens.muted },
});
