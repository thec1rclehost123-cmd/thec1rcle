...
export default function Index() {
  const { user, initialized, serverSynced, authSyncInProgress, authSyncFailed, authSyncError, isGuest } = useAuthStore();
  ...
  useEffect(() => {
    if (enforceFirstRunV2 && effectiveUser?.uid && (serverSynced || authSyncFailed) && !hydrated) void load();
  ...
  if (boot.type === 'recoverable-error' || boot.type === 'fatal-error' || (enforceFirstRunV2 && error && !profile)) {
    const retry = async () => {
