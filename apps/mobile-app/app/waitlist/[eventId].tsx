...
export default function WaitlistScreen() {
    ...
    return (
        <SafeAreaView style={[styles.container, { paddingTop: insets.top }]}>
            <LinearGradient colors={["rgba(244,74,34,0.08)", "transparent"]} style={StyleSheet.absoluteFill} />
            <View style={styles.header}>
                <Pressable onPress={() => router.back()} style={styles.backBtn}><Text style={styles.backIcon}>←</Text></Pressable>
            ...
            {!checking && !alreadyJoined && (
                <View style={styles.footer}>
                    <Pressable onPress={handleJoin} disabled={joining} style={[styles.joinBtn, joining && { opacity: 0.6 }]}>
                        <LinearGradient colors={gradients.primary as [string, string]} style={styles.joinGradient}>
                            {joining ? <ActivityIndicator color="#fff" /> : <Text style={styles.joinText}>Join Waitlist</Text>}
                        </LinearGradient>
                    </Pressable>
                </View>
            )}
...
const styles = StyleSheet.create({
    ...
    backBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: colors.base[50], alignItems: "center", justifyContent: "center" },
    ...
    card: { width: "100%", backgroundColor: colors.base[50], borderRadius: radii["2xl"], padding: 24, alignItems: "center", borderWidth: 1, borderColor: "rgba(244,74,34,0.15)", overflow: "hidden" },
    soldOutBadge: { backgroundColor: "rgba(255,61,113,0.15)", color: colors.error, fontSize: 11, fontWeight: "800", letterSpacing: 1.5, paddingHorizontal: 12, paddingVertical: 5, borderRadius: radii.pill, borderWidth: 1, borderColor: "rgba(255,61,113,0.25)", marginBottom: 16 },
    positionCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: "rgba(244,74,34,0.15)", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "rgba(244,74,34,0.3)", marginBottom: 20 },
    positionNumber: { color: colors.iris, fontSize: 28, fontWeight: "900" },
    cardTitle: { color: colors.gold, fontSize: 22, fontWeight: "800", textAlign: "center", marginBottom: 12 },
    cardSubtitle: { color: colors.goldMetallic, fontSize: 15, textAlign: "center", lineHeight: 22, marginBottom: 20 },
    infoRow: { backgroundColor: "rgba(255,255,255,0.04)", paddingHorizontal: 14, paddingVertical: 10, borderRadius: radii.xl },
    infoText: { color: colors.goldMetallic, fontSize: 13 },
    howItWorks: { width: "100%", gap: 12, marginTop: 4 },
    howTitle: { color: colors.gold, fontSize: 13, fontWeight: "700", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 },
    howText: { color: colors.goldMetallic, fontSize: 14 },
    footer: { paddingHorizontal: 20, paddingBottom: 32, paddingTop: 12, alignItems: "center", gap: 12 },
