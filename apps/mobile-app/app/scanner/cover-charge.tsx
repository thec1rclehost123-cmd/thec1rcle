...
export default function CoverChargeScreen() {
    ...
    const handleConfirmCharge = async () => {
        ...
        const newBalance = result.balanceAfterPaise ?? (wallet.currentBalancePaise - selectedItem.amountPaise);
        safeSetState(setWallet, (prev) => prev ? { ...prev, currentBalancePaise: newBalance } : prev);
                ...
                <View style={styles.cameraOverlay}>
                    <SafeAreaView edges={["top"]}>
                        <View style={styles.topBar}>
                            ...
                            <Text style={styles.eventName} numberOfLines={1}>{eventData?.event.title}</Text>
                        </View>
                    </SafeAreaView>
                ...
                <SafeAreaView style={styles.walletContainer} edges={["top", "bottom"]}>
                    <View style={styles.walletHeader}>
                        ...
                        <View style={styles.balanceChip}>
                            ...
                            <Text style={styles.balanceAmount}>{formatPaiseCurrency(wallet.currentBalancePaise)}</Text>
                        </View>
                    </View>
                    ...
                    {chargeState === "SUCCESS" && successReceipt && (
                        <View style={styles.successBanner}>
                            ...
                            <Text style={styles.successBannerAmount}>−{formatPaiseCurrency(successReceipt.amountPaise)}</Text>
            ...
            {chargeState === "CONFIRM_DEBIT" && wallet && selectedItem && (
                <SafeAreaView style={[styles.container, styles.confirmContainer]} edges={["top", "bottom"]}>
                    <View style={styles.confirmCard}>
                        ...
                        <Text style={styles.confirmAmount}>−{formatPaiseCurrency(selectedItem.amountPaise)}</Text>
                        <Text style={styles.confirmAfter}>
                            Balance after: {formatPaiseCurrency(wallet.currentBalancePaise - selectedItem.amountPaise)}
                        </Text>
                    </View>
...
const styles = StyleSheet.create({
    ...
    permTitle: { color: colors.gold, fontSize: 20, fontWeight: "700", marginBottom: 8, textAlign: "center", paddingHorizontal: 24 },
    permSubtitle: { color: "rgba(255,255,255,0.5)", fontSize: 14, textAlign: "center", marginBottom: 24, paddingHorizontal: 24 },
    primaryBtn: { backgroundColor: colors.iris, borderRadius: 14, paddingHorizontal: 28, paddingVertical: 14, marginTop: 8 },
    primaryBtnText: { color: "#fff", fontSize: 16, fontWeight: "700" },

    // Camera overlay
    cameraOverlay: { flex: 1, justifyContent: "space-between" },
    topBar: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "rgba(0,0,0,0.65)", gap: 12 },
    chargeModeLabel: { backgroundColor: colors.iris, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4 },
    ...
    scanFrame: { width: 260, height: 260, borderWidth: 2, borderColor: "rgba(255,255,255,0.4)", borderRadius: 24 },
    ...
    cornerTR: { top: -1, right: -1, borderTopWidth: 4, borderRightWidth: 4, borderTopRightRadius: 16 },
    cornerBL: { bottom: -1, left: -1, borderBottomWidth: 4, borderLeftWidth: 4, borderBottomLeftRadius: 16 },
    cornerBR: { bottom: -1, right: -1, borderBottomWidth: 4, borderRightWidth: 4, borderBottomRightRadius: 16 },
    scanHint: { color: "rgba(255,255,255,0.6)", fontSize: 14, marginTop: 20, textAlign: "center", paddingHorizontal: 32 },
    ...
    walletHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.06)" },
    guestName: { color: "#fff", fontSize: 26, fontWeight: "800" },
    balanceChip: { alignItems: "flex-end" },
    balanceLabel: { color: "rgba(255,255,255,0.45)", fontSize: 11, fontWeight: "600", letterSpacing: 0.8 },
    balanceAmount: { color: colors.success, fontSize: 22, fontWeight: "800" },

    terminationWarning: { backgroundColor: "rgba(251,191,36,0.12)", marginHorizontal: 16, marginTop: 8, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: "rgba(251,191,36,0.3)" },
    terminationWarningText: { color: colors.warning, fontSize: 13, fontWeight: "600", textAlign: "center" },

    successBanner: { backgroundColor: "rgba(0,214,143,0.12)", margin: 16, borderRadius: 14, padding: 16, borderWidth: 1, borderColor: "rgba(0,214,143,0.3)", alignItems: "center" },
    ...
    selectLabel: { color: "rgba(255,255,255,0.35)", fontSize: 11, fontWeight: "700", letterSpacing: 1.2, paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8 },
    ...
    confirmCard: { backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 20, padding: 28, alignItems: "center", marginBottom: 24, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" },
    confirmTitle: { color: "rgba(255,255,255,0.45)", fontSize: 12, fontWeight: "700", letterSpacing: 1.2, marginBottom: 12 },
    ...
    chargeBtn: { backgroundColor: colors.success, borderRadius: 16, paddingVertical: 18, alignItems: "center", marginHorizontal: 16, marginBottom: 8 },
    ...
    errorMessage: { color: colors.error, fontSize: 15, textAlign: "center", paddingHorizontal: 32, marginBottom: 28, lineHeight: 22 },
});
