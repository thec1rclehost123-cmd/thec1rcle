    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Animated.View entering={FadeInDown.duration(600)}>
          ...
          <Text style={styles.subtitle}>Add up to 3 prompts to your profile.</Text>
        </Animated.View>
      ...
      <View style={styles.footer}>
        ...
        >
          <Text style={[styles.buttonText, localPrompts.length === 0 && styles.buttonTextDisabled]}>
            Next
          </Text>
          <ArrowRight
            size={20}
            color={localPrompts.length === 0 ? colors.gray500 : colors.midnight}
          />
        </Pressable>
      </View>
          ...
          {selectedPromptId ? (
            <View style={styles.answerContainer}>
              ...
              <Pressable
                ...
                style={[styles.saveButton, !draftAnswer.trim() && styles.saveButtonDisabled]}
              >
              ...
              {Array.from(new Set(PROMPT_OPTIONS.map((p) => p.category))).map((category) => (
                <View key={category} style={styles.categoryGroup}>
                  ...
                  {PROMPT_OPTIONS.filter(
                    (p) =>
                      p.category === category && !localPrompts.some((lp) => lp.promptId === p.id),
                  ).map((p) => (
                    <Pressable
                      key={p.id}
                      style={styles.promptListItem}
                      onPress={() => selectPrompt(p.id)}
                    >
