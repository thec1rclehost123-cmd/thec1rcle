...
export default function ExploreScreen() {
    ...
    const exploreSections = useMemo<ExploreSection[]>(() => [
        ...
        {
            key: "featured",
            render: () => quickFilter === "all" && !isInitialLoading && heroSlides.length > 0 ? <FeaturedCarousel events={heroSlides} /> : null,
        },
        {
            key: "choose-scene",
        ...
        {
            key: "worth-it",
            render: () => quickFilter === "all" && freeEvents.length > 0 ? <ScenesWorthIt events={freeEvents} /> : null,
        },
        {
            key: "top-venues",
        ...
        {
            key: "editors-picks",
            render: () => quickFilter === "all" && recommendations.length > 0 ? <EditorsPicks events={recommendations} /> : null,
        },
        {
            key: "trending",
            render: () => quickFilter === "all" && trendingThisWeek.length > 0 ? <TrendingRightNow events={trendingThisWeek} /> : null,
        },
        {
            key: "coming-up",
            render: () => quickFilter === "all" && similarEvents.length > 0 ? <ComingUpThisWeek events={similarEvents} /> : null,
        },
        {
            key: "all-scenes",
            render: () => filteredEvents.length > 0 ? (
                <View onLayout={(e) => setAllScenesY(e.nativeEvent.layout.y)}>
                    <AllScenes
                        events={filteredEvents}
                        onPageChange={() => {
                            mainScrollRef.current?.scrollToOffset({ offset: Math.max(0, allScenesY - 20), animated: true });
                        }}
                    />
                </View>
            ) : (!loading ? (
        ...
        {
            key: "no-content",
            render: () => !loading && allEvents.length === 0 && !isOffline ? (
                <View style={styles.emptyState}>
        ...
        {
            key: "map",
            render: () => !isInitialLoading && allEvents.length > 0 ? <MapSection events={allEvents} /> : null,
        },
    ], [
        activeCityLabel,
        allEvents,
