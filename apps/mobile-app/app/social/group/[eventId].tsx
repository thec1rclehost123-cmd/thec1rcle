      ...
      <View className="flex-row items-center">
        <View className="flex-row items-center bg-midnight-100 px-3 py-2 rounded-bubble border border-white/10">
          <View className="flex-row mr-2">
            <Animated.Text entering={FadeIn.delay(0)} className="text-gold-stone">
              •
            </Animated.Text>
            <Animated.Text entering={FadeIn.delay(200)} className="text-gold-stone">
              •
            </Animated.Text>
            <Animated.Text entering={FadeIn.delay(400)} className="text-gold-stone">
              •
            </Animated.Text>
          </View>
          <Text className="text-gold-stone text-xs">{formatTypingText(status.users)}</Text>
        </View>
      </View>
    </Animated.View>
  );
  ...
  return (
    ...
    >
      ...
      {message.type === "image" ? (
        ...
        <View
          className={`max-w-[80%] px-4 py-3 rounded-bubble ${
            isOwnMessage
...
export default function EventGroupChatScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-midnight" edges={["top"]}>
      ...
      {/* Phase info banner */}
      <LinearGradient colors={[`${phaseInfo.color}15`, "transparent"]} className="px-4 py-3">
        ...
        >
          {loading ? (
            ...
            <View className="items-center py-20 px-6">
              <Text className="text-6xl mb-4">💬</Text>
              <Text className="text-gold font-semibold text-lg mb-2">Start the Conversation!</Text>
            ...
            <View className="flex-row items-end">
              ...
              <Pressable
                ...
                className={`w-12 h-12 rounded-full items-center justify-center mr-2 ${
                  imageUploading ? "bg-iris/50" : "bg-surface border border-white/10"
                }`}
              >
                {imageUploading ? (
              ...
              <Pressable
                ...
                className={`w-12 h-12 rounded-full items-center justify-center ${
                  inputText.trim() && !sending && canSend ? "bg-iris" : "bg-iris/50"
                }`}
              >
                {sending ? (
