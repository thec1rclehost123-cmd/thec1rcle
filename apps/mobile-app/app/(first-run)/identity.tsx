...
import { calculateAge, formatDateOfBirth, parseDateOfBirth, resolveMinimumAccountAge } from '@/lib/firstRun';
...
export default function IdentityScreen() {
  ...
  const [name, setName] = useState(snapshot?.displayName || profile?.displayName || user?.displayName || '');
  const [dob, setDob] = useState<Date | null>(() => parseDateOfBirth(snapshot?.dateOfBirth || profile?.dateOfBirth));
  const [showPicker, setShowPicker] = useState(false);
  const [localError, setLocalError] = useState<string | null>(null);
  const maxDate = useMemo(() => { const date = new Date(); date.setFullYear(date.getFullYear() - minimumAccountAge); return date; }, [minimumAccountAge]);
  ...
  const submit = async () => {
    ...
    if (calculateAge(dob) < minimumAccountAge) return setLocalError(`You must be at least ${minimumAccountAge} to use THE C1RCLE.`);
