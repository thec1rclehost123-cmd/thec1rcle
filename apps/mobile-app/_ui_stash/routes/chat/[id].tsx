    ...
    return (
        <Animated.View entering={FadeIn} style={styles.typingRow}>
            <View style={styles.typingAvatar}>
                {attendee.avatar ? (
                    <Image source={{ uri: attendee.avatar }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />
                ) : (
...
function MessageBubble({
    ...
    if (message.type === "announcement") {
        return (
            <Animated.View entering={FadeInDown.delay(Math.min(index * 20, 160)).duration(240)} style={styles.announcement}>
                ...
                {!isOwnMessage && (
                    <View style={styles.messageAvatarPeep}>
                        {message.senderAvatar ? (
                            <Image source={{ uri: message.senderAvatar }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />
                        ) : (
                            <Text style={styles.avatarFallback}>{message.senderName.slice(0, 1).toUpperCase()}</Text>
                        )}
                    </View>
                )}
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={styles.sheetScreen}>
                ...
                <View style={[styles.sheet, { paddingBottom: insets.bottom + spacing.base }]}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.attendeeList}>
                        {attendees.map((attendee, index) => (
                            <View key={attendee.userId} style={styles.attendeeRow}>
                                <View style={styles.attendeeAvatar}>
                                    ...
                                    {!subscribed && (
                                        <BlurView intensity={72} tint="dark" style={[StyleSheet.absoluteFillObject, styles.attendeeBlur]} />
                                    )}
                                </View>
                    ...
                    >
                        <View style={styles.groupIcon}>
                            {eventChat?.eventCover ? (
                                <Image source={{ uri: eventChat.eventCover }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />
                            ) : (
                                <Users size={19} color={colors.gold} />
                            )}
                        </View>
                        <View style={styles.headerCopy}>
                            <Text style={styles.headerTitle} numberOfLines={1}>{title || eventChat?.eventTitle || "Event chat"}</Text>
                            <Text style={styles.headerStatus}>
                                {eventChat ? `${eventChat.participantCount} people going · Tap to view` : "Tap to view attendees"}
                            </Text>
                        </View>
...
const styles = StyleSheet.create({
    ...
    backButton: { width: 34, height: 42, alignItems: "center", justifyContent: "center", marginRight: spacing.sm },
    ...
    centerState: { flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: spacing.xxl },
    ...
    avatarFallback: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.xs },
    messageBubble: {
        flexDirection: "row",
        alignItems: "flex-end",
        flexWrap: "wrap",
    ...
    inlineTime: { color: "rgba(255,255,255,0.64)", fontFamily: fonts.medium, fontSize: 8, marginBottom: 1 },
    announcement: {
        alignSelf: "stretch",
    ...
    announcementTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: spacing.xs },
    announcementLabel: {
        color: colors.gold,
        fontFamily: fonts.heading,
        fontSize: typography.fontSize.xs,
    ...
    announcementText: { color: colors.goldLight, fontFamily: fonts.medium, fontSize: typography.fontSize.sm, lineHeight: 18 },
    announcementTime: { color: "rgba(255,255,255,0.64)", fontFamily: fonts.medium, fontSize: 8 },
    typingRow: { alignSelf: "flex-start", flexDirection: "row", alignItems: "flex-end", marginTop: spacing.xs, marginLeft: 4 },
    typingAvatar: {
        width: 28,
        height: 28,
        marginRight: -7,
    ...
    composerDock: { backgroundColor: colors.base.DEFAULT, paddingHorizontal: spacing.md, paddingTop: spacing.md, paddingBottom: spacing.sm },
    composerRow: {
        minHeight: 48,
        flexDirection: "row",
        alignItems: "flex-end",
    ...
    errorText: { color: colors.error, fontFamily: fonts.medium, fontSize: typography.fontSize.xs, textAlign: "center", marginBottom: spacing.sm },
    ...
    sheetHandle: { alignSelf: "center", width: 42, height: 4, borderRadius: radii.pill, backgroundColor: colors.base[300], marginBottom: spacing.lg },
    sheetHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: spacing.lg },
    sheetEyebrow: { color: colors.irisGlow, fontFamily: fonts.heading, fontSize: typography.fontSize.xs, letterSpacing: 1.2 },
    sheetTitle: { color: colors.gold, fontFamily: fonts.display, fontSize: typography.fontSize["2xl"], letterSpacing: -0.7, marginTop: 2 },
    sheetSubtitle: { color: colors.goldMetallic, fontFamily: fonts.body, fontSize: typography.fontSize.sm, marginTop: 2 },
    closeButton: { width: 36, height: 36, borderRadius: radii.pill, alignItems: "center", justifyContent: "center", backgroundColor: colors.base[100] },
    lockNotice: {
        flexDirection: "row",
        alignItems: "center",
        gap: spacing.md,
    ...
    lockIcon: { width: 38, height: 38, borderRadius: radii.pill, alignItems: "center", justifyContent: "center", backgroundColor: colors.iris },
    lockCopy: { flex: 1 },
    lockTitle: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.base },
    lockBody: { color: colors.goldMetallic, fontFamily: fonts.body, fontSize: typography.fontSize.xs, marginTop: 2 },
    ...
    attendeeInitial: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.base },
    attendeeCopy: { flex: 1 },
    attendeeName: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.base },
    attendeeMeta: { color: colors.goldMetallic, fontFamily: fonts.body, fontSize: typography.fontSize.xs, marginTop: 2 },
    messageAttendeeButton: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: radii.pill, backgroundColor: colors.base[100] },
    messageAttendeeText: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.xs },
    subscribeButton: {
        minHeight: 48,
        flexDirection: "row",
        alignItems: "center",
    ...
    subscribeText: { color: colors.gold, fontFamily: fonts.heading, fontSize: typography.fontSize.base },
});
