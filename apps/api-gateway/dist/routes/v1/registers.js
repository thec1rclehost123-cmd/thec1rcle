export default async function registerRoutes(fastify) {
    ...
    function makeId(venueId, date) { return `${venueId}_${date}`; }
    async function getOrCreate(db, venueId, date) {
        ...
        if (!doc.exists) {
            const blank = {
                ...
                expectedFootfall: 0, actualFootfall: 0,
                staffAssignments: [], incidents: [], inspections: [],
                reminders: [], expenses: [],
                revenue: { cover: 0, drinks: 0, food: 0, vip: 0, other: 0 },
                weather: null, status: 'scheduled',
     ...
     */
    fastify.get('/:venueId/:date', async (request, reply) => {
     ...
     */
    fastify.get('/:venueId/range', async (request, reply) => {
     ...
     */
    fastify.patch('/:venueId/:date', async (request, reply) => {
        const { venueId, date } = request.params;
        const id = makeId(venueId, date);
        const now = new Date().toISOString();
        await getOrCreate(fastify.db, venueId, date);
        await fastify.db.collection(COL).doc(id).set({ ...request.body, updatedAt: now }, { merge: true });
        return fastify.db.collection(COL).doc(id).get().then((d) => ({ id: d.id, ...d.data() }));
    });
}
