 ...
 */
export default async function venueSettingsRoutes(fastify) {
    // ── Venue Settings ────────────────────────────────────────────────────────
    /**
     ...
     */
    fastify.patch('/venue', async (request, reply) => {
        const { venueId, updates } = request.body;
        if (!venueId || !updates)
            return reply.status(400).send({ error: 'venueId and updates required' });
     ...
     */
    fastify.get('/venue/reservations', async (request, reply) => {
        const { venueId, status, limit = 50 } = request.query;
        if (!venueId)
            return reply.status(400).send({ error: 'venueId required' });
        let q = fastify.db.collection('table_reservations').where('venueId', '==', venueId);
     ...
     */
    fastify.patch('/venue/reservations/:id', async (request, reply) => {
    ...
    fastify.get('/host', async (request, reply) => {
        ...
        if (!hostId)
            return reply.status(400).send({ error: 'hostId required' });
     ...
     */
    fastify.patch('/host', async (request, reply) => {
        const { hostId, settings } = request.body;
        if (!hostId)
            return reply.status(400).send({ error: 'hostId required' });
    ...
    fastify.get('/host/overview', async (request, reply) => {
        ...
        const eventIds = events.map((e) => e.id).slice(0, 10);
        let totalRevenue = 0;
        if (eventIds.length > 0) {
            const ordersSnap = await fastify.db.collection('orders')
                ...
                .get();
            totalRevenue = ordersSnap.docs.reduce((s, d) => s + (d.data().hostPayout || d.data().total || 0), 0);
        }
        return {
            hostId,
...
//# sourceMappingURL=venue-settings.js.map