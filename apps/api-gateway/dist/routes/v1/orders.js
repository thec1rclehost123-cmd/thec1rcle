...
export default async function orderRoutes(fastify) {
    ...
    fastify.get('/event/:eventId', async (request, reply) => {
        ...
        if (!eventData)
            return reply.status(404).send({ error: "Event data missing" });
        // 2. RBAC Check
        ...
        const rsvps = rsvpsSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data(), isRSVP: true }));
        const allOrders = [...orders, ...rsvps].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
