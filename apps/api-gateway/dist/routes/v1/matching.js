...
export default async function matchingRoutes(fastify) {
    ...
    fastify.get('/feed', {
        ...
        const { lat, lng, limit = 20, type = 'event' } = request.query;
        try {
    ...
    fastify.post('/swipe', {
        ...
        if (!userId)
            return reply.status(401).send({ error: "Unauthorized" });
        const { targetId, targetType = 'event', direction } = request.body;
    ...
    fastify.post('/report', {
        ...
        if (!userId)
            return reply.status(401).send({ error: "Unauthorized" });
        const { targetId, targetType = 'event', reason, details } = request.body;
        if (!targetId || !reason) {
