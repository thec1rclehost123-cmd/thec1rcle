import { generateSecureToken, validateTransfer } from '@c1rcle/core/ticket-engine';
export default async function ticketRoutes(fastify) {
    ...
    fastify.post('/claim', async (request, reply) => {
        ...
        if (!recipientId)
            return reply.status(401).send({ error: "Unauthorized" });
        if (!transferToken)
            return reply.status(400).send({ error: "transferToken is required" });
     ...
     */
    fastify.get('/my-tickets', async (request, reply) => {
    ...
    fastify.get('/transfer', async (request, reply) => {
        ...
        if (!code)
            return reply.status(400).send({ error: 'Transfer code is required' });
