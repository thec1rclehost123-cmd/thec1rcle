...
export default async function scanRoutes(fastify) {
     ...
     */
    fastify.get('/guestlist', async (request, reply) => {
        const { eventId } = request.query;
        if (!eventId)
            return reply.status(400).send({ error: 'eventId required' });
     ...
     */
    fastify.post('/door-entry', async (request, reply) => {
     ...
     */
    fastify.get('/door-entry', async (request, reply) => {
        const { eventId } = request.query;
        if (!eventId)
            return reply.status(400).send({ error: 'eventId required' });
     ...
     */
    fastify.get('/entitlements/:id/qr', async (request, reply) => {
        const { id } = request.params;
