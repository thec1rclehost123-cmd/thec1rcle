  ...
  const requireRoles = (
    ...
    return async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const activeMembership =
        request.authContext?.activeMembership || user.activeMembership || null;
      ...
      if (workspaceId) {
        try {
          ...
          if (!memberDoc.exists || memberDoc.data()?.isActive === false) {
            fastify.log.warn(
              `RBAC: workspace access revoked for user ${user.uid} in workspace ${workspaceId}`,
            );
            return reply
              .status(403)
              .send({ error: 'Forbidden: Workspace access has been revoked.' });
          }
          ...
          return reply
            .status(403)
            .send({ error: 'Forbidden: Could not verify workspace permissions.' });
        }
      }
      ...
      if (!effectiveRole || !allowedRoles.includes(effectiveRole as Role)) {
        fastify.log.warn(
          `RBAC Denied: User ${user.uid} (Role: ${effectiveRole}) attempted to access restricted route requiring [${allowedRoles.join(', ')}] in workspace ${workspaceId || 'global'}.`,
        );
