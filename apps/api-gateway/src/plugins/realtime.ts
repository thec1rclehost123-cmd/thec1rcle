...
export default fp(async (fastify: FastifyInstance) => {
  ...
  const subRedis = new Redis(redisUrl, {
    ...
    lazyConnect: true,
    retryStrategy: (times) => {
      // Back off heavily in development to prevent console log spam
      if (process.env.NODE_ENV === 'development') {
        return 30000; // Retry once every 30 seconds
      }
      return Math.min(times * 500, 10000); // Back off up to 10s in production
    },
  });
