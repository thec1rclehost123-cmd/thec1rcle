...
function normalizeHostSeo(data: any, baseUrl: string) {
  ...
  return {
    ...
    description:
      host.bio ||
      `Discover events by ${host.name || host.displayName || 'this host'} on THE C1RCLE`,
...
function normalizeVenueSeo(data: any, baseUrl: string) {
  ...
  return {
    ...
    description:
      venue.bio ||
      venue.description ||
      `Discover events at ${venue.name || 'this venue'} on THE C1RCLE`,
...
export default async function seoRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/seo/public/events/:idOrSlug', async (request: any, reply) => {
    try {
      ...
      request.log.error(
        ...
        'GET /seo/public/events/:idOrSlug failed',
      );
      return reply.status(500).send(
        buildErrorResponse({
          code: 'INTERNAL_ERROR',
  ...
  fastify.get('/seo/public/hosts/:slug', async (request: any, reply) => {
    try {
      ...
      request.log.error(
        { requestId: request.id, error: error.message },
        'GET /seo/public/hosts/:slug failed',
      );
      return reply.status(500).send(
        buildErrorResponse({
          code: 'INTERNAL_ERROR',
  ...
  fastify.get('/seo/public/venues/:slug', async (request: any, reply) => {
    try {
      ...
      request.log.error(
        { requestId: request.id, error: error.message },
        'GET /seo/public/venues/:slug failed',
      );
      return reply.status(500).send(
        buildErrorResponse({
          code: 'INTERNAL_ERROR',
  ...
  fastify.get('/seo/public/promoters/:username', async (request: any, reply) => {
    try {
      const promoter = await getPromoterByUsername(
        String(request.params.username || '').toLowerCase(),
      );
      ...
      request.log.error(
        { requestId: request.id, error: error.message },
        'GET /seo/public/promoters/:username failed',
      );
      return reply.status(500).send(
        buildErrorResponse({
          code: 'INTERNAL_ERROR',
