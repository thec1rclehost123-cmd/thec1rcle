...
export default async function publicRoutes(fastify: FastifyInstance) {
  fastify.decorate(
    ...
    async (target: 'events' | 'hosts' | 'venues' | 'search' | 'all' = 'all') => {
      const namespaces =
        target === 'all' ? ['events', 'hosts', 'venues', 'search', 'homepage'] : [target];
  ...
  fastify.get('/homepage/selects', async (request: any, reply) => {
    try {
      ...
      return reply
        .status(500)
        .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load homepage selects'));
    }
  });
  ...
  fastify.get('/homepage/interviews', async (request: any, reply) => {
    try {
      ...
      applyPublicCacheHeaders(reply, 300);
      return await cachedPublic(fastify, 'homepage', 'interviews', 300, () =>
      ...
      return reply
        .status(500)
        .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load homepage interviews'));
    }
  });

  fastify.get(
    '/events',
    { preHandler: fastify.validate({ querystring: publicListQuerySchema }) },
    async (request: any, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public events'));
      }
    },
  );
    ...
    async (request: any, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send(
            createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load featured public events'),
          );
      }
    },
  );
  ...
  fastify.get(
    '/events/:idOrSlug',
    { preHandler: fastify.validate({ params: eventParamSchema }) },
    async (request: any, reply) => {
      try {
        ...
        if (!result)
          return reply.status(404).send(createApiError(request.id, 'NOT_FOUND', 'Event not found'));
        return result;
      } catch (error: any) {
        if (error.message === 'RATE_LIMITED')
          return reply
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public event'));
      }
    },
  );
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      try {
        ...
        return await cachedPublic(fastify, 'hosts', JSON.stringify(sortedQuery), 120, () =>
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public hosts'));
      }
    },
  );
  ...
  fastify.get(
    '/hosts/:slug',
    { preHandler: fastify.validate({ params: slugParamSchema }) },
    async (request: any, reply) => {
      try {
        ...
        if (!result)
          return reply.status(404).send(createApiError(request.id, 'NOT_FOUND', 'Host not found'));
        return result;
      } catch (error: any) {
        if (error.message === 'RATE_LIMITED')
          return reply
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public host'));
      }
    },
  );
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      try {
        ...
        applyPublicCacheHeaders(reply, 120);
        const sortedQuery = sortObjectKeys(request.query || {});
        return await cachedPublic(fastify, 'venues', JSON.stringify(sortedQuery), 120, () =>
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public venues'));
      }
    },
  );
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      try {
        ...
        if (!result)
          return reply.status(404).send(createApiError(request.id, 'NOT_FOUND', 'Venue not found'));
        return result;
      } catch (error: any) {
        if (error.message === 'RATE_LIMITED')
          return reply
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to load public venue'));
      }
    },
  );
  ...
  fastify.get(
    '/search',
    { preHandler: fastify.validate({ querystring: publicSearchQuerySchema }) },
    async (request: any, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send(createApiError(request.id, 'INTERNAL_ERROR', 'Unable to search public discovery'));
      }
    },
  );
}
