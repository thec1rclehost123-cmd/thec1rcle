...
const VenueQuerySchema = z
  .object({
    venueId: z.string(),
  })
  .strict();

const StaffIdParamSchema = z
  .object({
    id: z.string(),
  })
  .strict();
...
export default async function staffRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      if (!hasAccess) {
        fastify.log.warn(
          { venueId, actorId, membershipId },
          '🛡️ SECURITY ALERT: Cross-tenant staff resolution attempt blocked!',
        );
      ...
      if (data.staffProfileId) {
        const profileDoc = await fastify.db
          .collection('staff_profiles')
          .doc(data.staffProfileId)
          .get();
