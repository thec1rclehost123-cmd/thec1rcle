...
const ConnectionRespondSchema = z
  .object({
    action: z.string().optional(),
...
function normalizePromoterLink(
  legacyLink: PlainRecord = {},
  unifiedLink: PlainRecord = {},
  rawLink: PlainRecord = {},
) {
  ...
  const clicks = toNumber(
    unifiedLink.clickCount ??
      legacyLink.clickCount ??
      legacyLink.clicks ??
      rawLink.clickCount ??
      rawLink.clicks,
  );
  const conversions = toNumber(
    unifiedLink.conversionCount ??
      legacyLink.conversionCount ??
      legacyLink.conversions ??
      rawLink.conversionCount ??
      rawLink.conversions,
  );
  ...
  return {
    ...
    commissionRate: toNumber(
      legacyLink.commissionRate ?? unifiedLink.commissionRate ?? rawLink.commissionRate,
    ),
    ...
    clearedCommission: toNumber(
      legacyLink.clearedCommission ?? legacyLink.commission ?? rawLink.commission,
    ),
...
function normalizePromoterConnection(
  legacyConnection: PlainRecord = {},
  unifiedConnection: PlainRecord = {},
  rawConnection: PlainRecord = {},
) {
  const id = String(
    legacyConnection.id || unifiedConnection.connectionId || rawConnection.id || '',
  );
  ...
  return {
    ...
    connectionId: id,
    promoterId:
      legacyConnection.promoterId ??
      rawConnection.promoterId ??
      unifiedConnection.fromPartnerId ??
      null,
    fromPartnerId:
      unifiedConnection.fromPartnerId ??
      rawConnection.fromPartnerId ??
      legacyConnection.promoterId ??
      null,
    toPartnerId:
      unifiedConnection.toPartnerId ??
      rawConnection.toPartnerId ??
      legacyConnection.targetId ??
      legacyConnection.otherId ??
      null,
    targetId:
      legacyConnection.targetId ?? rawConnection.targetId ?? unifiedConnection.toPartnerId ?? null,
    targetName:
      legacyConnection.targetName ??
      rawConnection.targetName ??
      legacyConnection.otherName ??
      'Partner',
    targetType:
      legacyConnection.targetType ??
      rawConnection.targetType ??
      legacyConnection.otherType ??
      'venue',
    otherId:
      legacyConnection.otherId ??
      legacyConnection.targetId ??
      rawConnection.targetId ??
      unifiedConnection.toPartnerId ??
      null,
    otherName:
      legacyConnection.otherName ??
    ...
    status:
      legacyConnection.status ?? unifiedConnection.status ?? rawConnection.status ?? 'pending',
    createdAt:
      legacyConnection.createdAt ?? unifiedConnection.createdAt ?? toIso(rawConnection.createdAt),
    updatedAt:
...
export default async function partnersPromoterRoutes(fastify: FastifyInstance) {
  ...
  const buildLegacyAssignments = async (promoterId: string, status?: string) => {
    ...
    const linkSnap = await fastify.db
      ...
      .get()
      .catch(() => ({ docs: [] as any[] }));
    const assignments = (assignmentSnap as any).docs.map((doc: any) => ({
      id: doc.id,
      ...(doc.data() || {}),
    }));
  ...
  const updateLegacyConnection = async (
    promoterId: string,
    connectionId: string,
    body: Record<string, any>,
  ) => {
  ...
  const resolvePromoterGuests = async (
    ...
    const guests = orderDocs.map((doc: any) => {
      ...
      return {
        ...
        eventTitle: eventMap.get(String(order.eventId || '')) || 'Event',
        eventId: order.eventId,
        amount,
        commission: Math.round(amount * commissionRate * 100) / 100,
        ticketCount: toNumber(order.ticketCount || 1),
        status: String(order.status || 'paid'),
        checkedIn: !!order.checkedIn,
        source: order.promoterCode || 'link',
  ...
  fastify.post('/partners/promoters/network/connect', async (request: any, reply: any) => {
    ...
    try {
      return reply.send(
        await manageConnection(String(body.action || ''), { ...body, actor: request.user }),
      );
    } catch {
    ...
    async (request: any, reply: any) => {
      ...
      try {
        ...
        fastify.log.error(
          ...
          'partners/promoters/overview error',
        );
        if (err.statusCode)
          return reply
  ...
  fastify.patch(
    ...
    async (request: any, reply: any) => {
      ...
      try {
        requireType(ctx, 'promoter');
        const legacyBody = await updateLegacyLink(
          ctx.partnerId,
  ...
  fastify.patch(
    ...
    async (request: any, reply: any) => {
      ...
      try {
        requireType(ctx, 'promoter');
        const legacyBody = await updateLegacyConnection(
          ctx.partnerId,
    ...
    async (request: any, reply: any) => {
      try {
        ...
        const work = async () => {
          ...
          const balances: any = await financeService.getBalances(promoterCtx);
