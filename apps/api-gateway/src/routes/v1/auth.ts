...
const HostVerificationSchema = z
  .object({
...
function setSessionCookie(reply: any, sessionCookie: string, rememberMe = true) {
  appendSetCookieHeader(
    ...
    serializeCookie(
      SESSION_COOKIE_NAME,
      sessionCookie,
      rememberMe ? SESSION_MAX_AGE_SECONDS : 60 * 60 * 24,
    ),
  );
}
...
async function createSessionFromIdToken(
  fastify: FastifyInstance,
  reply: any,
  idToken: string,
  rememberMe = true,
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          user: activeMembership ? { ...bootstrap.user, activeMembership } : bootstrap.user,
          activeMembership,
          csrfToken,
        };
        ...
        if (error.code === 'auth/invalid-phone-number' || error.message === 'INVALID_LENGTH') {
          return reply.status(400).send(
            buildErrorResponse({
              ...
              message:
                'Invalid phone number. Please include your country code (e.g. +91 9876543210).',
              requestId: request.id,
            }),
          );
        }
    ...
    async (request: any, reply) => {
      ...
      if (request.query.error) {
        return reply.redirect(
          `${getPortalBaseUrl()}${nextPath}?error=${encodeURIComponent(request.query.error)}`,
        );
      }
      ...
      if (!clientId || !clientSecret || !request.query.code) {
        return reply.redirect(
          `${getPortalBaseUrl()}${nextPath}?error=${encodeURIComponent('google_sign_in_unavailable')}`,
        );
      }
      ...
      try {
        ...
        const errorCode = isAccountConflict
          ? 'account_exists_with_different_credential'
          : 'google_sign_in_failed';
        return reply.redirect(
          `${getPortalBaseUrl()}${nextPath}?error=${encodeURIComponent(errorCode)}`,
        );
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        fastify.log.warn(
          { requestId: request.id, recipient, error: error.message },
