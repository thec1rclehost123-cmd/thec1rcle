...
export default async function recommendationRoutes(fastify: FastifyInstance) {
    fastify.get('/recommendations', {
        ...
        try {
            ...
            if (type === 'similar') {
                if (!eventId) {
                    return reply.status(400).send({ error: 'Event ID required for similar recommendations' });
                }
            ...
            request.log.error({ requestId: request.id, error: error?.message }, 'GET /recommendations failed');
