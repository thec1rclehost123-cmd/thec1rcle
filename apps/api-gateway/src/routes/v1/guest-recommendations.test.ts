...
describe('guest recommendations routes', () => {
    ...
    it('GET /api/v1/recommendations keeps similar recommendation validation', async () => {
        ...
        const missing = await server.inject({ method: 'GET', url: '/api/v1/recommendations?type=similar' });
        const found = await server.inject({ method: 'GET', url: '/api/v1/recommendations?type=similar&eventId=event_1&limit=4' });
