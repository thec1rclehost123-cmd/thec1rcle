...
export default async function promoterRoutes(fastify: FastifyInstance) {
    ...
    fastify.post('/promoter/links', {
        ...
        if (!trackingCode) {
            if (body.customTrackingCode) {
                ...
                if (cleanCode.length < 3) return reply.status(400).send({ error: 'Custom code must be at least 3 characters' });
                ...
                if (!existingGlobal.empty) {
                    return reply.status(409).send({ error: 'This custom code is already taken. Please choose another.' });
                }
    ...
    fastify.post('/promoter/links/click', {
        preHandler: [fastify.validate({ body: z.object({ code: z.string(), source: z.string().optional(), eventId: z.string().optional() }) })]
    }, async (request, reply) => {
    ...
    fastify.patch('/promoter/connections', {
        ...
        if (['revoked', 'blocked', 'rejected', 'removed'].includes(nextStatus)) {
            try {
                const connDoc = await fastify.db.collection('promoter_connections').doc(connectionId).get();
                if (connDoc.exists) {
                    ...
                    if (eventIds.length > 0) {
                        const linksSnap = await fastify.db.collection('promoter_links').where('promoterId', '==', current.promoterId).get();
