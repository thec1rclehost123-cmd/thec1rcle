...
export default async function scanRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/", async (request: any, reply) => {
    ...
    const existingScan = await fastify.db
    ...
    if (!existingScan.empty) {
      ...
      return reply.status(400).send({
        ...
        result: "already_scanned",
        previousScan: {
          scannedAt: existingScan.docs[0].data().scannedAt,
          scannedBy: existingScan.docs[0].data().scannedBy,
        },
      });
    }
  ...
  fastify.post("/auth", async (request: any, reply) => {
    ...
    const doorRevenue = doorOrdersSnap.docs.reduce(
      (s: number, d: any) => s + (d.data().total || 0),
      0,
    );
  ...
  fastify.get("/stats", async (request: any, reply) => {
    ...
    scansSnap.docs.forEach((d: any) => {
      const et = d.data().entryType || "general";
      byEntryType[et] = (byEntryType[et] || 0) + (d.data().quantity || 1);
    });
    const totalEntered =
      prebookedScans.reduce((s: number, d: any) => s + (d.data().quantity || 1), 0) +
      doorSnap.docs.length;
    return {
      totalEntered,
      prebooked: prebookedScans.length,
      doorEntries: doorSnap.docs.length,
      doorRevenue,
      byEntryType,
    };
  });
  ...
  fastify.get("/guestlist", async (request: any, reply) => {
    ...
    guests.sort((a: any, b: any) =>
      a.status !== b.status ? (a.status === "not_entered" ? -1 : 1) : a.name.localeCompare(b.name),
    );
    return { guests };
  });
  ...
  fastify.post("/door-entry", async (request: any, reply) => {
    ...
    qrPayload.sig = createHmac("sha256", QR_SECRET)
      .update(`${orderId}:${eventId}:${ticketId}:${qrPayload.u}:${quantity}:${ts}`)
      .digest("hex")
      .substring(0, 16);
