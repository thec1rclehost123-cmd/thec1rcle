...
const EventTrackBody = z
  .object({
    type: z.enum(['view', 'click', 'share', 'rsvp_intent', 'impression']),
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        await fastify.publicDiscoveryService.syncEventReadModels(event.id).catch((err: any) => {
          fastify.log.error(
            { eventId: event.id, error: err?.message || String(err) },
            '[syncEventReadModels] Failed to sync event card index after create',
          );
        });
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        await fastify.publicDiscoveryService.syncEventReadModels(event.id).catch((err: any) => {
          fastify.log.error(
            { eventId: event.id, error: err?.message || String(err) },
            '[syncEventReadModels] Failed to sync event card index after update',
          );
        });
        return { success: true, id: event.id };
      } catch (error: any) {
