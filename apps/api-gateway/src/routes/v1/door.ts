...
const DoorQuerySchema = z.object({
...
export default async function doorRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/venue/walk-ins', {
        ...
        try {
            const snap = await fastify.db.collection('walk_ins')
    ...
    fastify.get('/venue/door/dinein', {
        ...
        try {
            const snap = await fastify.db.collection('dine_in_entries')
                .where('eventId', '==', eventId)
                .limit(200)
                .get();
     ...
     */
    fastify.post('/venue/door/dinein', {
