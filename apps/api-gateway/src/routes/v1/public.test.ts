...
async function buildServer() {
  ...
  const publicDiscoveryService = {
    ...
    listHosts: vi.fn(async (query) => ({
      items: [{ id: 'host_1' }],
      nextCursor: null,
      hasMore: false,
      appliedFilters: query,
    })),
    getHostPublicProfile: vi.fn(async (slug) => ({
      host: { id: 'host_1', slug },
      stats: {},
      posts: [],
      highlights: [],
    ...
    listVenues: vi.fn(async (query) => ({
      items: [{ id: 'venue_1' }],
      nextCursor: null,
      hasMore: false,
      appliedFilters: query,
    })),
    getVenuePublicProfile: vi.fn(async (slug) => ({
      venue: { id: 'venue_1', slug },
      stats: {},
      highlights: [],
      upcomingEvents: [],
      pastEvents: [],
...
describe('public discovery routes GP-2 contracts', () => {
  it('GET /public/events accepts legacy guest query params and returns paginated items', async () => {
    ...
    expect(publicDiscoveryService.listEvents).toHaveBeenCalledWith(
    ...
    expect(publicDiscoveryService.listEvents).not.toHaveBeenCalledWith(
      expect.objectContaining({ city: 'Pune, IN' }),
    );
  ...
  it('GET /public/events/featured is registered before event detail and returns featured items', async () => {
    ...
    const response = await server.inject({
      method: 'GET',
      url: '/public/events/featured?limit=6&city=Pune&sort=Trending',
    });

    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ items: [{ id: 'event_featured' }] });
    expect(publicDiscoveryService.listFeaturedEvents).toHaveBeenCalledWith(
      expect.objectContaining({
        limit: 6,
        cityKey: 'pune-in',
        sort: 'heat',
  ...
  it('GET /public/hosts and /public/venues accept GP-2 filters and sort aliases', async () => {
    ...
    const hosts = await server.inject({
      method: 'GET',
      url: '/public/hosts?role=DJ&vibe=House&status=Verified&sort=Most%20followed',
    });
    const venues = await server.inject({
      method: 'GET',
      url: '/public/venues?area=Koregaon%20Park&vibe=Techno&tablesOnly=true&sort=Soonest%20event',
    });

    expect(hosts.statusCode).toBe(200);
    expect(venues.statusCode).toBe(200);
    expect(publicDiscoveryService.listHosts).toHaveBeenCalledWith(
      expect.objectContaining({
        role: 'DJ',
        vibe: 'House',
        status: 'Verified',
    ...
    expect(publicDiscoveryService.listVenues).toHaveBeenCalledWith(
      expect.objectContaining({
        area: 'Koregaon Park',
        vibe: 'Techno',
        tablesOnly: 'true',
