...
export default async function promoterLinksRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/by-code/:code', {
        preHandler: [fastify.validate({ params: CodeParam, querystring: z.object({ eventId: z.string().optional() }) })]
    }, async (request: any, reply) => {
        const { code } = request.params;
        const { eventId } = request.query;
        let query = fastify.db.collection(LINKS_COL).where('code', '==', code).where('isActive', '==', true);
        if (eventId) {
