...
const GuestNotificationsQuery = z
  .object({
...
const GuestNotificationIdParam = z
  .object({
    id: z.string(),
  })
  .strict();

const GuestNotificationsPatchBody = z
  .object({
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        fastify.log.error(
          ...
          'GET /guest-notifications failed',
        );
  ...
  fastify.patch(
    ...
    async (request: any, reply) => {
      ...
      try {
        ...
        fastify.log.error(
          { requestId: request.id, userId, error: error.message },
          'PATCH /guest-notifications failed',
        );
        return reply.status(500).send(
          buildErrorResponse({
            code: 'INTERNAL_ERROR',
