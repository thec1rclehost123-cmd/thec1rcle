...
export class PromoterServiceV2 {
  ...
  async getAnalytics(
    context: PromoterActorContext,
    filters: { range?: TimelineRange; eventId?: string },
  ) {
    ...
    const timelineMap = new Map<
      string,
      { date: string; conversions: number; revenue: number; commission: number; clicks: number }
    >();
    rows.forEach((row) => {
      const bucket = dateBucket(row.date);
      if (!bucket) return;
      const existing = timelineMap.get(bucket) || {
        date: bucket,
        conversions: 0,
        revenue: 0,
        commission: 0,
    ...
    const timeline = [...timelineMap.values()].sort((left, right) =>
    ...
    const summary = {
      ...
      conversionRate:
        linksPayload.summary.totalClicks > 0
  ...
  async listEvents(context: PromoterActorContext, filters: EventFilters = {}) {
    ...
    return {
      ...
      summary: {
        ...
        scheduledEvents: items.filter((item) => String(item.lifecycle).toLowerCase() !== 'live')
          .length,
      },
      nextCursor: null,
      meta: {
  ...
  async getFinance(context: PromoterActorContext, filters: FinanceFilters = {}) {
    ...
    return {
      ...
      payouts,
      payoutSummary: {
        pendingCount: payouts.filter((payout) =>
          isTruthyStatus(payout.status, ['pending', 'processing', 'in_transit']),
        ).length,
        paidCount: payouts.filter((payout) =>
          isTruthyStatus(payout.status, ['completed', 'paid', 'cleared']),
        ).length,
  ...
  async getProfile(context: PromoterActorContext) {
    ...
    return {
      profile: {
        ...
        isVerified:
          promoter.isVerified === true ||
          String(promoter.kycStatus || '').toLowerCase() === 'verified',
  ...
  private async loadEventsByIds(eventIds: string[]) {
    ...
    const docs = await Promise.all(
      uniqueIds.map((eventId) => this.db.collection('events').doc(eventId).get()),
    );
  ...
  private async loadPartnerLedgerRows(promoterId: string, eventId?: string, limit = 200) {
    ...
    return rows
      ...
      .map((row) => {
        ...
        const status =
          row.status === 'settled' ? 'cleared' : row.status === 'failed' ? 'rejected' : 'pending';
        return {
          id: String(row.id),
  ...
  private async loadOrdersByIds(orderIds: string[]) {
    ...
    const docs = await Promise.all(
      uniqueIds.map((orderId) => this.db.collection('orders').doc(orderId).get()),
    );
    return docs.reduce((map, doc) => {
      if (doc.exists) map.set(doc.id, doc.data() || {});
      return map;
