...
describe('PublicDiscoveryService', () => {
    ...
    it('listEvents hides ended and non-public lifecycle items by default and dedupes ids', async () => {
        ...
        expect(result.items.map((item: any) => item.id)).toEqual(['event_live', 'event_upcoming', 'event_approved']);
    });
    ...
    it('listEvents uses bounded read-model queries for first-page city browsing', async () => {
        ...
        expect(service.events.queryList).toHaveBeenCalledWith(expect.objectContaining({
            cityKey: 'pune-in',
            limit: 24,
        }));
    ...
    it('listHosts and listVenues use bounded read-model queries on first-page browse requests', async () => {
        const service = buildService();
        service.hosts = {
            queryList: vi.fn(async () => ([{ id: 'host_1', visibility: 'public', followersCount: 10, cityKey: 'pune-in' }])),
            listAll: vi.fn(async () => {
                throw new Error('should not full-scan hosts');
            }),
        };
        service.venues = {
            queryList: vi.fn(async () => ([{ id: 'venue_1', visibility: 'public', followersCount: 20, tablesAvailable: true, cityKey: 'pune-in' }])),
            listAll: vi.fn(async () => {
        ...
        const venues = await service.listVenues({ limit: 12, city: 'Pune', tablesOnly: 'true', sort: 'Most followed' });
    ...
    it('listHosts keeps scanning backend slices until filtered matches fill the page', async () => {
        ...
        service.hosts = {
            queryList: vi.fn(async ({ cursor }: any) => {
                ...
                if (cursor.id === 'host_48') {
                    return [
                        { id: 'host_49', visibility: 'public', cityKey: 'pune-in', role: 'DJ', vibes: ['House'], followersCount: 70, trending: true },
                    ];
                }
                return [];
            }),
        ...
        const result = await service.listHosts({ limit: 1, city: 'Pune', role: 'DJ', vibe: 'House', status: 'Trending', sort: 'Most followed' });
    ...
    it('listVenues keeps scanning backend slices until filtered matches fill the page', async () => {
        ...
        service.venues = {
            queryList: vi.fn(async ({ cursor }: any) => {
                ...
                if (cursor.id === 'venue_48') {
                    return [
                        { id: 'venue_49', visibility: 'public', cityKey: 'pune-in', areaKey: 'koregaon-park', tags: ['House'], tablesAvailable: true, followersCount: 100 },
                    ];
                }
                return [];
            }),
        ...
        const result = await service.listVenues({ limit: 1, city: 'Pune', area: 'Koregaon Park', vibe: 'House', tablesOnly: 'true', sort: 'Most followed' });
    ...
    it('search uses bounded prefix queries before falling back to full scans', async () => {
        const service = buildService();
        service.events = {
            querySearchPrefix: vi.fn(async () => ([{ id: 'event_1', visibility: 'public', searchText: 'after dark', heatScore: 40 }])),
            listAll: vi.fn(async () => {
                throw new Error('should not full-scan events');
            }),
        };
        service.hosts = {
            querySearchPrefix: vi.fn(async () => ([{ id: 'host_1', visibility: 'public', searchText: 'after host', followersCount: 10 }])),
            listAll: vi.fn(async () => {
                throw new Error('should not full-scan hosts');
            }),
        };
        service.venues = {
            querySearchPrefix: vi.fn(async () => ([{ id: 'venue_1', visibility: 'public', searchText: 'after venue', followersCount: 20 }])),
            listAll: vi.fn(async () => {
                throw new Error('should not full-scan venues');
            }),
        };
    ...
    it('host and venue detail helpers avoid full collection scans when queryList fails', async () => {
        const service = buildService();
        service.hosts = {
            getBySlug: vi.fn(async () => ({ id: 'host_1', slug: 'after-dark', followersCount: 10, upcomingEventsCount: 1 })),
        };
        service.venues = {
            getBySlug: vi.fn(async () => ({ id: 'venue_1', slug: 'high-spirits', followersCount: 20, upcomingEventsCount: 2, cityKey: 'pune-in', areaKey: 'kp' })),
            queryList: vi.fn(async () => {
                throw new Error('missing index');
            }),
        };
    ...
    it('ensureEventCardsSeeded backfills legacy event card read models', async () => {
        ...
        const db = {
            collection: vi.fn((name: string) => {
                if (name === 'system_meta') return { doc: vi.fn(() => ({ get: vi.fn(async () => ({ exists: false })), set: vi.fn(async () => undefined) })) };
    ...
    it('bootstrapReadModels shares a single in-flight bootstrap run', async () => {
        ...
        const ensureEventCardsSeeded = vi.fn(() => new Promise<void>((resolve) => {
