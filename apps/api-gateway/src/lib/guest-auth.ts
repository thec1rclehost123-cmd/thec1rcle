...
export function normalizeGuestProfile(raw: Record<string, any> | null | undefined, fallback: Record<string, any> = {}) {
    if (!raw) return null;
...
export function buildGuestProfileCreatePayload(body: Record<string, any>, user: Record<string, any>, now = new Date().toISOString()) {
    const uid = String(user.uid);
    const email = String(body.email || user.email || '');
    const displayName = asString(body.displayName || body.name || user.name || user.displayName || 'Member');
...
export function buildGuestProfileUpdates(updates: Record<string, any>, existing: Record<string, any> = {}, now = new Date().toISOString()) {
    ...
    if (safeUpdates.gender !== undefined) {
        ...
        if (existingGender && existingGender !== safeUpdates.gender) {
            const lastChangedAt = existing?.genderLastChangedAt ? new Date(existing.genderLastChangedAt).getTime() : 0;
