...
export async function resolvePromoterRequestContext(
  ...
  const claimedMembership =
    user.activeMembership && isPromoterMembership(user.activeMembership, { requireActive: false })
  ...
  if (String(user.partnerType || '').toLowerCase() === 'promoter' && user.partnerId) {
    return {
      uid: String(user.uid),
  ...
  const activeMembership = Array.isArray(request.authContext?.memberships)
    ? request.authContext?.memberships.find((membership: Record<string, any>) =>
        isPromoterMembership(membership),
      )
    : null;
