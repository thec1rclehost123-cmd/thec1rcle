...
export async function createScannerSession(
...
): Promise<{ ok: true; sessionToken: string; sessionExpiresAt: string; sessionId: string } | { ok: false; error: string }> {
    const db = (fastify as any).db;
    const now = new Date();
    const nowIso = now.toISOString();
    const maxDevices = codeData.maxDevices ?? 5;

    const existingSnap = await db.collection(SCANNER_AUTH_SESSION_COLLECTION)
        .where('codeId', '==', codeId)
    ...
    await db.collection(SCANNER_AUTH_SESSION_COLLECTION).doc(sessionId).set({
        codeId,
        code: codeData.code,
...
export async function touchScannerSession(sessionRef: any, when = new Date().toISOString()): Promise<void> {
    try {
