...
const VENUE_PERMISSIONS: Record<string, Permission[]> = {
    OWNER:        ['VIEW_FINANCIALS', 'MANAGE_STAFF', 'MANAGE_EVENTS', 'EDIT_EVENT_RULES', 'MANAGE_TABLES', 'VIEW_GUESTLIST', 'SCAN_ENTRY', 'LOG_INCIDENTS', 'VIEW_ANALYTICS', 'MANAGE_SETTINGS', 'MANAGE_PARTNERSHIPS', 'MANAGE_PAGE_CONTENT', 'MANAGE_GUEST_OPS', 'EXPORT_GUESTS'],
    MANAGER:      ['VIEW_FINANCIALS', 'MANAGE_EVENTS', 'EDIT_EVENT_RULES', 'MANAGE_TABLES', 'VIEW_GUESTLIST', 'SCAN_ENTRY', 'LOG_INCIDENTS', 'VIEW_ANALYTICS', 'MANAGE_PAGE_CONTENT', 'MANAGE_GUEST_OPS'],
...
const HOST_PERMISSIONS: Record<string, Permission[]> = {
    OWNER:   ['VIEW_FINANCIALS', 'MANAGE_STAFF', 'MANAGE_EVENTS', 'MANAGE_PROMOTERS', 'MANAGE_PAGE_CONTENT', 'VIEW_ANALYTICS', 'MANAGE_PAYOUTS', 'MANAGE_PARTNERSHIPS', 'VIEW_REAL_TIME_SCANS'],
    COHOST:  ['MANAGE_EVENTS', 'MANAGE_PROMOTERS', 'VIEW_ANALYTICS', 'VIEW_REAL_TIME_SCANS', 'VIEW_GUESTLIST'],
...
export function getDefaultTabVisibility(partnerType: string, role: string): Partial<Record<string, boolean>> | null {
    const r = role.toUpperCase();
    const type = (partnerType === 'club' ? 'venue' : partnerType).toLowerCase();

    if (type === 'host') {
        if (r === 'OWNER') return null;
        if (r === 'COHOST') return { overview: true, events: true, calendar: true, network: true, audience: true, analytics: true, finance: false, settings: false };
        if (r === 'MANAGER') return { overview: true, events: true, calendar: true, network: false, audience: true, analytics: true, finance: false, settings: false };
        return { overview: false, events: true, calendar: true, network: false, audience: true, analytics: false, finance: false, settings: false };
    }
    if (type === 'promoter') {
