...
export async function uploadPartnerAsset(
  ...
  if (options.allowedMimeTypes?.length && data.mimetype && !options.allowedMimeTypes.includes(data.mimetype)) {
  ...
  if (buffer.length > maxBytes) {
    throw makeUploadError(`File must be ${Math.floor(maxBytes / (1024 * 1024))}MB or smaller`, 400, 'FILE_TOO_LARGE');
  }
