...
function buildEntryTypeIncrement(
  entryType: string | undefined,
  quantity: number,
): Record<string, any> {
  const normalized = (entryType || 'general').replace(/[^\w-]+/g, '_').toLowerCase();
...
export async function getScannerSummarySnapshot(
  db: any,
  eventId: string,
): Promise<ScannerSummaryData> {
...
export async function recordScannerLiveEvent(
  ...
  const summaryExtra =
    options.entryType && options.entryTypeQuantity
