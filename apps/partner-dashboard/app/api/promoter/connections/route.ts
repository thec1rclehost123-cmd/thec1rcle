...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(
      `[Promoter API] GET request. Promoter: ${promoterId}, Action: ${searchParams.get("action")}`,
    );
    ...
    switch (action) {
      case "discover": {
        ...
        const partnersWithStatus = await Promise.all(
          partners.map(async (partner) => {
            const status = await getConnectionStatus(promoterId, partner.id, partner.type);
            return {
              ...partner,
      ...
      default: {
        ...
        const connections = await listPromoterConnections(promoterId, status || undefined);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { promoterId, promoterName, promoterEmail, targetId, targetType, targetName, message } =
      body;
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    switch (action) {
      case "cancel": {
        if (!promoterId) {
          return NextResponse.json({ error: "promoterId required to cancel" }, { status: 400 });
        }
      ...
      case "revoke": {
        if (!promoterId) {
          return NextResponse.json({ error: "promoterId required to revoke" }, { status: 400 });
        }
