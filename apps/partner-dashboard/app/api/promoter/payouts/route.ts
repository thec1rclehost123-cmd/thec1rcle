...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!promoterId && !payoutId) {
      return NextResponse.json({ error: "promoterId or payoutId is required" }, { status: 400 });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (paymentMethod === "upi" && !paymentDetails?.upiId) {
      return NextResponse.json({ error: "UPI ID is required for UPI payment" }, { status: 400 });
    }
