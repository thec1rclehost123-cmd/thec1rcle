/**
 * GET /api/venue/events/[id]/overview
 * Aggregated metrics for a single venue event.
 */
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        if (isPartnerHostedEvent(ev)) {
            return NextResponse.json({ error: "Analytics are hidden for host-managed events" }, { status: 403 });
        }
        ...
        return NextResponse.json({
            ...
            estimatedEarnings: Math.round(venueNet),
            guestListSize,
            totalCheckedIn,
            conversionRate,
            sellThrough,
        ...
        console.error("[venue/events/[id]/overview]", err.message);
