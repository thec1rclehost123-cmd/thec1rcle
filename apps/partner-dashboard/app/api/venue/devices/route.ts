...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await bindDevice(venueId, { deviceId, name }, staffId || null, {
      uid: auth.uid,
    ...
    return NextResponse.json({ error: error.message || "Failed to bind device" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await revokeDevice(deviceId, { uid: auth.uid, name: auth.name || auth.email });
