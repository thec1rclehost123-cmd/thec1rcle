...
function makeRequest(authHeader: string = 'Bearer test-token') {
  return {
    ...
    url: 'http://localhost/api/host/events/evt_123/computed-analytics',
    method: 'GET',
  } as unknown as NextRequest;
}

describe('host computed-analytics route', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });
  ...
  it('returns 403 Forbidden if the user lacks view_analytics permission', async () => {
    ...
    const response = await GET(makeRequest(), {
      params: Promise.resolve({ id: 'evt_123' }),
    });
  ...
  it('returns 502 BAD_GATEWAY when the gateway responds with an error', async () => {
    requireHostAccessMock.mockResolvedValue({ uid: 'user_123', hostId: 'host_A' });
  ...
  it('returns 502 BAD_GATEWAY when the gateway rejects a cross-tenant request', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 403,
      json: () => Promise.resolve({ error: { code: 'FORBIDDEN', message: 'Host mismatch' } }),
    }) as any;
