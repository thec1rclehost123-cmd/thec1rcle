...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string; attendeeId: string }> }) {
    ...
    return proxyToGateway(req, `${GATEWAY_URL}/api/v1/host/events/${id}/attendees/${attendeeId}?${searchParams}`, {});
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; attendeeId: string }> }) {
    ...
    return proxyToGateway(req, `${GATEWAY_URL}/api/v1/host/events/${id}/attendees/${attendeeId}`, {
        method: "PATCH",
