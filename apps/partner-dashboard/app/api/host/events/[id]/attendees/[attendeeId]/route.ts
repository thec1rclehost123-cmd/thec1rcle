...
export async function GET(
    ...
    const ctx = await requireHostAccess(request, "VIEW_GUESTLIST");
    ...
    try {
        ...
        if (!ownsHostEvent(eventData, ctx.hostId)) {
        ...
        for (const snapshot of orderSnapshots) {
            for (const doc of snapshot.docs) {
                const raw = doc.data() || {};
                if ((raw.hostId && raw.hostId !== ctx.hostId) || (raw.eventId && raw.eventId !== eventId && raw.hostId !== ctx.hostId)) {
                    continue;
                }
        ...
        const orders = Array.from(matchedDocs.values())
            ...
            .filter((order) =>
                (order.eventId === eventId || order.eventId === String(eventId)) &&
                !((order.hostId && order.hostId !== ctx.hostId) || (order.creatorId && order.creatorId !== ctx.hostId)) &&
                (attendeeMatchesOrder(order as any, identity) || order.id === attendee.orderId)
            )
        ...
        console.error("[host/events/[id]/attendees/[attendeeId]]", error?.message || error);
