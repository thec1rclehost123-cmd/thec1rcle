...
export default function TablesPage() {
    ...
    const fetchTables = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await fetch(`/api/club/tables?clubId=${profile.activeMembership.partnerId}`);
            if (res.ok) {
    ...
    const fetchTonightEvent = async () => {
        if (!profile?.activeMembership?.partnerId) return;
        try {
            const res = await fetch(`/api/club/events?venueId=${profile.activeMembership.partnerId}&limit=1`);
            if (res.ok) {
                const data = await res.json();
                if (data.events?.[0]) {
    ...
    const fetchEventStatus = async (eventId: string) => {
        try {
            const res = await fetch(`/api/club/tables?eventId=${eventId}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const handleSaveTable = async (tableData: any) => {
        ...
        setIsSaving(true);
        try {
            const res = await fetch("/api/club/tables", {
                method: "POST",
    ...
    const handleDeleteTable = async (tableId: string) => {
        ...
        try {
            const res = await fetch(`/api/club/tables?tableId=${tableId}`, { method: "DELETE" });
