...
export default function GateSecurityPage() {
    ...
    const fetchSecurityData = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/club/security/sync?clubId=${clubId}`);
            if (res.ok) {
    ...
    const generateSyncCode = async (eventId: string) => {
        ...
        try {
            const clubId = profile.activeMembership.partnerId;
            const res = await fetch(`/api/club/security/sync`, {
                method: "POST",
                body: JSON.stringify({ eventId, clubId, userId: profile.uid })
            });
            if (res.ok) {
                fetchSecurityData();
            }
    ...
    const deactivateSync = async (eventId: string) => {
        ...
        try {
            ...
            const res = await fetch(`/api/club/security/sync`, {
                method: "POST",
                body: JSON.stringify({ eventId, clubId, action: "deactivate" })
            });
            if (res.ok) {
                fetchSecurityData();
            }
