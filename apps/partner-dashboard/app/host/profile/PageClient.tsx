...
export default function HostProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'posts' | 'highlights' | 'followers'>(
    'details',
  );
    ...
    if (isComposerOpen) {
      const timer = setTimeout(() => {
        ...
        localStorage.setItem(
          `host_post_draft_${profile?.activeMembership?.partnerId}`,
          JSON.stringify({ content: composerContent, image: composerImage }),
        );
    ...
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 border-b border-border-subtle pb-8">
        <div>
          ...
          <p className="text-text-tertiary text-[13px] font-medium mt-2 max-w-xl">
            Control how your host brand appears to guests and partner venues on the C1RCLE platform.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div
            key={stat.label}
            className="p-5 rounded-2xl bg-surface-secondary border border-border-subtle hover:bg-surface-tertiary transition-all"
          >
            <p
        ...
        <div className="p-6">
          {activeTab === 'details' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-5">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-text-tertiary mb-2 ml-1">
                    Identity & Branding
                  </label>
                  <input
                    type="text"
              ...
              <div className="col-span-full space-y-5 pt-6 border-t border-border-subtle">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between bg-indigo-900 rounded-xl p-5 text-text-primary shadow-xl shadow-indigo-100">
                <div>
                  ...
                  <p className="text-text-primary/60 text-[10px] font-bold mt-0.5 uppercase tracking-widest">
                    Share news, lineups, or limited drops.
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data?.posts?.map((post: any) => (
                  <div
                    key={post.id}
                    className="p-0 bg-surface-tertiary rounded-xl border border-border-subtle group relative overflow-hidden transition-all hover:shadow-md"
                  >
                    {post.imageUrl && (
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between bg-surface-secondary rounded-xl p-5 text-text-primary shadow-xl shadow-slate-200">
                <div>
                  ...
                  <p className="text-text-primary/60 text-[10px] font-bold mt-0.5 uppercase tracking-widest">
                    Pin your best performance reels to the top.
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {data?.highlights?.map((h: any) => (
                  <div key={h.id} className="group relative">
                    ...
                    <p className="text-center mt-2 text-[9px] font-black uppercase tracking-widest text-text-tertiary">
                      {h.title}
                    </p>
                    <button
          ...
          {activeTab === 'followers' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-base font-black uppercase tracking-tight text-text-primary border-b border-border-subtle pb-4">
                  Your Community
                </h3>
                <p className="text-text-tertiary text-sm font-medium mt-3">
                  Engage with the {data?.stats?.followersCount || 0} guests following your journey.
                </p>
              </div>
              ...
              <div className="bg-surface-tertiary rounded-2xl border border-border-default p-12 text-center">
                ...
                <h4 className="text-base font-black text-text-primary mb-2 uppercase tracking-tight">
                  Host Insights Incoming
                </h4>
                <p className="text-text-tertiary text-sm max-w-sm mx-auto font-medium">
                  We're building advanced audience analytics for our top hosts. Stay tuned.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className="mb-5">
                ...
                <p className="text-text-tertiary text-[10px] font-black uppercase tracking-widest mt-0.5">
                  Specify {photoModal.field === 'photos' ? 'gallery' : photoModal.field} source URL
                </p>
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-12">
            ...
            >
              ...
              <div className="flex-1 flex flex-col border-r border-border-subtle">
                <div className="p-5 border-b border-slate-50 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setIsComposerOpen(false)}
                      className="p-1.5 hover:bg-surface-tertiary rounded-full transition-colors"
                    >
              ...
              <div className="w-full md:w-[320px] bg-surface-tertiary p-6 flex flex-col">
                <label className="text-[10px] font-black uppercase tracking-widest text-text-tertiary mb-5">
                  Page Preview
                </label>
...
function TabButton({ active, icon: Icon, label, onClick }: any) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 min-w-[90px] py-4 flex flex-col items-center gap-1.5 border-b-2 transition-all ${
        active
