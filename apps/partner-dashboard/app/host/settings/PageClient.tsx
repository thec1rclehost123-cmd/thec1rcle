...
import {
  sendOperationalPasswordResetEmail,
...
export default function HostSettingsPage() {
    ...
    try {
      ...
      if (pid === 'password') {
        ...
        await reauthenticateWithCredential(
          user,
          EmailAuthProvider.credential(user.email ?? '', reAuthPw),
        );
      } else {
      ...
      setReAuthErr(
        e.code === 'auth/wrong-password' ? 'Incorrect password.' : 'Authentication failed.',
      );
    } finally {
      setReAuthBusy(false);
    }
  ...
  if (hubTab === 'profile') {
    return <ProfileTabShell hubTab={hubTab} setHubTab={setHubTab} />;
  }
  ...
  if (isLoading) {
    return (
      <div className="py-24 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[var(--v-text-tertiary)]">
          Loading
        </p>
      </div>
    );
    ...
    <VenuePageShell title="Settings" subtitle="Organisation configuration">
      ...
      <div className="max-w-2xl space-y-10">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Currency">
              <Select
                value={local?.defaultCurrency ?? 'INR'}
        ...
        <section className="space-y-6">
          ...
          {isGoogleUser && (
            <div className="flex items-center justify-between p-5 rounded-2xl bg-[var(--v-card)] border border-[var(--v-border)]">
              <div>
                ...
                <p className="text-[12px] text-[var(--v-text-tertiary)] mt-0.5">
                  Signed in via Google — manage password at accounts.google.com
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[var(--v-text-tertiary)] px-1">
              Active Sessions
            </p>
            ...
            {sessionsLoading ? (
              ...
              <>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[14px] font-black text-text-primary">
                          {parseBrowser(
                            sessions.find((s) => s.sessionId === currentSid)?.userAgent ?? null,
                          )}
                        </p>
                        <p className="text-[12px] text-[var(--v-text-tertiary)] mt-0.5">
                          Active{' '}
                          {relativeTime(
                            sessions.find((s) => s.sessionId === currentSid)?.lastActiveAt ??
                              new Date().toISOString(),
                          )}
                        </p>
                      </div>
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-[var(--v-orange)]">
                      Current
                    </span>
                  </div>
                )}
                ...
                {otherSessions.map((s) => (
                  <div
                    key={s.sessionId}
                    className="flex items-center justify-between p-5 rounded-2xl bg-[var(--v-card)] border border-[var(--v-border)]"
                  >
                    <div className="flex items-center gap-4">
                      {s.deviceType === 'mobile' ? (
                        <Smartphone className="w-4 h-4 text-[var(--v-text-tertiary)] shrink-0" />
                ...
                {sessions.length === 0 && (
                  <p className="text-[13px] text-[var(--v-text-tertiary)] px-1">
        ...
        <section className="space-y-5">
          ...
          <div className="grid grid-cols-4 gap-3 px-1">
            ...
            {NOTIF_CHANNELS.map((ch) => (
              <p
                key={ch.key}
                className="text-[11px] font-black uppercase tracking-[0.15em] text-[var(--v-text-tertiary)] text-center"
              >
                {ch.label}
              </p>
            ))}
          ...
          <div className="space-y-2">
            {NOTIF_CATEGORIES.map((cat) => (
              <div
                key={cat.key}
                className="grid grid-cols-4 gap-3 items-center px-5 py-4 rounded-2xl bg-[var(--v-card)] border border-[var(--v-border)]"
              >
                ...
                {NOTIF_CHANNELS.map((ch) => {
                  ...
                  return (
                    <div key={ch.key} className="flex items-center justify-center">
                      ...
                      >
                        <div
                          className={`absolute top-0.5 w-[18px] h-[18px] rounded-full bg-white shadow-sm transition-all ${on ? 'left-[20px]' : 'left-0.5'}`}
                        />
                      </button>
                    </div>
                  );
      ...
      <AnimatePresence>
        {isDirty && (
          ...
          >
            {saveError && (
              <span className="text-[12px] font-bold text-[var(--v-error)]">{saveError}</span>
            )}
            {!saveError && (
              <span className="text-[12px] font-black uppercase tracking-wider text-[var(--v-text-secondary)]">
                Unsaved changes
              </span>
            )}
            <button
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div>
                ...
                <p className="text-[13px] font-medium mt-1 text-[var(--v-text-tertiary)]">
              ...
              {!isGoogleUser ? (
                <div className="space-y-4">
                  ...
                  />
                  {reAuthErr && (
                    <p className="text-[12px] font-bold text-[var(--v-error)]">{reAuthErr}</p>
                  )}
                  <VenueActionButton
                    variant="primary"
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[var(--v-text-tertiary)]">
        {label}
      </p>
      {children}
    </div>
