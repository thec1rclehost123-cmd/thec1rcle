...
const StatPill = ({
  ...
  <div className="flex flex-col items-center gap-2 px-6 py-5 rounded-[2rem] bg-surface-secondary/50 border border-border-subtle hover:bg-surface-secondary/80 hover:border-border-default transition-all group">
    <div
      className={cn(
        'w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300',
        color,
      )}
    >
      <Icon size={18} />
...
export default function HostProfileClient({
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const connectionsData =
          connectionsRes.status === 'fulfilled' && connectionsRes.value.ok
        ...
        const computed = {
          ...
          venuesConnected:
            summaryData?.summary?.activeVenuePartnerships ??
            active.filter((c) => c.otherType === 'venue').length,
          promotersConnected:
            summaryData?.summary?.activePromoterPartnerships ??
            active.filter((c) => c.otherType === 'promoter').length,
        };
        setStats(computed);
    ...
    if (hasChanges || isSaving) {
      setActions(
        <VenueActionButton
          variant="primary"
          onClick={handleSave}
          disabled={isSaving || photoUploading || backdropUploading}
        >
          {isSaving ? (
            'Saving...'
          ) : (
  ...
  const statItems = [
    ...
    {
      icon: Handshake,
      value: stats.venuesConnected,
      label: 'Venues Connected',
      color: 'bg-amber-500/10 text-amber-400',
    },
    {
      icon: Megaphone,
      value: stats.promotersConnected,
      label: 'Promoters Connected',
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      >
        ...
        {/* Stats strip */}
        <div
          className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-6"
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-tertiary mt-0.5">
                      Public-facing info shown to venues and guests
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <FormGroup
                    label="City / Location"
                    description="Primary city displayed on your profile"
                  >
                    <input
                      type="text"
                      value={form.city}
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-tertiary mt-0.5">
                      Connect your social presence for discovery
                    </p>
                  </div>
                </div>
