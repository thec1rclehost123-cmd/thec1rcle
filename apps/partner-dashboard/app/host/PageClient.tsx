...
async function fetchOrdersPage(token: string, hostId: string, limit: number) {
...
export default function HostDashboardStreaming() {
  ...
  const canViewOrders = hasPermission('VIEW_GUESTLIST') || canViewAnalytics;
  ...
  const emptyChartLabels = useMemo(() => buildEmptyRangeLabels(selectedRange), [selectedRange]);
  ...
  const performanceLoading = preferUnifiedOverview
    ? unifiedOverviewQuery.isLoading
    : timeSeriesQuery.isLoading;
  const ordersLoading = preferUnifiedOverview
    ? unifiedOverviewQuery.isLoading
    : recentOrdersQuery.isLoading;
  const upcomingEventsLoading = preferUnifiedOverview
    ? unifiedOverviewQuery.isLoading
      ...
      <VenuePageShell
        ...
        actions={
          <Link href="/host/create/select-venue">
            <VenueActionButton variant="primary">Create Event</VenueActionButton>
          </Link>
        }
      >
              ...
              <div className="flex flex-col gap-1">
                ...
                {!canViewAnalytics ? (
                  ...
                  >
                    ...
                    >
                      This graph is built from cross-event host performance data, so it only appears
                      for roles with analytics access.
                    </p>
                  </div>
                ) : performanceLoading ? (
              ...
              <div className="mt-4 flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar min-h-0">
                {!canViewOrders ? (
                  ...
                  >
                    ...
                    <p className="text-[12px] mt-2" style={{ color: 'var(--v-text-tertiary)' }}>
                      Host order activity is only shown for roles with host analytics or guestlist
                      access.
                    </p>
                  </div>
                ) : ordersLoading ? (
                  Array.from({ length: 5 }).map((_, index) => (
                    <div key={index} className="h-[88px] rounded-[24px] v-skeleton" />
                  ))
                  ...
                  filteredLatestOrders.map((order) => {
                    ...
                    return (
                      <Link key={order.id} href={`/host/events/${order.eventId}`}>
                        <motion.div
