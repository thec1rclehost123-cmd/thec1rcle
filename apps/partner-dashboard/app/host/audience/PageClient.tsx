...
function GuestRow({
  ...
  guest: AudienceGuest;
  onVipToggle: (id: string, val: boolean) => void;
}) {
  return (
...
export default function HostAudiencePage() {
  ...
  const fetchGuests = useCallback(
    async (cursor: string | null = null, replace = true) => {
  ...
  const filtered = guests.filter((g) => {
    if (
      filters.search &&
      !(g.maskedName ?? '').toLowerCase().includes(filters.search.toLowerCase())
    )
      return false;
    ...
    <div className="space-y-8 pb-16 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border-subtle">
        <div>
          ...
          <p className="text-text-tertiary text-sm mt-2 max-w-lg">
            Aggregated audience across all your events. Names are pseudonymized to protect guest
            privacy.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => fetchGuests()}
            className="p-2.5 bg-surface-elevated border border-border-subtle rounded-xl hover:bg-surface-tertiary transition-all text-text-tertiary"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            },
            {
              ...
              value: guests.filter((g) => g.isRepeat).length,
              icon: Repeat2,
              color: 'text-violet-400',
            },
            {
              label: 'Ticketed',
              value: guests.filter((g) => g.source === 'ticket').length,
              icon: UserCheck,
            ...
            return (
              <div
                key={stat.label}
                className="bg-surface-elevated border border-border-subtle rounded-2xl p-5"
              >
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <button
            onClick={() => fetchGuests()}
            className="mt-4 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[11px] font-black uppercase tracking-widest transition-all"
          >
            Retry
          </button>
        </div>
        ...
        <div className="py-20 bg-surface-elevated border border-dashed border-border-subtle rounded-2xl text-center">
          ...
          <p className="text-text-tertiary text-sm mt-2">
            {guests.length === 0
              ? 'Guests will appear here once tickets are sold.'
        ...
        <div className="bg-surface-elevated border border-border-subtle rounded-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-4 px-6 py-3 border-b border-border-subtle">
            {['Guest', 'Events', 'Source', 'Last Seen', ''].map((col, i) => (
              <p
                key={i}
                className="text-[10px] font-black text-text-placeholder uppercase tracking-widest"
              >
                {col}
              </p>
            ))}
