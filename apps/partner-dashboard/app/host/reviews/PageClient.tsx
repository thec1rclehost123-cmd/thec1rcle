...
export default function HostReviewsPage() {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/events?creatorId=${hostId}&lifecycle=completed,past,ended,scheduled`,
        {
        ...
        const enriched: EventReview[] = (data.events || [])
          ...
          .map((event: any) => {
            ...
            const rawRepeatGuests =
              event.stats?.repeatGuests ?? event.analytics?.repeatGuests ?? event.repeatGuests;
  ...
  const filteredEvents = useMemo(() => {
    if (filter === 'high')
      return events.filter((e) => e.satisfactionScore !== null && e.satisfactionScore >= 80);
    if (filter === 'low')
      return events.filter((e) => e.satisfactionScore !== null && e.satisfactionScore < 60);
    return events;
  }, [events, filter]);
  ...
  const aggregateStats = useMemo(() => {
    ...
    const avgSatisfaction =
      knownSatisfactionScores.length > 0
        ? Math.round(
            knownSatisfactionScores.reduce((a, score) => a + score, 0) /
              knownSatisfactionScores.length,
          )
        : null;
      ...
      <div className="flex flex-col items-center justify-center py-24 rounded-2xl border border-red-500/20 bg-red-500/5 gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-sm text-text-secondary opacity-80 mt-1">
            Could not fetch your past event data. Check your connection.
          </p>
        </div>
        <button
      ...
      {events.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            value={
              aggregateStats.avgSatisfaction !== null
                ? `${aggregateStats.avgSatisfaction}/100`
                : 'Not tracked'
            }
            icon={Star}
            trend={
              aggregateStats.avgSatisfaction !== null
                ? aggregateStats.avgSatisfaction >= 75
                  ? 'up'
                  : 'down'
                : null
            }
...
function ScoreBadge({ score }: { score: number | null }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-${color}-50 border border-${color}-100`}
    >
...
function generateHighlights(turnout: number, satisfaction: number | null): string[] {
  ...
  if (satisfaction !== null && satisfaction >= 85)
    highlights.push('Exceptional audience engagement');
