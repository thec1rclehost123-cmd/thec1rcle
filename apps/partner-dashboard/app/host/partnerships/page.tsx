  ...
  const fetchPartnerships = async () => {
    try {
      const res = await fetch(
        `/api/venue/partnerships?hostId=${profile?.activeMembership?.partnerId}`,
      );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 pb-10">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium mt-2">
            Manage your verified venue partnerships and slot permissions.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Active Partners (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activePartnerships.map((p) => (
              <div
                key={p.id}
                className="bg-white border border-slate-200 rounded-[2.5rem] p-8 hover:border-slate-300 hover:shadow-xl hover:shadow-slate-100 transition-all group"
              >
            ...
            {activePartnerships.length === 0 && (
              <div className="md:col-span-2 py-20 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200 flex flex-col items-center text-center px-10">
                ...
                <p className="text-slate-500 text-sm font-medium mt-1 max-w-xs">
                  You need a verified partnership to see club calendars and book slots.
                </p>
                <Link
                  href="/host/discover"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {pendingPartnerships.map((p) => (
              <div
                key={p.id}
                className="bg-white border border-slate-200 rounded-[2rem] p-6 flex items-center justify-between border-l-4 border-l-amber-400"
              >
                <div>
                  <h4 className="font-bold text-slate-900 uppercase text-sm">{p.venueName}</h4>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                    Requested{" "}
                    {new Date(p.createdAt?.toDate?.() || p.createdAt).toLocaleDateString()}
                  </p>
                </div>
          ...
          <div className="p-6 bg-white border border-slate-200 rounded-[2rem] space-y-4">
            <h4 className="font-black uppercase tracking-widest text-[10px] text-slate-400">
              Quick Actions
            </h4>
            <Link
              href="/host/events/requests"
          ...
          <div className="p-8 bg-indigo-900 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200">
            <h4 className="font-black uppercase tracking-widest text-[10px] text-indigo-300 mb-4">
              Pro Tip
            </h4>
            <p className="text-sm font-medium leading-relaxed italic">
              "Venues are more likely to approve hosts with a high profile completion and previous
              successful events."
            </p>
            <Link
              href="/host/profile"
