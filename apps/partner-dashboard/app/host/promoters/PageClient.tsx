  ...
  const handleConnectionAction = async (connectionId: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      setConnectionRequests((prev) =>
        prev.filter((r) => r.id !== connectionId || action === 'approve'),
      );
      if (action === 'approve') {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-text-primary tracking-tight transition-all">
            Promoter Network
          </h1>
          <p className="text-text-tertiary font-medium mt-0.5 text-sm">
            Manage and track your promoter team performance.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="bg-surface-elevated p-4 rounded-2xl border border-border-default shadow-sm flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-black text-text-primary">
              {connectionRequests.filter((c) => c.status === 'pending').length}
            </p>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-50">
                  <th className="px-5 py-3 text-[9px] font-black text-text-tertiary uppercase tracking-widest">
                    Promoter
                  </th>
                  <th className="px-5 py-3 text-[9px] font-black text-text-tertiary uppercase tracking-widest">
                    Performance
                  ...
                  <th className="px-5 py-3 text-[9px] font-black text-text-tertiary uppercase tracking-widest text-right">
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {promoters.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-5 py-12 text-center">
                      <div className="max-w-xs mx-auto">
                        ...
                        <p className="text-text-tertiary font-bold text-sm">
                          No promoters found in your network.
                        </p>
                        <p className="text-xs text-text-tertiary mt-1">
                          Invite your team to start tracking sales and performance.
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  promoters
                    ...
                    .map((promoter) => (
                      <tr
                        key={promoter.id}
                        className="hover:bg-surface-tertiary group transition-all"
                      >
          ...
          <div>
            ...
            {pendingRequests.length === 0 ? (
              <div className="bg-surface-elevated border border-border-default rounded-2xl p-8 text-center">
                ...
                <p className="text-xs text-text-tertiary mt-1">
                  When promoters request to connect with you, they'll appear here.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {pendingRequests.map((request) => (
                  <div
                    key={request.id}
                    className="bg-surface-elevated border border-border-default rounded-2xl p-5 shadow-sm hover:shadow-md transition-all"
                  >
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="text-base font-bold text-text-primary">
                              {request.promoterName || 'Unknown Promoter'}
                            </h4>
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                              ...
                              {request.promoterInstagram && (
                                <p className="text-xs font-bold text-pink-500 flex items-center gap-1 uppercase tracking-wider">
                                  ...
                                  {request.promoterInstagram.startsWith('@')
                                    ? request.promoterInstagram
                                    : `@${request.promoterInstagram}`}
                                </p>
                              )}
            ...
            {approvedConnections.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {approvedConnections.map((connection) => (
                  <div
                    key={connection.id}
                    className="bg-surface-elevated border border-border-default rounded-xl p-3 flex items-center justify-between"
                  >
          ...
          <div className="max-w-md w-full bg-surface-elevated rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 duration-200 relative">
            ...
            <div className="text-center mb-5">
              ...
              <p className="text-text-tertiary font-medium mt-1 text-sm">
                Generate a unique onboarding link for your promoter.
              </p>
            </div>
            ...
            {!inviteLink ? (
              <form onSubmit={handleInvite} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-text-tertiary ml-1">
                    Promoter Email
                  </label>
                  <input
                    required
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-center text-[10px] font-black uppercase tracking-widest text-text-tertiary">
                    Share this link with {inviteData.name}
                  </p>
                </div>
