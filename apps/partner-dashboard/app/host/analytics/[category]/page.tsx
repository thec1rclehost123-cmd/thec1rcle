    ...
    const fetchAnalytics = async () => {
      ...
      try {
        const res = await fetch(
          `/api/host/analytics/${category}?hostId=${profile.uid}&range=${range}`,
        );
        if (res.ok) {
    ...
    return (
      <div className="py-24 flex flex-col items-center text-center">
        ...
        <p className="text-slate-500 text-sm font-medium mb-10 max-w-xs mx-auto">
          Get your first event approved to start tracking your Host Impact Score.
        </p>
        <Link
          href="/host/create"
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm">
          ...
          <div className="h-[300px] w-full flex items-end justify-between px-4 gap-2">
            {stats.revenueTimeline?.map((item: any, i: number) => (
              <div
                key={i}
                className="flex-1 bg-slate-900 rounded-t-xl hover:bg-emerald-500 transition-all group relative"
                style={{ height: `${Math.min(100, (item.revenue / 1000) * 10)}%` }}
              >
        ...
        <div className="bg-slate-900 text-white rounded-[2.5rem] p-10 shadow-sm">
          <h3 className="text-xl font-black uppercase tracking-tight opacity-40 mb-8">
            Top Productions
          </h3>
  ...
  return (
    <div className="space-y-10">
      <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm">
        <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-8 text-center">
          RSVP → Entry Conversion
        </h3>
        <div className="flex items-center justify-around py-10">
          <div className="text-center group">
            <div className="text-5xl font-black text-slate-900 mb-2">{stats.rsvps}</div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-indigo-500 transition-colors">
              Total RSVPs
            </div>
          </div>
          ...
          <div className="text-center group">
            ...
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-emerald-500 transition-colors">
              Actual Entry
            </div>
          </div>
          <div className="h-1 w-20 bg-slate-100 rounded-full" />
          <div className="text-center group">
            <div className="text-5xl font-black text-slate-900 mb-2">
              {Math.round(stats.avgConversion)}%
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {stats.eventPerformance?.slice(0, 4).map((perf: any) => (
          <div
            key={perf.id}
            className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm transition-all hover:scale-[1.01] group"
          >
            <div className="flex justify-between items-start mb-6">
              <h4 className="font-bold text-slate-900 truncate max-w-[200px]">{perf.title}</h4>
              <div
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${perf.score > 80 ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
              >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-slate-900 text-white rounded-[2.5rem] p-10 shadow-sm flex flex-col items-center justify-center">
        <h3 className="text-xl font-black uppercase tracking-tight mb-12 opacity-40">
          Gender Mapping
        </h3>
          ...
          {Object.entries(stats.genderRatio || {}).map(([g, count]: any) => {
            ...
            return (
              <div key={g} className="text-center group">
                <div className="text-4xl font-black mb-1 group-hover:text-emerald-400 transition-colors">
                  {Math.round(percent)}%
                </div>
  ...
  return (
    <div className="space-y-10">
      <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm text-center">
        ...
        <div
          className={`inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] ${stats.status === "Trusted" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <StatMetric
          label="Denial Rate"
          value={`${Math.round(stats.denialRate)}%`}
          trend="Target: <5%"
          color="rose"
        />
        <StatMetric
          label="Cancellation Rate"
          value={`${Math.round(stats.cancellationRate)}%`}
          trend="Target: 0%"
          color="indigo"
        />
      </div>
...
function HostPartnerView({ stats }: { stats: any }) {
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm">
      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-10">
        Venue Performance
      </h3>
      <div className="space-y-8">
        {stats.clubPerformance?.map((club: any) => (
          <div
            key={club.name}
            className="flex items-center justify-between group p-6 hover:bg-slate-50 transition-all rounded-3xl border border-transparent hover:border-slate-100"
          >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {stats.recommendations?.map((rec: any, i: number) => (
        <div
          key={i}
          className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm hover:border-indigo-200 transition-all group"
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            <span
              className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${rec.impact === "Critical" ? "bg-rose-100 text-rose-600" : "bg-indigo-100 text-indigo-600"}`}
            >
              {rec.impact} Priority
            </span>
          </div>
...
function StatMetric({ label, value, trend, color }: any) {
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:scale-[1.02] transition-transform group">
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 group-hover:text-slate-900 transition-colors">
        {label}
      </p>
