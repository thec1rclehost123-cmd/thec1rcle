...
export default function HostPartnersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const venueConns = allConns.filter((c: any) => c.type === 'partnership');
      setVenues(
        venueConns.map((c: any) => ({
          connectionId: c.id,
      ...
      const promoterConns = allConns.filter((c: any) => c.type === 'promoter_connection');
      setPromoterConnections(
        promoterConns.map((c: any) => ({
          connectionId: c.id,
  ...
  const handleApprove = async (connectionId: string) => {
    ...
    try {
      const token =
        typeof getIdToken === 'function' ? await getIdToken() : await user?.getIdToken();
  ...
  const pendingIncoming = [
    ...venues
      .filter((v) => v.partnershipStatus === 'pending' && v.initiatedBy === 'venue')
      .map((v) => ({ ...v, type: 'venue' })),
    ...promoterConnections
      .filter((p) => p.status === 'pending' && p.initiatedBy === 'promoter')
      .map((p) => ({ ...p, type: 'promoter' })),
  ];
  ...
  const declined = [
    ...venues
      .filter((v) => v.partnershipStatus === 'declined')
      .map((v) => ({ ...v, type: 'venue' })),
    ...promoterConnections
  ...
  return (
    <VenuePageShell title="Partners" subtitle="Venues and promoters connected to your host profile">
        ...
        <motion.div {...mp(0)}>
          ...
          >
            ...
            <div className="relative z-10">
              <p className="text-[11px] font-black uppercase tracking-widest text-white/40 mb-1">
                Partner Network
              </p>
              <p className="text-[13px] font-medium text-white/70 max-w-lg">
                Connect with venues and promoters to access production calendars, book slots, and
                grow your event portfolio.
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div {...mp(0.06)}>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatTrendCard
              ...
              trendUp={activeVenues.length > 0}
              color="#34d399"
              icon={<CheckCircle2 className="w-4 h-4" />}
            />
            <StatTrendCard
              label="Active Promoters"
              value={activePromoters.length}
              trendUp={activePromoters.length > 0}
              color="#818cf8"
        ...
        <div className="min-h-[400px]">
          <AnimatePresence mode="wait">
            {error ? (
              <motion.div
                key="error"
                className="py-20 text-center border border-dashed border-white/10 rounded-[32px] bg-white/2 flex flex-col items-center gap-4"
              >
                <p className="text-white/40 text-[13px] font-medium">
                  Failed to load partner connections.
                </p>
                <button
                  onClick={fetchData}
                  className="px-6 py-3 rounded-xl bg-white/5 border border-white/10 text-[11px] font-black uppercase tracking-widest text-white hover:bg-white/10"
                >
                  Retry Sync
                </button>
              </motion.div>
              ...
              >
                {(() => {
                  const list =
                    activeTab === 'active'
                      ? [
                          ...activeVenues.map((v) => ({ ...v, type: 'venue' })),
                          ...activePromoters.map((p) => ({ ...p, type: 'promoter' })),
                        ]
                      : activeTab === 'incoming'
                        ? pendingIncoming
                        : activeTab === 'pending'
