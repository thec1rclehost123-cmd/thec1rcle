                    ...
                    <div className="flex items-center gap-3">
                        {[
                            ...
                            >
                                ...
                                <p className="mt-1.5 text-[10px] font-black uppercase tracking-[0.15em] opacity-40 text-white">{metric.label}</p>
                            </div>
                        ))}
                        <Link
                ...
                <div className="space-y-6">
                    <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                        ...
                        <div className="relative w-full lg:max-w-md px-4 py-2 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-3 focus-within:border-orange-500/50 transition-colors">
                            ...
                            />
                            {searchQuery && <button onClick={() => setSearchQuery("")} className="text-white/30 hover:text-white">×</button>}
                        </div>
                    </div>

                    <AnimatePresence mode="wait">
                        {fetchError ? (
                            <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 rounded-[40px] border border-red-500/20 bg-red-500/5 flex flex-col items-center gap-4 text-center">
                                <AlertCircle className="w-10 h-10 text-red-400" />
                                <div>
                                    <h3 className="text-lg font-black text-white">Roster Sync Failed</h3>
                                    <p className="text-sm text-white/40 mt-1">We couldn't load your event portfolio. Please try again.</p>
                                </div>
                                <button onClick={fetchEvents} className="px-8 py-3 rounded-xl bg-white/5 border border-white/10 text-[11px] font-black uppercase tracking-widest text-white">Retry</button>
                            </motion.div>
                        ) : loading ? (
                            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-80 rounded-[40px] bg-white/5 animate-pulse" />)}
                            </motion.div>
                        ) : filteredEvents.length === 0 ? (
                            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 rounded-[56px] border border-dashed border-white/10 bg-white/2 flex flex-col items-center text-center px-12">
                                <div className="p-6 rounded-3xl bg-white/5 mb-8"><CalendarDays className="w-12 h-12 text-white/20" /></div>
                                <h3 className="text-2xl font-black text-white tracking-tight">{searchQuery ? "No matches in roster" : "Roster empty"}</h3>
                                <p className="text-white/40 text-[15px] font-medium mt-4 mb-10 max-w-md">{searchQuery ? "Try refining your search terms for the current filter." : "Schedule your next production by securing a venue slot in the calendar."}</p>
                                <Link href="/host/calendar" className="h-14 px-10 rounded-2xl bg-orange-600 text-white text-[13px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl shadow-orange-950/20 hover:scale-105 transition-all"><Plus className="w-5 h-5" /> Browse Network Calendar</Link>
                            </motion.div>
                        ) : (
