...
export default function HostEventDetailPage() {
  ...
  if (!event) {
    return (
      <div className="py-24 text-center">
        ...
        <Link href="/host/events" className="text-indigo-600 mt-4 inline-block">
          Back to Events
        </Link>
      </div>
    );
  ...
  const isRejected =
    ...
    event.lifecycle === "needs_changes" ||
    event.lifecycle === "denied";
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-500">
          ...
          <div className="flex-1">
            ...
            <p className="text-rose-600 text-sm font-medium mt-1 leading-relaxed">
              {event.rejectionReason ||
                "Minimal changes required to comply with venue standards. Review requirements and resubmit."}
            </p>
          </div>
          <Link
            href={`/host/create?id=${id}`}
          ...
          <div className="flex-1 space-y-8">
            <div>
              ...
              <p className="text-slate-500 text-sm font-medium">
                Tracking the lifecycle of your event from draft to performance.
              </p>
            </div>
            <div className="flex flex-col md:flex-row gap-8">
              <Metric
                value={stats?.total || 0}
                label="Tickets Authorized"
                icon={Ticket}
                color="indigo"
              />
              <Metric
                value={stats?.checkedIn || 0}
                label="Turnout Verified"
                icon={Users}
                color="emerald"
              />
              <Metric
                value={`₹${finance?.gross || 0}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
        <div className="bg-slate-900 rounded-[2.5rem] p-10 shadow-2xl shadow-slate-200 flex flex-col justify-between">
          <div>
            <h3 className="text-xl font-black uppercase tracking-tight opacity-60 mb-6">
              Promoter Pulse
            </h3>
            <p className="text-white/40 text-sm leading-relaxed mb-8 font-medium italic">
              "Current demand is high among your connected promoter network. Expect peak scan
              velocity 2 hours into doors."
            </p>
          </div>
          <Link
            href="/host/promoters"
            className="flex items-center justify-between text-xs font-black uppercase tracking-widest group"
          >
            Manage Affiliates{" "}
        ...
        <div className="bg-indigo-600 rounded-[2.5rem] p-10 shadow-2xl shadow-indigo-100 flex flex-col justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm font-medium leading-relaxed mb-8">
              Reports are synchronized with the primary ledger. Final settlement will trigger
              post-event audit.
            </p>
          </div>
...
function EventBadge({ lifecycle }: { lifecycle: string }) {
  ...
  return (
    <span
      className={`px-4 py-1.5 rounded-full ${style.bg} ${style.text} text-[9px] font-black uppercase tracking-widest border border-current/10 flex items-center gap-2`}
    >
...
function Metric({ value, label, icon: Icon, color }: any) {
  ...
  return (
    <div className="flex-1 p-6 rounded-3xl border border-slate-50 hover:border-slate-100 transition-all group">
      <div
        className={`h-10 w-10 rounded-xl ${colors[color]} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
      >
