...
import {
    ...
    Clock, Plus, Music, X, ArrowRight, CheckCircle2, CircleDot,
    MapPin, Zap, Info, ArrowLeft, Loader2,
} from "lucide-react";
...
export default function HostCalendarPage() {
    ...
    const year = parseInt(currentMonth.toLocaleString("en-US", { year: "numeric", timeZone: "Asia/Kolkata" }));
    const month = parseInt(currentMonth.toLocaleString("en-US", { month: "numeric", timeZone: "Asia/Kolkata" })) - 1;
                ...
                events
                    ...
                    .forEach(e => {
                        ...
                        map[dk].push({
                            ...
                            endTime: e.endTime,
                            state: ["submitted", "pending", "needs_changes"].includes((e.lifecycle || e.status || "").toLowerCase()) ? "pending_mine" : "approved_mine",
                ...
                if (Array.isArray(data)) {
                    data.forEach((d: any) => {
                        ...
                        map[d.date] = visible.map((e: any) => ({
                            ...
                            startTime: e.startTime,
                            endTime: e.endTime,
                            state: (e.hostId === hostId) ? (["submitted", "pending"].includes((e.lifecycle || e.status).toLowerCase()) ? "pending_mine" : "approved_mine") : "occupied_other",
                            eventTitle: e.title || "Occupied",
                            eventId: e.id,
                        }));
                        if (d.state === "BLOCKED") {
                            map[d.date].push({ date: d.date, state: "blocked", startTime: d.block?.startTime, endTime: d.block?.endTime });
                        }
                    });
                }
                setSlotsMap(map);
    ...
    const selectedDayData = useMemo(() => grid.find(d => d?.dateStr === selectedDate), [grid, selectedDate]);
    ...
    return (
        <VenuePageShell title="Slot Calendar" subtitle="Coordinate production slots and venue availability.">
            <div className="flex flex-col gap-6 h-[calc(100vh-13rem)]">
                
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                        <button
                            onClick={() => { setSelectedVenueId(HOST_SCOPE_ID); setSelectedDate(null); }}
                            ...
                            style={{
                                background: selectedVenueId === HOST_SCOPE_ID ? C.surface : "rgba(255,255,255,0.03)",
                                border: selectedVenueId === HOST_SCOPE_ID ? `1px solid ${C.orange}` : `1px solid ${C.borderDefault}`,
                        ...
                        {venues.map(v => (
                            <button
                                key={v.id}
                                onClick={() => { setSelectedVenueId(v.id); setSelectedDate(null); }}
                                className="flex items-center gap-2 px-4 py-2 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all border flex-shrink-0"
                                style={{
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center rounded-2xl overflow-hidden bg-[#1c1c22] border border-white/5">
                            <button onClick={() => navMonth(-1)} className="w-10 h-10 flex items-center justify-center hover:bg-white/5 text-white/40"><ChevronLeft className="w-4 h-4" /></button>
                            <span className="px-4 text-[13px] font-black uppercase text-white min-w-[150px] text-center border-x border-white/5">{MONTHS[month]} {year}</span>
                            <button onClick={() => navMonth(1)} className="w-10 h-10 flex items-center justify-center hover:bg-white/5 text-white/40"><ChevronRight className="w-4 h-4" /></button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col lg:flex-row gap-5 min-h-0">
                    <div className="lg:flex-[2.5] bg-[#16161b] rounded-[28px] border border-white/5 flex flex-col overflow-hidden shadow-2xl">
                        <div className="grid grid-cols-7 px-4 pt-5 pb-3">
                            {DAYS.map(d => <div key={d} className="text-center text-[9px] font-black uppercase tracking-[0.2em] text-white/20">{d}</div>)}
                        </div>
                        <div className="flex-1 grid grid-cols-7 gap-2 p-4 pt-0" style={{ gridTemplateRows: `repeat(${weeks}, 1fr)` }}>
                            {loading ? Array.from({ length: 35 }).map((_, i) => <div key={i} className="rounded-2xl bg-white/5 animate-pulse" />) : grid.map((cell, i) => {
                                ...
                                if (isSel) { bg = C.surfaceSelected; border = `2px solid ${C.borderSelected}`; }
                                if (isBlocked) { bg = C.surfaceBlocked; border = `1px solid ${C.borderBlocked}`; }
                                ...
                                return (
                                    ...
                                    >
                                        <span className={`text-[15px] font-black tabular-nums ${isToday ? "text-orange-500" : "text-white/80"}`}>{cell.day}</span>
                    ...
                    <div className="lg:flex-1 bg-[#101014] rounded-[28px] border border-white/5 overflow-hidden flex flex-col shadow-2xl">
                        <AnimatePresence mode="wait">
                            {selectedDate && selectedDayData ? (
                                <motion.div key={selectedDate} initial={rm ? {} : { opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={rm ? {} : { opacity: 0, x: -20 }} className="h-full flex flex-col">
                                    <Inspector date={selectedDate} slots={selectedDayData.slots} venueId={selectedVenueId} onClear={() => setSelectedDate(null)} />
                                </motion.div>
                            ) : (
                                <div className="flex-1 flex flex-col items-center justify-center p-10 text-center gap-5">
                                    <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/20"><CalendarDays className="w-6 h-6" /></div>
                                    <p className="text-[11px] font-black uppercase tracking-widest text-white/30 uppercase">Select a date to view details</p>
                                </div>
                            )}
                        </AnimatePresence>
                    </div>
...
function Inspector({ date, slots, venueId, onClear }: { date: string, slots: CalendarSlot[], venueId: string, onClear: () => void }) {
    const isBlocked = slots.some(s => s.state === "blocked");
    const events = slots.filter(s => s.state !== "blocked");
    
    return (
        <div className="flex flex-col h-full">
            <div className={`p-6 pb-5 relative overflow-hidden ${isBlocked ? "bg-red-950/20" : "bg-teal-950/20"}`}>
                <div className="absolute top-0 right-0 p-4"><button onClick={onClear} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/30 hover:text-white transition-colors"><X className="w-4 h-4" /></button></div>
                <div className="flex items-end gap-3 mt-4">
                    <span className="text-[52px] font-black leading-none tracking-tighter text-white/90">{date.split("-")[2]}</span>
                    <div className="pb-1">
                        <p className="text-[15px] font-black text-white">{new Date(date).toLocaleDateString("en-US", { weekday: "long" })}</p>
                        <p className="text-[11px] font-medium text-white/30">{new Date(date).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-5 no-scrollbar">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20">Night Schedule</span>
                    <div className="flex-1 h-px bg-white/5" />
                    <span className="text-[9px] font-black text-white/10">2 PM - 4 AM</span>
                </div>

                <div className="relative rounded-2xl bg-white/5 border border-white/5 overflow-hidden" style={{ height: 320 }}>
                    {TIMELINE_HOURS.map(({ label, mins }) => (
                        <div key={label} className="absolute inset-x-0 h-px bg-white/5" style={{ top: pct(mins) }} />
                    ))}
                    {slots.map((s, i) => {
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    background: isB ? "rgba(239,68,68,0.15)" : isOccupied ? "rgba(255,255,255,0.05)" : isPending ? "rgba(251,191,36,0.15)" : "rgba(52,211,153,0.15)",
                                    borderColor: isB ? "rgba(239,68,68,0.3)" : isOccupied ? "rgba(255,255,255,0.1)" : isPending ? "rgba(251,191,36,0.3)" : "rgba(52,211,153,0.3)",
                                }}
                            >
                                <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-l-xl" style={{ background: isB ? C.red : isOccupied ? C.slate : isPending ? C.amber : C.teal }} />
                                <p className="text-[10px] font-black text-white uppercase truncate">{s.eventTitle || (isB ? "Blocked" : "Occupied")}</p>
                                <p className="text-[8px] font-bold text-white/40 tabular-nums">{fmt12(s.startTime || "21:00")} - {fmt12(s.endTime || "04:00")}</p>
                            </div>
                        );
                    })}
                </div>
