...
import {
    CheckCircle2, Circle, Clock, AlertCircle, RefreshCcw,
    ChevronLeft, ShieldCheck, Building2, User, Landmark,
    FileText, ArrowRight, Upload, X, Loader2, Sparkles,
} from "lucide-react";
...
async function uploadFile(uid: string, stepId: string, fieldName: string, file: File): Promise<string> {
...
function FileZone({
    ...
    return (
        <div className="space-y-1.5">
            <label className="text-[11px] font-black uppercase tracking-widest text-text-tertiary">{label}</label>
            {value ? (
                ...
                <div className="p-4 rounded-xl border border-border-subtle bg-surface-secondary">
                    ...
                    <div className="h-1 rounded-full bg-surface-tertiary overflow-hidden">
                        <div className="h-full bg-[#FF5A00] rounded-full transition-all" style={{ width: `${progress}%` }} />
                    </div>
                </div>
            ) : (
                <button
...
function InputField({ label, value, onChange, placeholder, type = "text", readOnly }: {
...
function KycIdentityForm({ uid, initialData, onSubmit, submitting, resubmitReason }: {
    uid: string;
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            {resubmitReason && (
                <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30">
                    <p className="text-[11px] font-black uppercase tracking-widest text-red-400 mb-1">Resubmission required</p>
            ...
            <div className={`grid gap-4 ${needsBack ? "sm:grid-cols-2" : "grid-cols-1"}`}>
                <FileZone label="Document Front" fieldName="doc_front" value={docFront} onChange={setDocFront} uid={uid} stepId="kyc_identity" />
                {needsBack && (
                    <FileZone label="Document Back" fieldName="doc_back" value={docBack} onChange={setDocBack} uid={uid} stepId="kyc_identity" />
                )}
            </div>

            <FileZone label="Selfie Photo" fieldName="selfie" value={selfie} onChange={setSelfie} uid={uid} stepId="kyc_identity" />
...
function KycBusinessForm({ uid, initialData, onSubmit, submitting, resubmitReason }: {
    uid: string;
    initialData: Record<string, unknown>;
    onSubmit: (data: Record<string, unknown>) => void;
    submitting: boolean;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <InputField label="Legal Business Name" value={legalName} onChange={setLegalName} placeholder="As on registration certificate" />
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                ...
                <InputField label="CIN / Registration Number" value={cin} onChange={setCin} placeholder="U74999MH2021PTC..." />
            </div>

            <InputField label="GST Number (optional)" value={gst} onChange={setGst} placeholder="27AAACB1234C1Z5" />
            <InputField label="Registered Address" value={address} onChange={setAddress} placeholder="Full address as on documents" />
...
function KycSignatoryForm({ uid, initialData, onSubmit, submitting, resubmitReason }: {
    uid: string;
    initialData: Record<string, unknown>;
    onSubmit: (data: Record<string, unknown>) => void;
    submitting: boolean;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                <InputField label="Full Legal Name" value={fullName} onChange={setFullName} placeholder="As on government ID" />
                <SelectField
                    label="Designation"
                    value={designation}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                <InputField label="Email" value={email} onChange={setEmail} type="email" placeholder="representative@company.com" />
            ...
            <InputField label="ID Number" value={idNumber} onChange={setIdNumber} placeholder="Enter ID number" />

            <div className={`grid gap-4 ${needsBack ? "sm:grid-cols-2" : "grid-cols-1"}`}>
                <FileZone label="Document Front" fieldName="sig_doc_front" value={docFront} onChange={setDocFront} uid={uid} stepId="kyc_signatory" />
                {needsBack && (
                    <FileZone label="Document Back" fieldName="sig_doc_back" value={docBack} onChange={setDocBack} uid={uid} stepId="kyc_signatory" />
                )}
            </div>

            <FileZone label="Selfie Photo" fieldName="sig_selfie" value={selfie} onChange={setSelfie} uid={uid} stepId="kyc_signatory" />
...
function BankSetupForm({ uid, initialData, onSubmit, submitting, resubmitReason }: {
    uid: string;
    initialData: Record<string, unknown>;
    onSubmit: (data: Record<string, unknown>) => void;
    submitting: boolean;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <InputField label="Account Holder Name" value={accountHolder} onChange={setAccountHolder} placeholder="Exactly as on passbook" />

            <div className="grid sm:grid-cols-2 gap-4">
                <InputField label="Account Number" value={accountNumber} onChange={setAccountNumber} type="password" placeholder="Enter account number" />
                <div className="space-y-1.5">
                    <label className="text-[11px] font-black uppercase tracking-widest text-text-tertiary">Confirm Account Number</label>
                    <input
                        type="text"
                        value={confirmNumber}
...
function CelebrationScreen({ entityType }: { entityType: string }) {
    ...
    const dashPath = entityType === "venue" ? "/venue" : entityType === "promoter" ? "/promoter" : "/host";
    ...
    return (
        ...
        >
            ...
            <h1 className="text-4xl font-black text-text-primary tracking-tight mb-4">You're fully verified</h1>
            <p className="text-text-tertiary text-base leading-relaxed max-w-sm mb-10">
                Welcome to C1RCLE. All features are now unlocked — start creating events, managing your audience, and tracking your finances.
            </p>
            <button
...
export function VerifyPageClient() {
    const { user, isApproved, loading: authLoading, entityType: ctxEntityType, kycStatus: ctxKycStatus } = useDashboardAuth();
    const router = useRouter();
    ...
    return (
        <div className="min-h-screen bg-[var(--v-canvas)]">
            ...
            <div className="sticky top-0 z-30 bg-surface-base/90 backdrop-blur-xl border-b border-border-subtle">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="w-7 h-7 rounded-lg bg-[#FF5A00] flex items-center justify-center text-white text-[11px] font-black">C</span>
                        <span className="text-[13px] font-black text-text-primary tracking-wide">Verification Hub</span>
                    </div>
                    <div className="w-16 text-right">
                        <span className="text-[11px] font-black text-text-tertiary uppercase tracking-widest">{entityTypeName}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-2xl font-black text-text-primary tracking-tight">Complete your verification</h1>
                    ...
                    <div className="space-y-2">
                        {stepSequence.map((stepId, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${isActive ? "bg-[#FF5A00]/15" : "bg-surface-tertiary"}`}>
                                        <Icon className={`h-4 w-4 ${isActive ? "text-[#FF5A00]" : "text-text-tertiary"}`} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-[13px] font-bold truncate ${isActive ? "text-text-primary" : "text-text-secondary"}`}>
                                            {meta?.label ?? stepId}
                                        </p>
                            ...
                            >
                                    ...
                                    return (
                                        <div className="mb-6">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <div>
                                                    <h2 className="text-[16px] font-black text-text-primary">{meta?.label}</h2>
                                                    <p className={`text-[11px] font-bold ${stepStatusColor(status)}`}>{STATUS_LABEL[status]}</p>
                                                </div>
                                            </div>
                                            <p className="text-[13px] text-text-tertiary leading-relaxed">{meta?.description}</p>
                                        </div>
                                    );
                                })()}
                                ...
                                {kycStepStatus[activeStep] === "approved" && (
                                    <div className="flex flex-col items-center justify-center py-12 text-center">
                                        ...
                                        <p className="text-[13px] text-text-tertiary">This step has been reviewed and approved.</p>
                                    </div>
                                )}

                                {/* Under review state */}
                                {(kycStepStatus[activeStep] === "submitted" || kycStepStatus[activeStep] === "under_review") && (
                                    <div className="flex flex-col items-center justify-center py-12 text-center">
                                        <Clock className="h-12 w-12 text-indigo-400 mb-4" />
                                        <p className="text-[16px] font-black text-text-primary mb-1">Under review</p>
                                        <p className="text-[13px] text-text-tertiary">We're reviewing your submission. You'll be notified of any updates.</p>
                                    </div>
                                )}

                                {/* Actionable states: not_started, in_progress, needs_resubmission */}
                                {(!kycStepStatus[activeStep] || ["not_started", "in_progress", "needs_resubmission"].includes(kycStepStatus[activeStep])) && (
                                    <>
