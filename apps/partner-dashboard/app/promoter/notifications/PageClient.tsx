...
import { Bell, CheckCheck, RefreshCw, Loader2, BellOff } from "lucide-react";
...
export default function PromoterNotificationsPageClient() {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch("/api/promoter/notifications", {
    ...
    const markAllRead = async () => {
        ...
        try {
            const token = await user?.getIdToken();
            await fetch("/api/promoter/notifications", {
                method: "PATCH",
                headers: {
    ...
    const markOneRead = async (id: string) => {
        ...
        const token = await user?.getIdToken();
        fetch("/api/promoter/notifications", {
            method: "PATCH",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
        ...
        >
            {loading ? (
                ...
                <div className="flex flex-col items-center gap-3 py-24 text-center">
                    ...
                    <p className="text-text-placeholder text-xs">You&apos;ll see event updates, payout alerts, and commission activity here.</p>
                </div>
            ) : (
