    ...
    return (
      <div className="py-24 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">
          Processing Promoter Intelligence...
        </p>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm relative overflow-hidden">
          ...
          <div className="h-64 flex items-end justify-between gap-2">
            ...
            {[30, 50, 40, 70, 90, 60, 80].map((h, i) => (
              <div
                key={i}
                className="flex-1 bg-slate-900 rounded-t-xl hover:bg-emerald-500 transition-all cursor-pointer"
                style={{ height: `${h}%` }}
              />
            ))}
          </div>
    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm">
      ...
      <div className="space-y-6">
        {stats.eventPerformance?.map((ev: any) => (
          <div
            key={ev.id}
            className="flex items-center justify-between p-6 rounded-3xl border border-slate-100 hover:bg-slate-50 transition-all group"
          >
            ...
            <div className="text-right">
              <p className="font-black text-emerald-600">
                ₹{Math.round(ev.commission).toLocaleString()}
              </p>
              <div className="flex items-center gap-1 justify-end">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                  {Math.round(ev.conversion)}% Yield
                </span>
              </div>
            </div>
          </div>
...
function PromoterFunnelView({ stats }: { stats: any }) {
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm">
      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-12 text-center">
        Intent-to-Entry Funnel
      </h3>
    ...
    <div className="space-y-10">
      <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm text-center">
        ...
        <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8">
          Promoter Quality Index
        </div>

        <div
          className={`inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] ${stats.status.includes("Trusted") ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
        >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {stats.recommendations?.map((rec: any, i: number) => (
        <div
          key={i}
          className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm hover:border-emerald-200 transition-all group"
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            <span
              className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${rec.impact === "Critical" ? "bg-rose-100 text-rose-600" : "bg-emerald-100 text-emerald-600"}`}
            >
              {rec.impact} Priority
            </span>
          </div>
