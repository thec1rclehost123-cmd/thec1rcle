    ...
    <div className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-6">
        <div>
          ...
          <p className="text-sm md:text-base text-[var(--text-tertiary)] mt-1">
            Proof of impact and performance overview.
          </p>
        </div>
        ...
        <div className="flex items-center gap-6 px-6 py-4 rounded-2xl bg-[var(--surface-elevated)] border border-[var(--border-subtle)] shadow-sm w-full md:w-auto justify-between md:justify-start">
          <div className="text-right">
            <p className="text-[10px] md:text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-[0.05em] mb-1">
              Available Payout
            </p>
            <p className="text-2xl md:text-3xl font-black text-[var(--text-primary)]">
          ...
          <Link
            ...
            className="btn btn-primary btn-sm rounded-xl px-5 py-3 h-auto"
          >
            Withdraw
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <MetricCard
          ...
          accentColor={
            stats.checkInRate >= 70 ? "emerald" : stats.checkInRate >= 50 ? "amber" : "red"
          }
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="p-6 md:p-8 rounded-[2rem] bg-[var(--surface-elevated)] border border-[var(--border-subtle)] shadow-sm">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link
                href="/promoter/links"
                className="text-sm font-medium text-indigo-600 flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-indigo-50 transition-all"
              >
                All Links
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 md:p-8 rounded-[2rem] bg-stone-900 text-white shadow-xl shadow-stone-200">
            ...
            <Link
              href="/promoter/payouts"
              className="btn btn-secondary w-full mt-8 bg-white/10 border-white/10 hover:bg-white/20 text-white font-bold"
            >
              Financial Ledger
            </Link>
          </div>
          ...
          {/* Quick Tools */}
          <div className="p-6 md:p-8 rounded-[2rem] bg-[var(--surface-elevated)] border border-[var(--border-subtle)] shadow-sm">
            <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-6">
              Tools
            </h3>
  ...
  return (
    <div className="p-6 md:p-8 rounded-[2rem] bg-[var(--surface-elevated)] border border-[var(--border-subtle)] shadow-sm hover:border-[var(--border-strong)] transition-all group">
      <div
        ...
        style={{ backgroundColor: `${accents[accentColor]}10`, color: accents[accentColor] }}
      >
        <Icon className="w-5 h-5" />
      </div>
      <p className="text-3xl md:text-4xl font-black text-[var(--text-primary)] leading-none mb-2 tracking-tight">
        {value}
      </p>
      <p className="text-[10px] md:text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-[0.05em] mb-1">
        {label}
      </p>
      {subtext && <p className="text-xs md:text-sm text-stone-400 font-medium">{subtext}</p>}
    </div>
  );
}
  ...
  return (
    <div className="group flex flex-col sm:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-5 rounded-[1.5rem] bg-[var(--surface-secondary)]/50 border border-[var(--border-subtle)] hover:bg-[var(--surface-elevated)] hover:border-[var(--border-strong)] transition-all">
      ...
      <div className="flex-1 min-w-0 text-center sm:text-left w-full">
        ...
        <h4 className="text-[16px] font-bold text-[var(--text-primary)] truncate leading-tight">
          {event.title}
        </h4>
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto">
        <button
          onClick={onCopyLink}
          className={`h-11 px-6 rounded-xl font-bold text-[13px] transition-all flex flex-1 sm:flex-none items-center justify-center gap-2 ${
            isCopied
        ...
        >
          {isCopied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4 opacity-50" />}
