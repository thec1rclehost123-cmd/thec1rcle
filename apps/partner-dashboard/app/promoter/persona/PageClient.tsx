        ...
        <div className="relative z-10 p-10 flex flex-col md:flex-row items-center md:items-end gap-10">
          ...
          <div className="space-y-4 text-center md:text-left flex-1">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
              ...
              <Tag icon={ShieldCheck} className="bg-white/5 border-white/10 text-white/50">
                Lvl 1 Reputation
              </Tag>
            </div>
          </div>
        ...
        <BentoCard className="p-10 space-y-10 flex flex-col">
          ...
          <div className="flex-1 min-h-[160px]">
            {editMode ? (
              ...
              <div className="h-full p-8 rounded-2xl bg-white/[0.02] border border-white/[0.04] flex items-center justify-center relative group">
                <div className="absolute top-4 left-6 text-4xl text-white/5 font-serif font-black">
                  "
                </div>
                <p className="text-[15px] font-medium text-white/80 leading-relaxed text-center italic relative z-10 px-4">
                  {formData.bio || ''}
                </p>
                <div className="absolute bottom-4 right-6 text-4xl text-white/5 font-serif font-black translate-y-2">
                  "
                </div>
              </div>
            )}
          ...
          <div className="p-6 rounded-2xl bg-[var(--v-orange)]/[0.03] border border-[var(--v-orange)]/10">
            <header className="flex items-center justify-between mb-4">
              <p className="text-[11px] font-black uppercase tracking-widest text-[var(--v-orange)]">
                Personal Brand URL
              </p>
              {formData.username && !editMode && (
                ...
                >
                  {linkCopied ? (
                    <CheckCircle2 className="w-3 h-3 text-[var(--v-success)]" />
                  ) : (
...
function Tag({ children, icon: Icon, className }: any) {
  return (
    <div
      className={cn(
        'px-4 py-2 rounded-xl bg-white/5 border border-white/10 flex items-center gap-2 text-[12px] font-bold text-white/80 transition-all hover:bg-white/10 hover:border-white/20',
        className,
      )}
    >
...
function ProfileItem({ icon: Icon, label, value, readOnly, editing, onChange, placeholder }: any) {
  return (
    <div className="space-y-3">
      <label className="text-[11px] font-black uppercase tracking-[0.2em] text-[var(--v-text-tertiary)] ml-1">
        {label}
      </label>
      <div
        className={cn(
      ...
      >
        <Icon
          className={cn(
            'w-4 h-4',
            editing && !readOnly ? 'text-[var(--v-orange)]' : 'text-white/20',
          )}
        />
        {editing && !readOnly ? (
          <input
          ...
          <span
            className={cn(
              ...
              value ? 'text-white' : 'text-white/20 italic',
            )}
          >
