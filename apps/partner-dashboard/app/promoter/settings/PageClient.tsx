    ...
    if (isDirty && setActions) {
      setActions(
        <div className="flex items-center gap-3">
          {saveError && (
            <span className="text-[13px] font-bold text-red-400 mr-2">{saveError}</span>
          )}
          <button
            onClick={handleReset}
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-6">
        ...
        <div className="space-y-1 text-center">
          <p className="text-[13px] font-black uppercase tracking-[0.3em] text-text-primary">
            Initializing
          </p>
          <p className="text-[11px] font-medium text-[var(--v-text-tertiary)]">
            Fetching your promoter credentials...
          </p>
        </div>
      </div>
    );
              ...
              <BentoCard className="p-10">
                <div className="space-y-10">
                  <header>
                    ...
                    <p className="text-[14px] text-[var(--v-text-secondary)]">
                      Manage your professional branding and legal information.
                    </p>
                  </header>
              ...
              <BentoCard className="p-10">
                <div className="space-y-10">
                  <header>
                    ...
                    <p className="text-[14px] text-[var(--v-text-secondary)]">
                      Choose which events trigger immediate push and email notifications.
                    </p>
                  </header>
                  ...
                  <div className="grid gap-4">
                    {NOTIF_ROWS.map(({ key, label }) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="group flex items-center justify-between p-6 rounded-2xl bg-surface-secondary border border-border-subtle hover:bg-surface-tertiary hover:border-border-default transition-all duration-300"
                        >
                          <div>
                            <p className="text-[15px] font-bold text-text-primary group-hover:text-[var(--v-orange)] transition-colors">
                              {label}
                            </p>
                            <p className="text-[12px] text-[var(--v-text-tertiary)]">
                              Receive updates for every {label.toLowerCase()}
                            </p>
                          </div>
                          <button
                            ...
                            className={cn(
                              ...
                              on
                                ? 'bg-[var(--v-orange)] shadow-[0_0_15px_rgba(255,107,0,0.3)]'
                                : 'bg-surface-elevated',
                            )}
                          >
                            <div
              ...
              <BentoCard className="p-10">
                <div className="space-y-10">
                  <header>
                    ...
                    <p className="text-[14px] text-[var(--v-text-secondary)]">
                      Secure your account with multi-factor authentication and session management.
                    </p>
                    {resetError ? (
                      <p className="mt-3 text-[13px] text-red-400">{resetError}</p>
                    ) : null}
                  </header>
                  ...
                  <div className="grid gap-6">
                    {!isGoogleUser ? (
                      <div className="flex items-center justify-between p-7 rounded-2xl bg-surface-secondary border border-border-default hover:bg-surface-tertiary transition-all">
                        <div className="space-y-1">
                          ...
                          <p className="text-[13px] text-[var(--v-text-tertiary)]">
                            Send a secure reset link to{' '}
                            <span className="text-text-tertiary font-medium">{user?.email}</span>
                          </p>
                        </div>
                        {resetSent ? (
                          <div className="px-6 py-2 rounded-xl bg-[var(--v-success)]/10 border border-[var(--v-success)]/20 flex items-center gap-2">
                            ...
                            <span className="text-[13px] font-black text-[var(--v-success)] uppercase tracking-wider">
                              Email Sent
                            </span>
                          </div>
                        ) : (
                      ...
                      <div className="p-7 rounded-2xl bg-blue-500/5 border border-blue-500/10 flex items-center justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-[13px] text-[var(--v-text-tertiary)]">
                            Your account is secured via Google. Manage security at
                            accounts.google.com
                          </p>
                        </div>
                    ...
                    <div className="flex items-center justify-between p-7 rounded-2xl bg-red-500/[0.03] border border-red-500/10 hover:bg-red-500/[0.05] transition-all">
                      <div className="space-y-1">
                        ...
                        <p className="text-[13px] text-[var(--v-text-tertiary)]">
                          Immediate logout from all other active devices and browser sessions.
                        </p>
                      </div>
                      {logoutDone ? (
                        <div className="px-6 py-2 rounded-xl bg-[var(--v-success)]/10 border border-[var(--v-success)]/20 flex items-center gap-2">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <p className="text-[12px] font-black uppercase tracking-[0.2em] text-[var(--v-text-tertiary)] ml-1">
        {label}
      </p>
      {children}
    </div>
