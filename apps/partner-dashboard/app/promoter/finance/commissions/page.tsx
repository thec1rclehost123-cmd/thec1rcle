...
const STATUS_CONFIG: Record<string, { label: string; classes: string }> = {
    ...
    cleared:  { label: "Cleared",   classes: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" },
        ...
        <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col items-start gap-4 pb-6 border-b border-border-subtle sm:flex-row sm:items-center">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-text-secondary mt-0.5">Per-event commission breakdown across all your assignments.</p>
                </div>
                <button
                    onClick={fetchData}
                    disabled={loading}
            ...
            {!loading && (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {[
                        ...
                        >
                            <span className="text-[10px] font-bold uppercase tracking-widest text-text-muted mb-2">{s.label}</span>
                            <p className={`text-xl font-bold tabular-nums ${s.dim ? "text-amber-400" : "text-text-primary"}`}>{s.value}</p>
                        </div>
                    ))}
                </div>
            )}
            ...
            {loading ? (
                ...
                <div className="bg-surface-elevated rounded-2xl border border-border-subtle overflow-hidden">
                    <div className="p-5 border-b border-border-subtle bg-surface-base/50 flex items-center justify-between">
                        ...
                        <span className="text-xs text-text-muted">{commissions.length} record{commissions.length !== 1 ? "s" : ""}</span>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[640px]">
                            <thead>
                                <tr className="border-b border-border-subtle bg-surface-base/30">
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Revenue</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Rate</th>
                                    ...
                                    return (
                                        <tr key={c.id} className="hover:bg-surface-hover/30 transition-colors">
                                            <td className="px-6 py-4">
                                                <p className="font-semibold text-text-primary text-sm">{c.eventName || "Event"}</p>
                                                {c.linkCode && (
