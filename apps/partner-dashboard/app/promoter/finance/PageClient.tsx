...
export default function PromoterFinancePageClient() {
  ...
  const [incomeItems, setIncomeItems] = useState<
    NonNullable<PromoterFinanceResponse['commissionDetails']>
  >([]);
  ...
  const balanceRows: FinanceRow[] = [
    { label: 'Available', value: loading ? '...' : formatINR(balance?.available || 0) },
    {
      ...
      helpLabel: 'Commissions clear after event settlement.',
    },
    {
      label: 'Instant Available',
      value: loading ? '...' : formatINR(balance?.instantAvailable || 0),
      helpLabel: 'Instant transfers are not enabled for promoters yet.',
    },
  ];
  ...
  const bankAccounts: FinanceBankAccount[] = accounts.map((account) => ({
    id: account.id,
    name:
      account.paymentType === 'debit_card' ? `${account.bankName} Debit Card` : account.bankName,
          ...
          <div className="mt-5 grid gap-3">
            ...
            >
              ...
              <p className="text-[13px] leading-6" style={{ color: 'rgba(255,255,255,0.72)' }}>
                Transfers are confirmed on the payouts page, where you can review the exact amount,
                destination, and request status before submission.
              </p>
            </div>
          </div>
