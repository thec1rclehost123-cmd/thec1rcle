...
export default function PromoterPayoutsPage() {
  ...
  const handleRequestPayout = async () => {
    ...
    if (!amt || amt < 100) {
      setFormError('Minimum payout is ₹100');
      return;
    }
      ...
      <div className="flex items-center gap-4 pb-6 border-b border-border-subtle">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-text-secondary mt-0.5">
            Manage your payout requests and withdrawal history.
          </p>
        </div>
      ...
      {!loading && balance && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            {
              label: 'Available',
              value: formatINR(balance.available),
              icon: Wallet,
              highlight: true,
            },
            ...
            {
              label: 'Total Paid',
              value: formatINR(balance.totalPaid),
              icon: CheckCircle2,
              highlight: false,
            },
          ].map((s) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <Icon
                    className={`h-4 w-4 ${s.highlight ? 'text-emerald-400' : 'text-text-muted'}`}
                  />
                ...
                <p
                  className={`text-2xl font-bold tabular-nums ${s.highlight ? 'text-emerald-400' : 'text-text-primary'}`}
                >
                  {s.value}
                </p>
              </div>
              ...
              <tbody className="divide-y divide-border-subtle">
                {payouts.map((p) => {
                  const normalizedStatus = (
                    p.status === 'cleared' ? 'completed' : p.status
                  ) as PayoutStatus;
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-surface-hover/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-text-primary font-medium capitalize">
                        ...
                        {p.buyerName && (
                          <span className="text-text-muted text-xs ml-2">({p.buyerName})</span>
                        )}
                      </td>
