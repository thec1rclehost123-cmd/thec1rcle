...
import { Trophy, Medal, Star, MapPin, ChevronDown, Calendar, TrendingUp, TrendingDown, Minus } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const fetchLeaderboard = async () => {
            try {
                ...
                const res = await fetch(`/api/v1/partners/promoters/leaderboard${queryString}`, { headers });
    ...
    return (
        <div className="min-h-screen relative px-4 pt-0 pb-32 md:px-8">
            ...
            <div className="absolute -top-6 bottom-0 -left-4 -right-4 md:-top-8 md:-left-8 md:-right-8 pointer-events-none z-0 bg-[#0a0a0b] overflow-hidden">
                ...
                <div 
                    className="absolute -top-10 -left-[10%] w-[60vw] h-[150vh] bg-gradient-to-b from-red-600/25 via-red-600/5 to-transparent blur-[120px] origin-top-left rotate-[35deg] mix-blend-screen pointer-events-none"
                />
                ...
                <div className="absolute top-[20%] left-1/2 -translate-x-1/2 flex flex-col items-center z-0 opacity-30 mix-blend-screen pointer-events-none blur-[12px]">
                    <div className="relative w-[300px] h-[300px] md:w-[600px] md:h-[600px]">
                        ...
                        <div className="w-full h-full rounded-full overflow-hidden bg-zinc-900 border border-zinc-800 shadow-[0_0_80px_rgba(255,255,255,0.1)]">
                            <div className="w-[200%] h-full animate-[spin-bg_60s_linear_infinite]" style={{
                                backgroundImage: `
                    ...
                    <div className="flex justify-center gap-4">
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <span className="font-semibold text-sm tracking-wide">
                                    {timeframe === 'month' ? 'This Month' : timeframe === 'week' ? 'This Week' : 'All Time'}
                                </span>
                                ...
                                <>
                                    ...
                                    <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-40 bg-surface-elevated border border-border-subtle rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] z-50 py-1 overflow-hidden backdrop-blur-xl">
                                        {[
                                            ...
                                            <button
                                                key={t.id}
                                                onClick={() => { setTimeframe(t.id); setTimeDropdownOpen(false); setLoading(true); }}
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <span className="font-semibold text-sm tracking-wide">{city === 'All' ? 'Global (All Cities)' : city}</span>
                                <ChevronDown className="w-4 h-4 text-text-tertiary ml-2" />
                            </button>
                                ...
                                <>
                                    ...
                                    <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-48 bg-surface-elevated border border-border-subtle rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] z-50 py-1 overflow-hidden backdrop-blur-xl">
                                        {['All', 'Pune', 'Mumbai', 'Goa', 'Bengaluru'].map((c) => (
                                            <button
                                                key={c}
                                                onClick={() => { setCity(c); setCityDropdownOpen(false); setLoading(true); }}
                        ...
                        {visualPodium.map((promoter, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 w-full h-full bg-[#1a1a1a] shadow-2xl" style={{ clipPath: 'polygon(0 8%, 100% 0, 100% 100%, 0% 100%)' }}>
                                        {/* Colored Header Area inside card */}
                                        <div className={`absolute top-0 w-full h-1/2 ${styles.gradient} opacity-80`} style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 85%)' }} />
                                    </div>

                                    {/* Avatar (Breaking Out) */}
                                    <div className="relative z-10 -mt-6 md:-mt-10">
                                        <div className={`h-20 w-20 md:h-28 md:w-28 overflow-hidden rounded-full border-4 border-[#1a1a1a] shadow-xl bg-surface-elevated`}>
                                            {promoter.avatarUrl ? (
                                                <img src={promoter.avatarUrl} alt={promoter.displayName} className="h-full w-full object-cover" />
                                            ) : (
                                        ...
                                        {/* Rank Badge */}
                                        <div className={`absolute -bottom-1 -right-1 flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-full text-lg md:text-xl font-black bg-black border-2 border-[#1a1a1a] ${styles.accent} z-20 shadow-lg`}>
                                            {promoter.rank}
                                        </div>
                                        {/* Crown for #1 */}
                                        {isFirst && (
                                    ...
                                    <div className="relative z-10 w-full px-2 pt-6 pb-4 text-center mt-auto">
                                        <h3 className={`font-black uppercase tracking-tighter truncate w-full mb-4 ${isFirst ? 'text-lg md:text-2xl text-white' : 'text-base md:text-xl text-white/90'}`}>
                                        ...
                                        <div className="flex justify-between items-end border-t border-white/10 pt-3 px-2">
                                            <div className="flex flex-col items-center">
                                                <span className="text-[9px] md:text-[11px] font-bold text-white/40 uppercase tracking-widest mb-1">Tickets</span>
                                                <span className="text-xl md:text-2xl font-black text-white tracking-tighter">
                                                    {Math.floor(promoter.xpScore * 0.45) + (10 - (i % 10))}
                                                </span>
                                            </div>
                                            <div className="flex flex-col items-center">
                                                <span className="text-[9px] md:text-[11px] font-bold text-white/40 uppercase tracking-widest mb-1">XP</span>
                                                <span className={`text-xl md:text-3xl font-black tracking-tighter ${styles.accent}`}>
                                                    {promoter.xpScore > 9999 ? (promoter.xpScore/1000).toFixed(1) + 'k' : promoter.xpScore.toLocaleString()}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="absolute left-0 top-0 bottom-0 w-16 md:w-20 flex flex-col justify-center items-center border-r border-white/10 bg-black/30">
                                    <span className="text-2xl md:text-4xl font-black tracking-tighter opacity-90">{promoter.rank}</span>
                                </div>
                                ...
                                <div className="pl-[140px] md:pl-[180px] flex flex-col justify-center flex-1 h-full z-10 py-2">
                                    <span className={`font-black uppercase tracking-tighter text-lg md:text-2xl truncate ${promoter.promoterId === promoterId ? 'text-c1rcle-orange-light' : 'text-white'}`}>
                                        {promoter.displayName}
                                    </span>
                                ...
                                <div className="pr-4 md:pr-6 flex flex-col items-end justify-center h-full border-l border-white/10 bg-black/30 px-3 md:px-6">
                                    <span className="text-xl md:text-3xl font-black tracking-tighter">{promoter.xpScore > 9999 ? (promoter.xpScore/1000).toFixed(1) + 'k' : promoter.xpScore.toLocaleString()}</span>
                                    <span className="text-[8px] md:text-[9px] uppercase tracking-widest font-bold opacity-60 mt-1">XP Score</span>
                                </div>
                            </motion.div>
                        );
                    })}
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-xs font-semibold text-c1rcle-orange/80 uppercase tracking-wider">Your Rank</p>
                                    <p className="font-bold text-white">{currentUserRank.xpScore.toLocaleString()} XP</p>
                                </div>
                            </div>
