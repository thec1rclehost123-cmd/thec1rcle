...
import {
    Users, CheckCircle2, AlertTriangle, Loader2, CalendarDays,
} from "lucide-react";
...
export function DoorSellClient() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            showFlash(
                "success",
                purpose === "party" ? "Walk-in entry added" : "Dine-in entry added"
            );
            ...
            if (data.remainingCapacity !== null && selectedEventId) {
                setCapacity(prev =>
                    prev ? { ...prev, available: data.remainingCapacity, isSoldOut: data.remainingCapacity === 0 } : prev
                );
            }
        } finally {
            setSubmitting(false);
    ...
    return (
        <div className="space-y-4 max-w-xl">
            ...
            {entryType === "walkins" && selectedEventId && (
                capLoading ? (
            ...
            {soldOut && (
                <div className="rounded-2xl border px-5 py-4 flex items-center gap-3"
                    ...
                    <div>
                        ...
                        <p className="text-[12px] text-red-400/70">Walk-in entries are disabled. Dine-in is still available.</p>
                    </div>
                </div>
            )}
            ...
            >
                ...
                <Field label="Type" required>
                    ...
                    >
                        <option value="" disabled style={{ background: "var(--v-elevated)" }}>Select type…</option>
                        <option value="walkins" style={{ background: "var(--v-elevated)" }}>Walk-ins</option>
                        <option value="dinein" style={{ background: "var(--v-elevated)" }}>Dine-ins</option>
                    </select>
                </Field>
