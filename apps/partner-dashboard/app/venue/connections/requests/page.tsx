...
export default function VenueConnectionsPage() {
  ...
  const approvedPromoterConnections = promoterRequests.filter(
    (r) => r.status === "approved" || r.status === "active",
  );
  const pendingHostRequests = hostPartnerships.filter((h) => h.status === "pending");
  const approvedHostPartnerships = hostPartnerships.filter(
    (h) => h.status === "approved" || h.status === "active",
  );
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center p-1.5 bg-[var(--surface-secondary)] backdrop-blur-sm rounded-2xl w-fit border border-[var(--border-subtle)]">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-[13px] font-semibold transition-all flex items-center gap-2.5 ${
              activeTab === "promoters"
          ...
          >
            <Zap
              className={`w-4 h-4 ${activeTab === "promoters" ? "text-[var(--c1rcle-orange)]" : ""}`}
            />
            Promoters
      ...
      <div className="min-h-[600px]">
        {activeTab === "discover" ? (
          <DiscoveryView allowedTypes={["host", "promoter"]} partnerId={venueId} role="venue" />
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              ...
              <AnimatePresence mode="popLayout">
                {loading ? (
                ...
                ) : (activeTab === "promoters" ? pendingPromoterRequests : pendingHostRequests)
                    .length === 0 ? (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    ...
                    <h4 className="text-title text-[var(--text-primary)] font-semibold">
                      Quiet for now
                    </h4>
                  ...
                  (activeTab === "promoters" ? pendingPromoterRequests : pendingHostRequests).map(
                    (request: any) => (
                      <motion.div
                        key={request.id}
                        layout
                            ...
                            <button
                              onClick={() =>
                                handleAction(
                                  request.id,
                                  "approve",
                                  activeTab === "promoters" ? "promoter" : "host",
                                )
                              }
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                ...
                <span className="text-label text-[var(--state-success)] bg-[var(--state-success-bg)] px-2 py-1 rounded-md">
                  {activeTab === "promoters"
                    ? approvedPromoterConnections.length
                    : approvedHostPartnerships.length}{" "}
                  Active
                </span>
              </div>
              ...
              <div className="space-y-3">
                {(activeTab === "promoters"
                  ? approvedPromoterConnections
                  : approvedHostPartnerships
                ).length === 0 ? (
                  <div className="py-16 card border-2 border-dashed border-[var(--border-default)] flex flex-col items-center text-center px-10">
                    ...
                    <p className="text-body-sm text-[var(--text-secondary)] mt-2">
                      Once approved, partners will appear here.
                    </p>
                  </div>
                ) : (
                  (activeTab === "promoters"
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-caption font-semibold text-[var(--state-success)]">
                              Active
                            </span>
                          </div>
                        </div>
