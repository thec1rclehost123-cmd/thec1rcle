  ...
  return (
    ...
    >
      ...
      <td className="py-2.5 px-3 text-xs text-text-secondary">
        <span className="font-mono">{PAYMENT_ICONS[entry.paymentMode]}</span> {entry.paymentMode}
      </td>
      <td className="py-2.5 px-3 text-xs text-text-secondary tabular-nums">
        {entry.amountPaise > 0 ? `₹${(entry.amountPaise / 100).toLocaleString('en-IN')}` : '—'}
      </td>
      ...
      <td className="py-2.5 px-3 text-xs text-text-tertiary truncate max-w-[120px]">
        {entry.addedByName}
      </td>
      <td className="py-2.5 px-3 text-xs text-text-tertiary truncate max-w-[160px]">
        {entry.note}
      </td>
...
export function WalkInsClient() {
  ...
  // State
  const [selectedEventId, setSelectedEventId] = useState<string>(sp.get('eventId') ?? '');
  ...
  return (
    ...
    >
      <div className="space-y-4 no-print">
        ...
        {selectedEventId && (
          <QuickEntryRow eventId={selectedEventId} venueId={venueId} onSuccess={() => {}} />
        )}
          ...
          <thead>
            <tr className="border-b border-border-default">
              {[
                ...
                <th
                  key={h}
                  className="py-2.5 px-3 text-xs font-medium text-text-tertiary no-print:sticky top-0 bg-surface-secondary"
                >
                  {h}
                </th>
              ))}
