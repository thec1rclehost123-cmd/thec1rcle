  ...
  const fetchTonightEvent = async () => {
    ...
    try {
      const res = await fetch(
        `/api/venue/events?venueId=${profile.activeMembership.partnerId}&limit=1`,
      );
      if (res.ok) {
        const data = await res.json();
        if (data.events?.[0]) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 pb-10">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium mt-3">
            Manage floor inventory and real-time nightly reservations.
          </p>
        </div>
      ...
      {mode === "setup" ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <StatCard
              label="Sections"
              value={new Set(tables.map((t) => t.location)).size}
              icon={Layers}
              color="amber"
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {tables.map((table) => {
              ...
              return (
                <div
                  key={table.id}
                  className="bg-white border border-slate-200 rounded-[2rem] p-8 flex items-center justify-between group hover:shadow-xl transition-all"
                >
                  <div className="flex items-center gap-8">
                    <div
                      className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all ${isBlocked ? "bg-amber-50 text-amber-500" : booking ? "bg-emerald-50 text-emerald-500" : "bg-slate-50 text-slate-400"}`}
                    >
                    ...
                    {booking ? (
                      <div className="text-right flex items-center gap-6">
                        ...
                        <button
                          ...
                          className="p-4 bg-slate-50 text-slate-400 hover:text-rose-500 rounded-2xl transition-all"
                        >
                          <X className="h-5 w-5" />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <button
                          onClick={() => updateStatus(table.id, "available")}
                          className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                        >
                          Release
                        </button>
                      </div>
...
function StatCard({ label, value, icon: Icon, color }: any) {
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm transition-all hover:scale-[1.02]">
      <div
        className={`h-12 w-12 rounded-2xl ${colors[color]} flex items-center justify-center mb-6 border`}
      >
...
function TableCard({ table, onEdit, onDelete }: any) {
  ...
  return (
    ...
    >
      <div
        className={`h-24 p-8 flex items-center justify-between border-b border-slate-50 shadow-inner`}
        style={{
          backgroundColor: `${type.color === "slate" ? "#f8fafc" : type.color === "orange" ? "#fff7ed" : type.color === "purple" ? "#faf5ff" : type.color === "blue" ? "#eff6ff" : "#ecfdf5"}`,
        }}
      >
        <div
          className={`h-12 w-12 rounded-2xl bg-white shadow-sm flex items-center justify-center text-slate-900`}
        >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onEdit}
            className="p-3 bg-white text-slate-400 hover:text-slate-900 rounded-xl shadow-sm transition-all hover:scale-110 active:scale-95"
          >
            <Edit3 className="h-4 w-4" />
          </button>
          <button
            onClick={onDelete}
            className="p-3 bg-white text-slate-400 hover:text-rose-500 rounded-xl shadow-sm transition-all hover:scale-110 active:scale-95"
          >
