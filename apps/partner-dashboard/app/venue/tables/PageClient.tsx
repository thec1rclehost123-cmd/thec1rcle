...
const TABLE_TYPES = [
    { id: "standard", label: "Standard", icon: Armchair, color: "var(--text-tertiary)" },
    { id: "premium", label: "Premium", icon: Wine, color: "var(--accent)" },
    { id: "vvip", label: "VVIP", icon: Sparkles, color: "#A78BFA" },
    { id: "booth", label: "Booth", icon: Users, color: "var(--color-info)" },
    { id: "cabana", label: "Cabana", icon: Layout, color: "var(--color-success)" },
];
    ...
    if (isLoading) {
        return (
            <div className="py-16 flex flex-col items-center justify-center">
                <Loader2 className="h-8 w-8 animate-spin mb-3" style={{ color: "var(--text-tertiary)" }} />
        ...
        <VenuePageShell
            ...
            actions={
                <div className="flex items-center gap-2">
                    {/* Mode toggle */}
                    <div className="flex p-1 rounded-xl" style={{ background: "var(--bg-base)", border: "1px solid var(--border-subtle)" }}>
                        <button
                            onClick={() => setMode("setup")}
                            className="px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all"
                            style={{
                                background: mode === "setup" ? "var(--bg-fill)" : "transparent",
                                color: mode === "setup" ? "var(--text-primary)" : "var(--text-tertiary)",
                            }}
                        >
                            Floor Setup
                        </button>
                        <button
                            ...
                            className="px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all"
                            style={{
                                background: mode === "tonight" ? "var(--bg-fill)" : "transparent",
                                color: mode === "tonight" ? "var(--text-primary)" : "var(--text-tertiary)",
                            }}
                        >
                            Tonight
                        </button>
            ...
            {mode === "setup" ? (
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                            { label: "TABLES", value: tables.length, icon: Armchair, bg: "var(--color-info-bg)", text: "var(--color-info)" },
                            { label: "CAPACITY", value: totalCapacity, icon: Users, bg: "var(--color-success-bg)", text: "var(--color-success)" },
                            { label: "VVIP", value: vvipCount, icon: Sparkles, bg: "rgba(167,139,250,0.1)", text: "#A78BFA" },
                            { label: "SECTIONS", value: new Set(tables.map(t => t.location)).size, icon: Layers, bg: "var(--color-warning-bg)", text: "var(--color-warning)" },
                        ].map((stat, i) => (
                            <div key={i} className="rounded-2xl p-4 flex items-center gap-3 border border-[var(--border-subtle)]" style={{ background: "var(--bg-elevated)" }}>
                                ...
                                <div>
                                    ...
                                    <p className="text-[18px] font-black leading-tight tabular-nums" style={{ color: "var(--text-primary)" }}>
                                        {stat.value}
                                    </p>
                                </div>
                            </div>
                    ...
                    <div className="relative">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-3.5 w-3.5" style={{ color: "var(--text-tertiary)" }} />
                        <input
                            type="text"
                ...
                <>
                    {/* Tonight header */}
                    <div className="rounded-2xl p-5" style={{ background: "var(--bg-base)", border: "1px solid var(--border-subtle)" }}>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="px-4 py-3 rounded-xl" style={{ background: "var(--bg-fill)", border: "1px solid var(--border-subtle)" }}>
                                ...
                                <div className="px-4 py-3 rounded-xl" style={{ background: "var(--bg-fill)", border: "1px solid var(--border-subtle)" }}>
                                    ...
                                    <p className="text-xl font-black tabular-nums" style={{ color: "var(--color-success)" }}>
                                    ...
                                    <div key={table.id} className="px-4 py-3 flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div
                                                ...
                                                style={{
                                                    background: isBlocked ? "var(--color-warning-bg)" : booking ? "var(--color-success-bg)" : "var(--bg-fill)",
                                                    color: isBlocked ? "var(--color-warning)" : booking ? "var(--color-success)" : "var(--text-tertiary)",
                                                }}
                                            >
                                                <type.icon className="h-4 w-4" />
                                            </div>
                                            <div>
                                                <h3 className="text-[14px] font-bold" style={{ color: "var(--text-primary)" }}>{table.name}</h3>
                                            ...
                                            {booking ? (
                                                <>
                                                    <div className="text-right">
                                                        <p className="text-[12px] font-bold" style={{ color: "var(--text-primary)" }}>{booking.guestName || "Reserved"}</p>
                                                        <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "var(--color-success)" }}>Confirmed</p>
                                                    </div>
                                                    <button onClick={() => updateStatus(table.id, 'available')} className="p-2 rounded-lg transition-colors" style={{ color: "var(--text-tertiary)", background: "var(--bg-fill)" }}>
                                                        <X className="h-3.5 w-3.5" />
                                                    </button>
                                                </>
                                            ) : isBlocked ? (
                                                <>
                                                    <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "var(--color-warning)" }}>Held</span>
                                                    <button onClick={() => updateStatus(table.id, 'available')} className="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest" style={{ background: "var(--bg-fill)", color: "var(--text-secondary)" }}>
                                                        Release
                                                    </button>
                                                </>
                                            ) : (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all"
                                                        style={{ background: "var(--color-success-bg)", color: "var(--color-success)", border: "1px solid rgba(52,211,153,0.2)" }}
                                                    >
                                                        Reserve
                                                    </button>
                                                </div>
        ...
        >
            ...
            <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <p className="text-[14px] font-bold leading-tight" style={{ color: "var(--text-primary)" }}>{table.name}</p>
                        <p className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-1" style={{ color: "var(--text-tertiary)" }}>
                            <MapPin className="h-2.5 w-2.5" /> {table.location || "Main Floor"}
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={onEdit} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--text-tertiary)" }}>
                        <Edit3 className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={onDelete} className="p-1.5 rounded-lg transition-colors hover:text-red-400" style={{ color: "var(--text-tertiary)" }}>
            ...
            <div className="px-4 py-3">
                <div className="grid grid-cols-2 gap-2">
                    <div className="p-2.5 rounded-xl" style={{ background: "var(--bg-fill)" }}>
                        <p className="text-[8px] font-black uppercase tracking-widest mb-0.5" style={{ color: "var(--text-tertiary)" }}>Min Spend</p>
                        <p className="text-[15px] font-black leading-none tabular-nums" style={{ color: "var(--text-primary)" }}>₹{table.minSpend || 0}</p>
                    </div>
                    <div className="p-2.5 rounded-xl" style={{ background: "var(--bg-fill)" }}>
                        <p className="text-[8px] font-black uppercase tracking-widest mb-0.5" style={{ color: "var(--text-tertiary)" }}>Seats</p>
                        <p className="text-[15px] font-black leading-none tabular-nums" style={{ color: "var(--text-primary)" }}>{table.capacity} Pax</p>
                    </div>
                </div>
                <div className="flex items-center justify-between mt-3 pt-2" style={{ borderTop: "1px solid var(--border-subtle)" }}>
                    <span
                        ...
                        style={{
                            background: table.isActive !== false ? "var(--color-success-bg)" : "var(--bg-fill)",
                            color: table.isActive !== false ? "var(--color-success)" : "var(--text-tertiary)",
                        }}
                    >
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
            <motion.div
                ...
                className="relative w-full max-w-lg rounded-[var(--r-xl)] overflow-hidden"
                style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-subtle)" }}
            >
                {/* Modal header */}
                <div className="p-6 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                    <div>
                        ...
                        <p className="text-[12px] mt-0.5" style={{ color: "var(--text-tertiary)" }}>Configure layout properties</p>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-xl transition-colors" style={{ color: "var(--text-tertiary)", background: "var(--bg-fill)" }}>
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-5 max-h-[65vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="relative">
                                <Users className="absolute left-3.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5" style={{ color: "var(--text-tertiary)" }} />
                                <input
                                    type="number"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <DollarSign className="absolute left-3.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5" style={{ color: "var(--text-tertiary)" }} />
                                <input
                                    type="number"
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                        <button
                            ...
                            style={{ background: formData.isActive ? "var(--accent)" : "var(--bg-fill)" }}
                        >
                            <div
                        ...
                        <span className="text-[12px] font-semibold" style={{ color: "var(--text-secondary)" }}>Active in Discovery</span>
                    </label>
                </div>

                {/* Modal footer */}
                <div className="p-6 flex items-center justify-end gap-3" style={{ borderTop: "1px solid var(--border-subtle)", background: "var(--bg-base)" }}>
                    <button
                        onClick={onClose}
