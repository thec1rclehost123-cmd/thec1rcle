...
export default function VenueDashboardHome() {
  ...
  const { data: alertsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/partners/venues/notifications?venueId=${venueId}&limit=3`, {
        headers,
      });
      return res.json();
    },
  ...
  const {
    ...
    queryFn: async () => {
      const token = await getIdToken();
      const headers: HeadersInit = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await fetch(`/api/partners/venues/overview/summary?venueId=${venueId}`, {
        headers,
      });
      return res.json();
    },
  ...
  const { data: tonight, isLoading: tonightLoading } = useQuery({
    ...
    queryFn: async () => {
      const token = await getIdToken();
      const headers: HeadersInit = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await fetch(`/api/partners/venues/overview/tonight?eventId=${tonightEvent?.id}`, {
        headers,
      });
      return res.json();
    },
    ...
    return (
      <VenuePageShell title="Dashboard">
        <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
          ...
          <p className="text-[16px] font-semibold" style={{ color: 'var(--v-text-primary)' }}>
  ...
  return (
    ...
    >
            ...
            <div className="relative z-10">
              ...
              >
                {loading
                  ? 'Fetching live data...'
                  : tonight
                    ? `${tonight.checkedIn} checked in · ${capacityPct}% capacity`
                    : 'Next event ' +
                      (nextEvent
            ...
            <div className="flex items-center justify-between relative z-10 mt-4">
              {tonight ? (
                <Link
                  ...
                  className="v-btn-primary px-5 py-2 text-[12px]"
                >
                  Entry Control →
                </Link>
              ) : (
          ...
          <div className="v-hero-card p-5 flex flex-col justify-between min-h-[140px] border-l-[3px] border-l-[var(--v-orange)]">
            ...
            <div className="flex items-center gap-2 mt-2">
              {summary?.revenueTrend && (
                <span
                  className={cn(
                    'v-trend-chip text-[10px]',
                    summary.revenueTrendDirection === 'down' ? 'v-trend-down' : 'v-trend-up',
                  )}
                >
                  {summary.revenueTrendDirection === 'down' ? '↓ ' : '↑ '}
                  {summary.revenueTrend}
                </span>
              )}
      ...
      <motion.div {...mp(0.12)} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Upcoming schedule */}
        <div
          className="xl:col-span-2 rounded-[32px] p-4 sm:p-5"
        ...
        >
          ...
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="v-skeleton rounded-2xl h-24" />
              ))}
            </div>
          ) : events.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div
            ...
            style={{ background: 'var(--v-hero)' }}
          >
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="w-4 h-4" style={{ color: 'var(--v-orange)' }} />
                <span className="text-[13px] font-bold" style={{ color: 'var(--v-text-primary)' }}>
                  C1RCLE PRO
                </span>
              </div>
              <p
            ...
            <div
              className="absolute -right-8 -bottom-8 w-36 h-36 rounded-full blur-3xl pointer-events-none"
              style={{ background: 'var(--v-orange-glow)' }}
            />
          </div>
        </div>
      </motion.div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <h4 className="text-xl font-bold text-text-primary mt-2">
                    {tonight.entryVelocity ?? '--'}{' '}
                    <span className="text-[10px] opacity-40 font-black">Guests/hr</span>
                  </h4>
                </div>
                <Link
                  href={`/venue/events/${tonight.id}`}
...
function EventMiniCard({ event }: { event: any }) {
  ...
  return (
    ...
    >
      ...
      <p
        className="text-[14px] font-semibold line-clamp-2 leading-tight uppercase"
        style={{ color: 'var(--v-text-primary)' }}
      >
        {event.title ?? '—'}
      </p>
      <span
        className="v-status-pill self-start text-[10px]"
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: !alert.isRead ? 'var(--v-orange)' : 'var(--v-text-muted)' }}
      />
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-medium truncate" style={{ color: 'var(--v-text-primary)' }}>
          {alert.title ?? 'Notification'}
        </p>
        <p className="text-[11px] truncate" style={{ color: 'var(--v-text-tertiary)' }}>
          {alert.description ?? alert.message ?? ''}
        </p>
      </div>
      <span className="text-[10px] shrink-0 mt-0.5" style={{ color: 'var(--v-text-muted)' }}>
...
function QuickLink({ icon: Icon, label, href }: { icon: any; label: string; href: string }) {
  return (
    ...
    >
      ...
      <span className="text-[12px] font-medium" style={{ color: 'var(--v-text-tertiary)' }}>
        {label}
      </span>
    </Link>
  );
    ...
    <div className="flex items-end gap-1 h-10 group">
      {bars.map((h, i) => (
        <motion.div
          ...
          style={{
            background: i === bars.length - 1 ? 'var(--v-success)' : 'rgba(52,211,153,0.25)',
          }}
        />
      ))}
    </div>
