          ...
          <ActiveRoster
            ...
            onViewProfile={(conn) =>
              setProfileTarget({
                ...
                connectionStatus: conn.status === 'active' ? 'active' : 'approved',
              })
            }
          />
        )}
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-52 bg-surface-secondary rounded-[2rem] animate-pulse border border-border-subtle"
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="space-y-4">
        {[1, 2].map((i) => (
          <div
            key={i}
            className="h-28 bg-surface-secondary rounded-[2rem] animate-pulse border border-border-subtle"
          />
        ))}
      </div>
    );
          ...
          >
            ...
            {req.message && (
              <div className="mt-5 p-5 bg-surface-secondary rounded-2xl border border-border-subtle relative">
                ...
                <p className="text-[13px] text-text-tertiary italic pl-8 leading-relaxed">
                  "{req.message}"
                </p>
              </div>
            )}
...
function EmptyState({
  ...
  return (
    <div className="py-32 relative overflow-hidden bg-surface-secondary dark:bg-[#0D0D0F] border border-dashed border-border-default rounded-[3rem] flex flex-col items-center text-center px-10 group">
      ...
      <p className="relative text-[14px] text-text-tertiary font-medium max-w-xs leading-relaxed">
        {subtitle}
      </p>
    </div>
  );
