    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 pb-10">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium mt-3">
            Duty logs, incident reports, and floor handovers for the date.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard
          ...
          subtext="Needs Resolution"
          color="rose"
        />
        <StatCard
          label="VIPs Expected"
          value={stats.vipsExpected}
                ...
                <div key={log.id} className="p-8 hover:bg-slate-50/50 transition-colors group">
                  <div className="flex items-start justify-between gap-6">
                    <div className="flex gap-6">
                      <div
                        className={`mt-1 h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 border ${getIconColor(log.type)}`}
                      >
                        {getIcon(log.type)}
                      </div>
                      <div className="space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          {log.severity && (
                            <span
                              className={`px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${getSeverityColor(log.severity)}`}
                            >
                              {log.severity}
                            </span>
                          )}
...
function StatCard({
  ...
  return (
    <div
      className={`p-8 rounded-[2.5rem] border ${colors[color] || colors.slate} shadow-sm transition-all hover:scale-[1.02] cursor-default`}
    >
