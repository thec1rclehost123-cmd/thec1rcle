            ...
            <div className="space-y-2">
              {sections.map((section) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <section.icon
                      className={`w-4 h-4 ${activeSection === section.id ? "text-emerald-400" : "text-slate-400"}`}
                    />
                    <span className="text-sm font-bold">{section.label}</span>
                  </div>
                  <span
                    className={`text-[10px] font-bold ${activeSection === section.id ? "text-white/40" : "text-slate-300"}`}
                  >
                    {section.count}
                  </span>
                </button>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-900 flex items-center gap-3">
                        {item.name}
                        {!item.available && (
                          <span className="px-2 py-0.5 bg-slate-100 rounded text-[9px] text-slate-400 uppercase tracking-tighter">
                            Out of Stock
                          </span>
                        )}
                      </h4>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div
                      className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer ${item.available ? "bg-emerald-500" : "bg-slate-200"}`}
                    >
                      <div
                        className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full shadow-sm transition-all ${item.available ? "left-5.5" : "left-0.5"}`}
                      />
                    </div>
                  </div>
                </div>
