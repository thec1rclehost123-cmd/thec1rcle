      ...
      {!recon && !isLoading && (
        <div className="py-16 text-center rounded-xl border border-border-subtle bg-surface-secondary">
          ...
          <p className="text-text-tertiary text-sm font-medium">
            {events.length === 0
              ? 'No cover charge events found for this venue.'
              : 'Select an event to view reconciliation.'}
          </p>
        </div>
      )}
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="bg-surface-tertiary">
                    <th className="px-5 py-3 text-left text-[10px] font-bold text-text-tertiary uppercase tracking-widest">
                      Item
                    </th>
                    <th className="px-5 py-3 text-right text-[10px] font-bold text-text-tertiary uppercase tracking-widest">
                      Times Charged
                    </th>
                    <th className="px-5 py-3 text-right text-[10px] font-bold text-text-tertiary uppercase tracking-widest">
                      Total Value
                ...
                <tbody>
                  {recon.itemDistribution.map((item, i) => {
                    const pct =
                      recon.totalRedeemed > 0
                        ? Math.round((item.totalPaise / recon.totalRedeemed) * 100)
                        : 0;
                    return (
                      <tr
                        key={item.itemId || i}
                        className="border-t border-border-subtle hover:bg-surface-tertiary/50 transition-all"
                      >
                        ...
                        <td className="px-5 py-3 text-right text-text-primary tabular-nums font-semibold">
              ...
              <div className="space-y-2">
                {recon.exceptionList.map((exc, i) => (
                  <div key={i} className="flex items-start gap-3 text-[12px]">
                    ...
                    <span className="text-text-tertiary ml-auto font-mono text-[11px] flex-shrink-0">
                      {exc.walletId}
                    </span>
                  </div>
                ))}
    ...
    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '1.1rem' }}>
      ...
      >
        ...
        <p
          ...
          style={{ fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: '-0.015em' }}
        >
          {value}
        </p>
      </div>
