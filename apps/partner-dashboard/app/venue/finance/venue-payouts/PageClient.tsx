...
function PayoutPillRow({ payout }: { payout: PayoutRequest }) {
  ...
  const Icon = cfg.icon;
  const dateStr = new Date(payout.requestedAt).toLocaleDateString('en-IN', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  });
  const detail = payoutAny.eventName
    ...
    : payout.method
      ? `${payout.method}${payout.methodDetail ? ' · ' + payout.methodDetail : ''}`
      : 'Venue payout';
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <p
          ...
          style={{ fontSize: 15, fontWeight: 700, color: cfg.color, letterSpacing: '-0.01em' }}
        >
    ...
    >
        ...
        {[
          {
            ...
            value: formatINRFromPaise(balance?.withdrawablePaise ?? 0),
          ...
          },
          {
            ...
            value: formatINRFromPaise(balance?.pendingSettlementPaise ?? 0),
      ...
      >
        ...
        <div className="p-4 space-y-2">
          ...
          {history.map((p) => (
            <PayoutPillRow key={p.id} payout={p} />
          ))}
          {!isLoading && history.length === 0 && (
            <div
              className="py-12 text-center"
              style={{ color: 'rgba(255,255,255,0.36)', fontSize: 14 }}
            >
