...
export function PaymentsClient() {
  ...
  return (
    <VenuePageShell title="Payments" subtitle="Wallet balance, subscription, and billing methods">
      {/* Wallet KPIs — glassmorphism glow cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          {
            label: 'Available',
            value: formatINRFromPaise(wallet?.availablePaise ?? 0),
          ...
          {
            label: 'Pending',
            value: formatINRFromPaise(wallet?.pendingPaise ?? 0),
          ...
          {
            label: 'On Hold',
            value: formatINRFromPaise(wallet?.heldPaise ?? 0),
          ...
          <div
            key={card.label}
            style={{ position: 'relative', overflow: 'hidden', borderRadius: '1.25rem' }}
          >
            {/* Radial glow layer */}
            <div
              style={{
      ...
      {sub && (
        <div className="rounded-xl border border-border-default bg-surface-secondary p-5 mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`inline-flex items-center rounded-md border border-transparent px-2 py-0.5 text-xs font-medium capitalize ${PLAN_COLORS[sub.plan]}`}
            >
              {sub.plan}
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-text-primary tabular-nums">
                {formatINRFromPaise(sub.amountPaise)}/mo
              </p>
            </div>
            <div>
