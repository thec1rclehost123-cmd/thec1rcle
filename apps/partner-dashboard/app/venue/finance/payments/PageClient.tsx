...
export function PaymentsClient() {
    ...
    return (
        ...
        >
            ...
            {sub && (
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5 mb-4">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-semibold text-zinc-300">Subscription</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            ...
                            <p className="text-sm text-white capitalize">{sub.status.replace("_", " ")}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p className="text-sm text-white">{sub.nextBillingDate ?? "—"}</p>
                        </div>
                        <div>
                            <p className="text-xs text-zinc-600">Autopay</p>
            ...
            {/* Billing methods */}
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
                <h3 className="text-sm font-semibold text-zinc-300 mb-4">Billing Methods</h3>
                {(!data?.billingMethods || data.billingMethods.length === 0) ? (
                    <p className="text-sm text-zinc-600">No billing methods added yet.</p>
                ) : (
                        ...
                        {data.billingMethods.map((bm) => (
                            <div key={bm.id} className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm text-white">{bm.label}</p>
                                        <p className="text-xs text-zinc-500">{bm.maskedDetail}</p>
                                    </div>
                                </div>
                                {bm.isDefault && (
                                    <span className="text-xs text-zinc-500 border border-white/[0.06] rounded px-2 py-0.5">
                                        Default
                                    </span>
                                )}
                            </div>
