...
const REPORT_TYPES: ReportType[] = [
    { id: "ledger_csv",           title: "Full Ledger Export",         description: "All transactions for the selected period in CSV format.",                         available: true,  extension: "csv" },
    { id: "revenue_summary",      title: "Revenue Summary",            description: "Gross, net, fees, refunds, and partner obligations by source.",                   available: true,  extension: "csv" },
    { id: "payout_history",       title: "Payout History",             description: "All payouts with status, amounts, dates, and destinations.",                      available: true,  extension: "csv" },
...
function LedgerSection() {
    ...
    const fetchLedger = useCallback(async (opts?: {
        p?: number; q?: string; cat?: TransactionCategory | ""; st?: SettlementStatus | "";
    }) => {
        if (!venueId) return;
        const p   = opts?.p   ?? page;
    ...
    const handleSearch = (q: string) => {
        ...
        searchDebounce.current = setTimeout(() => { setPage(1); fetchLedger({ p: 1, q }); }, 350);
    };
    ...
    const handleExportCSV = async () => {
        ...
        try {
            ...
            const rows = transactions.map((tx) => [
                ...
                tx.amount, tx.direction, tx.category, tx.status,
                `"${(tx.description || "").replace(/"/g, '""')}"`,
                tx.eventName || "", tx.partnerName || "", tx.paymentSource || "",
                tx.settlementBatchId || "", tx.processorFee || 0, tx.platformFee || 0, tx.netAmount || 0,
            ]);
        ...
        <div className="space-y-4">
            ...
            <BentoCard padding="sm" empty={false}>
                <LedgerTable
                    ...
                    onCategoryFilter={(cat) => { setCategory(cat); setPage(1); fetchLedger({ p: 1, cat }); }}
                    onStatusFilter={(st) => { setStatus(st); setPage(1); fetchLedger({ p: 1, st }); }}
                    totalCount={totalCount}
                    page={page}
                    onPageChange={(newPage) => { setPage(newPage); fetchLedger({ p: newPage }); }}
                    pageSize={PAGE_SIZE}
                />
            </BentoCard>
        </div>
...
function ReportsSection() {
    ...
    const handleGenerate = async (report: ReportType) => {
        ...
        try {
            if (report.extension === "csv") {
                ...
                const headers = ["Transaction ID", "Date", "Amount (₹)", "Direction", "Category", "Status", "Description", "Event", "Partner", "Net Amount (₹)"];
                const rows = transactions.map((tx: any) => [
                    `TR-${tx.id}`,
                    new Date(tx.timestamp).toISOString().slice(0, 10),
                    tx.amount, tx.direction, tx.category, tx.status,
                    `"${(tx.description || "").replace(/"/g, '""')}"`,
                    tx.eventName || "", tx.partnerName || "", tx.netAmount || 0,
                ]);
    ...
    return (
        <div className="space-y-4">
            {/* Date range */}
            <BentoCard header={<div className="flex items-center gap-2"><Calendar className="w-4 h-4" style={{ color: "var(--v-text-muted)" }} /><span className="v-label">DATE RANGE</span></div>}>
                <div className="flex items-center gap-4 flex-wrap">
                    <div>
                        <label className="block text-[11px] font-bold uppercase tracking-widest mb-1.5" style={{ color: "var(--v-text-muted)" }}>FROM</label>
                    ...
                    <div>
                        <label className="block text-[11px] font-bold uppercase tracking-widest mb-1.5" style={{ color: "var(--v-text-muted)" }}>TO</label>
                        <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} min={fromDate} max={new Date().toISOString().slice(0, 10)}
                            className="px-4 py-2.5 rounded-xl text-[13px] outline-none focus:ring-1 focus:ring-[var(--v-orange)]"
                            style={{ background: "var(--v-elevated)", color: "var(--v-text-primary)", border: "1px solid var(--v-border)" }} />
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {REPORT_TYPES.map((report) => (
                    <ReportTile key={report.id} report={report} isGenerating={generating === report.id} onGenerate={() => handleGenerate(report)} />
                ))}
            </div>
...
function ReportTile({ report, isGenerating, onGenerate }: { report: ReportType; isGenerating: boolean; onGenerate: () => void }) {
    return (
        <div className="flex flex-col p-5 rounded-[var(--v-r-xl)]" style={{ background: "var(--v-card)", border: "1px solid var(--v-border)", opacity: report.comingSoon ? 0.7 : 1 }}>
            <div className="flex items-start gap-3 mb-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: report.extension === "pdf" ? "rgba(129,140,248,0.12)" : "rgba(52,211,153,0.12)" }}>
                    <FileText className="w-4 h-4" style={{ color: report.extension === "pdf" ? "#818CF8" : "#34D399" }} />
                </div>
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-[14px] font-semibold" style={{ color: "var(--v-text-primary)" }}>{report.title}</p>
                        <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded" style={{ background: report.extension === "pdf" ? "rgba(129,140,248,0.15)" : "rgba(52,211,153,0.15)", color: report.extension === "pdf" ? "#818CF8" : "#34D399" }}>
                            {report.extension.toUpperCase()}
                        </span>
                        {report.comingSoon && (
                            <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded" style={{ background: "rgba(245,158,11,0.12)", color: "#F59E0B" }}>Soon</span>
                        )}
                    </div>
                    <p className="text-[12px] mt-0.5" style={{ color: "var(--v-text-muted)" }}>{report.description}</p>
                </div>
            </div>
            <button onClick={onGenerate} disabled={!report.available || isGenerating}
                ...
                style={{ background: report.available ? "var(--v-elevated)" : "transparent", color: "var(--v-text-secondary)", border: "1px solid var(--v-border)" }}>
                {isGenerating ? <>Generating…</> : report.comingSoon ? <>Coming Soon</> : <><Download className="w-3.5 h-3.5" />Download {report.extension.toUpperCase()}</>}
            </button>
        </div>
    );
}
...
export function RecordsClient() {
    ...
    return (
        ...
        >
            {/* Sub-tab toggle */}
            <div className="flex p-1 rounded-xl gap-0.5 mb-6 w-fit" style={{ background: "var(--v-card)", border: "1px solid var(--v-border)" }}>
