...
function SettlementRow({ s, partnerLabel }: { s: PartnerSettlement; partnerLabel: string }) {
    return (
...
function SettlementTable({ title, rows, isLoading, partnerLabel }: {
    title: string; rows: PartnerSettlement[]; isLoading: boolean; partnerLabel: string;
}) {
    return (
                ...
                <table className="w-full min-w-[560px]">
                    <thead>
                        <tr className="border-b border-border-subtle">
                            {[partnerLabel, "Event", "Date", "Net Due", "Status", "Settled", "Note"].map((h) => (
                                <th key={h} className="py-2.5 px-4 text-left text-xs font-medium text-text-tertiary">{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        {rows.map((s) => <SettlementRow key={s.id} s={s} partnerLabel={partnerLabel} />)}
...
function HostSection() {
    ...
    return (
        <div className="space-y-4">
            ...
            <SettlementTable title="Pending Settlements" rows={data?.pendingSettlements ?? []} isLoading={isLoading} partnerLabel="Host" />
            <SettlementTable title="Settlement History" rows={data?.historySettlements ?? []} isLoading={isLoading} partnerLabel="Host" />
        </div>
    );
}
...
function PromoterSection() {
    ...
    return (
        <div className="space-y-4">
            ...
            <SettlementTable title="All Settlements" rows={allRows} isLoading={isLoading} partnerLabel="Promoter" />
        </div>
    );
}

export function PartnerPayoutsClient() {
    const [sub, setSub] = useState<"hosts" | "promoters">("hosts");

    return (
        <VenuePageShell
            title="Partner Payouts"
            subtitle="Settlement obligations owed to hosts and promoters"
        >
            {/* Sub-tab toggle */}
            <div className="flex p-1 rounded-xl gap-0.5 mb-6 w-fit" style={{ background: "var(--v-card)", border: "1px solid var(--v-border)" }}>
                {(["hosts", "promoters"] as const).map((t) => (
                    <button
                        key={t}
                        onClick={() => setSub(t)}
                    ...
                    >
                        {t === "hosts" ? "Hosts" : "Promoters"}
                    </button>
                ))}
            </div>

            {sub === "hosts" ? <HostSection /> : <PromoterSection />}
        </VenuePageShell>
    );
}
