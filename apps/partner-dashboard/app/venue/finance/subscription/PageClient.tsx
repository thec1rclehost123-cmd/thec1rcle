        ...
        <div className="space-y-6">
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-[12px] mt-0.5" style={{ color: "var(--v-text-secondary)" }}>
                        Once active, your plan cost will be automatically debited from your C1RCLE wallet on the same date each month. You can cancel or upgrade anytime.
                    </p>
                </div>
            </div>
            ...
            >
                ...
                <div
                    ...
                    style={{ background: "rgba(255,255,255,0.04)", color: "var(--v-text-tertiary)", border: "1px solid var(--v-border)" }}
                >
                    Inactive
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {PLANS.map((plan) => (
                        ...
                        >
                            ...
                            <div>
                                <p className="text-[12px] font-bold uppercase tracking-widest" style={{ color: plan.accent }}>
                                    {plan.name}
                                </p>
                                <p className="text-[24px] font-black mt-1" style={{ color: "var(--v-text-primary)" }}>
                                    {plan.price}
                                </p>
                                <p className="text-[11px]" style={{ color: "var(--v-text-tertiary)" }}>{plan.billing}</p>
                                <p className="text-[12px] mt-2" style={{ color: "var(--v-text-secondary)" }}>{plan.description}</p>
                            </div>

                            <ul className="flex flex-col gap-2 flex-1">
                                {plan.features.map((f) => (
                                    <li key={f} className="flex items-start gap-2">
                                        <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 shrink-0" style={{ color: plan.accent }} />
                                        <span className="text-[12px]" style={{ color: "var(--v-text-secondary)" }}>{f}</span>
                                    </li>
                                ))}
                            </ul>
