...
import { Megaphone, ArrowRight, Info, Users, IndianRupee, Building2, MessageCircle, Zap, BarChart2 } from "lucide-react";
...
function FlowStep({
    ...
    return (
        ...
        >
            ...
            <p className="text-[11px]" style={{ color: "var(--v-text-tertiary)" }}>{sublabel}</p>
        </div>
    );
}
    ...
    return (
        <div className="space-y-6">
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-[12px] mt-0.5" style={{ color: "var(--v-text-secondary)" }}>
                        When activated, campaign costs are automatically deducted from your C1RCLE wallet. All funds pass through the C1RCLE platform account before being routed to carrier APIs (WhatsApp Business, SMS).
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    { label: "Spent This Month", value: "₹0", icon: IndianRupee, color: "#F44A22", bg: "rgba(244,74,34,0.08)" },
                    { label: "Users Reached", value: "0", icon: Users, color: "#34D399", bg: "rgba(52,211,153,0.08)" },
                    { label: "Active Campaigns", value: "0", icon: Megaphone, color: "#818CF8", bg: "rgba(129,140,248,0.08)" },
                    { label: "Avg. Cost / User", value: "—", icon: BarChart2, color: "#FBBF24", bg: "rgba(251,191,36,0.08)" },
                ].map((stat) => (
                    <div
                        key={stat.label}
            ...
            >
                ...
                >
                    <p className="text-[12px] font-semibold" style={{ color: "var(--v-text-primary)" }}>Cost model</p>
                    <ul className="flex flex-col gap-1.5">
                        {[
                            ...
                            <li key={line} className="flex items-start gap-2">
                                <span className="w-1 h-1 rounded-full mt-1.5 shrink-0" style={{ background: "#F44A22" }} />
                                <span className="text-[12px]" style={{ color: "var(--v-text-secondary)" }}>{line}</span>
                            </li>
                        ))}
                    </ul>
                </div>
