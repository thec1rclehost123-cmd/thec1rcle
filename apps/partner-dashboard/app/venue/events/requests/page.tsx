  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const slotsRes = await fetch(
        `/api/slots?venueId=${venueId}&status=${activeTab === "pending" ? "pending" : ""}`,
      );
      ...
      const eventRequestsFromSubmitted = (submittedData || [])
        .filter((e: any) => !slotEventIds.has(e.id))
        .map((e: any) => ({
          ...e,
  ...
  const handleAction = async (action: "approve" | "reject" | "suggest") => {
    ...
    try {
      ...
      if (action === "approve" || action === "reject") {
        ...
        if (
          action === "reject" ||
          (action === "approve" && actionModal.request.lifecycle === "submitted")
        ) {
                ...
                {(request.slotRequest?.status === "pending" ||
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      ...
                      {request.lifecycle === "submitted" ? "Publish" : "Approve"}
                    </button>
                    <button
            ...
            >
              ...
              <p className="text-body-sm text-[#86868b] mb-6">
                {actionModal.type === "approve" &&
                  "This will confirm the slot and approve the event for publication."}
                {actionModal.type === "reject" &&
                  "The host will be notified of the rejection and can request a different date."}
                {actionModal.type === "changes" &&
                  "Suggest alternative dates or request modifications."}
              </p>
              ...
              <div className="p-4 rounded-xl bg-[#f5f5f7] mb-4">
                ...
                <p className="text-[13px] text-[#86868b]">
                  {actionModal.request.slotRequest?.requestedDate} •{" "}
