...
export default function EventManagementPage() {
  ...
  const handleTogglePromoterSelection = async (promoterId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPromoters((prev) =>
          prev.map((p) => (p.id === promoterId ? { ...p, isSelected: !p.isSelected } : p)),
        );
      }
    } finally {
      setIsUpdating(false);
  ...
  if (loading)
    return <div className="p-12 text-center text-slate-400">Loading management console...</div>;
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-rose-600 mt-0.5">
              ...
              {event.refundStatus && (
                <span className="ml-2">
                  • Refund status: <strong>{event.refundStatus}</strong>
                </span>
              )}
            </p>
          </div>
          {event.cancellationSummary && (
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-lg font-bold text-rose-900">
                {event.cancellationSummary.refundsProcessed || 0} /{" "}
                {event.cancellationSummary.totalOrders || 0}
              </p>
            </div>
          )}
        </motion.div>
            ...
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                  event.lifecycle === "scheduled" || event.lifecycle === "live"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                    ...
                    <p className="text-sm text-slate-500 mb-4">
                      Your event is live at thec1rcle.in/e/{id}
                    </p>
                  ...
                  <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                    ...
                    <p className="text-sm text-slate-500 mb-4">
                      {event.settings?.showGuestlist ? "Enabled" : "Disabled"} - Guests can see
                      who's coming
                    </p>
                    <button
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-900 rounded-3xl p-6 text-white text-center">
                  ...
                  <p className="text-xs text-slate-400 mb-4">
                    Our partner support team is available 24/7 for event emergencies.
                  </p>
                  <button className="w-full py-2.5 bg-white text-slate-900 rounded-xl font-bold text-xs hover:bg-slate-100 transition-all">
                    Support Chat
                  </button>
                </div>
              </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 pb-8 border-b border-slate-100">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Enable selected promoters to generate affiliate links and track their sales
                      conversions.
                    </p>
                  </div>
                  <button
                ...
                {event.promoterSettings?.enabled ? (
                  ...
                  <div className="py-20 text-center space-y-4">
                    ...
                    <p className="text-slate-500 max-w-sm mx-auto">
                      Toggle the active network switch above to allow promoters to generate tracking
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                  <div>
                    <h3 className="text-xl font-bold mb-1 font-mono uppercase tracking-tight">
                      Identity Guestlist
                    </h3>
                    <p className="text-sm text-slate-500">
                      Real-time view of claimed tickets and pending slots.
                    </p>
                  </div>
                  ...
                  {guestlist.length === 0 && (
                    <div className="py-20 text-center">
                      ...
                      <p className="text-slate-500 font-bold">
                        No identities found for this event yet.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between py-6 border-b border-slate-100">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Allow guests on the website to see who else is attending.
                    </p>
                  </div>
                  <button className="flex items-center gap-2 p-1 rounded-full bg-slate-100 scale-110">
                    <div
                      className={`h-6 w-12 rounded-full relative transition-all ${event.settings?.showGuestlist ? "bg-indigo-600" : "bg-slate-300"}`}
                    >
                      <div
                        className={`absolute top-1 h-4 w-4 bg-white rounded-full transition-all ${event.settings?.showGuestlist ? "right-1" : "left-1"}`}
                      />
                    </div>
                  </button>
                </div>
                <div className="flex items-center justify-between py-6 border-b border-slate-100">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Check for 21+ status before ticket purchase.
                    </p>
                  </div>
