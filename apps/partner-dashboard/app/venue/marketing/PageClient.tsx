...
export default function VenueMarketingPageClient() {
    ...
    const openComposer = () => {
        if (selectedIds.length === 0) {
            error("Select recipients", "Choose at least one attendee before opening the campaign composer.");
            return;
        }
    ...
    const handleSend = async () => {
        if (!user || !venueId) {
            error("Unavailable", "You must be signed in with an active venue membership to queue campaigns.");
            return;
        }
                    ...
                    <div className="overflow-x-auto">
                        <table className="dashboard-data-table min-w-[880px]">
                            <thead>
                                <tr>
                                    <th>
                                        <button type="button" onClick={toggleAll} className="inline-flex h-5 w-5 rounded-full border border-[var(--v-border)] bg-[var(--v-elevated)]" />
                                    </th>
                                    <th>Name</th>
                                    <th>Tickets</th>
                                    <th>Total Spend</th>
                                        ...
                                        <tr key={recipient.id}>
                                            ...
                                            <td>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-medium text-[var(--v-text-primary)]">{recipient.name}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div className="space-y-2">
                                                    ...
                                                    {!recipient.directShareAllowed && (
                                                        <div className="text-[11px] text-[var(--v-text-tertiary)]">Direct export disabled</div>
                                                    )}
                                                </div>
                                            </td>
                                            <td>{recipient.lastPurchase ? new Date(recipient.lastPurchase).toLocaleDateString("en-IN", { month: "short", day: "numeric" }) : "—"}</td>
                                            <td>
                                                <button type="button" className="venue-utility-button" onClick={openComposer}>
                    ...
                    <>
                        ...
                        >
                            <div className="flex items-center justify-between border-b border-[var(--v-border)] px-8 py-6">
                                <div>
                                    <h2 className="text-[34px] font-semibold tracking-[-0.05em] text-white">New SMS Campaign</h2>
                                    <p className="mt-2 text-sm text-white/48">Compose a custom blast for the selected venue audience.</p>
                                </div>
                                <button type="button" onClick={() => setComposerOpen(false)} className="venue-utility-button">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                                ...
                                <div className="space-y-6">
                                    <div className="rounded-[24px] border border-[var(--v-border)] bg-[#131722] p-5">
                                        ...
                                        <p className="mt-3 text-sm text-white/72">
                                            This campaign will be queued to THE C1RCLE delivery layer for {selectedRecipients.length} attendee{selectedRecipients.length === 1 ? "" : "s"}.
                                        </p>
                                        <p className="mt-2 text-xs text-white/45">
                                            Contact data stays masked in the dashboard. Direct sharing remains disabled unless venue-specific consent has been recorded.
                                        </p>
                                    </div>

                                    <div className="space-y-3">
                                        <div className="dashboard-filter-row">
                                            <button type="button" className="dashboard-filter-chip" onClick={() => setMessage((value) => `${value} {{first_name}}`)}>
                                                Insert Field
                                            </button>
                                            <button type="button" className="dashboard-filter-chip" onClick={() => setMessage((value) => `${value} {{event_link}}`)}>
                                                Attach Event Link
                                            </button>
                                        </div>
                                        <label className="block rounded-[24px] border border-[var(--v-border)] bg-[#11151f] p-5">
                                            <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-white/35">Compose your message</div>
                                            <textarea
                                                value={message}
                                            ...
                                            />
                                            <div className="mt-3 text-right text-sm text-white/42">{message.length}/140 Characters</div>
                                        </label>
                                    </div>

                                    <div className="flex justify-end gap-3">
                                        <button type="button" onClick={() => setComposerOpen(false)} className="dashboard-filter-chip">
                                            Cancel
                                        </button>
                                        <button type="button" onClick={handleSend} className="venue-cta-primary" disabled={isSending}>
