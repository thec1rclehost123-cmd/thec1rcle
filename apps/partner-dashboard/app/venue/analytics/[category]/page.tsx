...
import {
  ...
  StudioCard,
  ChartPlaceholder,
} from "@/components/studio/StudioComponents";
    ...
    return (
      <div className="py-24 flex flex-col items-center text-center">
        ...
        <p className="text-body text-[var(--text-tertiary)] mb-10 max-w-sm mx-auto">
          Complete your first event to see {categoryLabels[category].toLowerCase()} analytics and
            ...
            <div className="w-full flex items-end justify-between h-48 px-4 gap-2">
              {stats.revenueTimeline?.map((item: any, i: number) => (
                <div
                  key={i}
                  style={{
                    height: `${Math.min(100, (item.revenue / (stats.totalRevenue / stats.eventCount)) * 50)}%`,
                  }}
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-10 shadow-2xl shadow-slate-200 text-white">
          ...
          <div className="space-y-6">
            {stats.topEvents?.map((event: any, i: number) => (
              <div
                key={event.id}
                className="flex items-center justify-between p-2 hover:bg-white/5 rounded-2xl transition-all group"
              >
                ...
                <div key={band}>
                  <div className="flex justify-between text-xs font-black uppercase tracking-widest mb-2">
                    ...
                    <span className="text-slate-900">
                      {count} guests ({percent.toFixed(1)}%)
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">
                      {promoter.tickets} Tickets
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-black text-slate-900">
                    ₹{(promoter.revenue / 1000).toFixed(1)}K
                  </p>
      ...
      {stats.recommendations?.map((rec: any, i: number) => (
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            <span
              className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${rec.impact === "High" ? "bg-amber-100 text-amber-600" : "bg-emerald-100 text-emerald-600"}`}
            >
              {rec.impact} Impact
            </span>
          </div>
            ...
            {[1, 2, 3].map((i) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <span className="text-[8px] font-black uppercase px-2 py-0.5 bg-emerald-100 text-emerald-600 rounded">
                    Success
                  </span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <StudioCard title="Timeline Narrative">
          <p className="text-slate-500 text-sm leading-relaxed font-medium">
            The event maintained high demand pressure for 4 hours starting at 9 PM. Most conversions
            happened within 15 minutes of queue joins, indicating healthy inventory availability.
          </p>
        </StudioCard>
