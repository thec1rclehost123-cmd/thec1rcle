...
function RevenueSection({ data, loading }: { data: AnalyticsDisplayModel; loading: boolean }) {
    ...
    const mergedTimeline = useMemo(
        ...
        [data.revenueTimeline, data.ticketsTimeline]
    );
...
function AudienceSection({ data, loading }: { data: AnalyticsDisplayModel; loading: boolean }) {
    ...
    const peakInterestDay = useMemo(() => {
        if (!data.hasData || data.interestTimeline.length === 0) return "—";
        return data.interestTimeline.reduce(
            (a, b) => (b.count > a.count ? b : a)
        ).date;
    }, [data.hasData, data.interestTimeline]);
...
function EventComparisonSection({ data, loading }: { data: AnalyticsDisplayModel; loading: boolean }) {
    ...
    const sorted = useMemo(() => {
        if (sortKey === "bubble") return data.topEvents;
        return [...data.topEvents].sort(
            (a, b) =>
                (b[sortKey as Exclude<EventSortKey, "bubble">] ?? 0) -
                (a[sortKey as Exclude<EventSortKey, "bubble">] ?? 0)
        );
    }, [data.topEvents, sortKey]);
...
function HeatmapGrid({ data }: { data: AnalyticsDisplayModel }) {
    // Pre-index heatmap into a Map so each cell lookup is O(1) instead of O(n)
    const heatIndex = useMemo(() => {
        const m = new Map<string, number>();
        for (const h of data.heatmap) m.set(`${h.day}|${h.hour}`, h.value);
        return m;
    }, [data.heatmap]);

    const maxVal = useMemo(
        () => Math.max(...data.heatmap.map(h => h.value), 1),
        [data.heatmap]
    );
                ...
                {HEATMAP_DAYS.map(day => (
                    <div key={day} className="flex items-center gap-0.5 mb-0.5">
                        ...
                        {HEATMAP_HOURS.map(hour => {
                            const cellVal = heatIndex.get(`${day}|${hour}`) ?? 0;
                            const intensity = maxVal > 1 ? cellVal / maxVal : 0;
                            return (
                                <div
                                    key={hour}
            ...
            {!loading && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
                    {data.insights.map((insight, i) => {
                        const accent = INSIGHT_ACCENT_COLORS[i % INSIGHT_ACCENT_COLORS.length];
                        return (
                        <div
                            key={i}
                            ...
                            style={{
                                ...
                                border: `1px solid ${
                                    insight.placeholder ? "var(--v-border)" : accent.color + "40"
                                }`,
