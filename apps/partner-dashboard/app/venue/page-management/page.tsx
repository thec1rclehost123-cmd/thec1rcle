  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await authedFetch(
        `/api/venue/page?venueId=${venueId}&dashboard=true&events=true`,
      );
      ...
      if (res.ok && json.venue) {
        ...
        if (fallbackRes.ok && fallbackData.profile) {
          setData({
            ...
            gallery: (fallbackData.profile?.photos || []).map((url: string, i: number) => ({
              id: `photo-${i}`,
              imageUrl: url,
              order: i,
            })),
      ...
      <div className="py-24 max-w-xl mx-auto text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[var(--text-tertiary)]">
            We couldn't retrieve the configuration for your venue. This might be due to incorrect
            permissions or the venue ID not being linked to your account.
          </p>
        </div>
        {error && (
