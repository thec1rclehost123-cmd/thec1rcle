  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await authedFetch(
        `/api/partners/venues/page?venueId=${venueId}&dashboard=true&events=true`,
      );
    ...
    return (
      <div className="py-24 max-w-xl mx-auto text-center space-y-6">
        <div
          ...
          style={{ background: 'var(--v-warning-bg)' }}
        >
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-text-primary uppercase tracking-tight">
            Venue Profile Not Set Up
          </h2>
          <p className="text-text-tertiary">
            Your venue profile hasn&apos;t been configured yet. Use the{' '}
            <strong className="text-text-secondary">Public Page</strong> tab to set up your venue
            name, description, images, and table booking — then return here.
          </p>
        </div>
        {error && (
          <div className="p-4 bg-red-500/5 text-red-500 text-xs font-mono rounded-xl border border-red-500/10">
            {error}
          </div>
        )}
