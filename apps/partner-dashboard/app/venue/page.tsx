...
export default function VenueDashboardHome() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const tonightEvent = normalizedEvents.find(
        (e: any) => e.dateStr === todayStr || e.startDateStr === todayStr,
      );

      const [summaryRes, tonightRes] = await Promise.all([
        fetch(`/api/venue/overview/summary?venueId=${venueId}`),
        tonightEvent
          ? fetch(`/api/venue/overview/tonight?eventId=${tonightEvent.id}`)
  ...
  const getAlertHref = (type: string): string | undefined => {
    switch (type) {
      case "host_request":
        return "/venue/connections/requests";
      case "promoter_request":
        return "/venue/connections/requests";
      case "reservation":
        ...
        <div>
          <div className="flex items-center gap-3 mb-3">
            <div className="live-indicator">
              <span className="text-[10px] font-bold text-[var(--state-success)] uppercase tracking-widest">
                Operational
              </span>
            </div>
          </div>
            ...
            {tonight ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-label-sm text-[var(--text-tertiary)] mb-3">
                      Peak Entry Rate
                    </p>
                    ...
                    <p className="text-caption font-medium text-[var(--state-success)]">
                      124 guests/hr peak
                    </p>
                  </div>
                  ...
                  >
                    <div>
                      <p className="text-label-sm text-[var(--text-tertiary)] mb-0.5">
                        Control Panel
                      </p>
              ...
              <div className="py-16 text-center border-2 border-dashed border-[var(--border-subtle)] rounded-2xl">
                ...
                <p className="text-caption text-[var(--text-placeholder)] mt-1">
...
function AlertItem({
  ...
  const stateClasses = {
    success:
      "bg-[var(--state-success-bg)] text-[var(--state-success)] border-[var(--state-success)]/20",
    warning:
      "bg-[var(--state-warning-bg)] text-[var(--state-warning)] border-[var(--state-warning)]/20",
  ...
  return (
    <Wrapper
      href={href as string}
      className={`flex items-start gap-3 p-3 rounded-xl border border-transparent transition-all ${
        href
          ? "hover:bg-[var(--surface-secondary)] hover:border-[var(--border-subtle)] cursor-pointer"
          : ""
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          ...
          <span className="text-caption text-[var(--text-placeholder)] whitespace-nowrap">
            {time}
          </span>
        </div>
