    ...
    <div className="space-y-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-8 h-full">
          <BentoCard
            padding="lg"
            className="border-border-default shadow-xl bg-surface-elevated/40 backdrop-blur-xl"
          >
            <div className="flex items-center gap-4 mb-10">
              ...
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary mt-0.5 opacity-60">
                  Terminal Management
                </p>
              </div>
            </div>
              ...
              {selectedEvent && (
                ...
                >
                  ...
                  <p className="text-text-primary/30 text-[10px] font-semibold mt-8 max-w-[240px] mx-auto leading-relaxed">
                    Scan this code within the{' '}
                    <span className="text-text-primary">C1rcle Guard App</span> to bridge
                    distributed handheld scanners.
                  </p>
                </motion.div>
              )}
            </div>
              ...
              <div className="relative py-10 flex justify-center">
                <div className="relative h-64 w-64">
                    ...
                    <motion.circle
                      ...
                      animate={{
                        strokeDashoffset:
                          ...
                          Math.PI *
                          114 *
                          (1 - selectedEvent.checkedIn / (selectedEvent.totalTickets || 1)),
                      }}
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-6xl font-black text-text-primary tracking-tighter tabular-nums">
                      {Math.round(
                        (selectedEvent.checkedIn / (selectedEvent.totalTickets || 1)) * 100,
                      )}
                      %
                    </span>
                    <span className="text-[11px] font-black text-text-tertiary uppercase tracking-[0.2em] mt-2 bg-surface-secondary px-4 py-1.5 rounded-full border border-border-subtle shadow-sm italic">
                      Validation Rate
                    </span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-8 h-full">
          <BentoCard
            padding="lg"
            className="border-border-default shadow-2xl flex flex-col h-full min-h-[960px] overflow-hidden bg-surface-elevated"
          >
            <div className="p-8 border-b-2 border-border-subtle flex flex-col md:flex-row md:items-center justify-between gap-8 bg-surface-secondary/20">
              <div>
                ...
                <p className="text-[11px] text-text-tertiary font-black uppercase tracking-[0.2em] mt-1 italic">
                  Manual Validation & Override Terminal
                </p>
              </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 z-10 bg-surface-elevated/95 backdrop-blur-xl border-b border-border-subtle shadow-sm">
                  <tr>
                    <th className="px-10 py-8 text-[11px] font-black text-text-tertiary uppercase tracking-[0.25em]">
                      Validated Name
                    </th>
                    <th className="px-10 py-8 text-[11px] font-black text-text-tertiary uppercase tracking-[0.25em]">
                      Classification
                    ...
                    <th className="px-10 py-8 text-[11px] font-black text-text-tertiary uppercase tracking-[0.25em] text-right">
                      Administrative
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-subtle/50">
                  {filteredGuests.map((guest) => (
                    <tr
                      key={guest.id}
                      className="hover:bg-surface-secondary/30 transition-all group cursor-pointer border-l-4 border-l-transparent hover:border-l-accent-primary"
                    >
                          ...
                          <div
                            className={cn(
                              ...
                              guest.status === 'checked_in'
                                ...
                                : 'bg-surface-secondary text-text-placeholder',
                            )}
                          >
                            {guest.name?.charAt(0) || '?'}
                          </div>
                          <div className="flex flex-col">
                            <span className="font-extrabold text-text-primary text-[17px] group-hover:text-accent-primary transition-colors tabular-nums">
                              {guest.name || 'Anonymous Guest'}
                            </span>
                            <span className="text-[11px] font-black text-text-tertiary uppercase tracking-widest mt-1 font-mono opacity-60">
                              Manifest ID: #{guest.id.substring(0, 8).toUpperCase()}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-10 py-8 text-right">
                        {guest.status !== 'checked_in' ? (
                          ...
                          <div className="flex flex-col items-end opacity-40 group-hover:opacity-100 transition-opacity">
                            <span className="text-[11px] font-black text-emerald-500 uppercase tracking-widest italic">
                              Entry: 22:45 IST
                            </span>
                            <span className="text-[10px] font-bold text-text-placeholder uppercase tracking-tighter">
                              Terminal A-12
                            </span>
                          </div>
                        )}
              ...
              {filteredGuests.length === 0 && (
                <div className="py-48 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="font-black text-text-primary uppercase tracking-[0.4em] text-xl">
                  ...
                  <p className="text-text-tertiary font-bold uppercase tracking-widest text-[11px] mt-4 max-w-[400px] leading-relaxed italic">
                    No matching identities found in current manifest. Ensure scanner is synced or
                    check manual ID entry.
                  </p>
                </div>
              )}
            </div>
