      ...
      {[
        ...
        <div key={key} className="flex items-center justify-between group">
          <div className="flex items-center gap-3">
            <div
              className={`p-1.5 rounded-lg transition-colors ${pii[key] ? 'bg-[var(--v-orange-dim)] text-[var(--v-orange)]' : 'bg-surface-tertiary text-[var(--v-text-muted)]'}`}
            >
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (aRes.ok) {
        ...
        (assignments as { uid: string; staffProfileId: string | null }[]).forEach((a) => {
          map[a.uid] = a.staffProfileId;
        });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-3">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-xs font-black uppercase tracking-widest text-[var(--v-text-muted)]">
              Active Profiles
            </h3>
            <button
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-bold text-[var(--v-text-primary)] truncate">
                        {p.profileName}
                      </p>
                    ...
                    <ChevronDown
                      className={`w-4 h-4 text-[var(--v-text-muted)] transition-transform ${isSelected ? 'rotate-180' : ''}`}
                    />
                  </div>
                </motion.div>
              );
          ...
          <AnimatePresence mode="wait">
            {selectedProfile ? (
              ...
              >
                ...
                <div className="px-8 py-6 border-b border-border-subtle bg-surface-tertiary/30 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-[var(--v-text-primary)]">
                        {selectedProfile.profileName}
                      </h2>
                      <p className="text-[11px] text-[var(--v-text-muted)] uppercase tracking-[0.2em] font-black">
                        Configuration Suite
                      </p>
                    </div>
                  </div>
                    ...
                    <Section title="Sidebar Visibility">
                      <div className="grid grid-cols-2 gap-2">
                        {TAB_META.map(({ key, label, icon: Icon }) => (
                          ...
                          >
                            <Icon
                              className={`w-4 h-4 ${editTabs[key] ? 'text-[var(--v-orange)]' : 'text-[var(--v-text-muted)]'}`}
                            />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                      {ACTION_GROUPS.map((grp) => (
                        <div key={grp.group} className="space-y-3">
                          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--v-orange)]">
                            {grp.group}
                          </p>
                          <div className="space-y-1">
                            {grp.actions.map(({ key, label }) => (
                              ...
                              >
                                <span className="text-[12px] text-[var(--v-text-secondary)]">
                                  {label}
                                </span>
                                <Toggle
                                  size="sm"
                                  ...
                                  onChange={(v) =>
                                    setEditActions((prev) => ({ ...prev, [key]: v }))
                                  }
                                />
                              </div>
                            ))}
                  ...
                  <Section title={`Assigned Members (${profileStaff.length})`}>
                    <div className="flex flex-wrap gap-2">
                      {profileStaff.map((s) => (
                        <div
                          key={s.id}
                          className="flex items-center gap-2 px-3 py-1.5 bg-surface-tertiary border border-border-subtle rounded-full group"
                        >
                          ...
                          <button
                            onClick={() => s.userId && handleRemove(s.userId)}
                            className="p-1 rounded-full hover:bg-red-400/10 text-[var(--v-text-muted)] hover:text-red-400 transition-colors"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        </div>
                      ...
                      {assignableStaff.length > 0 && (
                        <div className="flex items-center gap-2 ml-auto">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-full bg-[var(--v-orange)] text-white hover:scale-110 active:scale-90 transition-transform disabled:opacity-50"
                          >
                    ...
                    {saved ? (
                      <div className="flex items-center gap-2 text-[var(--v-success)]">
                        ...
                        <span className="text-[11px] font-black uppercase tracking-widest">
                          Saved — staff will see changes on their next page load
                        </span>
                      </div>
                    ) : saveError ? (
                      <span className="text-[11px] text-red-400 font-bold">{saveError}</span>
                    ) : (
                      <span className="text-[10px] text-[var(--v-text-muted)] font-bold uppercase tracking-widest">
              ...
              <div className="h-[70vh] flex flex-col items-center justify-center text-center p-12 bg-surface-tertiary/10 rounded-[2rem] border border-dashed border-white/[0.05]">
                ...
                <h3 className="text-lg font-bold text-[var(--v-text-primary)]">
                  Select an Access Profile
                </h3>
                <p className="text-[13px] text-[var(--v-text-muted)] max-w-xs mt-2">
                  Pick a profile from the registry to view and modify its granular permissions and
                  member assignments.
                </p>
                <button
                  onClick={() => setShowCreate(true)}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--v-text-muted)]">
                      Profile Name
                    </p>
                    <input
                      type="text"
                ...
                <Section title="Sidebar Visibility">
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                    {TAB_META.map(({ key, label, icon: Icon }) => (
                      ...
                      >
                        <Icon
                          className={`w-4 h-4 ${createTabs[key] ? 'text-[var(--v-orange)]' : 'text-[var(--v-text-muted)]'}`}
                        />
                          ...
                          {grp.actions.map(({ key, label }) => (
                            ...
                            >
                              ...
                              <Toggle
                                ...
                                onChange={(v) =>
                                  setCreateActions((prev) => ({ ...prev, [key]: v }))
                                }
                              />
                            </div>
                          ))}
