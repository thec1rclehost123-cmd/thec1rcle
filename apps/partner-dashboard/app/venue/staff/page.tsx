...
const ROLE_COLORS: Record<string, string> = {
  manager:
    "text-[var(--c1rcle-orange)] bg-[var(--c1rcle-orange-glow)] border-[var(--c1rcle-orange)]/20",
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="card overflow-hidden">
            <div className="px-6 py-4 bg-[var(--surface-secondary)] border-b border-[var(--border-subtle)] flex items-center justify-between">
              ...
              <span className="text-caption text-[var(--text-placeholder)] tabular-nums">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl">
                  <p className="text-stat-sm text-[var(--text-primary)] mb-1">
                    {activeStaff.length}
                  </p>
            ...
            {selectedStaff ? (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-title-sm text-[var(--text-primary)]">
                      {selectedStaff.name}
                    </h4>
                    <p className="text-label text-[var(--c1rcle-orange)] mt-0.5">
                ...
                <div className="space-y-4">
                  <div className="p-3 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl flex items-center gap-3">
                    ...
                    <span className="text-body-sm text-[var(--text-secondary)] truncate">
                      {selectedStaff.email}
                    </span>
                  </div>
                  {selectedStaff.phone && (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(selectedStaff.permissions || {}).map(([key, val]) => (
                      <div key={key} className="flex items-center gap-2">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${val ? "bg-[var(--state-success)] shadow-[0_0_5px_var(--state-success)]" : "bg-[var(--surface-tertiary)]"}`}
                        />
                        <span
                          className={`text-caption ${val ? "text-[var(--text-secondary)]" : "text-[var(--text-placeholder)]"}`}
                        >
                          {key.split("_").join(" ")}
                        </span>
                      </div>
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-label text-[var(--text-placeholder)]">
  ...
  return (
    ...
    >
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {member.isVerified ? (
              ...
              <div
                className="w-1.5 h-1.5 rounded-full bg-[var(--state-warning)] animate-pulse"
      ...
      <div className="flex items-center gap-6">
        <span
          className={`px-3 py-1 rounded-lg border text-caption font-semibold ${ROLE_COLORS[member.role] || ROLE_COLORS.viewer}`}
        >
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="card max-w-md w-full p-8 space-y-8 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-[var(--surface-tertiary)] rounded-lg text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[var(--state-info-bg)] border border-[var(--state-info)]/20 rounded-xl p-4 flex gap-3">
            ...
            <p className="text-body-sm text-[var(--text-tertiary)]">
              A verification link will be sent to their email. They will be active after they verify
              their account.
            </p>
          </div>
