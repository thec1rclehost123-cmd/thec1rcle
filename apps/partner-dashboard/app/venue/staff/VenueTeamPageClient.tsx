...
export default function VenueTeamPageClient() {
  ...
  const fetchStaff = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/partners/venues/staff?venueId=${venueId}&isActive=all`, {
        headers,
      });
  ...
  const handleAction = async (staffId: string, action: 'suspend' | 'reactivate' | 'remove') => {
    try {
      ...
      toastSuccess(
        ...
        action === 'remove'
          ? 'The team list is now up to date.'
          : 'The staff member status has been updated.',
      );
    } catch (err: any) {
  ...
  const handleSavePermissions = async (role: EditableVenueRole) => {
    ...
    try {
      ...
      setStaff((previous) =>
        previous.map((member) => (member.id === editTarget.id ? { ...member, role } : member)),
      );
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-base leading-relaxed text-[var(--v-text-secondary)]">
              Keep venue roles simple so managers, finance, and security teams only see what they
              need.
            </p>
          </div>
