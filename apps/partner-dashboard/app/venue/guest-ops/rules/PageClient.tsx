                ...
                <div className="max-w-2xl space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-[15px] font-semibold text-[var(--text-primary)]">Guest Rules</h2>
                        </div>
                        {isDirty && canEdit && (
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="ml-auto flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[var(--accent)] text-white text-[13px] font-semibold hover:brightness-110 disabled:opacity-60"
                            >
                    ...
                    {isLocked && (
                        <div
                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] text-[var(--text-secondary)]"
                        ...
                        <>
                            ...
                            <RulesSection title="Entry Policies">
                                ...
                                {draftRules.entryCutoffEnabled && (
                                    <div className="pl-4">
                                        <label className="block text-[11px] text-[var(--text-tertiary)] mb-1">Cutoff Time</label>
                                        <input
                                            type="time"
                                            value={draftRules.entryCutoffTime ?? ""}
                                            onChange={e => updateDraft("entryCutoffTime", e.target.value)}
                                            disabled={!canEdit}
                                            className="px-3 py-1.5 text-[13px] rounded-lg border bg-[var(--bg-fill)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] disabled:opacity-50"
                                ...
                                {draftRules.reEntryEnabled && (
                                    <div className="pl-4">
                                        <label className="block text-[11px] text-[var(--text-tertiary)] mb-1">Re-Entry Window (minutes)</label>
                                        <input
                                            type="number"
                                            min={5}
                                            max={480}
                                            ...
                                            disabled={!canEdit}
                                            className="w-24 px-3 py-1.5 text-[13px] rounded-lg border bg-[var(--bg-fill)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] disabled:opacity-50"
                                            style={{ borderColor: "var(--border-subtle)" }}
                                        />
                                    </div>
                                )}
                                {draftRules.minimumAgeRule !== undefined && (
                                    <div>
                                        <label className="block text-[11px] text-[var(--text-tertiary)] mb-1">Minimum Age</label>
                                        <input
                                            type="number"
                                            min={0}
                                            max={21}
                                        ...
                                        <>
                                            <div className="flex items-center gap-3 px-4 py-3">
                                                <div className="flex-1">
                                                    <div className="text-[13px] font-medium text-[var(--text-primary)]">PII Level</div>
                                                    <div className="text-[11px] text-[var(--text-tertiary)]">What personal data is included in exports</div>
                                                </div>
                                                <select
                                                    value={draftRules.exportPiiLevel ?? "masked"}
                                                    onChange={e => updateDraft("exportPiiLevel", e.target.value as GuestRules["exportPiiLevel"])}
                                                    disabled={!canEdit}
                                                    className="px-3 py-1.5 text-[13px] rounded-lg border bg-[var(--bg-fill)] text-[var(--text-primary)] focus:outline-none disabled:opacity-50"
                                                    style={{ borderColor: "var(--border-subtle)" }}
                                                >
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-[14px] font-semibold text-[var(--text-primary)]">Allocations</h3>
                                    {allocLoading && <Loader2 size={12} className="animate-spin text-[var(--text-tertiary)]" />}
                                </div>

                                {allocations.length === 0 ? (
                                    <div
                                        className="px-4 py-8 rounded-2xl border text-center text-[13px] text-[var(--text-tertiary)]"
                                        style={{ borderColor: "var(--border-subtle)" }}
                                    >
                                    ...
                                    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "var(--border-subtle)" }}>
                                        <div
                                            className="grid text-[11px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)] px-4 py-2 border-b"
                                            style={{ gridTemplateColumns: "1fr 80px 80px 80px", background: "var(--bg-fill)", borderColor: "var(--border-subtle)" }}
                                        >
                                            <span>Name</span>
                                        ...
                                        {allocations.map((alloc, i) => (
                                            <div
                                                ...
                                                style={{ gridTemplateColumns: "1fr 80px 80px 80px", borderColor: "var(--border-subtle)", background: "var(--bg-elevated)" }}
                                            >
                                                <div>
                                                    <div className="font-medium text-[var(--text-primary)]">{alloc.name}</div>
                                                    <div className="text-[11px] text-[var(--text-tertiary)]">
                                                        {alloc.hostId ? "Host" : "Promoter"}
                                                    </div>
                                                </div>
                                                <div className="text-center font-bold text-[var(--text-primary)] tabular-nums">{alloc.allocatedCount}</div>
                                                ...
                                                <div className={cn(
                                                    ...
                                                    alloc.remaining === 0 ? "text-red-500" : "text-[var(--text-primary)]"
                                                )}>
                                                    {alloc.remaining}
                                                </div>
                                            </div>
...
function RulesSection({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div>
            <div className="text-[12px] font-semibold text-[var(--text-tertiary)] uppercase tracking-wide mb-2">{title}</div>
            <div className="rounded-2xl border overflow-hidden divide-y" style={{ borderColor: "var(--border-subtle)" }}>
                {children}
            </div>
        </div>
    );
        ...
        >
            ...
            <button
                ...
                className={cn(
                    ...
                    checked ? "bg-[var(--accent)]" : "bg-[var(--bg-fill)]"
                )}
        ...
        >
            ...
            <div className="flex items-center gap-2 shrink-0">
                <button
                    ...
                    className="w-7 h-7 rounded-lg flex items-center justify-center bg-[var(--bg-fill)] hover:bg-[var(--bg-fill)] disabled:opacity-40 transition-all border"
                    style={{ borderColor: "var(--border-subtle)" }}
                >
                    <Minus size={12} />
                </button>
                <span className="text-[14px] font-bold text-[var(--text-primary)] tabular-nums w-10 text-center">
                    {value}
                </span>
                <button
    ...
    return (
        <div className="space-y-4 animate-pulse">
            {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="rounded-2xl border overflow-hidden" style={{ borderColor: "var(--border-subtle)" }}>
                    {Array.from({ length: 3 }).map((_, j) => (
                        <div key={j} className="flex items-center justify-between px-4 py-3 border-b last:border-b-0" style={{ background: "var(--bg-elevated)", borderColor: "var(--border-subtle)" }}>
                            <div className="space-y-1.5">
                                <div className="h-3.5 w-32 bg-[var(--bg-fill)] rounded" />
                                <div className="h-2.5 w-48 bg-[var(--bg-fill)] rounded" />
                            </div>
                            <div className="w-10 h-5.5 bg-[var(--bg-fill)] rounded-full" style={{ height: "22px" }} />
                        </div>
                    ))}
                </div>
            ))}
...
function NoEventState() {
    return (
        <div className="flex flex-col items-center justify-center py-24 text-center">
            <Settings2 size={40} className="text-[var(--text-tertiary)] mb-4" />
