      ...
      {!eventId ? (
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <ScanKPI
              label="Dupe Scans"
              value={summary?.kpis?.duplicateScans ?? 0}
              color="amber"
              icon={<AlertTriangle size={14} />}
            />
            <ScanKPI
              label="Invalid Scans"
              value={summary?.kpis ? ((summary.kpis as any).invalidScans ?? 0) : 0}
              color="red"
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              {devicesLoading && (
                <Loader2 size={12} className="animate-spin text-[var(--v-text-muted)] ml-1" />
              )}
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              {streamLoading && (
                <Loader2 size={12} className="animate-spin text-[var(--v-text-muted)] ml-auto" />
              )}
            </div>
  ...
  return (
    <div
      className={cn(
        ...
        device.isOnline ? 'border-green-200 dark:border-green-800/50' : 'opacity-70',
      )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            <div className="text-center">
              <div className="text-[13px] font-bold text-green-600 dark:text-green-400 tabular-nums">
                {device.validScans}
              </div>
              <div className="text-[9px] text-[var(--v-text-muted)] uppercase">valid</div>
            </div>
            <div className="text-center">
              <div className="text-[13px] font-bold text-amber-500 tabular-nums">
                {device.duplicateScans}
              </div>
              <div className="text-[9px] text-[var(--v-text-muted)] uppercase">dupes</div>
            </div>
            <div className="text-center">
              <div className="text-[13px] font-bold text-red-500 tabular-nums">
                {device.invalidScans}
              </div>
            ...
            {device.batteryLevel !== undefined && (
              <div className="ml-auto flex items-center gap-1">
                <BatteryMedium
                  size={12}
                  className={
                    device.batteryLevel < 20 ? 'text-red-400' : 'text-[var(--v-text-muted)]'
                  }
                />
                <span className="text-[11px] text-[var(--v-text-muted)]">
                  {device.batteryLevel}%
...
function ScanRow({ scan }: { scan: ScanEvent }) {
  const resultColor =
    RESULT_COLORS[scan.result] ?? 'text-slate-500 bg-slate-50 dark:bg-slate-800/50';
  return (
    <div
