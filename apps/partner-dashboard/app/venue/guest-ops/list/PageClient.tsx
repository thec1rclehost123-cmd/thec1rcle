...
export default function GuestListPageClient() {
  ...
  const { eventId, venueId, events, summary, openExceptions, authHeaders } = useGuestOpsShellData();
    ...
    async (cursor?: string) => {
      ...
      try {
        ...
        if (filter !== 'all') {
          ...
          if (['checked_in', 'expected', 'denied', 'flagged'].includes(filter))
            params.set('status', filter);
          else if (['vip', 'comp', 'table', 'host_manual', 'promoter_manual'].includes(filter))
            params.set('guestType', filter);
        }
  ...
  const handleGuestAction = useCallback(
    async (action: string, guestId: string, body: object = {}) => {
      if (!eventId || !venueId) return;
      await fetch(
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-md">
            <Search
              size={14}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--v-text-muted)]"
            />
            <input
              type="text"
        ...
        >
          ...
          <div
            ...
            style={{
              gridTemplateColumns: '1fr 100px 100px 100px 100px 140px 120px 80px',
...
function GuestRow({
  ...
  return (
    ...
    >
      ...
      <div className="text-[12px] text-[var(--v-text-muted)] font-mono">
        {guest.maskedPhone ?? '—'}
      </div>
      <div className="text-[11px] text-[var(--v-text-muted)]">
        {guest.checkedIn && guest.checkedInAt
          ? new Date(guest.checkedInAt).toLocaleTimeString('en-IN', {
              hour: '2-digit',
              minute: '2-digit',
            })
          : '—'}
      </div>
      <div className="text-[12px] text-[var(--v-text-muted)] truncate">
        {guest.addedByName || '—'}
      </div>
  ...
  return (
    <div className="divide-y" style={{ borderColor: 'var(--v-border)' }}>
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="grid items-center px-4 py-3 animate-pulse"
          style={{ gridTemplateColumns: '1fr 100px 100px 100px 100px 140px 120px 80px' }}
        >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-[12px] text-[var(--v-text-muted)]">
        {filter === 'all'
          ? 'Add the first guest or wait for ticket orders to arrive.'
