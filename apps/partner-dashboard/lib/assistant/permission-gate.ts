...
export function resolvePermissions(ctx: Pick<AssistantPermissionContext, 'partnerType' | 'role'>): Permission[] {
...
export function canViewEntryOps(ctx: AssistantPermissionContext): GateResult {
    const hasEntry = hasPermissions(ctx, ['SCAN_ENTRY']) ||
...
export function canViewPromoterCommissions(ctx: AssistantPermissionContext): GateResult {
    ...
    return {
        ...
        refusalText: 'Commission data is restricted to owners, finance admins, and promoters for their own data.',
    };
}
...
export function enforcePartnerScope(
    ...
    if (ctx.partnerId !== resourceOwnerId) {
        return {
            ...
            refusalText: "That data belongs to a different account. I can only show you data within your authorized scope.",
        };
    }
    return { allowed: true };
}
