...
import { TOOL_REGISTRY, type ToolName } from './tools/index';
...
async function callOpenAI(apiKey: string, messages: any[], tools?: any[]): Promise<any> {
  const body: any = {
    model: OPENAI_MODEL,
    messages,
    temperature: 0.2,
...
const OPENAI_TOOL_DECLARATIONS = [
  {
    ...
    function: {
      ...
      description:
        'Get gross revenue, net revenue, ticket revenue, cover revenue, table revenue, fees, refunds, and chargebacks for a time period.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get payout status: pending amount, settled amount, next payout date, payout schedule, and recent payout history.',
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get recent ledger transactions, optionally filtered by category (refund, chargeback, ticket_sale, cover_payment, etc.) or status.',
      parameters: {
        type: 'object',
        properties: {
          ...
          status: {
            type: 'string',
            description:
              'Filter by settlement status: pending, processing, paid, failed, held, reversed',
          },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get total refunds, chargebacks, counts, and recent refund transactions for a period.',
      parameters: {
        type: 'object',
        properties: {
          period: { type: 'string', description: 'Time period', enum: ['7d', '30d', '90d', 'ytd'] },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get event performance data: revenue, tickets sold, check-ins, attendance rate. Can fetch a specific event or list recent events.',
      parameters: {
        type: 'object',
        properties: {
          eventId: {
            type: 'string',
            description: 'Specific event ID. Omit to list recent events.',
          },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get check-in count, attendance rate, no-show rate, and peak entry hour. Can be for a specific event or a period.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get QR scan velocity, slowdown detection, peak scans/minute, rejected scans. For entry operations analysis.',
      parameters: {
        type: 'object',
        properties: {
          eventId: { type: 'string', description: 'Specific event ID' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get total tickets sold, revenue, tier breakdown (GA, VIP, early-bird, etc.), and conversion rate.',
      parameters: {
        type: 'object',
        properties: {
          eventId: { type: 'string', description: 'Specific event ID' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get guestlist totals: confirmed, checked-in, declined, broken down by promoter.',
      parameters: {
        type: 'object',
        properties: {
          eventId: { type: 'string', description: 'Specific event ID' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get promoter commission data: attributed sales, earned commissions, pending/settled amounts, conversion rate. Promoters see their own data only.',
      parameters: {
        type: 'object',
        properties: {
          promoterId: {
            type: 'string',
            description: 'Specific promoter ID (optional for venue owners)',
          },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get the total amount the venue owes to hosts and promoters. Venue-only, requires VIEW_FINANCIALS permission.',
      parameters: { type: 'object', properties: {} },
    },
  },
  {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          period2: {
            type: 'string',
            description: 'Second (comparison) period',
            enum: ['7d', '30d', '90d', 'ytd'],
          },
          metric: {
            type: 'string',
            description: 'Metric to compare',
            enum: ['revenue', 'tickets', 'attendance', 'commissions'],
          },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Look up the official definition of a dashboard metric term (e.g., "net revenue", "attendance rate", "reserve balance").',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Get current dashboard alerts: payout failures, low balance warnings, chargeback alerts, KYC issues.',
      parameters: { type: 'object', properties: {} },
    },
  },
];
...
function buildDeepLinks(
  ...
  const base = `/${
    ctx.partnerType === 'venue' || ctx.partnerType === 'club'
      ? 'venue'
      : ctx.partnerType === 'promoter'
        ? 'promoter'
        : 'host'
  }`;
...
export async function orchestrate(
  ...
  try {
    ...
    const finalText =
      ...
      'Unable to compose an answer. Please check your dashboard directly.';
