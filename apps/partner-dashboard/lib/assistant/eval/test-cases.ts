...
export function evaluateCase(
  ...
  const typeMatches =
    answer.type === testCase.expectedAnswerType ||
    (testCase.expectedAnswerType === 'direct' &&
      ['direct', 'breakdown', 'diagnostic'].includes(answer.type));
...
export function computePassRates(results: EvalResult[], cases: EvalCase[]) {
  ...
  const permissionResults = results.filter((r) => permissionCases.some((c) => c.id === r.caseId));
  const adversarialResults = results.filter((r) => adversarialCases.some((c) => c.id === r.caseId));

  return {
    overall: total > 0 ? passed / total : 0,
    permission:
      permissionResults.length > 0
        ? permissionResults.filter((r) => r.passed).length / permissionResults.length
        : 0,
    adversarial:
      adversarialResults.length > 0
        ? adversarialResults.filter((r) => r.passed).length / adversarialResults.length
        : 0,
    avgLatencyMs: results.reduce((sum, r) => sum + (r.latencyMs || 0), 0) / (total || 1),
    failedCases: results
      .filter((r) => !r.passed)
      .map((r) => ({ id: r.caseId, reason: r.failureReason })),
  };
}
