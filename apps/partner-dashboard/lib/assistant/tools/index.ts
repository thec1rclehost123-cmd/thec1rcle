  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        period,
        previousPeriodGross:
          raw?.comparedTo?.grossRevenue != null ? safeNum(raw.comparedTo.grossRevenue) : undefined,
        previousPeriodNet:
          raw?.comparedTo?.netRevenue != null ? safeNum(raw.comparedTo.netRevenue) : undefined,
      },
    };
  } catch (err: any) {
    return {
...
export async function getRefundAndChargebackSummary(
  ...
  try {
    ...
    const [summary, refunds] = await Promise.allSettled([
      ...
      client.request(
        `/finance/ledger?entityId=${ctx.partnerId}&type=${entityType}&category=refund&limit=5`,
      ),
    ]);
  ...
  try {
    ...
    return {
      ...
      source: input.eventId
        ? `Event Attendance`
        : `Attendance (${periodLabel(input.period || '30d')})`,
      data: {
...
export async function getPromoterCommissionSummary(
  ...
  const targetId =
    ctx.partnerType === 'promoter' ? ctx.partnerId : input.promoterId || ctx.partnerId;
...
export async function getTimeRangeComparison(
  ...
  try {
    ...
    const [r1, r2] = await Promise.allSettled([
      client.request(
        `/finance/summary?entityId=${ctx.partnerId}&type=${entityType}&period=${input.period1}`,
      ),
      client.request(
        `/finance/summary?entityId=${ctx.partnerId}&type=${entityType}&period=${input.period2}`,
      ),
    ]);
    ...
    const getValue = (raw: any, metric: string): number => {
      ...
      switch (metric) {
        case 'revenue':
          return safeNum(raw.gross || raw.grossRevenue);
        case 'tickets':
          return safeNum(raw.ticketsSold || raw.ticketRevenue);
        case 'attendance':
          return safeNum(raw.attendanceRate);
        case 'commissions':
          return safeNum(raw.commissions);
        default:
          return 0;
      }
    };
