...
export async function createSlotRequest(data, token, actor) {
    ...
    if (!availability) {
        throw new Error(`Could not load venue availability for venue ${data.venueId} on ${data.requestedDate}`);
    }
    ...
    if (!slotAvailable) {
        throw new Error(`Time slot ${data.requestedStartTime}-${data.requestedEndTime} on ${data.requestedDate} at venue ${data.venueId} is no longer available`);
    }
