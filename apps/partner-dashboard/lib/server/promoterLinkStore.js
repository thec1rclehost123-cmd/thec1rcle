...
function buildVanityAlias(promoterId, editableSlug, fallbackCode = "") {
    ...
    const normalizedEditable = sanitizeEditableSlug(editableSlug) || String(fallbackCode || "").toLowerCase();
...
function normalizeLink(doc, event = null, promoter = null) {
    ...
    return {
        ...
        promoterHandle: normalizePromoterHandle(promoter?.handle || promoter?.username || data.promoterHandle || null) || null,
...
async function loadEventsByIds(eventIds = []) {
    ...
    const docs = await Promise.all(ids.map((eventId) => db.collection(EVENTS_COLLECTION).doc(String(eventId)).get()));
...
async function loadPromotersByIds(promoterIds = []) {
    ...
    if (ids.length === 0) return new Map();
    const docs = await Promise.all(ids.map((promoterId) => db.collection(PROMOTERS_COLLECTION).doc(String(promoterId)).get()));
    return docs.reduce((map, doc) => {
        if (doc.exists) map.set(doc.id, { id: doc.id, ...doc.data() });
        return map;
    }, new Map());
...
async function generateUniqueCode(db) {
    for (let index = 0; index < 10; index += 1) {
        ...
        const existing = await db.collection(LINKS_COLLECTION)
            .where("code", "==", code)
            .limit(1)
            .get();
...
export async function createPromoterLink(payload) {
    ...
    const existingSnap = await db.collection(LINKS_COLLECTION)
    ...
    if (existingDoc) {
        ...
        error.status = 409;
        error.data = { link: normalizeLink(existingDoc, eventMap.get(String(eventId)) || null, promoterMap.get(String(promoterId)) || null) };
        throw error;
    }
    ...
    const resolvedPromoterHandle = normalizePromoterHandle(
        promoterHandle || promoter?.handle || promoter?.username || null
    ) || null;
    ...
    const record = {
        ...
        createdAt: now,
        updatedAt: now,
        fullUrl: buildFullUrl({ eventId, eventSlug: event?.slug || eventId, code, channel, promoterHandle: resolvedPromoterHandle, vanityAlias }, event, { ...promoter, handle: resolvedPromoterHandle, username: resolvedPromoterHandle }),
    };
...
export async function getPromoterLinkByCode(code) {
    ...
    return normalizeLink(doc, eventMap.get(String(doc.data().eventId)) || null, promoterMap.get(String(doc.data().promoterId)) || null);
}
...
export async function listPromoterLinks(filters = {}) {
    ...
    if (linkId) {
        ...
        const normalized = normalizeLink(doc, eventMap.get(String(data.eventId)) || null, promoterMap.get(String(data.promoterId)) || null);
    ...
    docs = docs.sort((a, b) => getSortableTimestamp(b.data().createdAt) - getSortableTimestamp(a.data().createdAt));
    const [eventMap, promoterMap] = await Promise.all([
    ...
    return docs
        ...
        .map((doc) => normalizeLink(doc, eventMap.get(String(doc.data().eventId)) || null, promoterMap.get(String(doc.data().promoterId)) || null));
}
...
export async function updatePromoterLinkAlias(linkId, promoterId, editableSlug) {
    ...
    const conflictSnap = await db.collection(LINKS_COLLECTION)
...
export async function listPromoterCommissions(promoterId) {
    const db = getAdminDb();
    const snapshot = await db.collection(COMMISSIONS_COLLECTION)
...
export async function getPromoterLinkAnalytics(linkId) {
    ...
    if (!linkDoc.exists) {
        return { link: null, funnel: { clicks: 0, conversions: 0, revenue: 0, cleared: 0 }, commissions: [] };
    }
    ...
    const commissionsSnap = await db.collection(COMMISSIONS_COLLECTION)
    ...
    const cleared = commissions
        .filter((entry) => ["cleared", "paid", "completed"].includes(String(entry.status || "").toLowerCase()))
