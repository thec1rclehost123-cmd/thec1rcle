...
function normalizeTicketTier(tier = {}) {
  ...
  const explicitRequirement = String(
    tier.genderRequirement || tier.requiredGender || tier.gender || '',
  ).toLowerCase();
...
function normalizeTicketCollections(payload = {}, built = {}) {
  const rawTiers = Array.isArray(payload.ticketTiers)
    ...
    : Array.isArray(payload.tickets)
      ? payload.tickets
      : null;
  const normalizedTiers = Array.isArray(rawTiers) ? rawTiers.map(normalizeTicketTier) : null;
  ...
  return {
    ticketTiers:
      normalizedTiers ||
      (Array.isArray(built.ticketTiers) ? built.ticketTiers.map(normalizeTicketTier) : undefined),
    tickets:
      normalizedTiers ||
      (Array.isArray(built.tickets) ? built.tickets.map(normalizeTicketTier) : undefined),
...
export async function listEvents(
  { city, limit, sort, search, host, venueId, lifecycle, creatorId, creatorRole } = {},
  token,
) {
  const db = getAdminDb();
...
export async function createEvent(payload, token) {
  ...
  const event = {
    ...
    endDate:
      built.endDate && built.endDate.length === 10
        ? built.endDate + 'T23:59:59.999Z'
        : built.endDate,
  };

  logger.info('eventStore', 'createEvent', { eventId: event.id });
...
export async function updateEvent(eventId, payload, token) {
  logger.info('eventStore', 'updateEvent', { eventId });
...
export async function deleteEvent(eventId, token) {
  logger.info('eventStore', 'deleteEvent', { eventId });
  if (!eventId || typeof eventId !== 'string') {
...
export async function updateEventLifecycle(eventId, status, context = {}, notes = '') {
  logger.info('eventStore', 'updateEventLifecycle', { eventId, status });
  if (!eventId || typeof eventId !== 'string') {
...
export async function listEventsForPromoter({ promoterId, city, limit = 20 } = {}, token) {
  ...
  try {
    ...
    const approvedTargetIds = new Set(connectionsSnap.docs.map((d) => d.data().targetId));
    ...
    if (city) {
      ...
      events = events.filter((e) => e.city?.toLowerCase().includes(c) || e.cityKey === c);
    }
    events.sort((a, b) => {
