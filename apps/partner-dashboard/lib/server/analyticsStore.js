...
function getGender(order) {
    const value = String(order?.gender || order?.sex || "").trim().toLowerCase();
...
function aggregateOps(events, orders) {
    ...
    return {
        ...
        capacityUtilisation: totalCapacity > 0 ? (events.reduce((sum, event) => sum + event.checkIns, 0) / totalCapacity) * 100 : 0,
        capacityUtilization: totalCapacity > 0 ? (events.reduce((sum, event) => sum + event.checkIns, 0) / totalCapacity) * 100 : 0,
...
function buildRecommendations(role, overview, audience, ops, partners) {
    ...
    if (leadPartner?.revenue > 0) {
        recommendations.push({
            impact: "medium",
            title: role === "venue" ? `Double down on ${leadPartner.hostName}` : `Expand ${leadPartner.partnerName}`,
...
function buildTopEvents(events) {
    return [...events]
        ...
        .map((event) => ({
            id: event.id,
            title: event.title,
            date: event.date ? event.date.toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : "—",
...
async function buildOverviewPayload(role, events, orders, range, db) {
    ...
    const recommendations = buildRecommendations(
        role,
        { totalRevenue, totalTicketsSold, totalCheckIns, dataReady: totalRevenue > 0 || totalTicketsSold > 0 || totalCheckIns > 0 },
        audience,
        ops,
        partners
    );
...
async function buildHostAnalyticsBundle(hostId, range = "30d") {
    ...
    const approvedEvents = events.filter((event) => ["approved", "live", "scheduled", "completed"].includes(event.lifecycle)).length;
    ...
    return {
        overview: {
            ...
            approvalRate: approvedEvents + deniedEvents > 0 ? (approvedEvents / (approvedEvents + deniedEvents)) * 100 : 0,
        ...
        partners,
        reliability: {
            reliabilityScore: totalEvents > 0 ? Math.max(0, Math.min(100, 100 - (cancelledEvents / totalEvents) * 100)) : 0,
            cancelledEvents,
            approvedEvents,
            totalEvents,
