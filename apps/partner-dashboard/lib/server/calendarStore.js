...
async function fetchCalendarEntriesWithRequests(venueId, startDate, endDate) {
  ...
  const [calendarSnap, eventsSnap] = await Promise.all([
    db
      .collection('venue_calendar')
      .where('venueId', '==', venueId)
      .where('date', '>=', startDate)
      .where('date', '<=', endDate)
      .get(),
    db.collection('events').where('venueId', '==', venueId).get(),
  ]);
...
async function getNormalizedVenueCalendar(venueId, startDate, endDate) {
  const { entries, requestsById, events } = await fetchCalendarEntriesWithRequests(
    venueId,
    startDate,
    endDate,
  );
...
export async function blockDate(
  ...
  return directBlockDate(
    venueId,
    date,
    reason,
    actor || { uid: 'system', role: 'venue' },
    startTime,
    endTime,
  );
}
...
export async function respondToSlotRequest(id, action, responseData = {}, actor) {
  return directRespondToSlotRequest(
    id,
    action,
    responseData,
    actor || { uid: 'system', role: 'venue' },
