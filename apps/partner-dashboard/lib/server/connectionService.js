...
async function appendPartnershipAuditLog(connectionId, connectionType, action, actorId, actorRole, reason = "") {
...
async function guardHostVenueRequest(hostId, venueId) {
    ...
    const existingSnap = await db.collection("partnerships")
        .where("hostId", "==", hostId)
    ...
    const rejectedSnap = await db.collection("partnerships")
        .where("hostId", "==", hostId)
        .where("venueId", "==", venueId)
...
async function guardPromoterRequest(promoterId) {
    ...
    const recentSnap = await db.collection("promoter_connections")
...
export async function createRequest({
    ...
    if (
        ...
        const result = await partnershipStore.requestPartnership(hostId, venueId, hostName, venueName, token);
        appendPartnershipAuditLog(result?.id || result?.partnershipId || "", "partnership", "requested", hostId, "host").catch(() => {});
        return result;
    }
...
async function writeCommissionableLedger(connectionId, tier) {
    ...
    try {
        ...
        await db.collection("commissionable_partners").doc(`${promoterId}_${targetId}`).set({
            promoterId,
            targetId,
            targetType,
            connectionId,
...
export async function approveRequest(connectionId, type, role, partnerId, partnerName, token, tier) {
    ...
    const result = await promoterConnectionStore.approveConnectionRequest(connectionId, { uid: partnerId, name: partnerName, role }, token, tier);
    appendPartnershipAuditLog(connectionId, type, "approved", partnerId, role).catch(() => {});
...
export async function rejectRequest(connectionId, type, role, partnerId, partnerName, reason = "", token) {
    ...
    if (type === "partnership") {
        ...
        result = await promoterConnectionStore.rejectConnectionRequest(connectionId, { uid: partnerId, name: partnerName, role }, reason, token);
    }
...
export async function blockRequest(connectionId, type, role, partnerId, partnerName, reason = "", token) {
    ...
    if (type === "partnership") {
        ...
        result = await promoterConnectionStore.blockConnectionRequest(connectionId, { uid: partnerId, role, name: partnerName }, reason, token);
    }
