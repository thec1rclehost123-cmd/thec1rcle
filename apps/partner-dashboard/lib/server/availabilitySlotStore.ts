...
function slotsRef(venueId: string) {
  return getAdminDb().collection('venues').doc(venueId).collection('availability_slots');
}

// ── Read ───────────────────────────────────────────────────────────────────────

export async function getSlot(venueId: string, slotId: string): Promise<AvailabilitySlot | null> {
...
export async function getHostVisibleSlots(
  ...
  if (!isFirebaseConfigured()) {
    slots = Array.from(_slots.values()).filter((s) => s.venueId === venueId && s.date === date);
  } else {
...
export async function consumeSlotForEvent(
  ...
  return (await slotEngine.consumeSlotForEvent(
    venueId,
    eventId,
    date,
    startTime,
