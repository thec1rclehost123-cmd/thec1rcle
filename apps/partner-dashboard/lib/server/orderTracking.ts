...
export function normalizeOrderRecord(doc: FirebaseFirestore.DocumentSnapshot, source: OrderSource = "ticket") {
    const data = doc.data() || {};
    ...
    return {
        ...
        opsLog: Array.isArray(data.opsLog) ? data.opsLog.map((entry: any, index: number) => ({
