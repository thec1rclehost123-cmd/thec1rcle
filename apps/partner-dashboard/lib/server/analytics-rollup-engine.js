...
export async function computeEventRollup(eventId, options = {}) {
  const db = getAdminDb();
  const { startTime, endTime, bucketType = BUCKET_TYPES.HOURLY, forceRecompute = false } = options;
  ...
  // 4. Compute Metrics
  const metrics = {
    gross_revenue: ledger
      .filter((l) => l.state === MONEY_STATES.CAPTURED && l.amount > 0)
      .reduce((s, l) => s + l.amount, 0),
    refunds: ledger
      .filter((l) => l.state === MONEY_STATES.REFUNDED)
      .reduce((s, l) => s + Math.abs(l.amount), 0),
    entries: entitlements.filter(
      (e) =>
        e.state === ENTITLEMENT_STATES.CONSUMED &&
        e.consumedAt >= startTime &&
...
export async function backfillEventAnalytics(eventId) {
  ...
  while (current < finalEnd) {
    ...
    results.push(
      await computeEventRollup(eventId, {
