...
export async function provisionRazorpayXPayoutMethod(
...
): Promise<Omit<PayoutProvisioningResult, "verificationStatus" | "verificationMessage" | "verificationReference">> {
    ...
    logger.info("payments/razorpayx", "Provider keys present; beneficiary creation remains gated until final activation", {
        partnerType: input.partnerType,
        partnerId: input.partnerId,
...
): Promise<{ providerPayoutId: string; providerReferenceId: string; status: PayoutJobRecord["status"] }> {
    const readiness = getPayoutProviderReadiness();
