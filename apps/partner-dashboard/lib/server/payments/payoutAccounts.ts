...
export async function listPayoutMethods(partnerType: PartnerPayoutOwnerType, partnerId: string) {
  const db = getAdminDb();
  const snap = await db
    .collection(COLLECTION_BY_PARTNER[partnerType])
  ...
  return snap.docs.map((doc: any) => mapDocToResponse(doc)).filter(Boolean);
}
...
export async function createPayoutMethod(input: CreatePayoutMethodInput) {
  ...
  const payload: Omit<PayoutMethodRecord, 'id'> & Record<string, unknown> = {
    ...
    accountHolderName:
      input.paymentType === 'bank_account' ? input.accountHolderName || null : null,
