...
function normalizeCommissionStatus(rawStatus: any, eventData?: Record<string, any> | null) {
  ...
  if (
    eventData &&
    isCompletedEventStatus(eventData.lifecycle || eventData.status || eventData.eventStatus)
  ) {
    return 'cleared';
  }
  return 'pending';
...
async function getLedgerCommissionRows(promoterId: string) {
  ...
  const orderMap = await getDocsByIds(
    'orders',
    ledgerEntries.map((entry) => String(entry.orderId || '')),
  );
  const eventMap = await getDocsByIds(
    'events',
    ledgerEntries.map((entry) =>
      String(entry.eventId || orderMap.get(String(entry.orderId || ''))?.eventId || ''),
    ),
  );
  ...
  return ledgerEntries
    .map((entry) => {
      ...
      const eventId =
        String(entry.eventId || orderMap.get(String(entry.orderId || ''))?.eventId || '') || null;
      ...
      const commissionRate = normalizeCommissionRate(
        entry.commissionRate || order?.promoterAttribution?.commissionRate,
      );
      const amount = Number(
        entry.commissionAmount || order?.promoterAttribution?.commissionAmount || 0,
      );
      ...
      return {
        ...
        buyerName: maskName(
          String(
            order?.guestName || order?.buyerName || order?.userName || entry.buyerName || 'Guest',
          ),
        ),
        amount,
        revenue,
...
async function getAssignmentFallbackRows(promoterId: string) {
  ...
  return assignmentSnap.docs.map((doc: any) => {
    ...
    return {
      ...
      settledAt:
        status === 'cleared' || status === 'paid'
          ? toIsoDate(data.settledAt || data.updatedAt)
          : null,
    } satisfies PromoterCommissionRow;
  });
}

export async function getPromoterFinanceSnapshot(
  promoterId: string,
): Promise<PromoterFinanceSnapshot> {
  ...
  const payouts: PromoterPayoutRow[] = payoutDocs
    ...
    .sort(
      (a, b) =>
        new Date(b.requestedAt || b.completedAt || 0).getTime() -
        new Date(a.requestedAt || a.completedAt || 0).getTime(),
    );

  const commissionDetails = (ledgerRows.length > 0 ? ledgerRows : fallbackAssignmentRows).slice(
    0,
    200,
  );

  const totals = commissionDetails.reduce(
    (acc: any, row: any) => {
  ...
  const bankAccounts = payouts
    ...
    .reduce((accounts: PromoterBankAccountRow[], payout) => {
      const exists = accounts.find(
        (account) =>
          account.last4 === (payout.last4 || '****') &&
          account.bankName === (payout.bankName || payout.paymentMethod),
      );
