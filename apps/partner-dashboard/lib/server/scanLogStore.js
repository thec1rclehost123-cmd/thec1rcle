...
export async function getGuestOpsSummary(eventId, venueId) {
  ...
  const overrideSnap = await db
    .collection('guest_override_logs')
    .doc(eventId)
    .collection('items')
...
export async function getExceptions(eventId, { status = 'open', type } = {}) {
  ...
  const openCount =
    status === 'all' ? exceptions.filter((e) => e.status === 'open').length : exceptions.length;
...
export async function resolveException(eventId, exceptionId, actor, { action, reason, notes }) {
  ...
  const exc = doc.data();
  if (exc.isLocked)
    throw { code: 'EVENT_LOCKED', message: 'Event is locked — exceptions cannot be modified' };
  if (exc.status !== 'open')
    throw { code: 'ALREADY_RESOLVED', message: 'Exception is already resolved' };

  const now = new Date().toISOString();
  await ref.update({
    status:
      action === 'escalated' ? 'escalated' : action === 'dismissed' ? 'dismissed' : 'resolved',
    resolution: {
  ...
  await writeOverrideLog(db, {
    ...
    reason,
    metadata: { exceptionType: exc.type, resolutionAction: action },
  });
}
...
export async function upsertHostAllocation(eventId, hostId, { allocatedCount, notes }, actor) {
  ...
  if (allocatedCount < (existing.usedCount || 0)) {
    throw {
      code: 'BELOW_USED_COUNT',
      message: `Cannot set allocation below already-used count (${existing.usedCount})`,
    };
  }
 ...
 */
export async function upsertPromoterAllocation(
  eventId,
  promoterId,
  { allocatedCount, notes },
  actor,
) {
  const db = getAdminDb();
  const ref = db
