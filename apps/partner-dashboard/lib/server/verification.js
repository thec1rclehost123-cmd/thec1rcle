...
export async function sendEmailOtp(email) {
  ...
  if (existing.exists) {
    ...
    if (Date.now() - lastSent.getTime() < cooldownMs) {
      throw new Error(
        `Please wait ${SECURITY_CONFIG.OTP_COOLDOWN_SECONDS}s before requesting another code.`,
      );
    }
  }
...
export async function sendSmsOtp(phone) {
  ...
  if (!MSG91_AUTH_KEY || !MSG91_TEMPLATE_ID) {
    ...
    await db
      .collection('otps')
      .doc(`signup_phone_${phone}`)
      .set({ code, expiresAt, lastSent: new Date(), attempts: 0 });
    console.warn(`\n=== DEV SMS OTP ===\nTo: ${phone}\nCode: ${code}\n===================\n`);
    return true;
  }
...
export async function checkAndConsumeOtpCompletions(email, phone) {
  ...
  const emailDoc = await db.collection('otp_completions').doc(recipientKey('email', email)).get();
  ...
  const phoneDoc = await db.collection('otp_completions').doc(recipientKey('phone', phone)).get();
