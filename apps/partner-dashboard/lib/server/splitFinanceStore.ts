...
export async function getPaymentsData(venueId: string, token: string): Promise<PaymentsPageData> {
...
async function getPartnerPayoutsData(
  ...
  try {
    ...
    const pending = pendingSnap.docs.map(
      (d: any) => ({ id: d.id, ...d.data() }) as PartnerSettlement,
    );
