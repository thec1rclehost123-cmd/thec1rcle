...
export async function writeLoginSession(
  ...
  const db = getAdminDb();
  await db
    .collection('host_login_sessions')
    .doc(hostId)
    .collection('sessions')
...
export async function getLoginSessions(hostId: string, userId: string): Promise<LoginSession[]> {
  const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();

  if (!isFirebaseConfigured()) {
    const all = _sessions.get(hostId) ?? [];
    return all.filter((s) => s.userId === userId && !s.isRevoked && s.lastActiveAt >= sevenDaysAgo);
  }
...
export async function revokeLoginSession(hostId: string, sessionId: string): Promise<void> {
  const now = new Date().toISOString();
...
export async function deactivateOrganization(hostId: string, actor: ActorContext): Promise<void> {
