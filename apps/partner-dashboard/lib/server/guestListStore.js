...
export async function listGuests(
  { eventId, cursor, limit = 50, filter = {}, sort = { field: 'addedAt', dir: 'desc' } },
  actorRole,
) {
  const db = getAdminDb();
  let query = db
...
export async function searchGuests({ eventId, q, field }, actorRole) {
  ...
  const results = snap.docs.map((doc) =>
    applyMasking({ guestId: doc.id, ...doc.data() }, actorRole),
  );
...
export async function addGuest(eventId, venueId, body, actor, rules) {
  ...
  if (normalizedPhone) {
    ...
    if (!dupSnap.empty) {
      throw {
        ...
        message: 'A guest with this phone number is already on the list',
        existingGuestId: dupSnap.docs[0].id,
      };
    }
  }
  ...
  const isCappedAdd = ['venue_manual', 'venue_comp', 'venue_vip', 'staff_override'].includes(
    guestType,
  );
  if (isCappedAdd && rules) {
    ...
    if (usedSnap.size >= cap) {
      throw {
        code: 'MANUAL_ADD_CAP_EXCEEDED',
        message: `Manual add cap (${cap}) has been reached`,
        used: usedSnap.size,
        cap,
      };
    }
  ...
  if (guestType === 'host_manual' && hostId && rules) {
    const allocSnap = await db
      .collection('event_guest_allocations')
      .doc(eventId)
      .collection('hosts')
      .doc(hostId)
      .get();
    if (allocSnap.exists) {
      ...
      if (alloc.remaining <= 0) {
        throw {
          code: 'HOST_ALLOCATION_EXCEEDED',
          message: 'Host allocation cap has been reached',
          hostId,
          used: alloc.usedCount,
          allocated: alloc.allocatedCount,
        };
      }
    }
  }
  ...
  if (guestType === 'promoter_manual' && promoterId && rules) {
    ...
    if (allocSnap.exists) {
      ...
      if (alloc.remaining <= 0) {
        throw {
          code: 'PROMOTER_ALLOCATION_EXCEEDED',
          message: 'Promoter allocation cap has been reached',
          promoterId,
          used: alloc.usedCount,
          allocated: alloc.allocatedCount,
        };
  ...
  if (guestType === 'host_manual' && hostId) {
    ...
    await allocRef.set(
      {
        usedCount: (await allocRef.get()).data()?.usedCount + 1 || 1,
        remaining: Math.max(0, ((await allocRef.get()).data()?.remaining || 0) - 1),
        updatedAt: now,
      },
      { merge: true },
    );
  ...
  if (needsApproval) {
    await db
      .collection('event_guest_exceptions')
      .doc(eventId)
      .collection('items')
      .add({
        eventId,
...
export async function checkInGuest(eventId, guestId, actor, { reason, gate } = {}) {
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    if (guest.status === 'denied')
      throw { code: 'GUEST_DENIED', message: 'Guest has been denied entry' };

    if (guest.checkedIn) {
      return {
        alreadyCheckedIn: true,
        checkedInAt: guest.checkedInAt,
        checkInSource: guest.checkInSource,
      };
    }
...
export async function denyGuest(eventId, guestId, actor, { reason, notes } = {}) {
  ...
  const guest = doc.data();
  if (guest.isLocked) throw { code: 'EVENT_LOCKED', message: 'Event is locked' };
  if (guest.checkedIn && actor.role !== 'OWNER')
    throw {
      code: 'ALREADY_CHECKED_IN',
      message: 'Guest has already checked in — only OWNER can deny after check-in',
    };
  ...
  await writeOverrideLog(db, {
    ...
    actorRole: actor.role,
    targetGuestId: guestId,
    targetGuestName: guest.displayName || '',
    reason: reason || '',
    metadata: { notes },
  });
 ...
 */
export async function flagGuest(
  eventId,
  guestId,
  actor,
  { reason, severity = 'medium', notes } = {},
) {
  const db = getAdminDb();
  const guestRef = db.collection('guest_lists').doc(eventId).collection('guests').doc(guestId);
  const doc = await guestRef.get();
