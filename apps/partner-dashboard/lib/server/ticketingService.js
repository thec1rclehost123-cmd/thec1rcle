...
export function calculateOrderPricing(items, event, options = {}) {
  ...
  if (
    ...
    event.promoterSettings?.buyerDiscountsEnabled
  ) {
  ...
  if (discountedSubtotal > 0) {
    result.fees.platformFee =
      Math.round(((discountedSubtotal * PLATFORM_FEES.platformFeePercent) / 100) * 100) / 100;
    result.fees.paymentFee =
      Math.round(((discountedSubtotal * PLATFORM_FEES.paymentFeePercent) / 100) * 100) / 100;
...
export function validatePricingMargins(pricing, commissionAmount) {
  ...
  if (netRevenue > 0 && netRevenue < MINIMUM_PAYOUT_FLOOR) {
    warnings.push(
      `Net revenue (₹${netRevenue.toFixed(2)}) is below minimum floor (₹${MINIMUM_PAYOUT_FLOOR})`,
    );
  }
