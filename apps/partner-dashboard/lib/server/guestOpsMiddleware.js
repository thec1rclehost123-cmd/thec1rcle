 ...
 */
export async function requireGuestOpsAccess(
  request,
  venueId,
  eventId,
  requiredPermissions = ['VIEW_GUESTLIST'],
) {
  ...
  if (eventId) {
    ...
    if (!event)
      return buildAuthError(request, 404, 'Event not found or does not belong to this venue');
  }
 ...
 */
export async function writeOverrideLog(
  db,
  {
    eventId,
    venueId,
