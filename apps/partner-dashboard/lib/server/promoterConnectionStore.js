...
export async function createConnectionRequest(
  { promoterId, promoterName, promoterEmail, targetId, targetType, targetName, message = '' },
  token,
) {
...
export async function discoverPartners(
  { type = 'host', city = '', search = '', limit = 20 },
  token,
) {
  const client = getApiClient(token);
  try {
...
export async function isConnected(promoterId, targetId, token) {
  ...
  try {
    const result = await client.request(
      `/promoter-connections/status?promoterId=${promoterId}&targetId=${targetId}`,
    );
...
export async function getConnectionStatus(promoterId, targetId, targetType, token) {
  ...
  try {
    return await client.request(
      `/promoter-connections/status?promoterId=${promoterId}&targetId=${targetId}&targetType=${targetType}`,
    );
  } catch (error) {
