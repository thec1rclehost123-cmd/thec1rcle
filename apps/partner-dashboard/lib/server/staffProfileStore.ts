...
import { DEFAULT_PII_POLICY, OWNER_PII_POLICY, ROLE_DEFAULT_TABS } from '../types/staffProfile';
...
export async function createStaffProfile(
  ...
  const basePii: PIIPolicy =
    payload.baseRole === 'OWNER' ? OWNER_PII_POLICY : { ...DEFAULT_PII_POLICY };
  ...
  const profile: StaffProfile = {
    ...
    tabVisibility: payload.tabVisibility ?? (ROLE_DEFAULT_TABS[payload.baseRole] as any),
  ...
  if (token) {
    try {
      ...
      const res = (await client.request(
        `/venue/staff-profiles/${profileId}?venueId=${venueId}`,
      )) as StaffProfile | null;
      if (res) return res;
    } catch (error: any) {
      console.warn(
        '[StaffProfileStore] API Gateway getStaffProfile failed, falling back to direct DB read:',
        error.message,
      );
    }
  }
...
export async function listStaffProfiles(venueId: string, token?: string): Promise<StaffProfile[]> {
  if (token) {
    try {
      const client = getApiClient(token);
      const res = (await client.request(`/venue/staff-profiles?venueId=${venueId}`)) as {
      ...
      console.warn(
        '[StaffProfileStore] API Gateway listStaffProfiles failed, falling back to direct DB read:',
        error.message,
      );
    }
  }
...
export async function updateStaffProfile(
  ...
  await profilesRef(venueId)
    .doc(profileId)
    .update({ ...updates, updatedBy: actor.uid, updatedAt: now });
  await writeAudit({
    venueId,
    profileId,
    action: 'updated',
...
export async function resolveEffectiveProfile(
  ...
  if (token) {
    try {
      const client = getApiClient(token);
      const res = (await client.request(
        `/venue/staff-profiles/resolve?venueId=${venueId}&membershipId=${membershipId}`,
      )) as {
        baseRole: VenueRole;
        tabVisibility: Partial<Record<VenueTab, boolean>>;
        actionPermissions: Partial<Record<StaffAction, boolean>>;
      ...
      console.warn(
        '[StaffProfileStore] API Gateway resolveEffectiveProfile failed, falling back to direct DB read:',
        error.message,
      );
    }
  }
