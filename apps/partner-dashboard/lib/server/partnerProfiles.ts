...
async function resolveByQuery(id: string) {
  ...
  for (const query of fallbackQueries) {
    const snapshot = await db
      .collection(COLLECTIONS[query.type])
      .where(query.field, '==', id)
      .limit(1)
      .get();
    if (!snapshot.empty) {
...
async function fetchEventsForPartner(partnerId: string, partnerType: PartnerEntityType) {
  ...
  if (partnerType === 'promoter') {
    ...
    const eventIds = Array.from(
      new Set(assignments.map((item: any) => item.eventId).filter(Boolean)),
    );
...
export async function getConnectionForViewer(params: {
  ...
  if (
    ...
    const snapshot = await db
      ...
      .get();
    if (!snapshot.empty)
      return { id: snapshot.docs[0].id, ...snapshot.docs[0].data(), type: 'partnership' };
  }
  ...
  if (promoterId && targetId) {
    const snapshot = await db
      ...
      .get();
    if (!snapshot.empty)
      return { id: snapshot.docs[0].id, ...snapshot.docs[0].data(), type: 'promoter_connection' };
  }
...
export async function getPartnerProfileSummary(id: string): Promise<PartnerProfileSummary | null> {
  ...
  const createdAt = toDate(
    doc.createdAt || doc.submittedAt || userData.createdAt || onboarding?.submittedAt,
  );
  const contactPoints = [
