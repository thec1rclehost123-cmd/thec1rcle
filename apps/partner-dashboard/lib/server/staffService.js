...
const ROLE_PRESETS = {
  ...
  door_staff: {
    ...
    permissions: ['scan:read', 'scan:create', 'guestlist:read'],
  },
  supervisor: {
    name: 'Supervisor',
  ...
  FINANCE_ADMIN: {
    ...
    permissions: ['calendar:read', 'events:read'],
  },
};
...
export async function revokeDevice(deviceRecordId, revokedBy) {
  ...
  const db = getAdminDb();
  await db
    .collection(BOUND_DEVICES_COLLECTION)
    .doc(deviceRecordId)
    .update({
      status: 'revoked',
...
async function getStaffByEmail(venueId, email) {
  ...
  const db = getAdminDb();
  const snapshot = await db
    .collection(CLUB_STAFF_COLLECTION)
...
export async function getVenueStaff(venueId, options = {}) {
  ...
  if (!isFirebaseConfigured()) {
    let staff = Array.from(fallbackStaff.values()).filter((s) => s.venueId === venueId);
  ...
  const db = getAdminDb();
  let query = db.collection(CLUB_STAFF_COLLECTION).where('venueId', '==', venueId);
...
export async function getVenueDevices(venueId, options = {}) {
  ...
  if (!isFirebaseConfigured()) {
    let devices = Array.from(fallbackDevices.values()).filter((d) => d.venueId === venueId);
  ...
  const db = getAdminDb();
  let query = db.collection(BOUND_DEVICES_COLLECTION).where('venueId', '==', venueId);
