...
export function normalizeAnalyticsV2(raw: Record<string, unknown> | null | undefined): AnalyticsV2 {
  ...
  const executive: ExecutiveOverview = {
    grossRevenue: buildMetric(
      totalRevenue,
      n(prior.totalRevenue),
      fmtCurrency(totalRevenue),
      true,
      'Total revenue before refunds and fees',
      revSparkline,
    ),
    netRevenue: buildMetric(
      totalNetPayable,
      n(prior.totalNetPayable),
      fmtCurrency(totalNetPayable),
      true,
      'Revenue after refunds, discounts, and platform fees',
      revSparkline,
    ),
    ...
    guestlistSignups: buildMetric(
      n(r.guestlistSignups),
      n(prior.guestlistSignups),
      fmtNum(n(r.guestlistSignups)),
      true,
    ...
    occupancyRate: buildMetric(
      n(r.occupancyRate),
      n(prior.occupancyRate),
      fmtPct(n(r.occupancyRate)),
      true,
      'Checked-in guests ÷ venue capacity',
    ),
    sellThroughRate: buildMetric(
      n(r.sellThroughRate),
      n(prior.sellThroughRate),
      fmtPct(n(r.sellThroughRate)),
      true,
      'Tickets sold ÷ total available tickets',
    ),
    avgTicketPrice: buildMetric(
      n(r.avgTicketPrice),
      n(prior.avgTicketPrice),
      fmtCurrency(n(r.avgTicketPrice)),
      true,
    ...
    noShowRate: buildMetric(
      n(r.noShowRate),
      n(prior.noShowRate),
      fmtPct(n(r.noShowRate)),
      false,
      'Purchased but did not arrive ÷ total purchases',
    ),
    repeatGuestRate: buildMetric(
      n((r as any).metrics?.repeatRate ?? r.repeatGuestRate),
      0,
      fmtPct(n((r as any).metrics?.repeatRate ?? r.repeatGuestRate)),
      true,
      'Guests who attended a previous event',
    ),
    firstTimeGuestRate: buildMetric(
      n(r.firstTimeGuestRate),
      0,
      fmtPct(n(r.firstTimeGuestRate)),
      true,
      'Guests attending for the first time',
    ),
    promoterDrivenSales: buildMetric(
      n(r.promoterDrivenSales),
      0,
      fmtCurrency(n(r.promoterDrivenSales)),
      true,
    ...
    pendingPayout: buildMetric(
      n(r.pendingPayout),
      0,
      fmtCurrency(n(r.pendingPayout)),
      true,
      'Payout amount awaiting disbursement',
    ),
    completedPayout: buildMetric(
      n(r.completedPayout ?? totalNetPayable),
      0,
      fmtCurrency(n(r.completedPayout ?? totalNetPayable)),
      true,
    ...
    profitEstimate: buildMetric(
      n(r.profitEstimate),
      0,
      fmtCurrency(n(r.profitEstimate)),
      true,
      'Net revenue minus estimated payouts and costs',
    ),
    contributionMargin: buildMetric(
      n(r.contributionMargin),
      0,
      fmtPct(n(r.contributionMargin)),
      true,
  ...
  const revenueIntelligence: RevenueIntelligence = {
    ...
    series: {
      ...
      revenueByHour: arr(
        r.revenueByHour,
        makeHourlySeries().map((h) => ({ ...h, revenue: 0 })),
      ),
  ...
  // ── Conversion funnel ────────────────────────────────────────────────────
  const rawFunnel =
    Array.isArray(r.funnel) && (r.funnel as any[]).length > 0 ? (r.funnel as any[]) : null;
  ...
  const conversionFunnel: ConversionFunnel = {
    stages: rawFunnel
      ? rawFunnel.map((s: any, i: number, arr: any[]) => ({
          ...
          count: n(s.count),
          dropPct:
            i > 0 && n(arr[i - 1].count) > 0
              ? ((n(arr[i - 1].count) - n(s.count)) / n(arr[i - 1].count)) * 100
              : 0,
          conversionFromTop: n(arr[0].count) > 0 ? (n(s.count) / n(arr[0].count)) * 100 : 0,
    ...
    purchaseToArrival: checkins > 0 && ticketsSold > 0 ? (checkins / ticketsSold) * 100 : 0,
  ...
  const audience: AudienceIntelligence = {
    ...
    arrivalByHour: arr(
      r.entryCurve ?? r.arrivalTimeline,
      makeHourlySeries().map((h) => ({
        hour: h.hour,
        count: 0,
        pct: 0,
      })),
  ...
  // ── Event scorecards ─────────────────────────────────────────────────────
  const eventScorecards: EventScorecard[] =
    Array.isArray(r.topEvents) && (r.topEvents as any[]).length > 0
  ...
  // ── Promoters ────────────────────────────────────────────────────────────
  const promoters: PromoterScore[] =
    Array.isArray(r.promoters) && (r.promoters as any[]).length > 0
  ...
  const doorOps: DoorOps = {
    ...
    scanIntegrityScore:
      totalScans > 0 ? (successfulScans / totalScans) * 100 : n(r.scanIntegrityScore),
  ...
  const finance: FinancialSummary = {
    ...
    refundAdjustedMargin:
      totalRevenue > 0 ? ((totalRevenue - n(r.refundAmount)) / totalRevenue) * 100 : 0,
  ...
  const retention: RetentionIntelligence = {
    ...
    avgLTV: n(r.avgLTV),
    topRetentionChannels: Array.isArray(r.topRetentionChannels)
      ? (r.topRetentionChannels as string[])
      : [],
