...
export function HostVenueCalendar() {
        ...
        async function fetchCalendar() {
            ...
            try {
                ...
                const res = await authedFetch(
                    `/api/venues/${venueId}/calendar?hostId=${hostId}&startDate=${startDate}&endDate=${endDate}`
                );
    ...
    useEffect(() => {
        if (!selectedDate || !venueId || !hostId) {
        ...
        async function fetchAvailability() {
            ...
            try {
                const res = await authedFetch(
                    `/api/venues/${venueId}/calendar?hostId=${hostId}&startDate=${selectedDate}&endDate=${selectedDate}`
                );
                const data = await res.json();
                const availableDays = data.calendar || data.days || [];
                setDateAvailability(availableDays[0] || { slots: [] });
            } catch (err) {
        ...
        fetchAvailability();
    }, [venueId, hostId, selectedDate, calendar, authedFetch]);
            ...
            <div className="min-h-[60vh] flex items-center justify-center">
                <div className="text-center">
                    ...
                    <button
                        ...
                        className="mt-4 btn btn-primary flex text-sm items-center gap-2 font-bold px-6 py-3 rounded-xl bg-[var(--c1rcle-orange)] hover:bg-[var(--c1rcle-orange-dim)] text-white transition-all transform hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                    >
                        Go Back
                    </button>
                </div>
    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-8">
                <button
                    onClick={() => router.push("/host/create/select-venue")}
                    className="flex items-center gap-2 text-text-tertiary hover:text-text-primary transition-colors mb-4 cursor-pointer"
                >
                    <ArrowLeft className="w-4 h-4" />
                    <span className="text-[11px] font-bold uppercase tracking-widest">Back to Venues</span>
                </button>
                <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-500/20 to-red-600/10 flex items-center justify-center border border-white/10">
                        <Building2 className="w-7 h-7 text-[var(--c1rcle-orange)]" />
                    </div>
                    <div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                {/* Calendar Section */}
                <div className="flex-[1.5] bg-[var(--v-card)] rounded-[2rem] border border-[var(--v-border)] p-8">
                    {/* Month Navigation */}
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center h-64">
                            <Loader2 className="w-8 h-8 text-[var(--c1rcle-orange)] animate-spin" />
                        </div>
                    ) : (
                            ...
                            {calendarGrid.map((day, idx) => (
                                <div
                                    ...
                                    className={`
                                        ...
                                        ${selectedDate === day.date ? "ring-2 ring-[var(--c1rcle-orange)] ring-offset-2 ring-offset-[var(--v-card)]" : ""}
                                    `}
                                >
                                    {!day.empty && (
                                        <>
                ...
                <div className="w-full lg:w-96 bg-[var(--v-card)] rounded-[2rem] border border-[var(--v-border)] p-8 lg:sticky lg:top-4 self-start">
