    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        <div className="flex items-start justify-between relative z-10">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:bg-white/10 text-white/30 hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
            ...
            >
              <div className="px-4 py-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black tabular-nums mt-0.5 text-red-300/70">
          ...
          <div className="px-5 pt-5 pb-4 space-y-5">
            ...
            <button
              ...
              style={{
                ...
                borderColor: timeConfirmed ? 'rgba(52,211,153,0.2)' : 'rgba(244,74,34,0.2)',
              }}
            >
              ...
              <span className="text-[12px] font-black tabular-nums px-3 py-1 rounded-full bg-white/5 border border-white/10 text-white/80">
      ...
      <div className="flex-shrink-0 p-5 mt-auto border-t border-white/5 bg-black/20">
        ...
        <button
          onClick={onClose}
          className="w-full mt-3 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest text-white/40 border border-white/5 hover:bg-white/5 transition-colors"
        >
          Close
        </button>
      </div>
      ...
      {timeModalOpen && (
        ...
        >
          <div
            className="w-full max-w-2xl bg-[#141418] rounded-3xl border border-white/10 overflow-hidden shadow-2xl"
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-white/30 uppercase font-black">
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex p-1 bg-white/5 rounded-xl border border-white/5">
            {['PM', 'AM'].map((p: any) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${period === p ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-white/30'}`}
              >
                {p}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center disabled:opacity-20"
            >
              <ChevronLeft className="w-4 h-4 text-white/60" />
            </button>
            <span className="text-[17px] font-black tabular-nums min-w-[90px] text-center text-orange-500">
              {fmt12(value)}
            </span>
            <button
              type="button"
      ...
      <div ref={scrollRef} className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
        {visibleTimes.map((t) => {
          const isSel = t === value,
            isDisabled = disabledTimes.has(t);
          return (
            <button
              key={t}
              data-selected={isSel}
              ...
              className={`flex-shrink-0 px-5 py-3 rounded-2xl text-[13px] font-black tabular-nums transition-all ${isSel ? 'bg-orange-600 text-white shadow-xl shadow-orange-600/20 scale-105' : 'bg-white/5 text-white/40 border border-white/5'} disabled:opacity-10`}
            >
              {fmt12(t)}
            </button>
          );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        ...
        <div
          className="flex-1 relative rounded-2xl bg-[#141418] border border-white/10 overflow-hidden"
          style={{ height: 340 }}
        >
          {TIMELINE_HOURS.slice(0, -1).map((_, i) => (
            <div
              ...
              style={{
                top: `${(TIMELINE_HOURS[i].mins / TOTAL_MINS) * 100}%`,
                height: `${((TIMELINE_HOURS[i + 1].mins - TIMELINE_HOURS[i].mins) / TOTAL_MINS) * 100}%`,
          ...
          {isActive && (
            <div className="absolute inset-x-0 z-30 pointer-events-none" style={{ top: nowPct }}>
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 px-2 py-1 rounded-full bg-orange-600 text-white text-[8px] font-black tracking-widest shadow-xl shadow-orange-600/30 flex items-center gap-1">
                <span className="w-1 h-1 rounded-full bg-white animate-pulse" /> {nowTimeStr}
              </div>
            </div>
          )}
          {isBlocked &&
            (() => {
              ...
              return (
                <div
                  className="absolute inset-x-0 z-10 flex flex-col items-center justify-center gap-1 bg-red-600/20 border-y-2 border-red-600/50"
                  style={{
                    top: pct(sMin),
                    height: `${((eMin - sMin) / TOTAL_MINS) * 100}%`,
                    backgroundImage:
                      'repeating-linear-gradient(135deg, rgba(239,68,68,0.1) 0px, rgba(239,68,68,0.1) 1px, transparent 1px, transparent 10px)',
                  }}
                >
                  <Lock className="w-3 h-3 text-red-300" />
                  <span className="text-[8px] font-black uppercase text-red-300">Blocked</span>
                </div>
              );
            })()}
          {events.map((e: any, i: number) => {
            ...
            return (
              <div
                key={e.id || i}
                className="absolute left-1 right-1 z-20 rounded-xl bg-teal-600/10 border border-teal-600/30"
                style={{
                  top: pct(sMin),
                  height: `${Math.max(5, ((eMin - sMin) / TOTAL_MINS) * 100)}%`,
                }}
              >
                <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-teal-400" />
                <p className="pl-3 py-2 text-[9px] font-black text-white uppercase truncate">
                  {e.title || 'Reserved'}
                </p>
              </div>
            );
