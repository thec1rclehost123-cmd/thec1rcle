...
export default function MenuManager({ venueId, menuItems, onRefresh }: MenuManagerProps) {
  ...
  return (
    <div className="space-y-6">
          ...
          {uploading ? (
            ...
            <>
              ...
              <p className="text-sm font-medium text-text-primary">
                Drop menu images here or click to upload
              </p>
      ...
      {/* Menu Pages List */}
      {menuItems.length > 0 && (
        <Reorder.Group axis="y" values={menuItems} onReorder={handleReorder} className="space-y-3">
          {menuItems.map((item, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-text-primary">Page {idx + 1}</p>
                <p className="text-xs text-text-tertiary truncate">{item.title || 'Menu page'}</p>
              </div>
                ...
                {menuItems.map((_, idx) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      idx === currentIndex ? 'bg-surface-elevated w-6' : 'bg-surface-elevated/40'
                    }`}
                  />
                ))}
              </div>
