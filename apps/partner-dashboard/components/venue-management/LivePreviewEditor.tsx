        ...
        <div className="space-y-8">
            ...
            <div className={`relative bg-[#0A0A0A] rounded-3xl overflow-hidden border border-white/10 shadow-2xl transition-all ${isEditing ? 'ring-2 ring-emerald-500/50' : ''}`}>
                    ...
                    <div className="relative aspect-[16/10] w-full overflow-hidden group">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 z-30 px-6 pb-6 pointer-events-none">
                            <div className="flex items-end gap-4">
                                <div onClick={(e) => { e.stopPropagation(); logoInputRef.current?.click(); }} className="relative w-20 h-20 rounded-2xl overflow-hidden border-4 border-[#0A0A0A] shadow-2xl cursor-pointer group pointer-events-auto bg-slate-800">
                                    {logo ? <img src={logo} className="w-full h-full object-cover" alt="" onError={(e) => console.error(`[LivePreviewEditor] Logo image failed to load:`, logo)} /> : <div className="w-full h-full flex items-center justify-center text-white/20"><Upload className="w-6 h-6" /></div>}
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">{uploadingField === "logo" ? <Loader2 className="w-5 h-5 text-white animate-spin" /> : <Upload className="w-5 h-5 text-white" />}</div>
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-2">
                                        {isEditing ? <input value={editValues.venueType} onChange={(e) => setEditValues(v => ({ ...v, venueType: e.target.value }))} className="px-3 py-1 bg-white/10 border border-white/20 rounded-full text-[10px] font-black uppercase tracking-widest text-white w-24 text-center pointer-events-auto" /> : <span className="px-3 py-1 bg-white/10 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-white/80">{venue.venueType || venue.category || "Venue"}</span>}
                                    ...
                                    {isEditing ? <input value={editValues.name} onChange={(e) => setEditValues(v => ({ ...v, name: e.target.value }))} className="text-3xl font-black uppercase tracking-tighter text-white bg-transparent border-b-2 border-white/30 focus:border-white outline-none w-full pointer-events-auto" /> : <h1 className="text-3xl font-black uppercase tracking-tighter text-white leading-[0.9] drop-shadow-lg">{venue.name}</h1>}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="px-6 py-4 border-b border-white/10 flex items-center gap-6">
                    <div className="flex items-center gap-2 text-white/60"><Users className="w-4 h-4" /><span className="text-sm font-bold">{venue.followers || 0} followers</span></div>
                    <div className="flex items-center gap-2 text-white/60"><Calendar className="w-4 h-4" /><span className="text-sm font-bold">{venue.eventsCount || 0} events</span></div>
                    <div className="flex items-center gap-2 text-white/60"><MapPin className="w-4 h-4" />{isEditing ? <input value={editValues.city} onChange={(e) => setEditValues(v => ({ ...v, city: e.target.value }))} placeholder="City" className="text-sm font-bold bg-transparent border-b border-white/30 text-white/60 w-32 focus:outline-none" /> : <span className="text-sm font-bold">{venue.city || "Add city"}</span>}</div>
                </div>

                <div className="px-6 py-6 border-b border-white/10">
                    <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">About</span></div>
                    {isEditing ? <textarea value={editValues.bio} onChange={(e) => setEditValues(v => ({ ...v, bio: e.target.value }))} placeholder="Tell visitors about your venue..." rows={3} className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white text-sm resize-none focus:outline-none focus:border-white/30" /> : <p className="text-white/70 text-sm leading-relaxed">{venue.bio || venue.description || "No description added yet."}</p>}
                </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar">
                        {highlights.map(h => (
                            <div key={h.id} className="flex-shrink-0 flex flex-col items-center gap-1">
                                ...
                                <span className="text-[8px] font-bold uppercase text-white/40">{h.title}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="px-6 py-6">
                    <div className="grid grid-cols-3 gap-2">
                        {gallery.slice(0, 3).map((p, i) => <div key={i} className="aspect-square rounded-xl overflow-hidden bg-white/5"><img src={p.imageUrl} className="w-full h-full object-cover" alt="" /></div>)}
                    </div>
                </div>
            </div>
        </div>
