...
export default function LivePreviewEditor({
  ...
  const coverImage =
    previewCover || venue.coverURL || venue.coverImage || venue.bannerImage || venue.image;
  ...
  return (
    <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-sm text-text-tertiary">
            {uploadingField
              ...
              : 'Design synced with your guest website.'}
          </p>
        </div>
      ...
      <div
        className={`relative bg-[#0A0A0A] rounded-3xl overflow-hidden border border-border-subtle shadow-2xl transition-all ${isEditing ? 'ring-2 ring-emerald-500/50' : ''}`}
      >
        <section className="relative w-full">
          <div className="relative aspect-[16/10] w-full overflow-hidden group">
            {coverImage ? (
              <img
                src={coverImage}
                className={`w-full h-full object-cover ${uploadingField === 'cover' ? 'opacity-50 blur-sm' : ''}`}
                alt=""
                onError={(e) =>
                  console.error(`[LivePreviewEditor] Cover image failed to load:`, coverImage)
                }
              />
            ) : (
              <div className="w-full h-full bg-surface-secondary flex items-center justify-center">
                <ImageIcon className="w-16 h-16 text-text-primary/10" />
              </div>
            )}
            ...
            {isEditing && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="px-6 py-3 bg-surface-elevated/10 backdrop-blur-md rounded-2xl border border-white/20 text-text-primary text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                  <Camera className="w-4 h-4" /> Click Cover to Change
                </div>
              </div>
            )}
            <div
            ...
            <div className="absolute bottom-0 left-0 right-0 z-30 px-6 pb-6 pointer-events-none">
              <div className="flex items-end gap-4">
                <div
                  ...
                  className="relative w-20 h-20 rounded-2xl overflow-hidden border-4 border-[#0A0A0A] shadow-2xl cursor-pointer group pointer-events-auto bg-surface-tertiary"
                >
                  {logo ? (
                    <img
                      src={logo}
                      ...
                      alt=""
                      onError={(e) =>
                        console.error(`[LivePreviewEditor] Logo image failed to load:`, logo)
                      }
                    />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    {isEditing ? (
                      <input
                        value={editValues.venueType}
                        onChange={(e) =>
                          setEditValues((v) => ({ ...v, venueType: e.target.value }))
                        }
                        className="px-3 py-1 bg-surface-elevated/10 border border-white/20 rounded-full text-[10px] font-black uppercase tracking-widest text-text-primary w-24 text-center pointer-events-auto"
                      />
                    ) : (
                      <span className="px-3 py-1 bg-surface-elevated/10 border border-border-subtle rounded-full text-[10px] font-black uppercase tracking-widest text-text-primary/80">
                        {venue.venueType || venue.category || 'Venue'}
                      </span>
                    )}
                    {isVerified && (
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="text-3xl font-black uppercase tracking-tighter text-text-primary bg-transparent border-b-2 border-white/30 focus:border-white outline-none w-full pointer-events-auto"
                    />
                  ) : (
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-text-primary leading-[0.9] drop-shadow-lg">
                      {venue.name}
                    </h1>
                  )}
                </div>
        ...
        <div className="px-6 py-4 border-b border-border-subtle flex items-center gap-6">
          <div className="flex items-center gap-2 text-text-primary/60">
            ...
            <span className="text-sm font-bold">{venue.followers || 0} followers</span>
          </div>
          <div className="flex items-center gap-2 text-text-primary/60">
            <Calendar className="w-4 h-4" />
            <span className="text-sm font-bold">{venue.eventsCount || 0} events</span>
          </div>
          <div className="flex items-center gap-2 text-text-primary/60">
            ...
            {isEditing ? (
              <input
                ...
                placeholder="City"
                className="text-sm font-bold bg-transparent border-b border-white/30 text-text-primary/60 w-32 focus:outline-none"
              />
            ) : (
              <span className="text-sm font-bold">{venue.city || 'Add city'}</span>
            )}
          </div>
        </div>
        ...
        <div className="px-6 py-6 border-b border-border-subtle">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-text-primary/40">
              About
            </span>
          </div>
          {isEditing ? (
            <textarea
              ...
              rows={3}
              className="w-full p-4 bg-surface-elevated/5 border border-border-subtle rounded-xl text-text-primary text-sm resize-none focus:outline-none focus:border-white/30"
            />
          ) : (
            <p className="text-text-primary/70 text-sm leading-relaxed">
              {venue.bio || venue.description || 'No description added yet.'}
            </p>
          )}
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {highlights.map((h) => (
              <div key={h.id} className="flex-shrink-0 flex flex-col items-center gap-1">
                <div className="w-14 h-14 rounded-full border-2 border-emerald-500/30 p-0.5 overflow-hidden">
                  <img
                ...
                <span className="text-[8px] font-bold uppercase text-text-primary/40">
                  {h.title}
                </span>
              </div>
            ))}
        ...
        <div className="px-6 py-6">
          <div className="grid grid-cols-3 gap-2">
            {gallery.slice(0, 3).map((p, i) => (
              <div
                key={i}
                className="aspect-square rounded-xl overflow-hidden bg-surface-elevated/5"
              >
