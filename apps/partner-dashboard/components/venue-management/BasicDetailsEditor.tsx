...
import { Camera, Upload, Loader2, MapPin, Clock, Phone, Globe, Instagram, Mail } from "lucide-react";
...
const VENUE_TYPES = ["Nightclub", "Lounge", "Bar", "Rooftop", "Pool Club", "Warehouse", "Festival Ground", "Concert Hall", "Restaurant & Bar"];
const CTA_OPTIONS = [
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-4">
                ...
                <div
                    ...
                    className="relative aspect-[21/9] w-full bg-[var(--surface-secondary)] rounded-2xl overflow-hidden cursor-pointer group border border-[var(--border-subtle)]"
                >
                    {bannerUrl ? (
                ...
                <div className="flex items-center gap-4">
                    <div
                        ...
                        className="relative w-24 h-24 rounded-2xl overflow-hidden cursor-pointer group border-2 border-[var(--border-subtle)] bg-[var(--surface-secondary)]"
                    >
                        {logoUrl ? (
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            {uploading === "logo" ? <Loader2 className="w-5 h-5 text-white animate-spin" /> : <Camera className="w-5 h-5 text-white" />}
                        </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <select value={venue.venueType || ""} onChange={(e) => onUpdate({ venueType: e.target.value })} className="w-full px-4 py-3 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl text-sm">
                            <option value="">Select type...</option>
                            {VENUE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                    </div>
                </div>
                <EditField label="Tagline" value={venue.tagline} placeholder="A short catchy phrase" onSave={(v) => onUpdate({ tagline: v })} />
                <EditField label="Description" value={venue.description || venue.bio} placeholder="Describe your venue..." multiline onSave={(v) => onUpdate({ description: v, bio: v })} />
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <EditField label="Neighborhood" value={venue.neighborhood} placeholder="e.g., Bandra West" onSave={(v) => onUpdate({ neighborhood: v })} />
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <EditField label="Email" value={venue.email} icon={<Mail className="w-4 h-4" />} onSave={(v) => onUpdate({ email: v })} />
                    <EditField label="Website" value={venue.website} icon={<Globe className="w-4 h-4" />} onSave={(v) => onUpdate({ website: v })} />
                    <EditField label="Instagram" value={venue.socialLinks?.instagram} icon={<Instagram className="w-4 h-4" />} placeholder="@yourhandle" onSave={(v) => onUpdate({ socialLinks: { ...venue.socialLinks, instagram: v } })} />
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <select value={venue.primaryCta || "reservation"} onChange={(e) => onUpdate({ primaryCta: e.target.value })} className="w-full px-4 py-3 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl text-sm">
...
function EditField({ label, value, placeholder, icon, multiline, onSave }: { label: string; value?: string; placeholder?: string; icon?: React.ReactNode; multiline?: boolean; onSave: (v: string) => void }) {
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="relative">
                ...
                {multiline ? (
                    <textarea value={localValue} onChange={(e) => { setLocalValue(e.target.value); setIsDirty(true); }} onBlur={handleBlur} placeholder={placeholder} rows={3} className="w-full px-4 py-3 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
                ) : (
                    <input type="text" value={localValue} onChange={(e) => { setLocalValue(e.target.value); setIsDirty(true); }} onBlur={handleBlur} placeholder={placeholder} className={`w-full px-4 py-3 bg-[var(--surface-secondary)] border border-[var(--border-subtle)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 ${icon ? "pl-10" : ""}`} />
                )}
            </div>
        </div>
    );
