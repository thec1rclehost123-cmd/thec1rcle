      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-tertiary)]">
            Show guests what amenities your venue offers
          </p>
        </div>
        <button
          ...
          >
            <div className="p-4 bg-[var(--surface-secondary)] rounded-xl border border-[var(--border-subtle)] space-y-4">
              <input
                ...
                className="w-full px-4 py-2 bg-[var(--surface-primary)] border border-[var(--border-subtle)] rounded-lg text-sm"
              />
              <div className="flex flex-wrap gap-2">
                {FACILITY_ICONS.map((icon) => (
                  <button
                    ...
                    className={`p-2 rounded-lg text-xl ${newIcon === icon.id ? "bg-emerald-500/20 ring-2 ring-emerald-500" : "bg-[var(--surface-primary)]"}`}
                  >
                    {icon.emoji}
                  </button>
                ))}
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm font-medium disabled:opacity-50"
                >
                  {saving ? (
        ...
        <Reorder.Group axis="y" values={facilities} onReorder={handleReorder} className="space-y-2">
          {facilities.map((f) => (
            <Reorder.Item
              key={f.id}
              value={f}
              className={`flex items-center gap-4 p-4 rounded-xl cursor-grab ${f.isEnabled ? "bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200" : "bg-[var(--surface-secondary)] border border-[var(--border-subtle)]"}`}
            >
              ...
              {editingId === f.id ? (
                <input
                  ...
                  className="flex-1 px-2 py-1 bg-white dark:bg-slate-800 border rounded text-sm"
                />
              ) : (
                <span
                  className={`flex-1 font-medium ${f.isEnabled ? "" : "text-[var(--text-tertiary)]"}`}
                >
                  {f.name}
                </span>
              )}
          ...
          <div className="flex flex-wrap gap-3">
            {facilities
              ...
              .map((f) => (
                <div
                  key={f.id}
                  className="flex items-center gap-2 px-3 py-2 bg-[var(--surface-primary)] rounded-full text-sm"
                >
