...
export default function FacilitiesManager({
  ...
  const handleToggle = (id: string, isEnabled: boolean) =>
    apiCall(
      'toggle',
      { facilityId: id, isEnabled },
      isEnabled ? 'Facility visible' : 'Facility hidden',
    );
  const handleUpdate = (id: string, updates: any) => {
    apiCall('update', { facilityId: id, updates }, 'Facility updated');
    setEditingId(null);
  };
  const handleDelete = (id: string) =>
    window.confirm('Remove?') && apiCall('delete', { facilityId: id }, 'Facility removed');
  const handleReorder = (newOrder: Facility[]) =>
    apiCall('reorder', { orderedIds: newOrder.map((f) => f.id) }, 'Layout saved');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-green-500 text-text-primary rounded-xl text-sm font-medium"
        >
          ...
          >
            <div className="p-4 bg-surface-secondary rounded-xl border border-border-subtle space-y-4">
              <input
                ...
                className="w-full px-4 py-2 bg-surface-base border border-border-subtle rounded-lg text-sm"
              />
              <div className="flex flex-wrap gap-2">
                {FACILITY_ICONS.map((icon) => (
                  <button
                    ...
                    className={`p-2 rounded-lg text-xl ${newIcon === icon.id ? 'bg-green-500/20 ring-2 ring-emerald-500' : 'bg-surface-base'}`}
                  >
                    {icon.emoji}
                  </button>
                ))}
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm">
                  Cancel
                </button>
                <button
                  onClick={handleAddFacility}
                  disabled={!newName.trim() || saving}
                  className="flex items-center gap-2 px-4 py-2 bg-green-500 text-text-primary rounded-lg text-sm font-medium disabled:opacity-50"
                >
                  {saving ? (
        ...
        <Reorder.Group axis="y" values={facilities} onReorder={handleReorder} className="space-y-2">
          {facilities.map((f) => (
            <Reorder.Item
              key={f.id}
              value={f}
              className={`flex items-center gap-4 p-4 rounded-xl cursor-grab ${f.isEnabled ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200' : 'bg-surface-secondary border border-border-subtle'}`}
            >
              ...
              {editingId === f.id ? (
                <input
                  autoFocus
                  defaultValue={f.name}
                  onBlur={(e) => handleUpdate(f.id, { name: e.target.value })}
                  className="flex-1 px-2 py-1 bg-surface-elevated dark:bg-surface-tertiary border rounded text-sm"
                />
              ) : (
                <span className={`flex-1 font-medium ${f.isEnabled ? '' : 'text-text-tertiary'}`}>
                  {f.name}
                </span>
              )}
              <button
              ...
              <button onClick={() => handleDelete(f.id)} className="p-2 text-red-500">
          ...
          <div className="flex flex-wrap gap-3">
            {facilities
              ...
              .map((f) => (
                <div
                  key={f.id}
                  className="flex items-center gap-2 px-3 py-2 bg-surface-base rounded-full text-sm"
                >
