...
export function VenueGuard({ children }: { children: React.ReactNode }) {
    ...
    if (loading) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-[var(--bg-fill)]">
                <div className="flex flex-col items-center gap-4">
                    <div className="h-8 w-8 animate-spin rounded-full border-b-2 border-t-2 border-indigo-500" />
                    <p className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-widest">
                        Verifying Access...
                    </p>
                </div>
            </div>
