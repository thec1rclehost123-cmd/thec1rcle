          ...
          {activeAction && (
            ...
            >
              ...
              {activeAction === 'deny' && (
                <>
                  <div>
                    <label className="block text-[11px] font-medium text-[var(--v-text-muted)] mb-1.5">
                      Reason *
                    </label>
                    <select
                      value={denyReason}
              ...
              {activeAction === 'check-in' && (
                <p className="text-[13px] text-[var(--v-text-secondary)]">
                  Manually check in <strong>{guest.displayName}</strong>. This will be logged as a
              ...
              {activeAction === 're-entry' && (
                <p className="text-[13px] text-[var(--v-text-secondary)]">
                  Allow re-entry for <strong>{guest.displayName}</strong>. This will be logged and
                  may require re-entry to be enabled in event rules.
                </p>
              )}
          ...
          <div className="px-5 py-4 space-y-5">
            ...
            <Section title="Identity">
              ...
              <DetailRow
                icon={<User size={13} />}
                label="Guest Type"
                value={<GuestSourceChip source={guest.guestType as any} size="xs" />}
              />
              <DetailRow
                icon={<User size={13} />}
                label="Source"
                value={<GuestSourceChip source={guest.source} size="xs" />}
              />
            </Section>
              ...
              <Section title="Check-In">
                {guest.checkedInAt && (
                  <DetailRow
                    icon={<Clock size={13} />}
                    label="Time"
                    value={new Date(guest.checkedInAt).toLocaleString('en-IN', {
                      weekday: 'short',
                      month: 'short',
                      day: 'numeric',
                      hour: '2-digit',
                ...
                {guest.checkInSource && (
                  <DetailRow
                    ...
                    label="Source"
                    value={guest.checkInSource.replace(/_/g, ' ')}
                  />
                )}
            ...
            <Section title="Metadata">
              <DetailRow
                icon={<User size={13} />}
                label="Added By"
                value={guest.addedByName || 'System'}
              />
              <DetailRow
                icon={<Clock size={13} />}
                label="Added At"
                value={new Date(guest.addedAt).toLocaleString('en-IN', {
                  weekday: 'short',
                  month: 'short',
                  day: 'numeric',
                  hour: '2-digit',
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-[11px] font-semibold text-[var(--v-text-muted)] uppercase tracking-wide mb-2">
        {title}
      </div>
      <div
...
function ActionButton({
  ...
  const colorMap = {
    ...
    slate:
      'text-slate-600 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/50 dark:hover:bg-slate-800/80 dark:text-slate-400',
  };
  return (
    <button
      onClick={onClick}
