...
function StyleCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative flex-shrink-0 w-[130px] p-4 rounded-[1.5rem] border-2 transition-all duration-300 text-left group ${
        isSelected
    ...
    >
      ...
      <p
        className={`text-[11px] font-black tracking-wide leading-tight ${isSelected ? 'text-indigo-400' : 'text-text-primary'}`}
      >
        {style.label}
      </p>
...
export function MediaStep({ formData, updateFormData }: MediaStepProps) {
  ...
  const [selectedQuality, setSelectedQuality] = useState<'quality' | 'default' | 'turbo'>(
    'quality',
  );
  ...
  const uploadPosterAsset = useCallback(async (asset: string) => {
    ...
    const posterRef = ref(
      storage,
      `${POSTER_STORAGE_PREFIX}/${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`,
    );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const result: PosterGenerationResult = await posterService.generatePoster({
        ...
        designPrompt:
          designPrompt.trim() || 'Modern, premium nightlife aesthetic with elegant typography',
      ...
      if (result.success && result.imageUrl) {
        ...
        setGenerationError(
          result.error?.userFriendlyMessage || 'Poster generation failed. Please try again.',
        );
        setGenerationState('error');
      }
    } catch (error: any) {
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    if (result.success && result.imageUrl) {
      ...
      setGenerationError(
        result.error?.userFriendlyMessage || 'Regeneration failed. Please try again.',
      );
      setGenerationState('error');
    }
  }, [
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-label text-text-secondary mt-1.5 max-w-lg">
            Define your event&apos;s visual identity through high-fidelity assets or creative AI
            synthesis.
          </p>
        </div>
        ...
        <div className="flex p-1 rounded-[1.25rem] bg-surface-secondary border border-border-subtle w-fit relative">
          <div className="relative flex">
            ...
            <button
              ...
              className={`relative px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-[0.1em] transition-colors duration-200 z-10 w-[140px] ${
                uploadMode === 'manual'
                  ? 'text-text-inverse'
            ...
            <button
              ...
              className={`relative px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-[0.1em] transition-colors duration-200 z-10 w-[140px] flex items-center justify-center gap-2 ${
                uploadMode === 'ai'
                  ? 'text-text-inverse'
                  : 'text-text-tertiary hover:text-text-primary'
              }`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            {uploadMode === 'manual' ? (
                ...
                <div className="rounded-[1.5rem] border border-dashed border-border-default bg-surface-base/80 px-5 py-6 md:px-6 md:py-7">
                  <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <p className="mt-1 max-w-md text-sm leading-relaxed text-text-secondary">
                      Recommended minimum: {POSTER_MIN_WIDTH} x {POSTER_MIN_HEIGHT}px. Other sizes
                      will be auto-cropped to fit.
                    </p>
              ...
              >
                ...
                <div className="p-4 rounded-[1.25rem] bg-surface-base border border-border-subtle shadow-sm relative overflow-hidden">
                  ...
                  <p
                    className={`text-body-sm font-bold truncate ${eventName ? 'text-text-primary' : 'text-text-tertiary italic'}`}
                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {MOOD_OPTIONS.map((mood) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-[11px] font-bold transition-all duration-200 flex items-center gap-1.5 ${
                  ...
                  <div className="flex items-center justify-between px-1">
                    ...
                    <div className="group relative">
                      ...
                      <div className="absolute bottom-full right-0 mb-4 w-72 p-4 bg-surface-elevated backdrop-blur-md text-text-primary text-[11px] rounded-[1.25rem] opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-20 shadow-2xl leading-relaxed border border-border-strong">
                        Describe what you want beyond the style preset. e.g. &ldquo;spotlight on a
                        DJ silhouette&rdquo; or &ldquo;fireworks and confetti explosion&rdquo;
                      </div>
                    </div>
                  </div>
                ...
                <div className="border-t border-border-subtle pt-6">
                  ...
                  >
                    <ChevronRight
                      className={`w-3.5 h-3.5 transition-transform duration-300 ${showAdvanced ? 'rotate-90' : ''}`}
                    />
                    Advanced Options
                  </button>
                          ...
                          <div className="flex gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`px-4 py-2.5 rounded-xl text-[10px] font-bold transition-all flex-1 ${
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[9px] text-text-tertiary">
                                AI renders event title directly
                              </p>
                            </div>
                          </div>
                          <button
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className={`btn w-full py-5 text-[12px] font-black uppercase tracking-[0.25em] flex items-center justify-center gap-3 transition-all duration-500 scale-100 active:scale-95 ${
                      isGenerating
                ...
                {generationHistory.length > 0 && (
                  <div className="pt-6 border-t border-border-subtle">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-3.5 h-3.5 transition-transform duration-500 ${showHistory ? 'rotate-180' : ''}`}
                      />
                    </button>
                        ...
                        >
                          {generationHistory.map((entry) => (
                            <button
                              ...
                              className={`relative w-20 h-28 rounded-[1.25rem] overflow-hidden flex-shrink-0 border-2 transition-all duration-500 shadow-lg ${
                                entry.isSelected
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-1">
            <p className="text-label font-black uppercase tracking-widest text-text-tertiary">
              Master Asset
            </p>
            {selectedImage && (
                ...
                {isGenerating ? (
                  <>
                    ...
                    <div className="relative">
                      ...
                      <div
                        className="absolute -inset-4 rounded-[3.5rem] border-2 border-dashed border-indigo-500/20 animate-spin"
                  ...
                  <>
                    ...
                    <div className="text-center space-y-2 px-12">
                      <p className="text-display-xs text-text-primary opacity-20">Void Buffer</p>
          ...
          {selectedImage && uploadMode === 'ai' && (
            ...
            >
              <RefreshCw
                className={`w-4 h-4 group-hover:text-indigo-600 transition-colors ${isGenerating ? 'animate-spin' : ''}`}
              />
        ...
        <div className="relative flex flex-wrap items-center justify-center gap-y-6 gap-x-12 py-8 px-12 rounded-[2.5rem] border border-border-subtle bg-surface-secondary backdrop-blur-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              ...
              <span className="block text-body-sm font-bold text-text-primary">
                Perfect Rendering
              </span>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="px-6 md:px-8 py-5 border-b border-border-subtle flex items-start justify-between gap-4 bg-surface-secondary">
                <div className="space-y-1">
                  <h3 className="text-xl md:text-2xl font-semibold tracking-tight text-text-primary">
                    Adjust Event Poster
                  </h3>
                  <p className="text-sm text-text-secondary max-w-xl">
                    Recommended minimum: {POSTER_MIN_WIDTH} x {POSTER_MIN_HEIGHT}px. We&apos;ll save
                    a higher-quality 4:5 poster master automatically.
                  </p>
                </div>
                <button
              ...
              <div className="px-6 md:px-8 py-6 bg-surface-base space-y-6 overflow-y-auto">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-[1.25rem] border border-border-subtle bg-surface-secondary p-4">
                    ...
                    <p className="mt-2 text-base font-bold text-text-primary">
                      {POSTER_MIN_WIDTH} x {POSTER_MIN_HEIGHT}px minimum
                    </p>
                    <p className="mt-1 text-sm leading-relaxed text-text-secondary">
                      We keep a higher-quality {POSTER_WIDTH} x {POSTER_HEIGHT}px master so your
                      poster stays sharper across event pages and shares.
                    </p>
                  </div>
                  <div className="rounded-[1.25rem] border border-border-subtle bg-surface-secondary p-4">
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-text-tertiary">
                    ...
                    <p className="mt-1 text-sm leading-relaxed text-text-secondary">
                      If your upload is wider or taller, we&apos;ll crop it to fit this poster ratio
                ...
                <div className="space-y-5">
                  <div className="flex justify-between items-center px-1">
                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-text-tertiary">
                      Zoom
                    </span>
                    <span className="text-indigo-500 px-4 py-1.5 rounded-full bg-indigo-500/10 text-[11px] font-black tracking-widest">
                      {zoom.toFixed(2)}x
                    </span>
                  </div>
                  <input
