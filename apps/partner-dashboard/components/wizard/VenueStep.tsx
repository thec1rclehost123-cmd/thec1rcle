...
export function VenueStep({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'available':
        return 'bg-green-500/10 text-accent-primary border-[var(--state-success)]/20';
      case 'blocked':
        return 'bg-red-500/10 text-red-500 border-[var(--state-error)]/20';
      case 'booked':
        return 'bg-surface-tertiary text-text-tertiary border-border-subtle';
      case 'partial':
        return 'bg-yellow-500/10 text-yellow-500 border-[var(--state-warning)]/20';
      case 'tentative':
        return 'bg-yellow-500/10 text-yellow-500 border-[var(--state-warning)]/40 ring-2 ring-[var(--state-warning)]/10';
      case 'approved_hold':
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      {validationErrors.venueId && (
        <div className="p-5 rounded-2xl bg-red-500/10 border border-[var(--state-error)]/20 flex items-center gap-3 shadow-sm border-l-4 border-l-rose-500">
          <AlertCircle className="w-5 h-5 text-red-500" />
          <p className="text-body-sm text-red-500 font-bold">{validationErrors.venueId}</p>
        </div>
      )}
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-1">
          <p className="text-label font-black uppercase tracking-widest text-text-secondary">
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-4 bg-surface-secondary px-5 py-2 rounded-xl border border-border-subtle">
                <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-text-tertiary">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]" />{' '}
                  Open
                </div>
                <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-text-tertiary">
            ...
            {loadingCalendar ? (
              <div className="p-16 rounded-[2.5rem] bg-surface-secondary border border-border-subtle text-center">
                ...
                <p className="text-[11px] font-black uppercase tracking-[0.2em] text-text-tertiary">
              ...
              <div className="grid grid-cols-4 md:grid-cols-7 gap-3">
                {calendar.slice(0, 28).map((day) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest ${isSelected ? 'text-text-primary/70' : 'text-text-tertiary'}`}
                      >
                        {date.toLocaleDateString('en-US', { weekday: 'short' })}
                      </span>
                      <span
                        className={`text-lg font-black ${isSelected ? 'text-text-primary' : 'text-text-primary'}`}
                      >
                        {date.getDate()}
                      </span>
            ...
            <div className="p-5 rounded-2xl bg-surface-secondary border border-border-subtle flex items-start gap-4">
              ...
              <div className="space-y-1">
                <p className="text-body-sm text-text-primary font-bold uppercase tracking-wider">
