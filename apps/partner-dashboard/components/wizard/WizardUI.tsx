...
export function Section({ title, description, children, className }: SectionProps) {
  return (
    <div
      className={clsx(
        "rounded-[2rem] border border-[var(--border-subtle)] bg-[var(--surface-base)] p-8 shadow-sm",
        className,
      )}
    >
      <div className="mb-6 space-y-1">
        ...
        {description && (
          <p className="text-body-sm text-[var(--text-tertiary)] uppercase tracking-widest font-bold">
            {description}
          </p>
        )}
      </div>
...
export function WizardInput({
  ...
  return (
    <div className="space-y-1.5 w-full">
      ...
      {hint && !error && (
        <p className="ml-1 text-[11px] text-[var(--text-tertiary)] uppercase tracking-widest font-bold">
          {hint}
        </p>
      )}
      {error && (
...
export function Reassurance({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-[var(--state-success)]/20 bg-[var(--state-success-bg)] p-5">
      ...
      <p className="text-body-sm leading-relaxed text-[var(--state-success)] font-medium opacity-90">
        {children}
      </p>
    </div>
  );
...
export function Hint({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-2 text-[11px] leading-relaxed text-[var(--text-tertiary)] uppercase tracking-widest font-black">
      {children}
    </p>
  );
}
