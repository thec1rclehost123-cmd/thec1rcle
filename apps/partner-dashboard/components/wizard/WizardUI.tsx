...
export function Section({ title, description, children, className }: SectionProps) {
  return (
    <div
      className={cn(
        'rounded-[2rem] border border-border-subtle bg-surface-base p-8 shadow-sm',
        className,
      )}
    >
      <div className="mb-6 space-y-1">
        ...
        {description && (
          <p className="text-body-sm text-text-tertiary uppercase tracking-widest font-bold">
            {description}
          </p>
        )}
      </div>
...
export function WizardInput({
  ...
  return (
    <div className="space-y-1.5 w-full">
      ...
      {hint && !error && (
        <p className="ml-1 text-[11px] text-text-tertiary uppercase tracking-widest font-bold">
          {hint}
        </p>
      )}
      {error && (
...
export function Reassurance({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-[var(--state-success)]/20 bg-green-500/10 p-5">
      ...
      <p className="text-body-sm leading-relaxed text-accent-primary font-medium opacity-90">
        {children}
      </p>
    </div>
  );
...
export function Hint({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-2 text-[11px] leading-relaxed text-text-tertiary uppercase tracking-widest font-black">
      {children}
    </p>
  );
}
