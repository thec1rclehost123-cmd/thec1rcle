...
function useDominantColor(imageUrl?: string | null) {
  const [color, setColor] = useState<string | null>(() =>
    imageUrl ? dominantColorCache[imageUrl] || null : null,
  );
            ...
            style={{
              ...
              background:
                index % 3 === 0
                  ...
                  : `rgba(${accent}, ${index % 2 === 0 ? 0.88 : 0.62})`,
              boxShadow:
                index % 3 === 0
...
export function GuestPortalEventPreview({
  ...
  const eventDate = useMemo(
    () => buildDate(event.startDate, event.startTime),
  ...
  const tickets = (event.tickets || []).filter(
    (ticket) => ticket && (ticket.price != null || ticket.amount != null),
  );
  const allFree =
    tickets.length > 0 &&
    tickets.every((ticket) => Number(ticket.price || ticket.amount || 0) === 0);
  const minPrice =
    tickets.length > 0
      ? Math.min(...tickets.map((ticket) => Number(ticket.price || ticket.amount || 0)))
      : 0;
  const entryLabel =
    tickets.length === 0
      ? 'TICKETS SOON'
        ...
        {extractedAccent ? (
          <>
            <div
              ...
              style={{
                background:
                  'radial-gradient(ellipse at 50% 30%, rgba(var(--event-accent), 0.9), transparent 85%)',
              }}
            />
            <div
              ...
              style={{
                background:
                  'radial-gradient(circle at 50% 50%, rgba(var(--event-accent), 0.5), transparent 85%)',
              }}
            />
          </>
        ) : poster ? (
        ...
        <div className="mx-auto flex max-w-[980px] items-center justify-between rounded-full border border-white/10 bg-[#140d0b]/82 px-4 py-3 shadow-[0_12px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          ...
          <div className="hidden items-center gap-2 md:flex">
            {['Explore', 'Hosts', 'Tickets', 'App'].map((item) => (
              <span
                key={item}
                className="rounded-full bg-white/[0.04] px-3.5 py-2 text-[8px] font-black uppercase tracking-[0.28em] text-white/36"
              >
                {item}
              </span>
            ))}
                ...
                <div className="mt-2 text-[1.45rem] font-semibold leading-tight text-white">
                  {tickets.length === 0
                    ...
                    : allFree
                      ...
                      : minPrice === 0
                        ? 'Entry starts free.'
                        : `Tickets start at ${formatINR(minPrice)}.`}
                </div>
                <div className="mt-2 text-[13px] text-white/42">
                  {venue} · {timeChip || dateChip}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <button className="rounded-full border border-white/10 bg-white/[0.05] px-5 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-white/65">
                  View Guestlist
                </button>
                <button
        ...
        <div className="mx-auto mt-3 grid max-w-[980px] items-start gap-3 lg:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <GlassCard accent={accent} className="overflow-hidden">
              <div className="flex items-center justify-between gap-3 px-5 py-5">
                <div>
                  ...
                  <div className="mt-3 text-[24px] font-black uppercase tracking-tight text-white">
                    {venue}
                  </div>
          ...
          <aside className="relative flex min-w-0 flex-col gap-3">
            <div
              ...
              style={{
                background:
                  'radial-gradient(circle, rgba(var(--event-accent), 0.5) 0%, transparent 60%)',
              }}
            />
            <GlassCard accent={accent} className="p-3">
              <div className="relative aspect-[0.78] overflow-hidden rounded-[22px] border border-white/10 bg-black/30">
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(180deg, rgba(10,10,10,0.06), rgba(10,10,10,0.36) 48%, rgba(10,10,10,0.86) 100%)`,
                  }}
                />
            ...
            <GlassCard accent={accent}>
              <div className="border-b border-white/[0.08] px-5 py-5">
                ...
                <div className="mt-3 text-[28px] font-black uppercase tracking-tight text-white">
                  {entryLabel}
                </div>
              ...
              <div className="flex flex-col gap-3 px-5 py-5">
                {tickets.length === 0 ? (
                  <div className="rounded-[18px] border border-white/[0.06] bg-white/[0.03] px-4 py-4 text-[13px] text-white/50">
                ...
                {tickets.map((ticket, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="rounded-[18px] border border-white/[0.06] bg-black/35 px-4 py-4"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-2 text-[16px] font-bold text-white">
                            {ticket.name || ticket.label || `Entry Tier ${index + 1}`}
                          </div>
                          <div className="mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-200">
                            {getAvailability(ticket)}
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <div className="mt-1 text-[9px] font-bold uppercase tracking-[0.18em] text-white/28">
                            {price === 0
                              ? 'Limit 1'
                              : `Limit ${Math.min(Number(ticket.quantity || 10) || 10, 10)}`}
                          </div>
                        </div>
                      </div>
                      <div className="mt-4 flex items-center justify-between border-t border-white/[0.06] pt-4">
                        ...
                        <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-black/40 px-1 py-1">
                          <button className="flex h-8 w-8 items-center justify-center rounded-full bg-white/[0.06] text-white">
                            −
                          </button>
              ...
              <div className="border-t border-white/[0.08] px-5 py-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-[18px] font-black text-white">
                    {allFree ? 'Free' : tickets.length ? formatINR(minPrice) : '--'}
                  </div>
                </div>
                <button
...
export function GuestPortalEventPreviewFrame({
  ...
  return (
    <div
      className="overflow-hidden rounded-[32px] shadow-2xl transition-transform hover:scale-[1.02]"
      style={{ width, height }}
    >
      <div
        style={{
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 40%, rgba(${accent}, 0.42), transparent 65%)`,
        }}
      />
      <div
