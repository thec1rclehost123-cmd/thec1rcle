...
const ENTRY_TYPES = [
  {
    ...
    label: 'Male',
    icon: User,
    description: 'Male entry',
    defaultName: 'Stag Entry',
  ...
  },
  {
    ...
    label: 'Female',
    icon: User,
    description: 'Female entry',
    defaultName: 'Ladies Entry',
...
function CoverChargeConfig({
  ...
  const updatePreset = (
    idx: number,
    patch: Partial<NonNullable<TicketTier['coverChargeConfig']>['presetItems'][0]>,
  ) => {
  ...
  const walletRupees =
    cfg.walletAmountPaise > 0 ? `= ₹${(cfg.walletAmountPaise / 100).toFixed(2)}` : null;

  return (
    <div
      className={`rounded-xl overflow-hidden transition-all ${isEnabled ? 'bg-violet-500/5 border border-violet-500/20' : 'bg-surface-tertiary'}`}
    >
      {/* Header row */}
      <button
        onClick={() => {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-black text-violet-500 uppercase tracking-widest">
            Financial Configuration
          </span>
          <span
            className={`text-[9px] px-1.5 py-0.5 rounded-full font-black uppercase tracking-widest ${isEnabled ? 'bg-violet-500 text-white' : 'bg-surface-elevated text-text-tertiary border border-border-subtle'}`}
          >
            {isEnabled ? 'WALLET ON' : 'DISABLED'}
          </span>
        </div>
        <span className="text-[11px] text-text-tertiary font-bold uppercase tracking-widest">
          {isEnabled && cfg.walletAmountPaise > 0
            ? `₹${(cfg.walletAmountPaise / 100).toFixed(0)}`
            : 'Configure'}
        </span>
      </button>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  {(
                    ...
                    >
                      <p
                        className={`text-[12px] font-black uppercase tracking-wider ${cfg.terminationPolicy === opt.value ? 'text-violet-400' : 'text-text-primary'}`}
                      >
                        {opt.label}
                      </p>
                ...
                <div className="space-y-1.5">
                  {cfg.presetItems.map((item, idx) => (
                    <div key={item.id} className="flex items-center gap-2">
                      ...
                      <div className="relative w-28 flex-shrink-0">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary font-bold text-[12px]">
                          ₹
                        </span>
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            updatePreset(idx, { amountPaise: parseInt(e.target.value) || 0 })
                          }
                          placeholder="40000"
                          min="0"
                  ...
                  {cfg.presetItems.length === 0 && (
                    <p className="text-[11px] text-text-tertiary py-1">
                      No presets yet — bartenders can still enter custom amounts.
                    </p>
                  )}
                </div>
                <button
...
>(
  (
    ...
    const convertDefaultToScheduledPrice = (
      defaultPrice: DefaultScheduledPrice,
      basePrice: number,
    ): ScheduledPrice => {
      const price =
        defaultPrice.type === 'discount'
    ...
    return (
      ...
      >
        ...
        <div className="px-4 py-3 flex items-center gap-3">
          <div
            className={`w-9 h-9 rounded-xl ${cardColor} shadow-md flex items-center justify-center flex-shrink-0`}
          >
          ...
          <p
            className={`text-[15px] font-black mr-1 flex-shrink-0 ${isRSVP ? 'text-indigo-500' : 'text-text-primary'}`}
          >
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (!tier.name || DEFAULT_NAMES.includes(tier.name))
                              updates.name = type.defaultName;
                            // Auto-assign gender for stag/female/couple; keep existing for vip/cover
                            if (type.genderRequirement)
                              updates.genderRequirement = type.genderRequirement;
                        ...
                        >
                          <Icon
                            className={`w-3.5 h-3.5 mx-auto mb-0.5 ${isSelected ? 'text-indigo-500' : 'text-text-tertiary'}`}
                          />
                          <p
                            className={`text-[10px] font-bold uppercase tracking-wider ${isSelected ? 'text-indigo-500' : 'text-text-primary'}`}
                          >
                            {type.label}
                          </p>
                        </button>
                ...
                <div>
                  ...
                  {tier.entryType === 'couple' ? (
                    ...
                    <div className="px-3 py-2.5 rounded-xl bg-surface-secondary border border-border-subtle text-[13px] font-bold text-text-tertiary flex items-center gap-2">
                      ...
                      {tier.entryType === 'stag'
                        ? 'Male only — set by entry type'
                        : 'Female only — set by entry type'}
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-1.5">
                  ...
                  {!tier.genderRequirement &&
                    (tier.entryType === 'vip' || tier.entryType === 'cover') && (
                      <p className="text-[10px] text-red-500 font-bold mt-1">
                        Gender category is required
                      </p>
                    )}
                </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const newPrices = newOverride
                          ? (defaultScheduledPrices || []).map((dp) =>
                              convertDefaultToScheduledPrice(dp, Number(tier.price) || 0),
                            )
                          : [];
                        onUpdate({
                          overrideScheduledPricing: newOverride,
                          scheduledPrices: newPrices,
                        });
                      }}
                      className="w-full p-3 flex items-center justify-between text-left"
                    >
...
export function TicketTierStep({
  formData,
  updateFormData,
  validationErrors,
}: TicketTierStepProps) {
        ...
        <div className="w-32">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[11px] font-bold text-text-primary">
              {totalTickets}
              <span className="text-text-tertiary font-normal">/{capacity}</span>
            </span>
          </div>
        ...
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.badge !== undefined && (
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${activeTab === tab.id ? 'bg-indigo-500/10 text-indigo-500' : 'bg-surface-tertiary text-text-tertiary'}`}
                >
                  {tab.badge}
                </span>
              )}
              {tab.dot && <div className="w-1.5 h-1.5 rounded-full bg-green-500 flex-shrink-0" />}
            </button>
          );
        })}
      </div>
        ...
        {activeTab === 'tiers' && (
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-1">
              <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest flex-shrink-0">
                Quick set:
              </span>
            ...
            <div className="p-4 rounded-xl border border-border-default bg-surface-secondary space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 flex-shrink-0 ${formData.scheduledPricingEnabled ? 'bg-gradient-to-br from-[#ff6b35] to-[#f7931e] text-white' : 'bg-surface-tertiary text-text-tertiary'}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-secondary">
                      Early Bird & Last Call pricing windows
                    </p>
                  </div>
                </div>
                <Toggle
                  on={formData.scheduledPricingEnabled}
                  onToggle={() =>
                    updateFormData({ scheduledPricingEnabled: !formData.scheduledPricingEnabled })
                  }
                  color="bg-green-500"
                />
              </div>
              ...
              {formData.scheduledPricingEnabled && (
                <motion.div
                  ...
                  className="pt-4 border-t border-border-default space-y-3"
                >
                  {/* Quick Presets */}
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => {
                        ...
                        updateFormData({
                          defaultScheduledPrices: [
                            ...(formData.defaultScheduledPrices || []),
                            {
                              id: `eb-${Date.now()}`,
                              name: 'Early Bird',
                              type: 'discount',
                  ...
                  {/* Current Schedules */}
                  <div className="space-y-2">
                    {(formData.defaultScheduledPrices || []).map(
                      (schedule: DefaultScheduledPrice) => (
                        <div
                          key={schedule.id}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 flex-grow">
                              <input
                                ...
                                value={schedule.name}
                                onChange={(e) => {
                                  const updated = formData.defaultScheduledPrices.map(
                                    (s: DefaultScheduledPrice) =>
                            ...
                            <button
                              onClick={() =>
                                updateFormData({
                                  defaultScheduledPrices: formData.defaultScheduledPrices.filter(
                                    (s: DefaultScheduledPrice) => s.id !== schedule.id,
                                  ),
                                })
                              }
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex p-0.5 bg-surface-elevated rounded-lg">
                              {[
                                { v: 'discount', l: 'DISCOUNT', c: 'bg-[#34c759]' },
                                { v: 'markup', l: 'MARKUP', c: 'bg-[#ff3b30]' },
                              ].map(({ v, l, c }) => (
                                <button
                                  key={v}
                                  onClick={() =>
                                    updateFormData({
                                      defaultScheduledPrices: formData.defaultScheduledPrices.map(
                                        (s: DefaultScheduledPrice) =>
                                          s.id === schedule.id ? { ...s, type: v } : s,
                                      ),
                                    })
                                  }
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <div key={k} className="space-y-1">
                                <label className="text-[10px] text-text-tertiary font-bold uppercase">
                                  {l}
                                </label>
                                <input
                                  type="datetime-local"
                                  value={(schedule as any)[k]}
                                  onChange={(e) =>
                                    updateFormData({
                                      defaultScheduledPrices: formData.defaultScheduledPrices.map(
                                        (s: DefaultScheduledPrice) =>
                                          s.id === schedule.id ? { ...s, [k]: e.target.value } : s,
                                      ),
                                    })
                                  }
                  ...
                  <p className="text-[11px] text-text-tertiary p-3 rounded-lg bg-surface-tertiary">
                    These apply to <strong>all tiers</strong> by default. Override individually
                    within each tier.
                  </p>
                </motion.div>
              )}
          ...
          >
            ...
            <div className="p-4 rounded-xl border border-border-default bg-surface-secondary space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 flex-shrink-0 ${formData.promotersEnabled ? 'bg-[#F44A22] text-white' : 'bg-surface-tertiary text-text-tertiary'}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-secondary">
                      Allow ambassadors to sell your tickets
                    </p>
                  </div>
                </div>
                <Toggle
                  on={formData.promotersEnabled}
                  onToggle={() => updateFormData({ promotersEnabled: !formData.promotersEnabled })}
                  color="bg-green-500"
                />
              </div>
                ...
                >
                  ...
                  {!formData.isRSVP && (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-[11px] font-black uppercase tracking-widest text-[#34c759]">
                          Buyer Discounts
                        </p>
                        <span className="px-2 py-0.5 rounded-full bg-green-500/10 text-[#34c759] text-[9px] font-black tracking-widest uppercase border border-emerald-500/20">
                          Via Links
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-xl bg-surface-tertiary border border-border-default">
                        <div>
                          ...
                          <p className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest">
                            Offer discounts via promoter links
                          </p>
                        </div>
                        <Toggle
                          on={formData.buyerDiscountsEnabled}
                          onToggle={() =>
                            updateFormData({
                              buyerDiscountsEnabled: !formData.buyerDiscountsEnabled,
                            })
                      ...
                      {formData.buyerDiscountsEnabled && (
                        ...
                        >
                          <div className="flex items-center justify-between p-3 rounded-xl bg-surface-tertiary border border-border-default">
                            ...
                            <Toggle
                              on={formData.useDefaultDiscount !== false}
                              onToggle={() =>
                                updateFormData({
                                  useDefaultDiscount: formData.useDefaultDiscount === false,
                                })
                              }
                              color="bg-[#34c759]"
                            />
                              ...
                              <div className="flex items-center gap-2">
                                <div className="flex p-1 bg-surface-secondary rounded-xl border border-border-default">
                                  {[
                                    ...
                                    <button
                                      key={v}
                                      onClick={() => updateFormData({ discountType: v })}
