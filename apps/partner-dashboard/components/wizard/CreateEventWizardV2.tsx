...
export function CreateEventWizardV2({ role }: { role: "venue" | "host" }) {
  ...
  const stepValidation = useMemo(() => {
    ...
    const totalTickets =
      formData.tickets?.reduce((sum: number, t: any) => sum + (Number(t.quantity) || 0), 0) || 0;
    if (totalTickets > formData.capacity) {
      validation.ticketing.issues.push(
        `Ticket quantity (${totalTickets}) exceeds capacity (${formData.capacity})`,
      );
  ...
  const grandTotal = useMemo(() => {
    ...
    const tableCapacity = (formData.tables || []).reduce(
      (acc: number, table: any) =>
        acc + Number(table.guestsPerTable || table.capacity || 0) * Number(table.quantity),
      0,
    );
          ...
          !formData.title ? (
            <div className="max-w-5xl mx-auto py-16 px-4">
              <div className="text-center mb-16 space-y-4">
                ...
                <h2 className="text-[32px] font-black tracking-tight text-[var(--text-primary)] uppercase">
                  Draft Sessions
                </h2>
                <p className="text-[var(--text-tertiary)] text-sm font-medium tracking-wide uppercase opacity-60">
                  Resume your creative sequence
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-bold text-[var(--text-primary)] group-hover:text-indigo-500 transition-colors uppercase tracking-widest text-xs">
                      Initialize New
                    </p>
                    <p className="text-[10px] text-[var(--text-tertiary)] font-medium uppercase mt-1">
                      From scratch
                    </p>
                  </div>
                </div>
            ...
            <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="w-full lg:w-[360px] lg:sticky lg:top-8 self-start space-y-6">
                ...
                <div className="flex justify-center">
                  ...
                  >
                    <EventCard event={formData} isPreview={true} device="desktop" height="h-full" />
                  </div>
                </div>
