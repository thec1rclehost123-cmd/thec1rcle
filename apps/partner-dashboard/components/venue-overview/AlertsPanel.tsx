...
function AlertRow({ alert, onDismiss }: { alert: VenueAlert; onDismiss: (id: string) => void }) {
...
export const AlertsPanel = memo(function AlertsPanel({ alerts, loading }: AlertsPanelProps) {
  ...
  return (
    <div className="rounded-[32px] p-5" style={{ background: 'var(--v-card)' }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h3 className="text-[14px] font-semibold" style={{ color: 'var(--v-text-primary)' }}>
            Alerts
          </h3>
          {unreadCount > 0 && (
            <span
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          {visible.length > 5 && (
            <p className="text-[11px] pt-2 text-center" style={{ color: 'var(--v-text-muted)' }}>
