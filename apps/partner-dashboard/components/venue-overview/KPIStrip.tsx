...
function TrendChip({ trend, direction }: { trend: string; direction: 'up' | 'down' | 'neutral' }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="v-label uppercase tracking-widest text-[9px]">{label}</span>
        <Icon className="w-4 h-4" style={{ color: 'var(--v-text-muted)' }} />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 flex-wrap min-h-[16px]">
            {trend && trendDirection && <TrendChip trend={trend} direction={trendDirection} />}
            {subtext && (
              <span className="text-[11px]" style={{ color: 'var(--v-text-tertiary)' }}>
                {subtext}
              </span>
            )}
          </div>
 ...
 */
export const KPIStrip = memo(function KPIStrip({ data, loading, filterUpdating }: KPIStripProps) {
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      ...
      <KPICard
        ...
        trend={data?.newGuestsThisWeek ? `+${data.newGuestsThisWeek} this week` : undefined}
