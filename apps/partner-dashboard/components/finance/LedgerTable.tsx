    ...
    <>
      ...
      {expanded && (
        ...
        >
          ...
          <DetailCell
            label="Net Amount"
            value={tx.netAmount != null ? formatINR(tx.netAmount) : '—'}
          />
          <DetailCell
            label="Processor Fee"
            value={tx.processorFee ? formatINR(tx.processorFee) : '—'}
          />
          <DetailCell
            label="Platform Fee"
            value={tx.platformFee ? formatINR(tx.platformFee) : '—'}
          />
    ...
    <div className="divide-y" style={{ borderColor: 'var(--v-border)' }}>
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          ...
          style={{ gridTemplateColumns: '120px 1fr 110px 120px 130px 100px 32px' }}
        >
          {Array.from({ length: 6 }).map((_, j) => (
            <div
              key={j}
              className="v-skeleton h-4 rounded"
              style={{ width: `${60 + j * 8}%`, maxWidth: 120 }}
            />
          ))}
          <div />
        </div>
...
function LedgerEmpty() {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-2">
      ...
      <p className="text-[14px] font-medium" style={{ color: 'var(--v-text-secondary)' }}>
