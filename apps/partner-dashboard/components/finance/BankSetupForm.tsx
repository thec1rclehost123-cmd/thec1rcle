...
import {
    CheckCircle2, Loader2, ArrowRight, Upload, X, AlertCircle,
} from "lucide-react";
...
export function BankSetupForm({
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4" onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }}>
            {error && (
                <div className="flex items-center gap-2 rounded-[18px] px-4 py-3 text-[13px]" style={{ background: "rgba(239,68,68,0.08)", color: "#fca5a5" }}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Confirm Account Number" error={numbersMismatch ? "Account numbers don't match" : undefined}>
                    <input
                        type="text"
                        value={confirmNumber}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Account Type">
                    <select
                        ...
                        style={{ background: inputBg, border: inputBorder, color: accountType ? textPrimary : textMuted }}
                    >
            ...
            {bankName && (
                <div className="flex items-center gap-2 rounded-[18px] px-4 py-3" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)" }}>
                    <CheckCircle2 size={14} style={{ color: "#34D399" }} />
                    <span className="text-[13px] font-medium" style={{ color: "#6ee7b7" }}>{bankName} — {branch}</span>
                </div>
            )}
...
function Field({ label, children, error }: { label: string; children: React.ReactNode; error?: string }) {
    return (
        <div>
            ...
            {children}
            {error && <p className="text-[11px] mt-1.5" style={{ color: "#fca5a5" }}>{error}</p>}
        </div>
    );
}
...
function ChequeUpload({
    value, onChange, getAuthToken,
}: {
    value: string | null; onChange: (url: string | null) => void; getAuthToken?: () => Promise<string>;
}) {
    ...
    if (value) {
        return (
            <div className="flex items-center gap-3 rounded-[18px] px-5 py-4" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)" }}>
                <CheckCircle2 size={16} style={{ color: "#34D399" }} />
                <span className="text-[13px] font-medium flex-1" style={{ color: "#6ee7b7" }}>Cheque uploaded</span>
                <button type="button" onClick={() => { onChange(null); setUploadError(""); }} className="p-1 rounded-lg hover:bg-red-500/20 transition-colors" style={{ color: textMuted }}>
                    <X size={14} />
                </button>
            </div>
        );
    ...
    if (uploading) {
        return (
            <div className="rounded-[18px] px-5 py-4" style={{ background: inputBg, border: inputBorder }}>
                <div className="flex items-center gap-2 mb-2">
                    <Loader2 size={14} className="animate-spin" style={{ color: textMuted }} />
                    <span className="text-[12px]" style={{ color: textMuted }}>Uploading… {progress}%</span>
                </div>
                <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, background: "rgba(255,255,255,0.5)" }} />
                </div>
            </div>
        );
    }
