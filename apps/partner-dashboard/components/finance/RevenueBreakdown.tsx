...
export function RevenueBreakdown({ items, loading = false, grossRevenue, className, layout = "grid" }: RevenueBreakdownProps) {
    if (loading) {
        const skeletonClasses = layout === "grid"
...
function BreakdownListItem({ item }: { item: RevenueBreakdownItem }) {
    const isOut = ["refund", "chargeback", "processor_fee", "platform_fee", "payout_initiated"].includes(item.category);
    ...
    if (item.ledgerHref) return <Link href={item.ledgerHref} className="block">{content}</Link>;
    if (item.analyticsHref) return <Link href={item.analyticsHref} className="block">{content}</Link>;
    return content;
}
    ...
    const tile = (
        ...
        >
            ...
            {item.trend.value !== "—" && (
                ...
                >
                    {isPositive && "↑ "}{isNegative && "↓ "}{item.trend.value} vs prev
                </span>
            )}
