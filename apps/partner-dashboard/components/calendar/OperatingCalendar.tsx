...
export function OperatingCalendar() {
  ...
  const month =
    parseInt(currentDate.toLocaleString("en-US", { month: "numeric", timeZone: "Asia/Kolkata" })) -
    1;
  ...
  const fetchCalendar = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/venue/calendar?${idParam}&view=operating&startDate=${startStr}&endDate=${endStr}`,
        {
  ...
  const stats = useMemo(() => {
    ...
    const pending = calendarData.filter(
      (d) => d.state === "PENDING" || d.stats?.pendingSlots > 0,
    ).length;
        ...
        <div className="flex items-center gap-10">
          ...
          <div className="hidden lg:flex items-center gap-8 pl-10 border-l border-[rgba(255,255,255,0.06)]">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-1">
                {cleanJargon("confirmed")}
              </span>
          ...
          <div className="glass-panel overflow-hidden border border-[var(--border-strong)] shadow-sm rounded-[32px]">
            <div className="grid grid-cols-7 border-b border-[var(--border-subtle)] bg-[var(--surface-secondary)]">
              {DAYS.map((d) => (
                <div
                  key={d}
                  className="py-4 text-center text-[10px] font-black text-[var(--text-tertiary)] uppercase tracking-[0.2em]"
                >
                  {d}
                </div>
              ))}
              ...
              {calendarGrid.map((cell, idx) => {
                if (!cell) {
                  return (
                    <div
                      key={`empty-${idx}`}
                      className="aspect-[1.15/1] bg-[#F9F9FB] border-r border-b border-[rgba(0,0,0,0.04)]"
                    />
                  );
                }
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between relative z-10">
                      <span
                        className={`text-[13px] font-black tabular-nums ${isToday ? "text-rose-500 animate-pulse" : "text-[var(--text-tertiary)] group-hover:text-[var(--text-primary)]"}`}
                      >
                        {cell.day}
                      </span>
                      {isToday && (
                        <div className="w-1 h-1 rounded-full bg-iris shadow-[0_0_5px_#F44A22]" />
                      )}
                    </div>
                      ...
                      <div className="mt-2 space-y-1 relative z-10">
                        {cell.events.slice(0, 2).map((e: any) => {
                          ...
                          return (
                            <div
                              key={e.id}
                              className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tight truncate border ${
                                isConfirmed
          ...
          <div className="glass-panel min-h-[600px] flex flex-col overflow-hidden">
            {selectedDateStr ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-10 text-center space-y-4">
                ...
                <div>
                  <h3 className="text-[12px] font-black text-[#1D1D1F] uppercase tracking-[0.2em]">
                    System Idle
                  </h3>
    ...
    <div className="h-full flex flex-col bg-transparent select-none">
      ...
      <div className="px-8 py-6 border-b border-[var(--border-subtle)] flex items-start justify-between bg-[var(--surface-secondary)]">
        <div>
          ...
          <p className="text-[10px] font-black text-[var(--text-tertiary)] uppercase tracking-widest mt-1">
            {formattedDate} — SYSTEM_READY
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar scrollbar-hide">
        ...
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <p className="text-[10px] font-black text-[var(--text-tertiary)] uppercase tracking-[0.2em]">
              Operational Timeline
            </p>
            {isBlockingMode && (
              <span className="text-[9px] font-black text-rose-500 animate-pulse uppercase tracking-widest">
                Awaiting Range Selection
              </span>
            )}
          </div>
          ...
          <div className="relative min-h-[700px] flex">
            ...
            <div className="w-16 flex flex-col justify-between pr-4 py-0">
              {hours.map((h, i) => (
                <span
                  key={i}
                  className="text-[9px] font-bold text-[var(--text-tertiary)] opacity-40 uppercase tracking-widest h-0 flex items-center justify-end tabular-nums"
                >
                  {h.label}
                </span>
              ))}
                ...
                {data?.events?.map((e: any) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`absolute inset-x-0 mr-4 rounded-xl p-5 flex flex-col border transition-all hover:translate-x-1 ${
                        isAnonymized
                          ? "bg-[#F2F2F7] border-[rgba(0,0,0,0.06)] text-[#86868B]"
                          : isConfirmed
                      ...
                      <div className="flex gap-4 h-full pointer-events-auto">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col">
                          <div className="flex items-center justify-between mb-2">
                            <span
                              className={`text-[10px] font-black uppercase tracking-widest tabular-nums ${isConfirmed ? "text-emerald-500/80" : "text-[var(--text-tertiary)]"}`}
                            >
                              {e.startTime} <span className="mx-1 opacity-50">/</span> {e.endTime}
                            </span>
                            {isConfirmed && !isAnonymized && (
                              <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.4)]" />
                            )}
                            {isAnonymized && (
                              <Lock className="h-3 w-3 opacity-30 text-[var(--text-tertiary)]" />
                            )}
                          </div>
                          ...
                          {!isAnonymized && (
                            <div className="flex items-center gap-2 mt-auto">
                              ...
                              <span className="text-[10px] font-black text-[var(--text-tertiary)] uppercase tracking-widest truncate">
                                {e.host || "Booking"}
                              </span>
                            </div>
                          )}
                ...
                {data?.slots?.map((s: any) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded bg-rose-500/10 text-[9px] font-black text-rose-500 uppercase tracking-widest border border-rose-500/20">
                        PENDING REQUEST
                      </span>
                    </div>
                    ...
                    <p className="text-[9px] text-rose-500 font-black uppercase tracking-[0.2em] mt-auto animate-pulse">
                      ACTION_REQUIRED
                    </p>
                  </div>
                ))}
                ...
                {(data?.block || isDragging || (isBlockingMode && startTime && endTime)) && (
                  ...
                  >
                    <div className="text-center p-6 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl">
                      <Lock
                        className={`h-6 w-6 mx-auto mb-3 ${isBlockingMode ? "text-white" : "text-white/30"}`}
                      />
                      ...
                      {isBlockingMode && (
                        <span className="text-[11px] font-black text-iris uppercase tracking-widest tabular-nums">
                          {startTime} — {endTime}
                        </span>
                      )}
                    </div>
          ...
          {!isBlockingMode && needsAttention && (
            <div className="mt-12 space-y-4">
              <p className="text-[10px] font-black text-[#86868B] uppercase tracking-[0.2em]">
                  ...
                  <div key={s.id} className="glass-panel p-6 border-iris/20 bg-iris/5 rounded-2xl">
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] font-black text-[var(--text-tertiary)] uppercase tracking-widest mt-1 tabular-nums">
                          {s.startTime} — {s.endTime}
                        </p>
                      </div>
                    </div>
      ...
      {role === "venue" && isBlockingMode && (
        <div className="px-8 py-8 border-t border-[var(--border-subtle)] bg-[var(--surface-secondary)] space-y-6 animate-in slide-in-from-bottom-4 duration-300">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-black text-[var(--text-primary)] uppercase tracking-[0.2em]">
              Manual Block Settings
            </h3>
          ...
          <div className="space-y-6">
            <div className="space-y-1.5 text-left">
              <label className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest ml-1">
                Reason
              </label>
              <input
                type="text"
