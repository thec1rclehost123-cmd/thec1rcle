...
export default function TierProgressBar({
  ...
  if (variant === 'inline') {
    return (
      <div className="flex items-center gap-2">
        <span
          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold ${style.badge}`}
        >
  ...
  if (variant === 'compact') {
    return (
      <div
        className={`p-4 rounded-2xl bg-surface-elevated border border-border-default ${style.glow}`}
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div
              className={`w-8 h-8 rounded-xl bg-gradient-to-br ${style.gradient} flex items-center justify-center`}
            >
  ...
  return (
    <div
      className={`p-8 rounded-[2rem] bg-surface-elevated border border-border-default ${style.glow}`}
    >
      {/* Current Tier Header */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary font-semibold mt-0.5">
              Earning <span className="text-text-primary">{currentTier.rate}%</span> commission per
              sale
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-8">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-text-secondary">
              {currentTier.label} ({currentTier.rate}%)
            </span>
          ...
          <p className="text-sm text-text-tertiary font-medium">
            <span className="font-bold text-text-primary">
              {salesToNext} more sale{salesToNext !== 1 ? 's' : ''}
            </span>{' '}
            to unlock <span className="font-bold text-text-primary">{nextTier.rate}%</span>{' '}
            commission
          </p>
        </div>
      )}
