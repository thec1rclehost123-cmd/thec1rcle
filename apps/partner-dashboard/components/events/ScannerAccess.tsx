...
import { Plus, Copy, Trash2, QrCode, Users, Smartphone, Check, RefreshCw } from 'lucide-react';
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  {/* Code */}
                  <div className="font-mono text-xl font-bold text-text-primary">{code.code}</div>
              ...
              {code.stats && !code.isRevoked && (
                <div className="flex items-center gap-6 mt-3 pt-3 border-t border-slate-700/50">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-text-tertiary">{code.stats.scansCount} scans</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-text-tertiary">
                ...
                {code.lastUsedAt && <span>Last: {new Date(code.lastUsedAt).toLocaleString()}</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-surface-tertiary rounded-2xl p-6 w-full max-w-md border border-slate-700">
            <h3 className="text-xl font-semibold text-text-primary mb-4">Generate Scanner Code</h3>

            {/* Type Selection */}
            <div className="mb-4">
              <label className="block text-sm text-text-tertiary mb-2">Access Type</label>
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-xs text-text-tertiary mt-1">Scan tickets + Door entry</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-xs text-text-tertiary mt-1">Ticket scanning only</div>
                </button>
              </div>
            </div>
