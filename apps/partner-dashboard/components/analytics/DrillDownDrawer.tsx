  ...
  return (
    ...
    >
      <div>
        ...
        {sub && (
          <p className="text-[10px] mt-0.5" style={{ color: 'var(--v-text-muted)' }}>
            {sub}
          </p>
        )}
      </div>
  ...
  return (
    <>
      ...
      <Section title="Revenue by source">
        <HorizRankBars
          rows={ranks(bySource, 'var(--v-chart-1)')}
          maxValue={br.grossTicketRevenue}
        />
      </Section>
      <Section title="Key figures">
        ...
        <StatRow
          label="Rev / Available Spot"
          value={cur(dr.revenuePerAvailableSpot)}
          sub="Total gross ÷ venue capacity"
        />
        <StatRow
          label="High-yield window"
          value={dr.highYieldWindow || '—'}
          sub="Hour bracket with highest revenue density"
          color="var(--v-success)"
        />
      </Section>
    </>
  ...
  return (
    <>
        ...
        <div className="space-y-2">
          {waterfall.map((row, i) => (
            <div key={i} className="flex items-center gap-3">
              <div
                className="h-7 rounded-lg flex items-center px-3 text-[12px] font-bold tabular-nums transition-all"
                style={{
      ...
      <Section title="Efficiency">
        ...
        <StatRow
          label="Revenue Lost to No-shows"
          value={cur(dr.revenueLostToNoShows)}
          color="var(--v-error)"
        />
        <StatRow
          label="Revenue Leakage (comps)"
          value={cur(dr.revenueLeakageFromComps)}
          color="var(--v-warning)"
        />
...
function TicketsSoldDrill({ data }: { data: AnalyticsV2 }) {
  ...
  const byEvent = data.eventScorecards.slice(0, 6).map((e) => ({
    label: e.title,
    value: e.attendance,
    subValue: `${pct(e.sellThrough)} sell-through`,
  }));
  return (
    <>
    ...
    <>
      ...
      <Section title="Door metrics">
        ...
        <StatRow
          ...
          value={pct(ops.capacityUtilization)}
          color="var(--v-chart-1)"
        />
      </Section>
    </>
  ...
  return (
    <>
      <Section title="Capacity breakdown">
        ...
        <StatRow
          label="Unused spots"
          value={num(cy.totalCapacity - cy.soldCapacity)}
          color="var(--v-error)"
        />
        <StatRow
          label="Unused cost est."
          value={cur(cy.unusedCapacityCostEstimate)}
          color="var(--v-error)"
      ...
      <Section title="Yield windows">
        <StatRow
          label="Best pricing lift window"
          value={cy.bestPricingLiftWindow || '—'}
          color="var(--v-success)"
        />
        <StatRow
          label="Best push notification time"
          value={cy.bestPushNotificationWindow || '—'}
          color="var(--v-chart-2)"
        />
        <StatRow
          label="Best inventory release time"
          value={cy.bestInventoryReleaseWindow || '—'}
          color="var(--v-chart-3)"
        />
...
function AvgTicketDrill({ data }: { data: AnalyticsV2 }) {
  const byType = data.revenueIntelligence.series.revenueByTicketType.map((t) => ({
    ...
    value:
      t.pct > 0
        ? Math.round(t.revenue / Math.max(1, (t.pct / 100) * data.executive.ticketsSold.raw))
        : 0,
  ...
  return (
    <>
      ...
      <Section title="Price metrics">
        ...
        <StatRow
          label="Avg Rev / Attendee"
          value={data.executive.avgRevenuePerAttendee.formatted}
          sub="Includes walk-ins, tables, and cover charges"
        />
        <StatRow
          label="Rev / Available Spot"
          value={cur(data.revenueIntelligence.derived.revenuePerAvailableSpot)}
...
function GuestlistDrill({ data }: { data: AnalyticsV2 }) {
  ...
  const scorecards = data.eventScorecards.slice(0, 6).map((e) => ({
    ...
    subValue:
      e.guestlistArrivals > 0
        ? `${pct((e.guestlistArrivals / Math.max(1, e.guestlistSignups)) * 100)} arrived`
        : '—',
  }));
  return (
    <>
      <Section title="Guestlist conversion">
        ...
        <StatRow
          ...
          value={ex.firstTimeGuestRate.formatted}
          color="var(--v-chart-2)"
        />
        <StatRow
          label="Promoter-driven signups"
          value={ex.promoterDrivenSales.formatted}
          color="var(--v-chart-3)"
        />
      </Section>
    ...
    <>
      <Section title="Refund reasons">
        {reasons.length > 0 ? (
          ...
          <p className="text-[12px] py-2" style={{ color: 'var(--v-text-muted)' }}>
            No refund data yet.
          </p>
        )}
      </Section>
      <Section title="Financial impact">
        ...
        <StatRow
          ...
          value={cur(data.revenueIntelligence.derived.refundAdjustedRevenue)}
        />
        <StatRow
          label="Discount value given"
          value={cur(br.discountValue)}
          color="var(--v-warning)"
        />
...
function NoShowDrill({ data }: { data: AnalyticsV2 }) {
  const sorted = [...data.eventScorecards]
    ...
    .map((e) => ({
      ...
      value: e.noShows,
      subValue:
        e.attendance > 0 ? pct((e.noShows / Math.max(1, e.attendance + e.noShows)) * 100) : '—',
    }));
  return (
    <>
      ...
      <Section title="Impact">
        <StatRow
          label="Revenue lost to no-shows"
          value={cur(data.revenueIntelligence.derived.revenueLostToNoShows)}
          color="var(--v-error)"
        />
        <StatRow
          label="Overall no-show rate"
          value={data.executive.noShowRate.formatted}
          color="var(--v-warning)"
        />
        <StatRow
          label="Purchase-to-arrival rate"
          value={pct(data.conversionFunnel.purchaseToArrival)}
        />
        <StatRow
          label="Guestlist arrival rate"
          value={pct(data.conversionFunnel.guestlistToArrival)}
        />
      </Section>
    </>
  );
  ...
  return (
    <>
      ...
      <Section title="Loyalty metrics">
        <StatRow
          label="Return rate after 1st visit"
          value={pct(ret.returnRateAfterFirst * 100)}
          color="var(--v-success)"
        />
        <StatRow
          label="Avg days between visits"
          value={`${ret.avgDaysBetweenVisits.toFixed(0)}d`}
        />
        <StatRow
          label="Avg events to become a regular"
          value={`${ret.avgTimeToBecomingRegular.toFixed(1)} events`}
        />
        <StatRow label="Avg guest LTV" value={cur(ret.avgLTV)} color="var(--v-chart-1)" />
        <StatRow
          label="Best retention channel"
          value={ret.topRetentionChannels[0] || '—'}
          color="var(--v-success)"
        />
      </Section>
    </>
  ...
  return (
    <>
      <Section title="Promoter-driven revenue">
        <StatRow
          label="Total promoter-driven"
          value={data.executive.promoterDrivenSales.formatted}
          color="var(--v-chart-1)"
        />
        <StatRow
          label="Direct sales"
          value={data.executive.directSales.formatted}
          color="var(--v-chart-2)"
        />
        <StatRow
          label="Promoter payout owed"
          value={cur(data.revenueIntelligence.breakdown.promoterPayoutExposure)}
          color="var(--v-warning)"
        />
      </Section>
  ...
  return (
    <>
      <Section title="Payout breakdown">
        ...
        <StatRow
          label="Processing"
          value={cur(data.finance.processingPayout)}
          color="var(--v-chart-2)"
        />
        <StatRow
          label="Completed"
          value={cur(data.finance.completedPayout)}
          color="var(--v-success)"
        />
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <span
                    ...
                    style={{
                      background:
                        p.status === 'completed' ? 'var(--v-success-bg)' : 'var(--v-warning-bg)',
...
const DRILL_META: Record<
  string,
  { label: string; description: string; valueFn: (d: AnalyticsV2) => string }
> = {
  'gross-revenue': {
    label: 'Gross Revenue',
    description: 'Total collected before deductions',
    valueFn: (d) => d.executive.grossRevenue.formatted,
  },
  'net-revenue': {
    label: 'Net Revenue',
    description: 'After fees, refunds & payouts',
    valueFn: (d) => d.executive.netRevenue.formatted,
  },
  'tickets-sold': {
    label: 'Tickets Sold',
    description: 'Total entitlements issued',
    valueFn: (d) => d.executive.ticketsSold.formatted,
  },
  'check-ins': {
    label: 'Check-ins',
    description: 'Confirmed door entries',
    valueFn: (d) => d.executive.confirmedCheckins.formatted,
  },
  'sell-through': {
    label: 'Sell-through Rate',
    description: 'Tickets sold ÷ total capacity',
    valueFn: (d) => d.executive.sellThroughRate.formatted,
  },
  occupancy: {
    label: 'Occupancy Rate',
    description: 'Check-ins ÷ total capacity',
    valueFn: (d) => d.executive.occupancyRate.formatted,
  },
  'avg-ticket': {
    label: 'Avg Ticket Price',
    description: 'Gross revenue ÷ tickets sold',
    valueFn: (d) => d.executive.avgTicketPrice.formatted,
  },
  'rev-attendee': {
    label: 'Rev / Attendee',
    description: 'Total revenue ÷ check-ins',
    valueFn: (d) => d.executive.avgRevenuePerAttendee.formatted,
  },
  guestlist: {
    label: 'Guestlist Signups',
    description: 'Total guestlist registrations',
    valueFn: (d) => d.executive.guestlistSignups.formatted,
  },
  'refund-rate': {
    label: 'Refund Rate',
    description: 'Refunds ÷ total tickets sold',
    valueFn: (d) => d.executive.refundRate.formatted,
  },
  'no-show': {
    label: 'No-show Rate',
    description: 'Tickets issued but never scanned',
    valueFn: (d) => d.executive.noShowRate.formatted,
  },
  'repeat-guest': {
    label: 'Repeat Guest Rate',
    description: 'Returning guests ÷ total guests',
    valueFn: (d) => d.executive.repeatGuestRate.formatted,
  },
  'promoter-sales': {
    label: 'Promoter-driven',
    description: 'Revenue attributed to promoters',
    valueFn: (d) => d.executive.promoterDrivenSales.formatted,
  },
  'pending-payout': {
    label: 'Pending Payout',
    description: 'Earnings queued for settlement',
    valueFn: (d) => d.executive.pendingPayout.formatted,
  },
};
...
function DrillContent({ drillKey, data }: { drillKey: string; data: AnalyticsV2 }) {
  switch (drillKey) {
    case 'gross-revenue':
      return <GrossRevenueDrill data={data} />;
    case 'net-revenue':
      return <NetRevenueDrill data={data} />;
    case 'tickets-sold':
      return <TicketsSoldDrill data={data} />;
    case 'check-ins':
      return <CheckInsDrill data={data} />;
    case 'sell-through':
    case 'occupancy':
      return <SellThroughDrill data={data} />;
    case 'avg-ticket':
    case 'rev-attendee':
      return <AvgTicketDrill data={data} />;
    case 'guestlist':
      return <GuestlistDrill data={data} />;
    case 'refund-rate':
      return <RefundRateDrill data={data} />;
    case 'no-show':
      return <NoShowDrill data={data} />;
    case 'repeat-guest':
      return <RepeatGuestDrill data={data} />;
    case 'promoter-sales':
      return <PromoterSalesDrill data={data} />;
    case 'pending-payout':
      return <PendingPayoutDrill data={data} />;
    default:
      return (
      ...
      {drillKey && (
        <>
          ...
          >
            ...
            {/* Footer */}
            <div className="px-6 py-4 shrink-0" style={{ borderTop: '1px solid var(--v-border)' }}>
              <p
