...
function ChartSkeleton({ height = 240 }: { height?: number }) {
  return (
    <div
      ...
      style={{ height, background: 'var(--v-elevated)' }}
    />
  );
}
...
export default function VenueCrossEventClient() {
  ...
  const overviewQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(`/api/partners/venues/analytics/overview?venueId=${venueId}&range=${range}`),
  ...
  const tsRevenueQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(
        `/api/partners/venues/analytics/time-series?venueId=${venueId}&range=${RANGES.find((r) => r.id === range)?.tsRange}&metric=revenue`,
      ),
  ...
  const tsTicketsQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(
        `/api/partners/venues/analytics/time-series?venueId=${venueId}&range=${RANGES.find((r) => r.id === range)?.tsRange}&metric=tickets`,
      ),
    enabled: Boolean(venueId && activeTab === 'revenue'),
    staleTime: Infinity,
    refetchOnWindowFocus: false,
  ...
  const audienceQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(`/api/partners/venues/analytics/audience?venueId=${venueId}&range=${range}`),
  ...
  const hostsQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(`/api/partners/venues/analytics/partners?venueId=${venueId}&range=${range}`),
  ...
  const opsQuery = useQuery({
    ...
    queryFn: () =>
      authedFetch(`/api/partners/venues/analytics/ops?venueId=${venueId}&range=${range}`),
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <KpiCard
                label="Total Revenue"
                value={formatINR(
                  rawOverview.totalRevenue ?? normOverview.executive?.grossRevenue?.raw ?? 0,
                )}
                icon={DollarSign}
                accent="#34d399"
              ...
              <KpiCard
                label="Tickets Sold"
                value={(
                  rawOverview.totalTicketsSold ??
                  normOverview.executive?.ticketsSold?.raw ??
                  0
              ...
              <KpiCard
                label="Check-ins"
                value={(
                  rawOverview.totalCheckIns ??
                  normOverview.executive?.confirmedCheckins?.raw ??
                  0
                ).toLocaleString('en-IN')}
                icon={CheckCircle2}
                accent="#f59e0b"
              ...
              <KpiCard
                label="Events Hosted"
                value={(rawOverview.totalEvents ?? 0).toLocaleString('en-IN')}
                icon={BarChart3}
                accent="#f44a22"
                loading={overviewQuery.isLoading}
              />
        ...
        {activeTab === 'revenue' && (
          <div className="space-y-6">
            <RefreshBar
              ...
              onRefresh={() => {
                tsRevenueQuery.refetch();
                tsTicketsQuery.refetch();
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  label: `Total Revenue · ${RANGES.find((r) => r.id === range)?.label}`,
                  value: formatINR(revTotal),
                  glow: 'rgba(52,211,153,0.22)',
                  border: 'rgba(52,211,153,0.22)',
                  accent: '#34d399',
                },
                {
                  label: `Tickets Sold · ${RANGES.find((r) => r.id === range)?.label}`,
                  value: tickTotal.toLocaleString('en-IN'),
                ...
                <div
                  key={card.label}
                  style={{ position: 'relative', overflow: 'hidden', borderRadius: '1.1rem' }}
                >
                  <div
                    style={{
                      position: 'absolute',
            ...
            <SectionCard
              ...
              subtitle="Daily ticket volume over the selected period"
            >
              {tsTicketsQuery.isLoading ? (
        ...
        {activeTab === 'audience' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <KpiCard
                label="Total Guests"
                value={(audience.totalGuests ?? 0).toLocaleString('en-IN')}
                icon={Users}
                accent="#818cf8"
                loading={audienceQuery.isLoading}
              />
              <KpiCard
                label="Repeat Guests"
                value={`${audience.repeatGuestPct ?? 0}%`}
                icon={Repeat}
                accent="#34d399"
                loading={audienceQuery.isLoading}
              />
              <KpiCard
                label="Avg Age"
                value={audience.avgAge ? `${audience.avgAge}y` : '—'}
                icon={Star}
                accent="#f59e0b"
                loading={audienceQuery.isLoading}
              />
              <KpiCard
                label="Top City"
                value={audience.topCities?.[0]?.city ?? '—'}
                icon={Building2}
                accent="#f44a22"
                loading={audienceQuery.isLoading}
              />
                            ...
                            <div key={d.name} className="flex items-center justify-between gap-3">
                              <div className="flex items-center gap-2">
                                <div
                                  ...
                                  style={{ background: ['#818cf8', '#f472b6', '#a78bfa'][i] }}
                                />
                                <span
                  ...
                  {(audience.topCities as any[]).slice(0, 8).map((c: any, i: number) => {
                    ...
                    return (
                      <div key={c.city} className="flex items-center gap-4">
                        <span
                          className="text-[11px] font-black tabular-nums w-4 shrink-0"
                          style={{ color: 'var(--v-text-tertiary)' }}
                        >
                          {i + 1}
                        </span>
                        <span
                          className="text-[13px] font-semibold w-24 shrink-0"
                          style={{ color: 'var(--v-text-primary)' }}
                        >
                          {c.city}
                        </span>
                        ...
                        >
                          <div
                            ...
                            style={{ width: `${pct}%`, background: 'var(--v-orange)' }}
                          />
                        </div>
                        <span
                          className="text-[12px] font-black tabular-nums w-12 text-right"
                          style={{ color: 'var(--v-text-secondary)' }}
                        >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: 'var(--v-text-primary)' }}
                  >
                    {(audience.repeatVsNew.new ?? 0).toLocaleString()}
                  </p>
                </div>
                <div
                  className="p-5 rounded-2xl"
                ...
                >
                  ...
                  <p className="text-[28px] font-black tabular-nums" style={{ color: '#34d399' }}>
                    {(audience.repeatVsNew.repeat ?? 0).toLocaleString()}
                  </p>
                </div>
              </div>
            )}
        ...
        {activeTab === 'hosts' && (
          <div className="space-y-6">
            <RefreshBar
              ...
              onRefresh={() => {
                hostsQuery.refetch();
                setHostsRefreshedAt(new Date());
              }}
            />
            {/* Host leaderboard */}
            <SectionCard
              title="Host Performance"
              subtitle="Revenue and attendance per host across all events"
            >
                  ...
                  {[...(partners.hosts as any[])]
                    ...
                    .map((host: any, i: number) => {
                      ...
                      const cardStyle = isFirst
                        ? {
                            background:
                              'linear-gradient(135deg, rgba(244,74,34,0.88) 0%, rgba(251,146,60,0.72) 100%)',
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <p style={{ fontSize: 11, color: subColor, marginTop: 1 }}>
                              {host.events ?? 0} events · {(host.tickets ?? 0).toLocaleString()}{' '}
                              tickets
                            </p>
                          </div>
            ...
            {(partners.promoters ?? []).length > 0 && (
              <SectionCard
                ...
                subtitle="Sales and revenue driven by each promoter"
              >
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--v-border)' }}>
                        {['Promoter', 'Sales', 'Revenue', 'Clicks', 'Conv Rate'].map((h) => (
                          <th
                            key={h}
                            className="pb-3 pr-4 text-[10px] font-black uppercase tracking-widest"
                            style={{ color: 'var(--v-text-secondary)' }}
                          >
                            {h}
                          </th>
                    ...
                    <tbody>
                      {(partners.promoters as any[]).map((p: any, i: number) => (
                        ...
                        >
                          <td
                            className="py-3 pr-4 text-[13px] font-semibold"
                            style={{ color: 'var(--v-text-primary)' }}
                          >
                            {p.promoterName || 'Unknown'}
                          </td>
                          <td
                            className="py-3 pr-4 text-[13px] tabular-nums"
                            style={{ color: 'var(--v-text-secondary)' }}
                          >
                          ...
                          >
                            {p.clicks > 0
                              ? `${Math.round(((p.sales ?? 0) / p.clicks) * 100)}%`
                              : '—'}
                          </td>
                        </tr>
                      ))}
        ...
        {activeTab === 'operations' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <KpiCard
                label="Avg Fill Rate"
                value={ops.avgFillRate ? `${Math.round(ops.avgFillRate)}%` : '—'}
                icon={Target}
                accent="#f44a22"
              ...
              <KpiCard
                label="Total Scans"
                value={(ops.totalScans ?? 0).toLocaleString('en-IN')}
                icon={Zap}
                accent="#818cf8"
                loading={opsQuery.isLoading}
              />
              ...
              <KpiCard
                label="Capacity Util."
                value={ops.capacityUtilisation ? `${Math.round(ops.capacityUtilisation)}%` : '—'}
                icon={BarChart3}
                accent="#34d399"
                loading={opsQuery.isLoading}
              />
            ...
            {(ops.dayOfWeekBreakdown ?? []).length > 0 && (
              <SectionCard
                ...
                subtitle="Average fill rate and events per day"
              >
                <Suspense fallback={<ChartSkeleton height={200} />}>
                  <LazyBar
                    data={ops.dayOfWeekBreakdown}
                    xKey="day"
                    yKey="avgFill"
                    color="#f44a22"
                    height={200}
            ...
            {(ops.scanVelocity ?? []).length > 0 && (
              <SectionCard
                ...
                subtitle="Average scans per hour across events"
              >
                <Suspense fallback={<ChartSkeleton height={200} />}>
                  <LazyBar
                    data={ops.scanVelocity}
                    xKey="hour"
                    yKey="scans"
                    color="#818cf8"
                    height={200}
            ...
            {(ops.capacityTimeline ?? []).length > 0 && (
              <SectionCard
                ...
                subtitle="How full each event was, chronologically"
              >
                <Suspense fallback={<ChartSkeleton height={220} />}>
                  <LazyArea
                    data={ops.capacityTimeline}
                    xKey="date"
                    yKey="fillRate"
                    color="#34d399"
                    height={220}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p className="text-[28px] font-black tabular-nums" style={{ color: '#818cf8' }}>
                    {(ops.channelSplit.online ?? 0).toLocaleString()}
                  </p>
                </div>
                <div
                  className="p-5 rounded-2xl"
                ...
                >
                  ...
                  <p className="text-[28px] font-black tabular-nums" style={{ color: '#f59e0b' }}>
                    {(ops.channelSplit.walkIn ?? 0).toLocaleString()}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-6">
            <RefreshBar
              ...
              onRefresh={() => {
                ...
                setStrategyRefreshedAt(new Date());
              }}
            />
              ...
              <div className="space-y-5">
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-[13px] font-bold" style={{ color: 'var(--v-text-primary)' }}>
                      Strategy insights require at least 30 days of event data.
                    </p>
                  </div>
                  <p className="text-[12px]" style={{ color: 'var(--v-text-tertiary)' }}>
                    Once your venue has sufficient event history, AI-powered recommendations will
                    appear here — covering optimal slot timing, host selection, pricing, and
                    capacity planning.
                  </p>
                </div>
              </div>
...
function KpiCard({
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div
          className="h-7 w-20 rounded-lg animate-pulse mb-1"
          style={{ background: 'var(--v-elevated)' }}
        />
      ) : (
        <p
          className="text-[22px] font-black tabular-nums mb-0.5"
          style={{ color: 'var(--v-text-primary)' }}
        >
          {value}
        </p>
...
function SectionCard({
  ...
  return (
    <div
      ...
      style={{ background: 'var(--v-card)', border: '1px solid var(--v-border)' }}
    >
      <p className="text-[13px] font-bold mb-0.5" style={{ color: 'var(--v-text-primary)' }}>
        {title}
      </p>
      {subtitle && (
        <p className="text-[11px] mb-4" style={{ color: 'var(--v-text-tertiary)' }}>
          {subtitle}
        </p>
      )}
      {children}
...
function RecommendationCard({ rec }: { rec: any }) {
  const impactColor =
    rec.impact === 'high' ? '#34d399' : rec.impact === 'medium' ? '#f59e0b' : '#818cf8';
  return (
    <div
    ...
    >
      <div
        ...
        style={{ background: `${impactColor}15`, border: `1px solid ${impactColor}25` }}
      >
      ...
      <div className="flex-1">
        ...
        {rec.action && (
          <p className="text-[11px] font-semibold mt-2" style={{ color: impactColor }}>
            {rec.action}
          </p>
        )}
      </div>
...
function EmptyChart({ label }: { label: string }) {
  return (
    ...
    >
      ...
      <p className="text-[12px] font-semibold" style={{ color: 'var(--v-text-tertiary)' }}>
        {label}
      </p>
    </div>
  );
...
function EmptyState({ label }: { label: string }) {
  return (
    <div
      className="py-16 flex flex-col items-center gap-3 rounded-2xl"
      style={{ border: '1px dashed var(--v-border)' }}
    >
      <Info className="w-8 h-8" style={{ color: 'var(--v-text-tertiary)' }} />
      <p className="text-[13px] font-semibold" style={{ color: 'var(--v-text-tertiary)' }}>
        {label}
      </p>
    </div>
  );
