      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="h-[420px] bg-slate-50/50 rounded-[2.5rem] animate-pulse border border-slate-100"
            />
          ))}
        </div>
      ) : partners.length === 0 ? (
        <div className="py-24 bg-white/50 backdrop-blur-sm rounded-[3rem] border border-dashed border-slate-200 flex flex-col items-center text-center px-10">
          ...
          <p className="text-slate-500 font-medium mt-2 max-w-xs">
            We couldn't find any partners matching those filters right now.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[8px]">
            ...
            >
              ...
              <p className="text-slate-500 font-medium leading-relaxed mb-8 px-4">
                ...
                <span className="text-slate-900 font-bold">{selectedPartner?.name}</span> is on its
                way.
              </p>
              <button
    ...
    >
      ...
      <div className="relative h-52 bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-20 flex gap-2">
          ...
          <div
            className={`px-3.5 py-1.5 backdrop-blur-md rounded-full text-[11px] font-bold tracking-tight text-white flex items-center gap-1.5 border border-white/20 shadow-sm ${
              partner.type === "host"
                ? "bg-purple-500/80"
                : partner.type === "venue"
                  ? "bg-blue-500/80"
