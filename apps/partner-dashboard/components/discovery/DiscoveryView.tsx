    ...
    try {
      ...
      const res = await fetch('/api/discovery', {
        ...
        body: JSON.stringify({
          ...
          requesterType: role,
          requesterName:
            profile?.activeMembership?.partnerName || profile?.displayName || undefined,
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="h-80 rounded-[32px] animate-pulse bg-[var(--v-card)] border border-[var(--v-border)]"
            />
          ))}
        </div>
      ) : partners.length === 0 ? (
      ...
      <AnimatePresence>
        {showSuccessModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            ...
            >
              <div
                className="w-20 h-20 rounded-[24px] flex items-center justify-center mx-auto mb-7"
              ...
              <p className="text-[var(--v-text-tertiary)] font-bold leading-relaxed mb-8 px-2">
                Your partnership request to{' '}
                <span className="text-text-primary">{selectedPartner?.name}</span> is now pending
                their approval.
              </p>
              <VenueActionButton
                variant="primary"
...
function PartnerCard({
  ...
  const typeColor =
    partner.type === 'venue'
      ? 'var(--v-info)'
      : partner.type === 'promoter'
        ? 'var(--v-orange)'
        : 'var(--v-success)';
  const TypeIcon =
    partner.type === 'venue' ? Building2 : partner.type === 'host' ? UserCircle : Users;
    ...
    >
      ...
      <div className="h-44 bg-surface-tertiary relative overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2">
          {partner.city && (
            <div
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-widest border border-white/10 backdrop-blur-md"
          ...
          <div
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-widest border backdrop-blur-md"
            style={{
        ...
        {partner.isVerified && (
          <div
            className="absolute top-4 right-4 p-2 rounded-xl border border-white/10 backdrop-blur-md"
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="text-[17px] font-black text-text-primary tracking-tight truncate mb-1">
          {partner.name}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="flex items-center gap-2 p-3 rounded-2xl bg-surface-tertiary border border-border-subtle">
            ...
            <span className="text-[12px] font-black text-text-primary tabular-nums">
        ...
        {partner.connectionStatus === 'approved' ? (
          <div
            className="flex items-center justify-center gap-2 h-11 rounded-2xl text-[12px] font-black uppercase tracking-widest"
            style={{
              background: 'var(--v-success-bg)',
              color: 'var(--v-success)',
              border: '1px solid var(--v-success)20',
            }}
          >
