...
const TYPE_CONFIG: Record<string, { icon: React.ReactNode; bg: string }> = {
    ...
    table_reservation: { icon: <Calendar   className="w-4 h-4 text-indigo-500" />,    bg: "bg-indigo-500/10" },
...
export function NotificationCenter() {
    ...
    const playNotificationSound = useCallback(() => {
        try {
            ...
            const play = (freq: number, startAt: number) => {
                ...
                osc.type = "sine"; osc.frequency.setValueAtTime(freq, ctx.currentTime + startAt);
                ...
                osc.start(ctx.currentTime + startAt); osc.stop(ctx.currentTime + startAt + 0.4);
            };
            play(880, 0); play(1108, 0.18);
        } catch { /* silent */ }
    }, []);
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            const incoming: Notification[] = (data.notifications || []).map((n: any) => normalizeNotification(n, partnerType));
    ...
    const handleQuickAction = async (notif: Notification, action: 'approve' | 'reject', e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    const handleNotifClick = (notif: Notification) => {
        const rolePrefix = partnerType === 'venue' ? 'venue' : partnerType === 'host' ? 'host' : 'promoter';
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                {unreadCount > 0 && <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-c1rcle-orange rounded-full animate-pulse" />}
            </button>
                            ...
                            <div className="px-5 pt-5 pb-4 border-b border-border-subtle">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {unreadCount > 0 && (
                                            <button onClick={handleMarkAllRead} className="text-[10px] font-black text-c1rcle-orange uppercase tracking-widest">Mark all read</button>
                                        )}
                                        <button onClick={fetchNotifications} className="p-1.5 rounded-lg hover:bg-surface-base"><RefreshCw className={`w-3.5 h-3.5 ${loading ? "animate-spin" : ""}`} /></button>
                                    </div>
                                </div>
                            ...
                            <div className="max-h-[440px] overflow-y-auto custom-scrollbar divide-y divide-white/5 bg-surface-elevated">
                                {loading && filtered.length === 0 ? (
                                    <div className="py-16 text-center">
                                        ...
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-text-tertiary">Loading...</p>
                                    </div>
                                ) : filtered.length === 0 ? (
                                    <div className="py-16 text-center opacity-40">
                                        <Bell className="w-8 h-8 mx-auto mb-2 text-text-placeholder" />
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-text-tertiary">All caught up</p>
                                    </div>
                                ) : (
                                    filtered.map(notif => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {!notif.isRead && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-c1rcle-orange rounded-r-full" />}
                                                <div className="flex gap-4">
                                                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${cfg.bg}`}>{cfg.icon}</div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-start justify-between gap-2 mb-0.5">
                                                            <h4 className="text-[12px] font-bold text-text-primary truncate">{notif.title}</h4>
                                                            <span className="text-[10px] text-text-tertiary tabular-nums shrink-0">{notif.timestamp}</span>
                                                        </div>
                                                        ...
                                                        {notif.actionable && partnerType === "venue" && (
                                                            <div className="flex gap-2 mt-3">
                                                                ...
                                                                >
                                                                    {actionLoading === `${notif.id}_approve` ? <Loader2 className="w-3 h-3 animate-spin"/> : <CheckCircle2 className="w-3.4 h-3.5"/>}
                                                                    Approve
                                                                </button>
                                                                <button
                                                                ...
                                                                >
                                                                    {actionLoading === `${notif.id}_reject` ? <Loader2 className="w-3 h-3 animate-spin"/> : <AlertCircle className="w-3.5 h-3.5"/>}
                                                                    Reject
                                                                </button>
                                                            </div>
                                                        )}
