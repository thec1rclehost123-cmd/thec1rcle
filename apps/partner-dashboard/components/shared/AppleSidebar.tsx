...
export function AppleSidebar({
  ...
  const isActive = (path: string) => {
    ...
    if (currentTab === 'calendar') {
      ...
      if (pathname.includes('/events') && (path.endsWith('/events') || path.endsWith('/events/')))
        return false;
    }
  ...
  const visibleSections = menuSections
    .map((section) => ({
      ...
      items: section.items.filter(
        (item) => !item.minPlan || PLAN_HIERARCHY[item.minPlan.toLowerCase()] <= currentPlanLevel,
      ),
    }))
  ...
  return (
    <aside
      className={`relative ${isCollapsed ? 'w-[80px]' : 'w-[280px]'} bg-[var(--sidebar-bg)] border-r border-[var(--sidebar-border)] flex flex-col h-full z-50 shrink-0 transition-all duration-300 ease-in-out`}
    >
      {/* Brand Header */}
      <div className={`p-6 ${isCollapsed ? 'px-4' : 'p-7'}`}>
        <div className="flex items-center gap-4">
          ...
          {!isCollapsed && (
            ...
            >
              <h1 className="text-[17px] font-bold text-text-primary tracking-tight leading-tight uppercase">
                THE C1RCLE
              </h1>
      ...
      <nav
        className={`flex-1 overflow-y-auto ${isCollapsed ? 'px-3' : 'px-5'} space-y-1 scrollbar-hide`}
      >
                      ...
                      {hasChildren ? (
                        ...
                        >
                          <div className="relative z-10 flex items-center gap-4 w-full">
                            <Icon
                              className={`h-6 w-6 transition-colors ${isChildActive || expanded ? 'text-text-primary' : 'text-text-tertiary/60 group-hover:text-text-primary/70'}`}
                            />
                            <span className="flex-1 text-left text-[20px] font-semibold leading-none">
                              {item.label}
                            </span>
                            <ChevronDown
                              className={`h-4 w-4 text-text-tertiary transition-transform duration-300 ease-out ${expanded ? 'rotate-180' : ''}`}
                            />
                          </div>
                        </button>
                      ) : (
                        ...
                        >
                          ...
                          <div className="relative z-10 flex items-center gap-4 w-full justify-center lg:justify-start">
                            <Icon
                              className={`h-6 w-6 min-w-[24px] transition-colors ${active ? 'text-text-primary' : 'text-text-tertiary/60 group-hover:text-text-primary/70'}`}
                            />
                            {!isCollapsed && (
                              <span className="flex-1 text-left text-[20px] leading-none">
                                {item.label}
                              </span>
                            )}
                            ...
                            {item.children?.map((child) => {
                              ...
                              return (
                                <Link
                                  key={child.href}
                                  href={child.href}
                                  className={`block px-4 py-3 rounded-xl text-[15px] font-semibold transition-all ${
                                    childActive
      ...
      {/* Account Footer */}
      <div
        className={`p-4 ${isCollapsed ? 'px-2' : 'p-6'} border-t border-border-subtle bg-surface-tertiary/10`}
      >
