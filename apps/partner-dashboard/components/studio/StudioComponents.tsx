...
export function StudioCard({ title, children, className, rightElement }: StudioCardProps) {
  return (
    <div
      className={cn(
        "bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm hover:shadow-md transition-shadow",
        className,
      )}
    >
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm group hover:border-slate-300 transition-all">
      <div className="flex justify-between items-start mb-4">
        <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-slate-900 transition-colors">
          {label}
        </p>
        {trend && (
          <div
...
export function ChartPlaceholder({
  ...
  return (
    <div
      className={cn(
        "w-full bg-slate-50/50 border border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center p-8",
        height,
      )}
    >
