...
export function StudioCard({ title, children, className, rightElement }: StudioCardProps) {
  return (
    <div
      className={cn(
        'bg-surface-elevated border border-border-default rounded-[2rem] p-8 shadow-sm hover:shadow-md transition-shadow',
        className,
      )}
    >
  ...
  return (
    <div className="bg-surface-elevated border border-border-default rounded-[2rem] p-8 shadow-sm group hover:border-border-strong transition-all">
      <div className="flex justify-between items-start mb-4">
        <p className="text-[11px] font-black uppercase tracking-[0.2em] text-text-tertiary group-hover:text-text-primary transition-colors">
          {label}
        </p>
        {trend && (
          <div
...
export function ChartPlaceholder({
  ...
  return (
    <div
      className={cn(
        'w-full bg-surface-tertiary/50 border border-dashed border-border-default rounded-2xl flex flex-col items-center justify-center p-8',
        height,
      )}
    >
      <div className="text-text-placeholder font-black uppercase tracking-widest text-[10px]">
        {label}
      </div>
    </div>
  );
