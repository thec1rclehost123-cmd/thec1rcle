...
function RankedItem({ row }: { row: RankedRow }) {
  return (
    <div
      className={`flex items-center gap-3 py-1.5 ${row.highlight ? 'opacity-100' : 'opacity-85'}`}
    >
...
function GroundingFooter({ sources }: { sources: GroundingSource[] }) {
  ...
  return <p className="text-[10px] text-text-muted mt-2.5 leading-relaxed">Based on: {labels}</p>;
}
...
function FreshnessTag({ freshness }: { freshness?: string }) {
  ...
  if (!cfg) return null;
  return <span className={`text-[10px] font-medium ${cfg.color} ml-2`}>{cfg.label}</span>;
}
    ...
    <div className="py-0.5 space-y-3">
      ...
      {answer.comparison && answer.comparison.length > 0 && (
        <div
          ...
          style={{ gridTemplateColumns: `repeat(${answer.comparison.length}, 1fr)` }}
        >
            ...
            <thead>
              <tr>
                {answer.table.headers.map((h, i) => (
                  <th
                    key={i}
                    className="text-left text-[10px] text-text-tertiary uppercase tracking-wider pb-1.5 font-normal"
                  >
                    {h}
                  </th>
                ))}
              ...
              {answer.table.rows.map((row, ri) => (
                <tr key={ri} className="border-t border-border-subtle">
                  {row.cells.map((cell, ci) => (
                    <td key={ci} className="py-1.5 text-text-secondary pr-3">
                      {cell}
                    </td>
                  ))}
                </tr>
      ...
      {/* Deep links */}
      {answer.links && answer.links.length > 0 && <DeepLinkRow links={answer.links} />}
