...
export default function HostSidebar() {
  ...
  const menuItems = [
    ...
    {
      category: 'personal',
      items: [{ icon: Layout, label: 'presence', href: '/host/presence' }],
    },
  ];
      ...
      <div className="p-8 border-b border-black/[0.05]">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h1 className="font-bold text-text-primary text-base tracking-tight truncate">
              C1RCLE
            </h1>
            ...
            <div className="space-y-1">
              {section.items.map((item) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <item.icon
                      className={`w-4 h-4 ${isActive(item.href) ? 'text-text-primary' : 'text-text-primary/30 group-hover:text-text-primary/50'}`}
                    />
                  ...
                  {isActive(item.href) && (
                    <div className="h-1 w-1 rounded-full bg-iris shadow-[0_0_10px_#F44A22]" />
                  )}
                </Link>
              ))}
            </div>
