...
export function DiscoverDirectory({
  ...
  const fetchPartners = useCallback(async () => {
    ...
    try {
      ...
      // Fetch current connections to overlay connectionStatus
      let connections: any[] = [];
      try {
        const connRes = await fetch(
          `/api/discovery?action=list&partnerId=${partnerId}&role=${role}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        console.error('DiscoverDirectory failed to fetch connections overlay:', connErr);
      }

      const mappedPartners = (data.partners || []).map((p: any) => {
        const conn = connections.find((c: any) => c.otherId === p.id);
        return {
          ...p,
