    ...
    const fetchFreshProfile = async () => {
      ...
      try {
        ...
        if (isHost) {
          ...
          const res = await fetch(`/api/partners/venues/profile?venueId=${profile.id}`, {
            headers,
          });
          if (res.ok && !cancelled) {
            const json = await res.json();
        ...
        if (!cancelled) {
          setLiveHostStats({
            ...
            venuesConnected:
              ...
              active.filter((c: any) => c.otherType === 'venue').length,
            promotersConnected:
              summaryData?.summary?.activePromoterPartnerships ??
              active.filter((c: any) => c.otherType === 'promoter').length,
          });
        }
      } catch {
      ...
      >
        ...
        <div className="px-8 -mt-12 flex items-end justify-between mb-8 relative z-10">
          <div className="w-24 h-24 rounded-3xl border-4 border-surface-elevated shadow-2xl overflow-hidden bg-gradient-to-br from-surface-elevated to-surface-secondary flex items-center justify-center">
            {profileLoading ? (
              ...
              <span className="text-4xl font-black text-white uppercase drop-shadow-md">
                {p.name[0]}
              </span>
            )}
          </div>
          {p.isVerified && (
            <div className="flex items-center gap-2 px-4 py-2 bg-accent-glow rounded-2xl border border-accent-primary/20 shadow-lg shadow-accent-primary/5">
              ...
              <span className="text-[12px] font-black uppercase tracking-tight text-accent-primary">
                Verified
              </span>
            </div>
          )}
        ...
        <div className="px-8 pb-8 space-y-8">
          ...
          <div>
            ...
            {p.bio && <p className="text-body text-text-secondary mt-4 leading-relaxed">{p.bio}</p>}
          </div>
...
function MetricTile({
  ...
  return (
    <div
      className={`p-4 rounded-3xl border transition-all hover:shadow-lg hover:bg-opacity-10 backdrop-blur-sm ${bgClass}`}
    >
