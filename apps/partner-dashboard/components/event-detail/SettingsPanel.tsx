...
function ToggleRow({
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-1">
      <div>
        <p className="text-[14px] font-medium" style={{ color: 'var(--v-text-primary)' }}>
          {label}
        </p>
        {hint && (
...
const MOCK_EVENT_INFO = {
  ...
  fullDesc:
    'Join us for an unforgettable evening as we open the doors to The C1rcle for the very first time. Expect curated music, bespoke cocktails, and an intimate crowd of culture-forward individuals.\n\nEntry by invite only. RSVP required.',
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <p
        className="text-[14px] font-medium leading-relaxed whitespace-pre-line"
...
function CheckoutForm() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <ToggleRow
        label="Include Fees in Price"
        hint="Fees are folded into the displayed ticket price."
        checked={includeFees}
        onChange={setIncludeFees}
      />
      <ToggleRow
        label="Long Form Add to Cart"
        hint="Use an expanded product-page style checkout."
        checked={longForm}
        onChange={setLongForm}
      />
      <ToggleRow
        label="Legacy RSVP Mode"
        hint="Disable payments and collect RSVPs only."
        checked={legacyRSVP}
        onChange={setLegacyRSVP}
      />

      <SectionDivider label="Custom Checkout Fields" />
      <p className="text-[13px]" style={{ color: 'var(--v-text-tertiary)' }}>
        Custom fields let you collect extra information at checkout (e.g. dietary preferences,
        T-shirt size).
      </p>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <ToggleRow
        ...
        hint="Guests must show proof of vaccination."
        checked={vaccinationReq}
        onChange={setVaccinationReq}
      />
      <ToggleRow
        label="Mask Required"
        hint="Guests must wear a mask at this event."
        checked={maskReq}
        onChange={setMaskReq}
      />
      <ToggleRow
        label="Negative COVID Test"
        hint="Guests must present a recent negative test."
        checked={covidTest}
        onChange={setCovidTest}
      />
      <ToggleRow
        label="Age Verification Required"
        hint="Guests must present valid ID at the door."
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">{FORM_MAP[settingsKey]}</div>
