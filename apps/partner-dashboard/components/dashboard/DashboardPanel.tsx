  ...
  return (
    <Wrapper
      className={cn(
        ...
        interactive &&
          'hover:bg-[var(--v-panel-hover)] hover:shadow-[var(--v-shadow-hover)] cursor-pointer active:scale-[0.99]',
        fullHeight && 'h-full',
        className,
      )}
      style={style}
    ...
    >
      ...
      {(title || icon || actions) && (
        <div
          className={cn(
            'px-6 py-5 flex items-center justify-between border-b border-[var(--v-divider)]',
            headerClassName,
          )}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              {title && (
                <h3 className="text-[15px] font-bold text-[var(--v-text-primary)] leading-tight">
                  {title}
                </h3>
              )}
              {subtitle && (
                <p className="text-[12px] font-medium text-[var(--v-text-tertiary)] mt-0.5">
                  {subtitle}
                </p>
              )}
            </div>
      ...
      {/* Content */}
      <div className={cn('flex-1', paddingClasses[padding])}>{children}</div>

      {/* Footer */}
      {footer && (
        <div className="px-6 py-4 border-t border-[var(--v-divider)] bg-transparent">{footer}</div>
      )}
    </Wrapper>
  );
}
