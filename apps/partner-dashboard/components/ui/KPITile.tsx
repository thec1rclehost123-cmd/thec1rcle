...
export function KPITile({
  ...
  const getTrendColor = () => {
    ...
    if (trend.direction === "neutral")
      return "text-[var(--text-tertiary)] bg-[var(--surface-tertiary)]";
    return isPositive
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <p className={clsx("kpi-label", compact ? "text-[11px] mb-2" : "text-[13px] mb-3")}>
        {label}
      </p>
      ...
      {/* Subtext */}
      {subtext && <p className="text-[12px] text-[var(--text-tertiary)] mt-2">{subtext}</p>}
    </Wrapper>
  );
}
...
export function KPIGrid({ children, columns = 4, className }: KPIGridProps) {
  ...
  return (
    <div className={clsx("grid gap-4 md:gap-6", colClasses[columns], className)}>{children}</div>
  );
}
  ...
  return (
    <div className={clsx("text-center py-8", className)}>
      ...
      <div className="flex items-baseline justify-center gap-2">
        {prefix && <span className="text-stat-lg text-[var(--text-tertiary)]">{prefix}</span>}
        <motion.span
        ...
        {suffix && <span className="text-stat-lg text-[var(--text-tertiary)]">{suffix}</span>}
      </div>
      {trend && (
      ...
      {description && (
        <p className="text-body-sm text-[var(--text-tertiary)] mt-3">{description}</p>
      )}
    </div>
  );
}
