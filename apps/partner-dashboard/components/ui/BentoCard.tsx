...
const variantStyles: Record<string, React.CSSProperties> = {
  ...
  accent: {
    ...
    boxShadow: '0 0 20px var(--v-orange-glow)',
  },
...
export function BentoCard({
  ...
  const inner = (
    ...
    >
      {/* Header slot */}
      {header && <div className="flex items-center justify-between mb-3 shrink-0">{header}</div>}
      ...
      {footer && !loading && !error && !empty && (
        <div className="mt-3 pt-3 border-t border-[var(--v-border)] shrink-0">{footer}</div>
      )}
    </div>
  );
...
function BentoEmpty({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
      ...
      <p className="text-[13px] font-medium" style={{ color: 'var(--v-text-tertiary)' }}>
        {title}
      </p>
      {action}
    </div>
...
function BentoError({ onRetry }: { onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
      <AlertCircle className="w-8 h-8" style={{ color: 'var(--v-error)' }} />
