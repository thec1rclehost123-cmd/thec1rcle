    ...
    return (
      <div
        ...
        className={cn(
          ...
          // Base styles
          glass ? 'card-glass backdrop-blur-xl' : 'bg-surface-elevated',
          // Border styles
          glow ? 'border-accent-primary shadow-glow' : 'border-border-subtle',
          // Elevation
          elevated && 'shadow-md',
          // Interactive states
          interactive &&
            'cursor-pointer hover:shadow-lg hover:border-border-default active:scale-[0.995]',
          paddingMap[padding],
          className,
        )}
        {...(rest as any)}
...
export const CardFooter = ({ className, ...rest }: HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'mt-auto flex flex-wrap items-center gap-3 pt-5 border-t border-border-subtle',
      className,
    )}
    {...(rest as any)}
  />
