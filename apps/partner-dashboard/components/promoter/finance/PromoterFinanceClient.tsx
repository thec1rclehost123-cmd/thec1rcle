...
export function PromoterFinanceClient() {
  ...
  const commissionsList =
    commissionsData?.pages.flatMap((page) => page.commissions) || data?.commissionDetails || [];
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500 pb-16">
      <header className="mb-2 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-display-sm text-text-primary tracking-tight font-bold">Finance</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 text-text-secondary ${isRefetching ? 'animate-spin' : ''}`}
            />
          </button>
        </div>
      </header>
        ...
        <div className="flex flex-col gap-6 w-full animate-pulse">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-36 bg-surface-elevated rounded-2xl border border-border-subtle p-6 flex flex-col justify-end gap-2"
              >
                  ...
                  <tbody className="divide-y divide-border-subtle">
                    {data.ledger.map((tx: any) => (
                      <tr key={tx.id} className="hover:bg-surface-hover/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-[11px] text-text-tertiary font-medium uppercase tracking-wider mt-0.5">
                                {tx.type}
                              </span>
                            </div>
                          </div>
                        ...
                        <td
                          className={`px-6 py-4 text-right font-bold tabular-nums tracking-tight ${
                            tx.amount > 0 ? 'text-emerald-500' : 'text-text-primary'
                          }`}
                        >
                          {tx.amount > 0 ? '+' : ''}
                          {formatCurrencyInline(tx.amount)}
                        </td>
                      </tr>
                    ))}
              ...
              <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-6">
                ...
                <div className="flex flex-col gap-3">
                  {data.bankAccounts.map((account: any) => (
                    <div
                      key={account.id}
                      className="border border-border-subtle rounded-xl p-4 bg-surface-base group relative overflow-hidden"
                    >
                      {account.isDefault && (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-text-tertiary text-xs font-mono mt-0.5">
                            •••• {account.last4}
                          </span>
                        </div>
                      </div>
              ...
              <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-6 flex-1 bg-[url('/noise.png')] relative overflow-hidden group">
                ...
                <div className="relative z-10 h-full flex flex-col justify-center">
                  ...
                  <p className="text-sm text-text-secondary">
                    Your cleared balance is automatically deposited to your default bank account on
                  ...
                  {commissionsList.map((cd: any) => (
                    <tr key={cd.id} className="hover:bg-surface-hover/30 transition-colors">
                      <td className="px-6 py-4 font-semibold text-text-primary text-sm">
                        {cd.eventName || cd.eventTitle}
                      </td>
                      <td className="px-6 py-4 text-sm text-text-secondary">
                        <span className="bg-surface-tertiary px-2 py-0.5 rounded-md border border-border-subtle text-xs font-medium">
                          {cd.ticketType || 'Ticket'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-text-secondary">
                        {cd.buyerName || cd.guestName}
                      </td>
                      <td className="px-6 py-4 text-sm text-text-secondary">
                        {new Date(cd.date || cd.createdAt).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                        })}
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                            cd.status === 'cleared' || cd.status === 'paid'
                              ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                              : 'bg-amber-500/10 text-amber-500 border-amber-500/20'
                          }`}
                        >
                      ...
                      <td className="px-6 py-4 text-right font-bold tabular-nums text-emerald-500">
                        +{formatCurrencyInline(cd.amount || cd.commission || 0)}
                      </td>
                    </tr>
                  ))}
