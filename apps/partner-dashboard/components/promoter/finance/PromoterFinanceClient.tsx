            ...
            {isLoading ? (
                <div className="flex flex-col gap-6 w-full animate-pulse">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                         {[1, 2, 3].map(i => (
                             <div key={i} className="h-36 bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] p-6 flex flex-col justify-end gap-2">
                                  <div className="w-1/2 h-4 bg-[var(--bg-secondary)] rounded-md"></div>
                                  <div className="w-2/3 h-8 bg-[var(--bg-secondary)] rounded-lg"></div>
                             </div>
                         ))}
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                         <div className="lg:col-span-2 h-[400px] bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] flex flex-col p-6 gap-6">
                              <div className="w-1/3 h-6 bg-[var(--bg-secondary)] rounded-md"></div>
                              {[1,2,3,4,5].map(j => (
                                  <div key={j} className="w-full h-12 bg-[var(--bg-secondary)] rounded-xl"></div>
                              ))}
                         </div>
                         <div className="h-[400px] bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] p-6 flex flex-col gap-4">
                              <div className="w-1/2 h-6 bg-[var(--bg-secondary)] rounded-md mb-2"></div>
                              <div className="w-full h-24 bg-[var(--bg-secondary)] rounded-xl"></div>
                              <div className="w-full h-24 bg-[var(--bg-secondary)] rounded-xl"></div>
                         </div>
                    </div>
                </div>
                ...
                <>
                    {/* Balanced KPI Grid */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                        <div className="bg-[var(--bg-elevated)] border-emerald-500/30 p-6 rounded-2xl border bg-gradient-to-br from-emerald-500/10 to-surface-elevated flex flex-col gap-2 relative overflow-hidden group">
                             ...
                             <span className="text-4xl font-black tabular-nums tracking-tighter text-[var(--text-primary)] mt-1">
                        ...
                        <div className="bg-[var(--bg-elevated)] border-[var(--border-subtle)] p-6 rounded-2xl border flex flex-col gap-2 relative overflow-hidden group">
                             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                                <Clock className="h-16 w-16 text-[var(--text-tertiary)]" />
                             </div>
                        ...
                        <div className="bg-[var(--bg-elevated)] border-[var(--border-subtle)] p-6 rounded-2xl border flex flex-col gap-2 relative overflow-hidden group">
                             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                                <DollarSign className="h-16 w-16 text-[var(--text-tertiary)]" />
                             </div>
                             <p className="text-sm font-semibold text-[var(--text-tertiary)] flex items-center gap-2">
                                 Lifetime Earnings
                             </p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                        {/* Transaction Ledger */}
                        <div className="lg:col-span-2 bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] flex flex-col min-h-0">
                            <div className="p-5 border-b border-[var(--border-subtle)] flex justify-between items-center bg-[var(--bg-base)]/50">
                                <h3 className="font-bold text-[var(--text-primary)]">Transaction Ledger</h3>
                            </div>
                            <div className="p-0 overflow-y-auto min-h-[300px]">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-[var(--border-subtle)]">
                                            <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Transaction</th>
                                    ...
                                    <tbody className="divide-y divide-border-subtle">
                                        {data.ledger.map((tx: any) => (
                                            <tr key={tx.id} className="hover:bg-[var(--bg-fill)]/30 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="font-semibold text-[var(--text-primary)] text-sm tracking-tight">{tx.description}</span>
                                                            <span className="text-[11px] text-[var(--text-tertiary)] font-medium uppercase tracking-wider mt-0.5">{tx.type}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className={`px-6 py-4 text-right font-bold tabular-nums tracking-tight ${
                                                    tx.amount > 0 ? "text-emerald-500" : "text-[var(--text-primary)]"
                                                }`}>
                        ...
                        <div className="flex flex-col gap-4 h-full">
                             <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] p-6">
                                 <h3 className="font-bold text-[var(--text-primary)] mb-4">Payout Method</h3>
                                 <div className="flex flex-col gap-3">
                                     {data.bankAccounts.map((account: any) => (
                                         <div key={account.id} className="border border-[var(--border-subtle)] rounded-xl p-4 bg-[var(--bg-base)] group relative overflow-hidden">
                                              {account.isDefault && (
                                              ...
                                              <div className="flex items-center gap-3">
                                                  ...
                                                  <div className="flex flex-col">
                                                      <span className="font-bold text-[var(--text-primary)] text-sm">{account.bankName}</span>
                                                      <span className="text-[var(--text-tertiary)] text-xs font-mono mt-0.5">•••• {account.last4}</span>
                                                  </div>
                                              </div>
                                         </div>
                                     ))}
                                     <button className="w-full py-3 border border-dashed border-[var(--border-subtle)] hover:border-emerald-500/50 hover:bg-emerald-500/5 text-emerald-500 transition-colors rounded-xl text-sm font-semibold mt-2">
                                         + Link Bank Account
                                     </button>
                                 </div>
                             </div>

                             <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] p-6 flex-1 bg-[url('/noise.png')] relative overflow-hidden group">
                                  ...
                                  <div className="relative z-10 h-full flex flex-col justify-center">
                                      ...
                                      <h4 className="font-bold text-[var(--text-primary)] mb-2">Automated Payouts</h4>
                    ...
                    <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] flex flex-col min-h-0 w-full">
                         <div className="p-5 border-b border-[var(--border-subtle)] flex justify-between items-center bg-[var(--bg-base)]/50">
                             <h3 className="font-bold text-[var(--text-primary)] flex items-center gap-2">
                                 <Receipt className="h-4 w-4 text-emerald-500" />
                                 Commission Ledger
                             </h3>
                             <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">Per-Ticket Breakdown</span>
                         </div>
                             ...
                             <table className="w-full text-left border-collapse min-w-[700px]">
                                 <thead>
                                     <tr className="border-b border-[var(--border-subtle)] bg-[var(--bg-base)]/30">
                                         ...
                                         <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Ticket Type</th>
                                         <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Buyer</th>
                                         <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Date</th>
                                         <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Status</th>
                                         <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider text-right">Commission</th>
                                     </tr>
                                 </thead>
                                 <tbody className="divide-y divide-border-subtle">
                                     {(data.commissionDetails || []).map((cd: any) => (
                                         <tr key={cd.id} className="hover:bg-[var(--bg-fill)]/30 transition-colors">
                                             <td className="px-6 py-4 font-semibold text-[var(--text-primary)] text-sm">{cd.eventName}</td>
                                             <td className="px-6 py-4 text-sm text-[var(--text-secondary)]">
                                                 <span className="bg-[var(--bg-secondary)] px-2 py-0.5 rounded-md border border-[var(--border-subtle)] text-xs font-medium">{cd.ticketType}</span>
                                             </td>
                                             <td className="px-6 py-4 text-sm text-[var(--text-secondary)]">{cd.buyerName}</td>
                                             <td className="px-6 py-4 text-sm text-[var(--text-secondary)]">
                                     ...
                                     {(!data.commissionDetails || data.commissionDetails.length === 0) && (
                                         <tr>
                                             <td colSpan={6} className="px-6 py-12 text-center text-[var(--text-tertiary)]">No commission records found.</td>
                                         </tr>
                                     )}
                                 </tbody>
                             </table>
