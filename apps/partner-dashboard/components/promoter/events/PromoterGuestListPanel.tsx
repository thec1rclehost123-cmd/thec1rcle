    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 gap-4">
                <div>
                    <h2 className="text-xl font-bold tracking-tight text-[var(--text-primary)]">Guest List</h2>
                    <p className="text-sm text-[var(--text-secondary)]">You have used {guestlist.used} out of {guestlist.allowance} complimentary spots.</p>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <button className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-[var(--bg-secondary)] hover:bg-[var(--bg-fill)] text-[var(--text-primary)] px-4 py-2 rounded-xl font-semibold transition-colors text-sm border border-[var(--border-subtle)]">
            ...
            <div className="flex items-center gap-4 bg-[var(--bg-base)] p-2 border border-[var(--border-subtle)] rounded-full overflow-hidden mb-2">
                <div className="h-2 w-full flex bg-[var(--bg-secondary)] rounded-full overflow-hidden mx-4">
                    <div 
                        className="bg-emerald-500 h-full rounded-full transition-all duration-1000" 
                        style={{ width: `${(guestlist.used / Math.max(guestlist.allowance, 1)) * 100}%` }} 
                    />
                </div>
                <div className="pr-4 whitespace-nowrap text-xs font-bold font-mono text-[var(--text-tertiary)]">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-[var(--border-subtle)] bg-[var(--bg-base)]/30">
                                <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Guest Name</th>
                                <th className="px-6 py-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">Tickets</th>
                                ...
                                <tr key={guest.id} className="hover:bg-[var(--bg-fill)]/30 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="h-8 w-8 rounded-full bg-[var(--bg-secondary)] flex items-center justify-center shrink-0">
                                                <User className="h-4 w-4 text-[var(--text-tertiary)]" />
                                            </div>
                                            <span className="font-semibold text-[var(--text-primary)]">{guest.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1 text-sm font-medium tabular-nums text-[var(--text-secondary)]">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button className="text-sm text-[var(--text-tertiary)] hover:text-emerald-500 font-medium transition-colors opacity-0 group-hover:opacity-100">
                                            Edit
                                        </button>
                                    </td>
                                </tr>
