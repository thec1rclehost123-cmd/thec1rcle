    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {max > 0 && value !== max && (
            <span style={{ color: 'var(--v-text-tertiary, #a1a1aa)' }}>({pct}%)</span>
          )}
        </span>
      </div>
      <div
...
export default function AnalyticsDrawer({
  ...
  const handleCopy = () => {
    ...
    const url =
      link.fullUrl || `${GUEST_PORTAL_URL}/e/${slug}?ref=${encodeURIComponent(ref)}${channel}`;
  ...
  const linkUrl = link
    ? link.fullUrl ||
      `${GUEST_PORTAL_URL}/e/${link.eventSlug || link.eventId || ''}?ref=${encodeURIComponent(link.code || link.shortId || link.token || link.id || '')}${link.channel ? `&s=${encodeURIComponent(link.channel)}` : ''}`
    : '';

  const statusColor = isActive ? '#34d399' : '#71717a';
  const statusBg = isActive ? 'rgba(52,211,153,0.1)' : 'rgba(113,113,122,0.1)';
  const statusLabel =
    status === 'deactivated' ? 'Deactivated' : status === 'expired' ? 'Expired' : 'Active';
                      ...
                      <button
                        ...
                        onMouseLeave={(e) => {
                          ...
                          e.currentTarget.style.borderColor =
                            'var(--v-border, rgba(255,255,255,0.08))';
                        }}
                      >
                        Deactivate Link
                      </button>
