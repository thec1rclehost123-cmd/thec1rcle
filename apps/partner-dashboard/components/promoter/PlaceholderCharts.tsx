  ...
  return (
    <div className="rounded-[32px] bg-surface-elevated border border-border-default p-6 flex flex-col gap-4">
      <div className="flex items-start justify-between">
        ...
        <span className="text-[10px] font-bold text-text-placeholder uppercase tracking-widest">
          Last 30 days
        </span>
      </div>
      ...
      <div className="flex justify-between px-1">
        {dayLabels.map((d) => (
          <span
            key={d}
            className="text-[9px] font-bold uppercase tracking-widest text-text-placeholder"
          >
            {d}
          </span>
        ))}
...
export function LeaderboardPlaceholder({
  title,
  items = DEFAULT_LEADERBOARD,
}: LeaderboardPlaceholderProps) {
  return (
    <div className="rounded-[32px] bg-surface-elevated border border-border-default p-6 flex flex-col gap-4">
      <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">{title}</p>
          ...
          return (
            ...
            >
              <span
                ...
                style={
                  style
                    ...
                    : { background: 'rgba(255,255,255,0.06)', color: 'var(--v-text-muted)' }
                }
              >
                {item.rank}
              </span>
              <div className="flex-1 min-w-0">
                ...
                {item.badge && (
                  <span className="text-[9px] font-black uppercase tracking-widest text-text-placeholder">
                    {item.badge}
                  </span>
                )}
              </div>
              <span
                ...
                style={{ color: style ? style.text : 'var(--v-text-primary)' }}
              >
                {item.value}
              </span>
            </div>
