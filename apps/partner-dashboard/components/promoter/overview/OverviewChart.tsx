...
function buildLinePath(values: number[], width: number, height: number, padding: number) {
  ...
  return safe
    .map((value, index) => {
      const x =
        padding + (safe.length === 1 ? innerWidth : (index / (safe.length - 1)) * innerWidth);
  ...
  const chartValues = useMemo(() => {
    const source =
      analyticsTimeline.length > 0
        ? analyticsTimeline
        : buildZeroStateDates(range).map((date) => ({ date }));
    ...
    >
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-[38px] leading-none font-semibold text-white mt-2">
            {loading
              ? '—'
              : metric === 'clicks'
                ? chartValue.toLocaleString('en-IN')
                : formatCompactINR(chartValue)}
          </p>
        </div>
      </div>
      ...
      <div className="relative h-[320px] rounded-[24px] overflow-hidden">
        <div className="absolute inset-0 flex flex-col justify-between py-4">
          {[0, 1, 2, 3].map((row) => (
            <div key={row} className="border-t" style={{ borderColor: 'rgba(255,255,255,0.04)' }} />
          ))}
        </div>
        ...
        {chartLabels.length > 0 && (
          <div className="absolute inset-x-3 bottom-0 grid grid-cols-6 gap-2 pb-1">
            {chartLabels
              .filter((_, index) => index % Math.max(1, Math.floor(chartLabels.length / 6)) === 0)
              .slice(0, 6)
              .map((label, i) => (
                <span
                  key={`${label}-${i}`}
