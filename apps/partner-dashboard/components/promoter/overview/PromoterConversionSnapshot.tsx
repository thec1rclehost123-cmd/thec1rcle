...
export function PromoterConversionSnapshot({ snapshot }: { snapshot?: any }) {
    return (
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-2xl p-6 flex flex-col h-[280px]">
            <h3 className="font-semibold text-[var(--text-primary)] text-sm tracking-wide mb-4">CONVERSION RATE</h3>
            <div className="flex-1 flex flex-col items-center justify-center text-center">
                <div className="h-12 w-12 rounded-full bg-[var(--bg-secondary)] flex items-center justify-center mb-3">
                    <Activity className="h-5 w-5 text-emerald-500" />
                </div>
                <div className="text-4xl font-black tabular-nums tracking-tighter text-[var(--text-primary)]">
                    {snapshot?.rate || "0.0%"}
                </div>
