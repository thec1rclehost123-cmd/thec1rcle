                ...
                <>
                    ...
                    <div className="flex items-center gap-4 bg-surface-elevated p-6 border border-border-subtle rounded-2xl">
                        <div className="flex-1">
                             <h4 className="font-bold text-text-primary text-sm mb-1 uppercase tracking-wider text-text-tertiary">Total Allocations</h4>
                            ...
                            <table className="w-full text-left border-collapse min-w-[800px]">
                                <thead>
                                    <tr className="border-b border-border-subtle bg-surface-base/30">
                                        <th className="px-6 py-4 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Guest Details</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Event</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Added Date</th>
                                    ...
                                    {filteredGuests.map((guest: any) => (
                                        <tr key={guest.id} className="hover:bg-surface-hover/30 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="font-semibold text-text-primary tracking-tight">{guest.name || guest.guestName}</span>
                                                        <span className="text-xs text-text-tertiary font-medium mt-0.5 flex items-center gap-1">
                                                            <Users className="h-3 w-3" />
                                                            {guest.tickets || guest.ticketCount} ticket{(guest.tickets || guest.ticketCount) > 1 ? 's' : ''}
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-sm text-text-secondary font-medium">
                                                {new Date(guest.dateAdded || guest.createdAt || Date.now()).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                                            </td>
                                            <td className="px-6 py-4">
