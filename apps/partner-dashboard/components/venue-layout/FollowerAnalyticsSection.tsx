...
export default function FollowerAnalyticsSection({ stats, venue }: FollowerAnalyticsSectionProps) {
  ...
  const cityBreakdown = stats?.cityBreakdown || [
    ...
    { city: "Delhi", count: Math.floor((stats?.followersCount || 100) * 0.1), percentage: 10 },
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            {topFollowers.map((follower: any, idx: number) => (
              <div
                key={idx}
                className="flex items-center gap-4 p-4 bg-[var(--surface-secondary)] rounded-2xl border border-[var(--border-subtle)]"
              >
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-white/60 text-sm max-w-md">
                ...
                {followerGrowth.total.toLocaleString()} followers
              </p>
            </div>
            <a
              href="#broadcast"
