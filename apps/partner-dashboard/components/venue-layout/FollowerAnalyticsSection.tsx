        ...
        <div className="space-y-12">
            ...
            <section className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    {/* Time Range Toggle */}
                    <div className="flex p-1 bg-[var(--bg-fill)] rounded-xl border border-[var(--border-subtle)]">
            ...
            <section className="space-y-4 pt-8 border-t border-[var(--border-subtle)]">
                ...
                <div className="p-8 bg-gradient-to-br from-violet-900/20 to-slate-900/40 rounded-3xl border border-violet-500/10">
                    ...
                    <div className="h-48 flex items-end justify-between gap-2">
                        {Array.from({ length: 12 }).map((_, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-black rounded-md text-[10px] text-[var(--text-primary)] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    ...
                    <div className="flex justify-between mt-4 text-[10px] text-[var(--text-primary)]/40 font-bold">
                        <span>Jan</span>
                        <span>Feb</span>
                        <span>Mar</span>
                        <span>Apr</span>
                    ...
                    <div className="space-y-3">
                        {cityBreakdown.map((city: any, idx: number) => (
                            <div key={city.city} className="space-y-2">
                                ...
                                <div className="h-2 bg-[var(--bg-fill)] rounded-full overflow-hidden">
                                    <motion.div
                                        initial={{ width: 0 }}
                ...
                <section className="space-y-4">
                    ...
                    <div className="space-y-3">
                        {topFollowers.map((follower: any, idx: number) => (
                            <div key={idx} className="flex items-center gap-4 p-4 bg-[var(--bg-fill)] rounded-2xl border border-[var(--border-subtle)]">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-[var(--text-primary)] font-bold text-sm">
                                    {follower.name[0]}
                                </div>
            ...
            <section className="pt-8 border-t border-[var(--border-subtle)]">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 via-fuchsia-600 to-violet-700 p-8">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-[var(--bg-elevated)]/10 rounded-full blur-3xl" />
                    <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            <div className="flex items-center gap-2 mb-3">
                                <Zap className="w-5 h-5 text-[var(--text-primary)]" />
                                <span className="text-[10px] font-bold text-[var(--text-primary)]/60 uppercase tracking-widest">Reach Your Audience</span>
                            </div>
                            <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Send a Broadcast</h3>
                            <p className="text-[var(--text-primary)]/60 text-sm max-w-md">
                                Push notifications, announcements, and event drops directly to your {followerGrowth.total.toLocaleString()} followers
                            </p>
                        </div>
                        <a
                            href="#broadcast"
                            className="flex items-center gap-2 px-8 py-4 bg-[var(--bg-elevated)] text-violet-600 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-[var(--bg-elevated)]/90 transition-all shadow-lg"
                        >
...
function StatCard({ label, value, change, positive, icon: Icon, iconColor, iconBg }: {
    ...
    return (
        <div className="p-6 bg-[var(--bg-fill)]/50 rounded-2xl border border-[var(--border-subtle)] hover:border-[var(--border-default)] transition-all">
