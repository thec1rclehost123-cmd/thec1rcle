...
export default function PublishingControlsSection({
  ...
  onPublish,
  isPublishing,
}: PublishingControlsSectionProps) {
  ...
  const versionHistory = [
    {
      ...
      action: 'Published',
      user: 'Admin',
      changes: ['Updated bio', 'Added 3 photos'],
    },
    {
      id: 2,
    ...
    <div className="space-y-8">
      ...
      <div
        className={`p-8 rounded-3xl border-2 ${
          publishStatus === 'published'
            ? 'bg-green-500/5 border-emerald-500/20'
            : publishStatus === 'draft'
          ...
          {validationChecks.map((check) => (
            <div
              ...
              className={`flex items-center gap-3 p-3 rounded-xl ${
                check.passed ? 'bg-green-500/5' : 'bg-amber-500/5'
              }`}
            >
              {check.passed ? (
              ...
              <span
                className={`text-sm ${check.passed ? 'text-text-primary' : 'text-text-secondary'}`}
              >
                {check.label}
              </span>
            </div>
            ...
            >
              ...
              <div className="max-h-96 overflow-y-auto p-6 space-y-4">
                {versionHistory.map((version) => (
                  <div
                    key={version.id}
                    className="p-4 bg-surface-secondary rounded-2xl border border-border-subtle"
                  >
                    ...
                    <ul className="space-y-1">
                      {version.changes.map((change, idx) => (
                        <li
                          key={idx}
                          className="text-[11px] text-text-tertiary flex items-center gap-2"
                        >
