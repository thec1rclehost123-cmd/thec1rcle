        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {AGE_POLICIES.map((policy) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-2.5 rounded-xl mb-3 w-fit ${isSelected ? "bg-blue-500/20" : "bg-[var(--surface-elevated)]"}`}
                >
                  <Icon
                    className={`w-5 h-5 ${isSelected ? "text-blue-500" : "text-[var(--text-tertiary)]"}`}
                  />
                </div>
                <p
                  className={`text-sm font-bold mb-1 ${isSelected ? "text-blue-600" : "text-[var(--text-primary)]"}`}
                >
                  {policy.label}
                </p>
      ...
      <section className="space-y-6 pt-8 border-t border-[var(--border-subtle)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {DAYS_OF_WEEK.map((day) => (
            <div
              key={day.key}
              className="flex items-center gap-4 p-4 bg-[var(--surface-secondary)] rounded-xl border border-[var(--border-subtle)]"
            >
      ...
      <section className="space-y-6 pt-8 border-t border-[var(--border-subtle)]">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-tertiary)]">
              Entry policies and venue guidelines
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {(venue?.rules || []).map((rule: string, idx: number) => (
            <div
              key={idx}
              className="flex items-center gap-3 p-4 bg-[var(--surface-secondary)] rounded-xl border border-[var(--border-subtle)] group"
            >
