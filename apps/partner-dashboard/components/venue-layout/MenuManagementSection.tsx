              ...
              {CUISINE_OPTIONS.map((cuisine) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-[11px] font-bold transition-all ${
                    venue?.cuisineTags?.includes(cuisine)
          ...
          <section className="space-y-4 pt-8 border-t border-border-subtle">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {venue?.popularDishes?.map((dish: any) => (
                <div
                  key={dish.id}
                  className="relative group rounded-2xl overflow-hidden border border-border-subtle bg-surface-secondary"
                >
                  {dish.image ? (
                    ...
                    <div className="flex gap-2">
                      {['Bestseller', "Chef's Pick", 'Must Try', 'New'].map((tag) => (
                        <button
                          key={tag}
                          onClick={() =>
                            setNewDish((prev) => ({ ...prev, tag: prev.tag === tag ? '' : tag }))
                          }
