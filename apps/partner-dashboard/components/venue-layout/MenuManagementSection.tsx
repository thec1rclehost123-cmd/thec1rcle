            ...
            <section className="space-y-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h3 className="text-lg font-bold text-text-primary">Restaurant & Menu</h3>
                        <p className="text-sm text-text-tertiary">Manage your food & beverage offerings</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        />
                        <div className="w-11 h-6 bg-surface-secondary peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-surface-elevated after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-500" />
                        <span className="ml-3 text-[11px] font-bold text-text-tertiary uppercase tracking-widest">
            ...
            {venue?.hasRestaurant && (
                <>
                    ...
                    <section className="space-y-4 pt-8 border-t border-border-subtle">
                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Price Range</label>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {PRICE_BANDS.map((band) => (
                                ...
                                >
                                    ...
                                    <p className="text-[11px] font-bold text-text-secondary">{band.label}</p>
                                    <p className="text-[10px] text-text-tertiary">{band.description}</p>
                                </button>
                            ))}
                        </div>
                    </section>
                    ...
                    {/* Cuisine Tags */}
                    <section className="space-y-4 pt-8 border-t border-border-subtle">
                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Cuisine Types</label>
                            ...
                            {CUISINE_OPTIONS.map((cuisine) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-[11px] font-bold transition-all ${venue?.cuisineTags?.includes(cuisine)
                                        ...
                                        : "bg-surface-secondary text-text-secondary border border-border-subtle hover:bg-surface-elevated"
                                        }`}
                                >
                                    {cuisine}
                                </button>
                    ...
                    {/* Menu PDF Upload */}
                    <section className="space-y-4 pt-8 border-t border-border-subtle">
                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Digital Menu (PDF)</label>
                        <input type="file" ref={menuFileRef} className="hidden" accept=".pdf" onChange={handleMenuPdfUpload} />

                        {venue?.menuURL ? (
                            <div className="flex items-center justify-between p-6 bg-surface-secondary rounded-2xl border border-border-subtle">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-red-500/10 rounded-xl">
                                        <FileText className="w-6 h-6 text-red-500" />
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-text-primary">Menu Uploaded</p>
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="px-4 py-2 bg-surface-elevated rounded-xl text-[11px] font-bold text-text-secondary hover:bg-surface-base border border-border-subtle"
                                    >
                                        Replace
                                    </button>
                                    <button
                            ...
                            <button
                                ...
                                className="w-full py-12 border-2 border-dashed border-border-subtle rounded-2xl flex flex-col items-center justify-center gap-4 hover:bg-surface-secondary transition-all group"
                            >
                                {isUploading ? (
                                    <Loader2 className="w-8 h-8 text-text-tertiary animate-spin" />
                                ) : (
                                    <>
                                        <div className="p-4 bg-surface-secondary rounded-2xl group-hover:scale-110 transition-transform">
                                            <Upload className="w-6 h-6 text-text-tertiary" />
                                        </div>
                                        <div className="text-center">
                                            <p className="text-[11px] font-bold text-text-primary uppercase tracking-widest">Upload Menu PDF</p>
                                            <p className="text-[10px] text-text-tertiary mt-1">Max 10MB</p>
                                        </div>
                                    </>
                                )}
                            </button>
                    ...
                    <section className="space-y-4 pt-8 border-t border-border-subtle">
                        <div className="flex items-center justify-between">
                            <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Popular Dishes</label>
                            <button
                                onClick={() => setShowAddDish(true)}
                                className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-text-primary rounded-xl text-[11px] font-bold hover:bg-amber-400 transition-all"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {venue?.popularDishes?.map((dish: any) => (
                                <div key={dish.id} className="relative group rounded-2xl overflow-hidden border border-border-subtle bg-surface-secondary">
                                    {dish.image ? (
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4">
                                        {dish.tag && (
                                            <span className="inline-block px-2 py-1 bg-amber-500 rounded-md text-[9px] font-black text-text-primary uppercase mb-2">
                                                {dish.tag}
                                            </span>
                                        )}
                                        <p className="text-text-primary font-bold">{dish.name}</p>
                                        <p className="text-amber-400 font-bold">{dish.price}</p>
                                    </div>
                                    <button
                                        onClick={() => handleRemoveDish(dish.id)}
                                        className="absolute top-3 right-3 p-2 bg-black/40 backdrop-blur-md rounded-xl text-text-primary opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500"
                                    >
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                </div>
                            ...
                            {(!venue?.popularDishes || venue.popularDishes.length === 0) && (
                                <div className="col-span-full py-16 text-center bg-surface-secondary/30 rounded-2xl border border-dashed border-border-subtle">
                                    <Utensils className="w-10 h-10 text-border-subtle mx-auto mb-4" />
                                    <p className="text-text-tertiary text-sm font-medium">No dishes added yet</p>
                                    <p className="text-text-placeholder text-xs mt-1">Showcase your best items</p>
                                </div>
                            )}
                        </div>
                    </section>
                ...
                {showAddDish && (
                    ...
                    >
                        <motion.div
                            ...
                            className="w-full max-w-md bg-surface-base rounded-3xl border border-border-subtle overflow-hidden shadow-2xl"
                        >
                            <div className="p-8">
                                <h2 className="text-xl font-bold text-text-primary mb-2">Add Popular Dish</h2>
                                <p className="text-sm text-text-tertiary mb-6">Highlight your best menu items</p>

                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Dish Name</label>
                                        <input
                                            type="text"
                                            value={newDish.name}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Price</label>
                                        <input
                                            type="text"
                                            value={newDish.price}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-bold text-text-tertiary uppercase tracking-widest">Tag (Optional)</label>
                                        <div className="flex gap-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <button
                                            ...
                                            className="w-full py-8 border-2 border-dashed border-border-subtle rounded-xl flex flex-col items-center justify-center gap-2 hover:bg-surface-secondary transition-all"
                                        >
                                            {newDish.image ? (
                                                ...
                                                <>
                                                    ...
                                                    <span className="text-[10px] font-bold text-text-tertiary">Upload Photo</span>
                                                </>
                                            )}
                                        </button>
                                    </div>
                                ...
                                <div className="flex gap-3 mt-6">
                                    <button
                                        onClick={() => setShowAddDish(false)}
                                        className="flex-1 py-3 bg-surface-secondary text-text-secondary rounded-xl text-sm font-bold"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 py-3 bg-amber-500 text-text-primary rounded-xl text-sm font-bold disabled:opacity-50"
                                    >
                                        Add Dish
                                    </button>
                                </div>
