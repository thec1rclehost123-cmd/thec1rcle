...
const ANALYTICS_SUB: AnalyticsSubGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: 'ROI & Partners',
        href: '/venue/analytics/attribution',
        icon: Handshake,
        minPlan: 'gold',
        badge: null,
        tabKey: 'analytics',
...
const MENU_GROUPS: MenuGroup[] = [
  ...
  },
  {
    ...
    items: [
      {
        label: 'Venue Page',
        href: '/venue/page-management',
        icon: Building2,
        minPlan: 'basic',
        badge: null,
        tabKey: 'page_management',
      },
      {
        label: 'Digital Menu',
        href: '/venue/menu',
...
export function VenueSidebar({
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      ...
      >
        ...
        {/* Icon */}
        <div
          className={`relative flex items-center justify-center w-8 h-8 rounded-lg transition-all duration-300 shrink-0
                    ${
                      active
        ...
        >
          <Icon
            className={`h-4 w-4 transition-all ${active ? 'text-orange-500' : 'opacity-70 group-hover:opacity-100'}`}
          />
        </div>
  ...
  return (
    <aside
      className={`flex flex-col h-full bg-[#0D0D0F] ${isCollapsed ? 'w-[80px]' : 'w-[260px]'} shrink-0 transition-all duration-300 ease-in-out ${className}`}
    >
      {/* Header / Brand */}
          ...
          <div className="relative shrink-0">
            ...
            <div className="relative h-12 w-12 rounded-xl bg-gradient-to-br from-[#1a1a1f] to-[#0f0f12] flex items-center justify-center border border-white/10 shadow-xl overflow-hidden">
              {(profile?.activeMembership as any)?.partnerLogo ? (
                <img
                  src={(profile!.activeMembership as any).partnerLogo}
                  alt="Logo"
                      ...
                      <div className="mt-0.5 space-y-1 pl-2">
                        {group.subGroups.map((sg) => {
                          const visibleItems = sg.items.filter(
                            (item) =>
                              PLAN_HIERARCHY[item.minPlan] <= currentPlanLevel &&
                              isTabAllowed(item),
                          );
          ...
          return (
            <div key={idx} className="mb-1">
              ...
              {group.collapsible && !isCollapsed ? (
                ...
                <div
                  className={`flex items-center gap-3 px-3 py-3 text-text-primary/30 ${isCollapsed ? 'justify-center' : ''}`}
                >
              ...
              <AnimatePresence initial={false}>
                {expanded && (
                  <motion.div
                    ...
                    className="overflow-hidden"
                  >
                    <div className="mt-0.5 space-y-0.5 pl-4">{visibleItems.map(renderNavItem)}</div>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <div className="relative overflow-hidden rounded-[1.5rem] p-5 bg-gradient-to-br from-orange-500/10 via-rose-500/5 to-transparent border border-white/[0.06] group/cta">
            ...
            <div className="relative">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[11px] font-black text-text-primary tracking-widest uppercase">
                  C1RCLE PRO
                </span>
              </div>
      ...
      <div className={`p-3 border-t border-white/[0.06] ${isCollapsed ? 'px-2' : 'p-3'}`}>
        <div
          className={`flex items-center gap-3 p-2 rounded-xl bg-surface-elevated/[0.02] hover:bg-surface-elevated/[0.04] transition-all cursor-pointer group ${isCollapsed ? 'flex-col' : ''}`}
        >
          <div className="relative shrink-0">
