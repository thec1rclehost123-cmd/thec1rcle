...
export function VenueCalendarPreview({
  ...
  useEffect(() => {
    ...
    async function fetchAvailability() {
      ...
      try {
        const res = await fetch(`/api/host/venue-calendar?venueId=${venueId}&date=${selectedDate}`);
        const data = await res.json();
                    ...
                    {!day.empty && (
                      <>
                        ...
                        {getStatusIcon(day.status) && (
                          <span className="absolute bottom-1">{getStatusIcon(day.status)}</span>
                        )}
                        {day.myRequest && (
                  ...
                  {loadingAvailability ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {TIME_SLOTS.map((slot) => {
                        ...
                        const isUnavailable = dateAvailability?.slots?.some(
                          (s: any) =>
                            s.status !== "available" &&
