...
const Zap = ({ className }) => (
  <svg
    ...
    className={className}
  >
    <path d="M4 14.71 13 4l-1 8h8l-9 10.71 1-8h-8z" />
  </svg>
);
    ...
    const fetchAlerts = async () => {
      ...
      try {
        ...
        if (alertErrorCountRef.current >= 3) {
          console.warn(
            '[AdminConsoleShell] Alert polling suspended after 3 failures. Reload to resume.',
          );
        }
      }
    };
            ...
            <div className="flex items-center justify-between px-2 py-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] text-zinc-500 font-bold mt-1.5 uppercase tracking-widest leading-none opacity-80">
                    Authority Node
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-0.5">
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <Icon
                          className={`h-4.5 w-4.5 ${isActive ? 'text-white' : 'opacity-40 group-hover:opacity-100 transition-opacity'}`}
                          strokeWidth={1.5}
                        />
            ...
            <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold leading-none">
                    {profile?.admin_role || 'Super'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 ml-[300px] flex flex-col relative bg-obsidian-base">
          <header className="h-16 flex items-center justify-between px-8 z-30 sticky top-0 bg-obsidian-base/80 backdrop-blur-xl border-b border-[#ffffff05]">
            <div className="flex items-center gap-3">
              <Link
                href="/"
                className="text-[10px] font-bold text-zinc-500 hover:text-iris transition-colors uppercase tracking-[0.2em]"
              >
                Console
              </Link>
            ...
            <div className="flex items-center gap-6">
              <div className="relative hidden lg:block group">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none">
                  <div className="h-5 w-5 rounded bg-zinc-800 border border-white/5 flex items-center justify-center text-[10px] font-bold text-zinc-500">
                    /
                  </div>
                </div>
              </div>
                  ...
                  {showNotifications && (
                    <div className="absolute right-0 mt-4 w-80 bg-obsidian-surface border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                      <div className="p-4 border-b border-white/5 flex items-center justify-between">
                        ...
                        <span className="px-2 py-0.5 rounded bg-iris/10 text-iris text-[9px] font-bold uppercase">
                      ...
                      <div className="max-h-96 overflow-y-auto custom-scrollbar">
                        {alerts.length > 0 ? (
                          alerts.map((alert, i) => (
                            ...
                            >
                              <div className="flex gap-3">
                                <div
                                  className={`mt-1 h-2 w-2 rounded-full flex-shrink-0 ${alert.priority === 'high' ? 'bg-iris' : 'bg-amber-500'}`}
                                ></div>
                                <div className="flex-1">
                                  <p className="text-xs text-white font-medium mb-1 leading-relaxed">
                                    {alert.message}
                                  </p>
                          ...
                          <div className="p-12 text-center">
                            <ShieldCheck
                              className="h-8 w-8 text-zinc-800 mx-auto mb-3"
                              strokeWidth={1}
                            />
                      ...
                      {alerts.length > 0 && (
                        <Link
                          ...
                          className="block p-3 text-center bg-white/[0.02] hover:bg-white/[0.05] transition-colors border-t border-white/5"
                        >
                          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-colors">
          ...
          {searchResults && (
            <div
              className="fixed inset-0 z-[100] flex items-start justify-center pt-24 px-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200"
              onClick={() => setSearchResults(null)}
            >
              <div
                className="bg-obsidian-surface rounded-xl border border-[#ffffff08] shadow-2xl w-full max-w-[600px] overflow-hidden"
              ...
              >
                <div className="p-4 border-b border-[#ffffff08] flex items-center justify-between bg-white/[0.02]">
                  ...
                  <button
                    onClick={() => setSearchResults(null)}
                    className="p-1.5 hover:bg-white/5 rounded-md transition-colors border border-transparent hover:border-white/5"
                  >
                      ...
                      {searchResults.map((result, idx) => (
                        ...
                        >
                          <div className="h-9 w-9 rounded-md bg-zinc-900 border border-white/5 flex items-center justify-center text-zinc-500 group-hover:text-white group-hover:border-white/10 transition-all">
                            ...
                            {result.type === 'order' && (
                              <Ticket className="h-4.5 w-4.5" strokeWidth={1.5} />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] font-bold uppercase tracking-widest text-[#A1A1AA] bg-white/5 px-1.5 py-0.5 rounded-sm">
                                {result.type}
                              </span>
                              <h4 className="text-sm font-semibold text-white truncate">
                                {result.displayName || result.name || result.email || result.id}
                              </h4>
                            </div>
                            <p className="text-[10px] text-zinc-500 font-medium mt-0.5 truncate uppercase tracking-tight">
                              ID: {result.id}
                            </p>
                          </div>
              ...
              {envStatus.isNonProd && (
                <div className="mb-8 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10 flex items-center justify-between group overflow-hidden relative">
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-500 leading-none">
                        Development Node
                      </h4>
                    </div>
                  </div>
