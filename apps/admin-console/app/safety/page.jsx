    ...
    const columns = useMemo(() => [
        {
            ...
            cell: ({ row }) => (
                <div className="flex flex-col gap-1">
                    <span className="text-sm font-semibold text-white tracking-tight uppercase truncate max-w-[300px]">{row.original.reason || 'Reported Incident'}</span>
                    <span className="text-[10px] text-zinc-600 font-mono tracking-widest">ID: {row.original.id?.slice(0, 12).toUpperCase()}</span>
                </div>
            )
        },
        {
            ...
            cell: ({ getValue }) => (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">{getValue() || 'Anonymous'}</span>
                </div>
            )
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                return (
                    <div className={`inline-flex px-2.5 py-1 rounded border text-[9px] font-bold uppercase tracking-widest ${isCritical ? 'bg-iris/10 border-iris/20 text-iris' : 'bg-amber-500/10 border-amber-500/20 text-amber-500'}`}>
                        {priority || 'NORMAL'}
                    </div>
                );
            }
        ...
        <div className="space-y-12 pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-1">
                <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-6 shadow-sm overflow-hidden relative group hover:border-iris/20 transition-all">
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Flame className="h-24 w-24 text-iris" /></div>
                ...
                <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-6 shadow-sm overflow-hidden relative group hover:border-amber-500/20 transition-all">
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><AlertCircle className="h-24 w-24 text-amber-500" /></div>
                    <div className="h-12 w-12 rounded-lg bg-amber-500/10 flex items-center justify-center border border-amber-500/20 shadow-inner"><AlertCircle className="h-5 w-5 text-amber-500" strokeWidth={1.5} /></div>
                    <div>
                ...
                <div className="p-8 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-6 shadow-lg shadow-emerald-500/5 overflow-hidden relative group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-emerald-500"><Activity className="h-24 w-24" /></div>
                    <div className="h-12 w-12 rounded-lg bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30"><ShieldCheck className="h-5 w-5 text-emerald-500" strokeWidth={1.5} /></div>
                    <div>
            ...
            >
                {selectedReport && (
                    <div className="space-y-8">
                        <div className={`p-6 rounded-xl border ${selectedReport.priority === 'CRITICAL' ? 'bg-iris/5 border-iris/10' : 'bg-amber-500/5 border-amber-500/10'}`}>
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Oversight Registry</span>
                                <div className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border ${selectedReport.priority === 'CRITICAL' ? 'bg-iris/10 text-iris border-iris/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                                    {selectedReport.priority || 'Normal'} Priority
                                </div>
                            </div>
                            <h3 className="text-xl font-semibold text-white tracking-tight uppercase leading-snug">{selectedReport.reason || 'Reported Incident'}</h3>
                        </div>
                            ...
                            <div className="grid grid-cols-1 gap-px bg-[#ffffff08] border border-[#ffffff08] rounded-xl overflow-hidden shadow-inner font-mono">
                                <div className="bg-obsidian-surface p-4 flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest">Reporter</span>
                                        <span className="text-[11px] font-bold text-white uppercase">{selectedReport.reporterEmail || 'Anonymous'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
