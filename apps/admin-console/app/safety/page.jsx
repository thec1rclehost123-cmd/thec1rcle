          ...
          {loading ? (
            <div className="p-12 space-y-4">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="h-20 bg-white/5 animate-pulse rounded-lg border border-white/5"
                />
              ))}
            </div>
          ) : reports.length === 0 ? (
            <div className="py-24 text-center">
              ...
              <p className="text-[10px] text-zinc-600 mt-2 font-bold uppercase tracking-widest leading-relaxed">
                No pending safety reports or security incidents at this time.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-[#ffffff05]">
              {filteredReports.length > 0 ? (
                  ...
                  <div key={r.id} className="p-8 hover:bg-white/[0.01] transition-colors group">
                    <div className="flex items-center gap-8">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-lg font-semibold tracking-tight text-white mb-1.5 truncate uppercase">
                          {r.reason || "Reported Incident"}
                        </p>
                        <div className="flex items-center gap-4 text-[9px] font-bold uppercase tracking-widest text-zinc-600">
                          ...
                          <span
                            className={`${r.priority === "CRITICAL" ? "text-iris" : "text-amber-500"}`}
                          >
                ...
                <div className="py-20 text-center">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-700 italic">
