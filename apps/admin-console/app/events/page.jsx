      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="relative group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-zinc-600 group-focus-within:text-zinc-300 transition-colors"
              strokeWidth={1.5}
            />
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loading ? (
              [...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="h-48 bg-white/5 animate-pulse rounded-xl border border-white/5"
                ></div>
              ))
            ) : filtered.length > 0 ? (
              filtered.map((event) => (
                ...
                >
                  <div className="bg-obsidian-surface border border-[#ffffff08] p-6 hover:border-[#ffffff15] transition-colors cursor-pointer">
                    <div className="flex items-start justify-between mb-5">
                      ...
                      <div
                        className={`px-2.5 py-1 rounded border text-[9px] font-bold uppercase tracking-widest ${event.status === "live" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : "bg-iris/10 border-iris/20 text-iris"}`}
                      >
                        {event.status}
                      </div>
                    </div>
                    <h3 className="text-base font-semibold text-white truncate mb-1 uppercase tracking-tight">
                      {event.title}
                    </h3>
                    <div className="flex items-center gap-3 text-zinc-600 text-[9px] font-bold uppercase tracking-widest">
                      <span className="flex items-center gap-1.5">
                        <Ticket className="h-3 w-3" strokeWidth={1.5} /> {event.ticketsSold || 0}{" "}
                        Sold
                      </span>
                      <div className="h-1 w-1 rounded-full bg-zinc-800"></div>
                      <span className="flex items-center gap-1.5 text-zinc-700 font-mono-numbers">
                        <TrendingUp className="h-3 w-3" strokeWidth={1.5} /> Priority:{" "}
                        {event.discoveryWeight || 0}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="col-span-full py-24 text-center border border-[#ffffff08] rounded-xl bg-obsidian-surface/50">
                ...
                <p className="text-[10px] text-zinc-600 mt-2 font-bold uppercase tracking-widest leading-relaxed">
                  Adjust your search parameters
                  <br />
                  to discover experiences.
                </p>
              </div>
            )}
          </div>
              ...
              <div className="bg-obsidian-surface border border-[#ffffff08] rounded-xl overflow-hidden shadow-2xl relative">
                ...
                <div className="p-8 space-y-8">
                  <div className="space-y-4 text-center">
                    <h3 className="text-2xl font-semibold tracking-tight text-white leading-tight uppercase tracking-tight">
                      {selectedEvent.title}
                    </h3>
                    <div className="flex flex-wrap items-center justify-center gap-2">
                      ...
                      <div
                        className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border ${selectedEvent.status === "live" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-iris/10 text-iris border-iris/20"}`}
                      >
                          ...
                          <button
                            onClick={() =>
                              setModalConfig({
                                ...
                                message:
                                  "Immediately stop all ticket sales for this listing. Requires secondary authorization.",
                          ...
                          <button
                            onClick={() =>
                              setModalConfig({
                                ...
                                message:
                                  "Authorized the experience to resume ticket sales and marketplace visibility.",
