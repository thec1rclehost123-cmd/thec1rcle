...
export default function AdminEvents() {
    ...
    const columns = useMemo(() => [
                ...
                <div className="flex items-center gap-4">
                    <div className="h-10 w-10 rounded-lg bg-zinc-900 border border-white/5 overflow-hidden flex-shrink-0">
                        {row.original.poster || row.original.image ? (
                            <img src={row.original.poster || row.original.image} className="h-full w-full object-cover" />
                        ) : (
                    ...
                    <div className="flex flex-col min-w-0">
                        <span className="text-sm font-semibold text-white truncate uppercase tracking-tight">{row.original.title}</span>
                        <span className="text-[10px] text-zinc-600 font-mono tracking-tighter">ID: {row.original.id.slice(0, 12)}</span>
                    </div>
                </div>
            )
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                return (
                    <div className={`inline-flex px-2.5 py-1 rounded border text-[9px] font-bold uppercase tracking-widest ${status === 'live' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-iris/10 border-iris/20 text-iris'}`}>
                        {status}
                    </div>
                );
            }
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-3 text-zinc-400 text-[10px] font-bold uppercase tracking-widest font-mono-numbers">
                    <span className="flex items-center gap-1.5"><Ticket className="h-3 w-3 text-zinc-700" /> {row.original.ticketsSold || 0}</span>
                </div>
            )
        },
        {
    ...
    const exportToCSV = () => {
        ...
        const rows = filtered.map(e => [e.id, e.title, e.status, e.ticketsSold || 0, e.discoveryWeight || 0]);
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-px bg-[#ffffff10] border border-[#ffffff10] rounded-xl overflow-hidden shadow-sm">
                            <div className="bg-obsidian-surface p-5">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1.5 font-mono">Discovery Weight</p>
                            ...
                            <div className="bg-obsidian-surface p-5 text-right">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1.5 font-mono">Total Sales</p>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-[#ffffff05]">
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Listing Status</p>
                                    <p className="text-sm font-bold text-white uppercase tracking-tight">{selectedEvent.status}</p>
                                </div>
                                <div className={`h-2.5 w-2.5 rounded-full ${selectedEvent.status === 'live' ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.4)]' : 'bg-iris shadow-[0_0_12px_rgba(163,163,255,0.4)]'}`} />
                            </div>
                        </div>

                        {/* Authority Actions */}
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-600 px-1">Authority Override</h4>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-300 font-mono">Issue Compliance Notice</span>
                                    </div>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-300 font-mono">Override Discovery Priority</span>
                                    </div>
                                    <ChevronRight className="h-4 w-4 text-zinc-700 group-hover:translate-x-1 transition-transform" />
                                </button>
                                    ...
                                    {selectedEvent.status === 'live' ? (
                                        <button
                                            onClick={() => setModalConfig({
                                                ...
                                                message: 'Immediately stop all ticket sales for this listing. Requires tier-2 auth.',
                                        ...
                                        <button
                                            onClick={() => setModalConfig({
                                                ...
                                                message: 'Authorize the experience to resume ticket sales and marketplace visibility.',
