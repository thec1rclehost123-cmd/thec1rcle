  ...
  const columns = [
    {
      ...
      label: 'Member',
      sortable: true,
      render: (val: string | undefined, row: UserData) => (
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest truncate mt-0.5">
              {row.email}
            </p>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (val: boolean | undefined) => (
        <div className="flex items-center gap-2.5">
          <div
            className={`h-1.5 w-1.5 rounded-full ${val ? 'bg-iris shadow-[0_0_8px_rgba(244,74,34,0.4)]' : 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]'}`}
          ></div>
          <span
            className={`text-[10px] font-bold uppercase tracking-widest ${val ? 'text-iris' : 'text-zinc-600'}`}
          >
            ...
            <button
              onClick={() =>
                setModalConfig({
                  ...
                  message:
                    'Prevent this user from accessing the platform. This action will be logged.',
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-6">
            <div className="flex items-center justify-between py-2 border-b border-white/[0.02]">
              ...
              <div className="flex items-center gap-1.5">
                {selectedUser?.isBanned ? (
                  <Lock className="h-3.5 w-3.5 text-iris" strokeWidth={1.5} />
                ) : (
                  <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" strokeWidth={1.5} />
                )}
                <span
                  className={`text-[9px] font-bold uppercase tracking-widest ${selectedUser?.isBanned ? 'text-iris' : 'text-emerald-500'}`}
                >
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 px-1">
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-iris">
                Governance Ledger
              </p>
            </div>
            <div className="p-5 rounded-2xl bg-iris/5 border border-iris/10 text-[11px] font-medium text-iris/80 leading-relaxed italic">
              All administrative actions taken against this identity are recorded in the global
              audit ledger with a high-priority priority.
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 px-1">
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-500">
                  Re-provision Partner Type
                </p>
              </div>
              <div className="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-4">
                <p className="text-[11px] text-zinc-400 leading-relaxed">
                  Use this if the account was approved with the wrong workspace type. This will
                  update Firebase claims, deactivate the old membership, and create the correct one.
                </p>
