...
export default function AdminsManagement() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      toast({ message: 'Failed to fetch admins', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAction = async (reason: string, targetId: string, inputValue: string) => {
    if (!modalConfig || !selectedAdmin) return;
    try {
      const token = await user.getIdToken();
      ...
      await fetchAdmins();
      setModalConfig(null);
      toast({ message: 'Action applied effectively.', type: 'success' });
    } catch (err) {
      toast({ message: (err as Error).message, type: 'error' });
    }
  };

  const handleProvision = async ({ email, name, role }: { email: string; name: string; role: string }) => {
    try {
      const token = await user.getIdToken();
      const res = await fetch('/api/actions', {
        method: 'POST',
      ...
      await fetchAdmins();
      toast({ message: `Provisioned ${name} as ${role} successfully.`, type: 'success' });
    } catch (err) {
      toast({ message: `Setup Error: ${(err as Error).message}`, type: 'error' });
      throw err;
    }
  };
    ...
    <div className="space-y-12 pb-24">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#ffffff05]">
                {loading ? (
                  ...
                  filteredAdmins.map((adm: AdminUser) => (
                    <tr
                      key={adm.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-right">
                        <ChevronRight
                          className={`h-4 w-4 transition-all ${selectedAdmin?.id === adm.id ? 'text-white translate-x-1' : 'text-zinc-700 group-hover:text-white group-hover:translate-x-1'}`}
                          strokeWidth={1.5}
                        />
                      </td>
                    </tr>
          ...
          {selectedAdmin ? (
              ...
              <div className="bg-obsidian-surface border border-[#ffffff08] rounded-xl p-8 space-y-8 shadow-2xl relative">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="space-y-2">
                      {rolesList.map((role: { id: string; label: string; desc: string }) => (
                        <button
                          key={role.id}
                ...
                <div className="pt-2 text-center">
                  <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest italic opacity-50">
                    Monitoring staff authority matrix.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="h-[400px] flex flex-col items-center justify-center rounded-xl border border-[#ffffff05] bg-white/[0.01] text-center p-8 sticky top-28">
              ...
              <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest leading-relaxed">
                ...
                <br />
                to modify organizational access.
              </p>
            </div>
          )}
        </aside>
      ...
      {modalConfig && (
        <AdminConfirmModal
          ...
          onConfirm={(reason: string) => handleAction(reason, selectedAdmin!.id, modalConfig!.inputValue!)}
