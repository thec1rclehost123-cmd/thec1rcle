...
export default function AdminLogs() {
    ...
    const cleanJargon = (text) => {
        ...
        return mapping[normalized] || text.replace(/_/g, ' ').split(' ').map(w => w.charAt(0) + w.slice(1).toLowerCase()).join(' ');
    };
        ...
        },
        {
            ...
            cell: ({ getValue }) => (
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold text-zinc-400 truncate max-w-[150px] font-mono">{getValue()}</span>
                </div>
            )
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white">{cleanJargon(row.original.actionType || row.original.action)}</span>
                    <span className="text-[9px] font-bold text-zinc-600 tracking-widest uppercase font-mono">ID: {row.original.targetId?.slice(0, 12)}</span>
                </div>
            )
        },
        {
            ...
            accessorKey: "status",
            cell: ({ getValue }) => (
                <div className={`inline-flex items-center gap-2 px-2.5 py-1 rounded border ${getValue() === 'failed' ? 'bg-iris/10 border-iris/20 text-iris' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500'}`}>
                    <div className={`h-1.5 w-1.5 rounded-full ${getValue() === 'failed' ? 'bg-iris' : 'bg-emerald-500'}`} />
                    ...
                    <div className="space-y-8">
                        <div className="flex flex-col items-center text-center space-y-4 font-mono">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest italic">Immutable System Record</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6 font-mono">
                            <div className="p-5 rounded-xl bg-white/[0.02] border border-[#ffffff05] space-y-6 shadow-inner">
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-2">Authority Narrative</p>
                                    <p className="text-sm font-medium text-zinc-400 italic leading-relaxed uppercase tracking-tight">&quot;{cleanJargon((selectedLog.reason || selectedLog.details?.reason) || 'Authenticated administrative action recorded by system protocol.')}&quot;</p>
                                </div>

                                <div className="pt-4 border-t border-[#ffffff05] grid grid-cols-2 gap-6">
                                    <div>
                                        <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mb-1">Target Identity</p>
                                        <p className="text-[10px] font-bold text-white truncate tracking-tight uppercase">#{selectedLog.targetId?.slice(0, 12)}</p>
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mb-1">Network Origin</p>
                                        <p className="text-[10px] font-bold text-white uppercase tracking-tight">{(selectedLog.ipAddress || selectedLog.ip) || 'Protocol-Secured'}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 p-4 rounded-xl bg-zinc-900 border border-white/5">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mb-0.5">Executor Identity</p>
                                        <p className="text-[11px] font-bold text-white truncate uppercase">{selectedLog.actorEmail}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-[9px] font-bold uppercase tracking-widest text-emerald-500/50 mb-0.5">Authorization Flow</p>
                                        <p className="text-[11px] font-bold text-emerald-500 uppercase tracking-widest">{selectedLog.proposalId ? 'Consensus Approved' : 'Authority Direct'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
