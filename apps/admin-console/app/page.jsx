...
export default function AdminDashboard() {
    ...
    const stats = [
        ...
        { label: "Pending Reviews", value: data?.alertsCount || 0, icon: ShieldAlert, color: "amber", href: "/approvals" },
        { label: "Upcoming Registry", value: snapshot.events?.upcoming || 0, icon: Clock, color: "sky", href: "/logs" },
    ];
        ...
        <div className="space-y-16 pb-20">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-12 border-b border-slate-200 pb-16">
                <div className="flex-1">
                    ...
                    <h1 className="text-7xl font-black tracking-tighter text-slate-900 leading-[0.85] mb-6">Platform Overview</h1>
                ...
                <div className="flex flex-shrink-0">
                    <Link href="/health">
                        <div className="px-10 py-8 rounded-[3rem] bg-white border border-slate-200 flex items-center gap-8 shadow-sm group hover:border-slate-300 cursor-pointer transition-all duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {stats.map((stat, i) => (
                        <Link href={stat.href} key={i}>
                            <div
                                className="group relative flex h-full flex-col p-10 rounded-[3.5rem] bg-white border border-slate-200 overflow-hidden transition-all duration-500 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.06)] hover:-translate-y-2 hover:border-indigo-100 shadow-sm cursor-pointer"
                            >
            ...
            <section>
                <div className="flex items-center gap-4 mb-10 px-4">
                    ...
                    <h2 className="text-lg font-black uppercase tracking-[0.2em] text-slate-900">Live Authority Queues</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
                    {[
                        { label: "Venues", count: snapshot.queues?.venues || 0, color: "blue", href: "/venues" },
                        { label: "Hosts", count: snapshot.queues?.hosts || 0, color: "indigo", href: "/hosts" },
                        { label: "Refunds", count: snapshot.queues?.refunds || 0, color: "rose", href: "/payments" },
                        { label: "Incidents", count: snapshot.queues?.incidents || 0, color: "red", href: "/safety" },
                        { label: "Webhooks", count: snapshot.queues?.webhooks || 0, color: "orange", href: "/logs" },
                        { label: "Payouts", count: snapshot.queues?.payouts || 0, color: "emerald", href: "/payments" },
                    ].map((q, i) => (
                        <Link href={q.href} key={i}>
                            <div className="p-8 rounded-[2.5rem] bg-white border border-slate-200 hover:border-slate-400 transition-all shadow-sm flex flex-col items-center text-center group cursor-pointer">
                                <span className="text-3xl font-black text-slate-900 mb-2 group-hover:scale-110 transition-transform">{q.count}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                ...
                {/* Left: Alerts & Top Workflows */}
                <div className="lg:col-span-2 space-y-16">
                    {/* Top Workflows */}
                    <section>
                        <div className="flex items-center gap-4 mb-10 px-4">
                            <div className="h-6 w-1.5 bg-indigo-600 rounded-full"></div>
                            <h2 className="text-lg font-black uppercase tracking-[0.2em] text-slate-900">Priority Workflows</h2>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {[
                                ...
                                <Link href={w.href} key={i} className="group">
                                    <div className="p-10 rounded-[3rem] bg-white border border-slate-200 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-50 transition-all h-full">
                                        <div className="h-14 w-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-indigo-50 transition-all">
                                            <w.icon className="h-6 w-6 text-slate-900 group-hover:text-indigo-600" />
                                        </div>
                                        <h3 className="text-2xl font-black tracking-tighter text-slate-900 mb-3">{w.title}</h3>
