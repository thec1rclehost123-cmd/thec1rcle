...
export default function AdminPromotions() {
    ...
    const [form, setForm] = useState({ code: '', type: 'percent', value: '', limit: '', expiry: '' });
    ...
    return (
        <div className="space-y-12 pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: "Active Codes", value: loading ? '--' : promos.filter(p => !p.disabled && (!p.expiry || new Date(p.expiry) > new Date())).length, icon: Percent, color: "text-emerald-500" },
                    { label: "Partner Codes", value: loading ? '--' : promos.filter(p => p.isPartner).length, icon: Users, color: "text-emerald-500" },
                    { label: "Total Usage", value: loading ? '--' : promos.reduce((sum, p) => sum + (p.used || 0), 0), icon: TrendingUp, color: "text-emerald-500" },
                    { label: "Flagged Codes", value: loading ? '--' : promos.filter(p => p.flagged).length, icon: AlertCircle, color: "text-zinc-600" },
                ].map((item, i) => (
                            ...
                            {loading ? (
                                ...
                                <tr key={p.id} className="hover:bg-white/[0.01] transition-all cursor-pointer group">
                                    ...
                                    <td className="px-8 py-8 text-right">
                                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded border ${p.disabled ? 'bg-zinc-800 border-white/5 text-zinc-500' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500'}`}>
                                            <div className={`h-1 w-1 rounded-full ${p.disabled ? 'bg-zinc-500' : 'bg-emerald-500'}`} />
                                            <span className="text-[9px] font-bold uppercase tracking-widest">{p.disabled ? 'Disabled' : 'Active'}</span>
                                        </div>
                                    </td>
                                </tr>
                            )) : (
                                <tr><td colSpan={5} className="px-8 py-24 text-center">
                                    ...
                                    <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-white">{searchTerm ? 'No matching codes' : 'No active promos'}</h4>
            ...
            {showCreate && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-zinc-950 border border-[#ffffff12] rounded-2xl p-8 w-full max-w-md shadow-2xl relative">
                        ...
                        <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest mb-8">Requires Super Admin clearance</p>
                        <form onSubmit={handleCreate} className="space-y-5">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Code</label>
                                <input
                                    required
                                    value={form.code}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Usage Limit</label>
                                    <input
                                        type="number"
                                        min="1"
                                        value={form.limit}
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Expiry Date</label>
                                    <input
                                        type="date"
                                        value={form.expiry}
