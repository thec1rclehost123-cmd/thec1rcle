        ...
        {[
          {
            ...
            value: loading
              ? '--'
              : promos.filter((p) => !p.disabled && (!p.expiry || new Date(p.expiry) > new Date()))
                  .length,
            icon: Percent,
          ...
          {
            label: 'Partner Codes',
            value: loading ? '--' : promos.filter((p) => p.isPartner).length,
            icon: Users,
            color: 'text-emerald-500',
          },
          {
            label: 'Total Usage',
            value: loading ? '--' : promos.reduce((sum, p) => sum + (p.used || 0), 0),
            icon: TrendingUp,
            color: 'text-emerald-500',
          },
          {
            label: 'Flagged Codes',
            value: loading ? '--' : promos.filter((p) => p.flagged).length,
            icon: AlertCircle,
            color: 'text-zinc-600',
          },
        ].map((item, i) => (
