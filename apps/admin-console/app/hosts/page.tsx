...
export default function AdminHosts() {
  ...
  const fetchHosts = async () => {
    try {
      ...
      const json = await res.json() as { data?: Host[] };
      setHosts(json.data || []);
    } catch (err) {
  ...
  const handleAction = async (reason: string, targetId: string, inputValue: string, evidence: string) => {
    ...
    try {
      ...
      const updatedJson = await updatedRes.json() as { data?: Host[] };
      const mappedHosts = updatedJson.data || [];
      setHosts(mappedHosts);

      const updated = mappedHosts.find((h: Host) => h.id === selectedHost!.id);
  ...
  const [showOnlySuspended, setShowOnlySuspended] = useState<boolean>(false);

  const exportToCSV = () => {
    const headers = ['ID', 'Name', 'Role', 'Status', 'Owner UID'];
    const rows = filteredHosts.map((h: Host) => [
      h.id,
    ...
    const csvContent = [headers, ...rows].map((row: string[]) => row.join(',')).join('\n');
  ...
  const columns = [
    {
      ...
      label: 'Organizer',
      sortable: true,
      render: (val: string | undefined, row: Host) => (
