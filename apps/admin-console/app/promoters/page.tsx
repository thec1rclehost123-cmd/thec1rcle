...
export default function AdminPromoters() {
  ...
  const fetchPromoters = async () => {
    try {
      ...
      const json = (await res.json()) as { data: Promoter[] };
  ...
  const handleAction = async (reason: string, targetId: string, inputValue: string, evidence: string) => {
    ...
    try {
      ...
      if (selectedPromoter && freshPromoters) {
        const updated = freshPromoters.find((p: Promoter) => p.id === selectedPromoter.id);
  ...
  const exportToCSV = () => {
    ...
    const rows = filtered.map((p: Promoter) => [
      p.id,
  ...
  const filtered = promoters.filter((p: Promoter) => {
    const matchesSearch =
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loading ? (
              ...
              filtered.map((promoter: Promoter) => (
                <div
                  key={promoter.id}
