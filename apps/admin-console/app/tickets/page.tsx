...
export default function AdminTickets() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const json = (await res.json()) as { data: TicketData[] };
  ...
  const handleExport = () => {
    ...
    const rows = tickets.map((t: TicketData) => [
      t.id,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-6 shadow-sm group hover:border-[#ffffff15] transition-all">
          ...
          <div>
            ...
            <p className="text-2xl font-light text-white tracking-tight font-mono-numbers">
              {tickets.filter((t: TicketData) => t.checkedIn).length || '--'}
            </p>
          </div>
        </div>
        <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-6 shadow-sm group hover:border-[#ffffff15] transition-all">
          ...
          <div>
            ...
            <p className="text-2xl font-light text-white tracking-tight font-mono-numbers">
              {tickets.length === 0
                ? '--'
                : `${Math.round((tickets.filter((t: TicketData) => t.paymentStatus === 'paid' || t.status === 'valid').length / tickets.length) * 100)}%`}
            </p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#ffffff05]">
              {loading ? (
                ...
                filteredTickets.map((t: TicketData) => (
                  <tr
                    key={t.id}
                  ...
                  >
                    <td className="px-8 py-8">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <p className="text-[14px] font-semibold tracking-tight text-white uppercase">
                            {t.ticketType || 'General Entry'}
                          </p>
                          <p className="text-[10px] text-emerald-500/80 font-bold uppercase tracking-widest mt-0.5 font-mono-numbers">
                    ...
                    <td className="px-8 py-8 text-right">
                      <div
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded border ${t.status === 'valid' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-zinc-800 border-white/5 text-zinc-500'}`}
                      >
                        <div
                          className={`h-1 w-1 rounded-full ${t.status === 'valid' ? 'bg-emerald-500' : 'bg-zinc-500'}`}
                        />
                ...
                <tr>
                  <td colSpan={4} className="px-8 py-24 text-center">
                    ...
                    <p className="text-[10px] text-zinc-600 mt-2 font-bold uppercase tracking-[0.2em]">
                      Check again later or adjust your search.
                    </p>
                  </td>
                </tr>
              )}
