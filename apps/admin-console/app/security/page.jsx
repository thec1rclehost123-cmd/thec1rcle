...
import {
    ShieldAlert, ShieldCheck, Shield, Ban, UserX, Activity,
    TrendingUp, AlertCircle, RefreshCw, ChevronRight,
    Globe, User, Crown, Clock, Zap, Eye, CheckCircle2, XCircle,
} from "lucide-react";
...
function Badge({ label, style }) {
    return (
        <span className={`inline-flex px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-widest ${style}`}>
            {label}
        </span>
    );
}

function KPICard({ icon: Icon, label, value, sub, accent = "zinc" }) {
    const colors = {
    ...
    return (
        <div className="p-6 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-5 overflow-hidden relative group hover:border-white/10 transition-all">
            <div className={`h-11 w-11 rounded-lg flex items-center justify-center border flex-shrink-0 ${colors[accent]}`}>
                <Icon className="h-5 w-5" strokeWidth={1.5} />
            </div>
            <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-500 mb-0.5">{label}</p>
                <p className="text-2xl font-light text-white tracking-tighter tabular-nums">{value ?? "—"}</p>
...
function SectionHeader({ children }) {
    return (
        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-600 mb-3 px-1">{children}</h3>
    );
}
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 px-1">
                <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-iris">Security Operations</span>
                    </div>
            ...
            {loading ? <LoadingSkeleton /> : (
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 px-1">
                        <KPICard icon={Ban}       label="Blocked IPs"    value={data?.counts?.blockedIps ?? 0}    accent="red"     />
                        <KPICard icon={UserX}     label="Blocked Users"  value={data?.counts?.blockedUsers ?? 0}  accent="iris"    />
                        <KPICard icon={AlertCircle} label="Flagged Users" value={data?.counts?.flaggedUsers ?? 0}  accent="amber"   />
                        <KPICard icon={Crown}     label="Suspended Admins" value={data?.counts?.suspendedAdmins ?? 0} accent="red" />
                        <KPICard icon={ShieldAlert} label="Open Incidents" value={data?.metrics?.openIncidentCount ?? 0} accent="amber" />
                        <KPICard icon={ShieldCheck} label="Block Rate"   value={`${data?.metrics?.blockRate ?? 0}%`} sub="of recent events" accent="emerald" />
                    </div>
                    ...
                    {tab === "Incidents" && <IncidentsTab data={data} user={user} onUpdate={() => fetchOverview(true)} />}
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 px-1">
            ...
            <div className="space-y-6">
                <RiskyEntityTable title="At-Risk Admins" icon={Crown} entities={rep.topRiskyAdmins || []} compact />
...
function RiskyEntityTable({ title, icon: Icon, entities, compact = false }) {
    return (
        <div className="rounded-xl bg-obsidian-surface border border-[#ffffff08] overflow-hidden">
            <div className="flex items-center gap-2 p-4 border-b border-[#ffffff08]">
                ...
                <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">{title}</span>
            </div>
            {entities.length === 0 ? (
                <p className="text-[11px] text-zinc-700 p-6 text-center">No high-risk entities</p>
            ) : (
                <div className="divide-y divide-[#ffffff05]">
                    {entities.slice(0, compact ? 5 : 10).map((e, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 hover:bg-white/2 transition-colors">
                            <span className="text-[11px] font-mono text-zinc-300 truncate max-w-[200px]">{e.id}</span>
                            <div className="flex items-center gap-2 flex-shrink-0">
                                <span className={`text-[10px] font-bold uppercase ${RISK_TIER_STYLES[e.tier] || "text-zinc-500"}`}>
                                    {e.tier}
                                </span>
...
function MostTargetedEndpoints({ endpoints }) {
    return (
        <div className="rounded-xl bg-obsidian-surface border border-[#ffffff08] overflow-hidden">
            <div className="flex items-center gap-2 p-4 border-b border-[#ffffff08]">
                <Zap className="h-4 w-4 text-zinc-600" strokeWidth={1.5} />
                <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">Most Targeted</span>
            </div>
            {endpoints.length === 0 ? (
                ...
                <div className="divide-y divide-[#ffffff05]">
                    {endpoints.slice(0, 8).map((ep, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3">
                            <span className="text-[10px] font-mono text-zinc-400 truncate max-w-[150px]">{ep.endpoint || "unknown"}</span>
...
function IncidentsTab({ data, user, onUpdate }) {
    const [incidents, setIncidents] = useState(data?.openIncidents || []);
    ...
    const handleTransition = async (incidentId, status, resolution = "") => {
        const token = await user.getIdToken();
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <Badge label={inc.entityType} style="bg-zinc-800 border-zinc-700 text-zinc-400" />
                                        <Badge label={inc.severity}   style={SEVERITY_STYLES[inc.severity] || SEVERITY_STYLES.low} />
                                ...
                                <div className="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    <ChevronRight className={`h-4 w-4 text-zinc-700 transition-transform ${selected?.id === inc.id ? "rotate-90" : ""}`} />
                                </div>
                            </div>

                            {/* Inline action panel */}
                            {selected?.id === inc.id && (
                                <div className="mt-4 pt-4 border-t border-[#ffffff08] flex flex-wrap gap-2" onClick={e => e.stopPropagation()}>
                                    {inc.status === "flagged" && (
                                        <button
                                    ...
                                    {(inc.status === "flagged" || inc.status === "under_review") && (
                                        <>
                                            <button
                                                onClick={() => handleTransition(inc.id, "resolved", "Investigated and cleared")}
                                            ...
                                            <button
                                                onClick={() => handleTransition(inc.id, "false_positive", "Confirmed false positive")}
...
function TrendsTab({ data }) {
    ...
    // Flatten hourly buckets → aggregated event-type totals across all hours
    const aggregated = {};
    Object.values(trends).forEach(bucket => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-1">
            ...
            <div className="rounded-xl bg-obsidian-surface border border-[#ffffff08] overflow-hidden">
                <div className="flex items-center gap-2 p-4 border-b border-[#ffffff08]">
                    ...
                    <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">Attack Volume (24h)</span>
                </div>
            ...
            <div className="rounded-xl bg-obsidian-surface border border-[#ffffff08] overflow-hidden">
                <div className="flex items-center gap-2 p-4 border-b border-[#ffffff08]">
                    <Zap className="h-4 w-4 text-zinc-600" strokeWidth={1.5} />
                    <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">Targeted Endpoints</span>
                </div>
                {endpoints.length === 0 ? (
                    <p className="text-[11px] text-zinc-700 p-6 text-center">No data</p>
                ) : (
                    <div className="divide-y divide-[#ffffff05]">
                        {endpoints.slice(0, 10).map((ep, i) => (
                            <div key={i} className="flex items-center justify-between px-4 py-3">
                                <span className="text-[11px] font-mono text-zinc-400 truncate max-w-[240px]">{ep.endpoint || "unknown"}</span>
                                <span className="text-[11px] font-bold text-iris tabular-nums flex-shrink-0">{ep.count}</span>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="rounded-xl bg-obsidian-surface border border-[#ffffff08] overflow-hidden lg:col-span-2">
                <div className="flex items-center gap-2 p-4 border-b border-[#ffffff08]">
                    ...
                    <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">Event Distribution (Last 20)</span>
                </div>
                {Object.keys(eventsByType).length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-3 p-4">
                        {Object.entries(eventsByType).map(([type, count]) => (
                            <div key={type} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-800 border border-zinc-700">
                                <span className="text-[10px] font-mono text-zinc-400">{type}</span>
                                <span className="text-[10px] font-bold text-white tabular-nums">{count}</span>
                            </div>
                        ))}
                    ...
                    {events.map(ev => (
                        <div key={ev.id} className="px-5 py-3 hover:bg-white/2 transition-colors">
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <Badge label={ev.type}     style={ev.severity ? SEVERITY_STYLES[ev.severity] : SEVERITY_STYLES.low} />
                                        <Badge label={ev.severity} style={SEVERITY_STYLES[ev.severity] || SEVERITY_STYLES.low} />
                                        {ev.mitigated && <Badge label="mitigated" style="bg-emerald-500/10 border-emerald-500/20 text-emerald-400" />}
                                    </div>
                                    <div className="flex items-center gap-3">
                                        {ev.ip  && <span className="text-[10px] font-mono text-zinc-500">IP: {ev.ip}</span>}
                                        {ev.uid && <span className="text-[10px] font-mono text-zinc-500">UID: {ev.uid?.slice(0, 12)}…</span>}
                                        {ev.endpoint && <span className="text-[10px] font-mono text-zinc-600">{ev.endpoint}</span>}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 flex-shrink-0">
                                    {ev.mitigationAction && (
                                        <span className="text-[9px] font-mono text-zinc-700">{ev.mitigationAction}</span>
                                    )}
                                    <span className="text-[9px] text-zinc-700 tabular-nums">
    ...
    return (
        <div className="space-y-6 animate-pulse px-1">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {[...Array(6)].map((_, i) => (
                    <div key={i} className="h-24 rounded-xl bg-zinc-900 border border-[#ffffff05]" />
                ))}
            </div>
            <div className="h-10 w-72 rounded-xl bg-zinc-900" />
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div className="xl:col-span-2 h-64 rounded-xl bg-zinc-900" />
