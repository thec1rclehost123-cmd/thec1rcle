...
export default function AdminPayments() {
  ...
  const handleAction = async (reason: string, targetId: string, inputValue: string, evidence: string) => {
    ...
    try {
      ...
      const successJson = await res.json() as { message?: string };
  ...
  const columns = [
    {
      ...
      label: 'Timeline',
      sortable: true,
      render: (val: { _seconds: number } | string | undefined) => {
        const date = val && typeof val === 'object' && '_seconds' in val ? new Date(val._seconds * 1000) : new Date(val ?? '');
        return (
          <div>
    ...
    },
    {
      ...
      label: 'Customer',
      sortable: true,
      render: (val: string | undefined, row: Transaction) => (
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 truncate">
              {row.eventTitle ?? 'Event Entry'}
            </p>
            <p className="text-[9px] text-zinc-600 font-medium truncate mt-0.5">
              #{val.slice(0, 8)}
            </p>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (val: string | undefined) => (
        <div className="flex items-center justify-end gap-2">
          <div
            className={`h-1.5 w-1.5 rounded-full ${val === 'captured' || val === 'paid' ? 'bg-emerald-500' : val === 'refunded' ? 'bg-iris' : 'bg-amber-500'}`}
          />
          <span
            className={`text-[10px] font-bold uppercase tracking-widest ${val === 'captured' || val === 'paid' ? 'text-emerald-500' : val === 'refunded' ? 'text-iris' : 'text-amber-500'}`}
          >
            {val ?? 'pending'}
          </span>
        </div>
            ...
            <button
              ...
              onClick={() =>
                setModalConfig({
                  ...
                  title: 'Issue Refund',
                  message:
                    'Verify a full fund reversal. The amount will be sent back to the customer origin.',
        ...
        <div className="space-y-8">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div>
              ...
              <div
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest ${selectedTxn?.status === 'refunded' ? 'bg-iris/10 text-iris border border-iris/20' : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'}`}
              >
                {selectedTxn?.status}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="p-4 rounded-xl bg-white/[0.02] border border-[#ffffff05] space-y-1">
              ...
              <p className="text-sm font-semibold text-white truncate">
                {selectedTxn?.customerName ?? 'Anonymous'}
              </p>
            </div>
            <div className="p-4 rounded-xl bg-white/[0.02] border border-[#ffffff05] space-y-1">
              <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-600">
              ...
              <p className="text-sm font-semibold text-white">
                {selectedTxn
                  ? new Date(
                      selectedTxn.createdAt && typeof selectedTxn.createdAt === 'object' && '_seconds' in selectedTxn.createdAt
                        ? selectedTxn.createdAt._seconds * 1000
                        : selectedTxn.createdAt as string | number,
                    ).toLocaleString()
                  : 'N/A'}
              </p>
            </div>
            ...
            <div className="space-y-2">
              {ledgerEntries.length > 0
                ? ledgerEntries.map((entry: LedgerEntry, i: number) => (
                    <div
                      key={i}
                      className="p-4 rounded-xl bg-black/20 border border-white/[0.02] flex items-center justify-between group hover:bg-white/[0.02] transition-all"
                    >
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-white/50">
                          {entry.state}
                        </p>
                        <p className="text-[9px] font-medium text-zinc-500 mt-0.5 truncate pr-4">
                          {entry.description}
                        </p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p
                          className={`text-[11px] font-bold ${entry.amount >= 0 ? 'text-emerald-500' : 'text-iris'}`}
                        >
                          {entry.amount >= 0 ? '+' : ''}
                          {entry.amount}
                        </p>
                        <p className="text-[8px] text-zinc-700 font-bold mt-0.5">
                ...
                : !loadingLedger && (
                    <p className="text-[10px] text-zinc-700 font-bold italic text-center py-6 bg-black/20 rounded-xl border border-white/[0.02]">
