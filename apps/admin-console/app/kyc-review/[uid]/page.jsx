...
export default function KycReviewDetail() {
  ...
  const { stepSequence, kycStepStatus, kycStepData, kycStepAdminNotes, kycStepResubmissionReason } =
    kycData;
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-black text-white">
              {approvedCount}
              <span className="text-zinc-600 text-lg">/{stepSequence.length}</span>
            </p>
        ...
        <div className="space-y-1.5">
          {stepSequence.map((stepId) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isActive ? 'bg-white/10' : 'bg-white/[0.03]'}`}
                >
                  <Icon className={`h-4 w-4 ${isActive ? 'text-white' : 'text-zinc-600'}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[12px] font-bold truncate ${isActive ? 'text-white' : 'text-zinc-400'}`}
                  >
                    {meta?.label ?? stepId}
                  </p>
          ...
          <div className="bg-obsidian-surface border border-white/[0.08] rounded-2xl overflow-hidden">
            ...
            <div className="p-6 border-b border-white/[0.05] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-[11px] font-bold ${STATUS_COLOR[activeStepStatus]}`}>
                    {STATUS_LABEL[activeStepStatus]}
                  </p>
                </div>
              </div>
            ...
            {kycStepResubmissionReason[activeStep] && (
              <div className="mx-6 mt-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">
                <p className="text-[10px] font-bold uppercase tracking-widest text-amber-400 mb-1">
                  Resubmission Reason (shown to user)
                </p>
            ...
            <div className="p-6">
              {Object.keys(activeStepData).length === 0 ? (
                <p className="text-[12px] text-zinc-600 font-medium text-center py-12">
                ...
                <div className="space-y-6">
                  ...
                  {Object.entries(activeStepData).filter(
                    ([key, value]) => key.toLowerCase().includes('url') && value,
                  ).length > 0 && (
                    <div>
