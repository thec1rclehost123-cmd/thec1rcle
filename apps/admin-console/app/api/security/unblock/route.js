...
import { unblockIp, unblockUser, unflagUser, clearAdminSuspension } from "@c1rcle/core/security-state";
...
async function handler(req) {
    ...
    if (!VALID_TYPES.has(type)) {
        return NextResponse.json({ error: "Invalid type. Must be ip|user|flag|admin" }, { status: 400 });
    }
    ...
    switch (type) {
        case "ip":    await unblockIp(target); break;
        case "user":  await unblockUser(target); break;
        case "flag":  await unflagUser(target); break;
        case "admin": await clearAdminSuspension(target); break;
    }
