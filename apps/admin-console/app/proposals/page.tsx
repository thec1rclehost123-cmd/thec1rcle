...
export default function AdminProposals() {
  ...
  const [selectedProp, setSelectedProp] = useState<ProposalAction | null>(null);
  ...
  const fetchProposals = async () => {
    try {
      ...
      const json = await res.json() as ProposalListResponse;
      const sorted = (json.data || []).sort(
        (a: ProposalAction, b: ProposalAction) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime(),
      );
      setProposals(sorted);
    } catch (err) {
  ...
  const handleResolve = async (status: string, reason = '') => {
    ...
    <div className="space-y-12 pb-24">
          ...
          <div className="flex gap-4 items-center px-1">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                {proposals.filter((p: ProposalAction) => p.status === 'pending').length} Pending
              </span>
            </div>
          </div>
                  ...
                  <tr>
                    <td colSpan={4} className="px-8 py-24 text-center">
                      ...
                      <p className="text-[10px] text-zinc-600 mt-2 font-bold uppercase tracking-widest leading-relaxed">
                        No requests require dual approval at this time.
                      </p>
                    </td>
                  </tr>
                ) : (
                  filtered.map((p: ProposalAction) => (
                    <tr
                      key={p.id}
                    ...
                    >
                      <td className="px-8 py-7">
                        <p className="text-sm font-semibold text-white flex items-center gap-2 uppercase tracking-tight">
                          ...
                          {p.action?.includes('REFUND') && (
                            <span className="px-2 py-0.5 rounded bg-iris/10 text-[8px] font-bold uppercase tracking-widest text-iris border border-iris/20">
                              Finance
                            </span>
                          )}
                        </p>
                        <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest mt-1">
                          Ref: {p.targetId?.slice(0, 12)}
                        </p>
                      </td>
                      <td className="px-8 py-7">
                      ...
                      <td className="px-8 py-7 text-right">
                        <ChevronRight
                          className={`h-4 w-4 transition-all ${selectedProp?.id === p.id ? 'text-white translate-x-1' : 'text-zinc-700 group-hover:text-white group-hover:translate-x-1'}`}
                          strokeWidth={1.5}
                        />
                      </td>
                    </tr>
                ...
                <div className="space-y-6">
                  ...
                  {selectedProp.params && Object.keys(selectedProp.params).length > 0 && (
                    <div className="space-y-3">
                      ...
                      <div className="p-5 rounded-xl bg-zinc-900/50 border border-white/5 space-y-3 shadow-inner overflow-hidden">
                        {Object.entries(selectedProp.params).map(([key, val]: [string, unknown]) => (
                          <div
                            key={key}
                            className="flex justify-between items-start gap-4 text-[10px]"
                          >
                            <span className="font-bold text-zinc-600 uppercase tracking-widest">
                              {key}:
                            </span>
                            <span className="font-bold text-zinc-400 text-right break-all">
                              {typeof val === 'object' ? 'Structured Data' : String(val)}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  {selectedProp.status === 'pending' && (
                    <div className="flex flex-col gap-3 pt-2">
                      <button
                        onClick={() =>
                          setModalConfig({ status: 'approved', label: 'Approve Request' })
                        }
                      ...
                      <button
                        onClick={() =>
                          setModalConfig({ status: 'rejected', label: 'Reject Request' })
                        }
      ...
      {modalConfig && (
        <AdminConfirmModal
          ...
          onConfirm={(reason: string) => handleResolve(modalConfig.status, reason)}
