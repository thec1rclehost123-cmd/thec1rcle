...
export default function ExploreCMS() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      const json = (await res.json()) as { data: { id: string; sections: ExploreSection[] }[] };
      const layout = json.data?.find((d) => d.id === 'explore_layout');

      if (layout && layout.sections) {
        setSections(layout.sections.sort((a: ExploreSection, b: ExploreSection) => (a.order || 0) - (b.order || 0)));
      } else {
        setSections([
          {
            id: 'for-you',
  ...
  const updateSection = (id: string, updates: Partial<ExploreSection>) => {
    ...
    <div className="space-y-12 pb-24">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium max-w-xl">
            Design the member experience. Reorder sections, update logic, and curate the discovery
            feed.
          </p>
        </div>
          ...
          <button
            onClick={() =>
              setModalConfig({
                ...
                message:
                  'This will immediately update the Explore page for all mobile users. This action is logged and immutable.',
      ...
      <div className="space-y-4">
        {sections.map((section, index) => (
          <div
            key={section.id}
            className={`p-6 rounded-2xl bg-obsidian-surface border ${section.enabled ? 'border-[#ffffff08]' : 'border-dashed border-[#ffffff15] opacity-60'} transition-all group relative`}
          >
              ...
              <div className="flex flex-col items-center gap-2 py-2">
                ...
                <div className="flex flex-col gap-1">
                  <button
                    ...
                    disabled={index === 0}
                    className="p-1 hover:bg-white/5 rounded-md disabled:opacity-20"
                  >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          value={section.icon}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(section.id, { icon: e.target.value })}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updateSection(section.id, { limit: parseInt(e.target.value) })
                        }
                ...
                <div className="md:col-span-5 space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateSection(section.id, { filterType: e.target.value })}
                  ...
                  {section.filterType === 'category' && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        value={
                          Array.isArray(section.filterValue)
                            ? section.filterValue.join(', ')
                            : section.filterValue || ''
                        }
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updateSection(section.id, {
                            filterValue: e.target.value.split(',').map((v) => v.trim()),
                          })
                        }
                  ...
                  {(section.filterType === 'trending' || section.filterType === 'social_proof') && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updateSection(section.id, { filterValue: parseInt(e.target.value) })
                        }
                        className="w-full bg-black/20 border border-white/5 rounded-lg px-4 py-2.5 text-sm focus:outline-none"
                      />
                    </div>
