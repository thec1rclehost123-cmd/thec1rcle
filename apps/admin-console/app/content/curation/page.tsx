...
export default function AdminCuration() {
  ...
  const [spotlights, setSpotlights] = useState<SpotlightBuckets>({
    featured: [],
    selects: [],
    trending: [],
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const eventsJson = (await eventsRes.json()) as { data: EventData[] };
      const mapped = (eventsJson.data || []).map((e: EventData) => mapEventForClient(e, e.id));
      setEvents(mapped);
      ...
      const spotlightDoc = configJson.data?.find((d: Record<string, unknown>) => d.id === 'spotlights') as Record<string, string[]> | undefined;

      if (spotlightDoc) {
        setSpotlights({
          featured: (spotlightDoc.featured as string[]) || [],
          selects: (spotlightDoc.selects as string[]) || [],
          trending: (spotlightDoc.trending as string[]) || [],
        });
      }
    } catch (err) {
  ...
  const toggleSpotlight = (id: string, bucket: keyof SpotlightBuckets) => {
    setSpotlights((prev) => {
  ...
  const filteredEvents = sortedEvents.filter((e) => {
    ...
    if (activeTab === 'live') {
      statusMatch =
        isPublicLifecycle(e.lifecycle) && (!endDate || endDate >= now) && !isDraft && hasRealPoster;
  ...
  if (loading) return <PageSkeleton sections={['header', 'table']} />;
    ...
    <div className="space-y-12 pb-24">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-iris">
              Authority / Curation Orchestrator
            </span>
          </div>
          ...
          <button
            onClick={() =>
              setModalConfig({
                ...
                message:
                  'This will immediately update Featured, Circle Select, and Trending sections across all platforms.',
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-obsidian-surface z-10">
                  <tr className="bg-white/[0.02] border-b border-[#ffffff08]">
                    ...
                    <th className="px-6 py-4 text-right text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">
                      Promote To
                    </th>
                  </tr>
                </thead>
                      ...
                      <tr key={event.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-tight mt-0.5">
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-2">
                            <div
                              className={`h-1.5 w-1.5 rounded-full ${event.lifecycle === 'live' ? 'bg-emerald-500' : event.lifecycle === 'scheduled' ? 'bg-iris' : 'bg-zinc-700'}`}
                            />
                            <span
                              className={`text-[10px] font-bold uppercase tracking-widest ${event.lifecycle === 'live' ? 'text-white' : 'text-zinc-500'}`}
                            >
                              {event.lifecycle}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            >
                              <Star
                                className="h-4 w-4"
                                fill={
                                  spotlights.featured.includes(event.id) ? 'currentColor' : 'none'
                                }
                                strokeWidth={1.5}
                              />
                            </button>
                            ...
                            >
                              <Zap
                                className="h-4 w-4"
                                fill={
                                  spotlights.selects.includes(event.id) ? 'currentColor' : 'none'
                                }
                                strokeWidth={1.5}
                              />
                            </button>
          ...
          <div className="bg-obsidian-surface border border-[#ffffff08] rounded-2xl p-6 shadow-xl sticky top-28">
            <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-6 flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 rounded bg-iris/10 text-iris text-[9px] font-bold uppercase tracking-widest">
                {spotlights.featured.length +
                  spotlights.selects.length +
                  spotlights.trending.length}{' '}
                Total
              </span>
            </h3>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {spotlights.featured.length > 0 ? (
                    spotlights.featured.map((id) => {
                      const e = getEventById(id);
                      return (
                        <div
                          key={id}
                          className="p-3 rounded-xl bg-black/20 border border-white/5 flex items-center justify-between group"
                        >
                          <span className="text-xs text-zinc-300 font-medium truncate pr-4">
                            {e?.title || id.slice(0, 8)}
                          </span>
                          <button
                            onClick={() => toggleSpotlight(id, 'featured')}
                            className="h-5 w-5 flex items-center justify-center text-zinc-600 hover:text-white transition-colors"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </div>
                    ...
                    <p className="text-[10px] text-zinc-700 italic px-1">
                      Curate a drop to show on homepage.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {spotlights.selects.length > 0 ? (
                    spotlights.selects.map((id) => {
                      const e = getEventById(id);
                      return (
                        <div
                          key={id}
                        ...
                        >
                          ...
                          <button
                            onClick={() => toggleSpotlight(id, 'selects')}
                            className="h-5 w-5 flex items-center justify-center text-zinc-600 hover:text-white transition-colors"
                          >
                            <X className="h-3 w-3" />
                          </button>
                    ...
                    <p className="text-[10px] text-zinc-700 italic px-1">
                      Pin events to the &quot;Selects&quot; grid.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {spotlights.trending.length > 0 ? (
                    spotlights.trending.map((id) => {
                      const e = getEventById(id);
                      return (
                        <div
                          key={id}
                        ...
                        >
                          ...
                          <button
                            onClick={() => toggleSpotlight(id, 'trending')}
                            className="h-5 w-5 flex items-center justify-center text-zinc-600 hover:text-white transition-colors"
                          >
                            <X className="h-3 w-3" />
                          </button>
                    ...
                    <p className="text-[10px] text-zinc-700 italic px-1">
                      Manual boost for discovery feed.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="mt-12 pt-6 border-t border-[#ffffff05] text-center">
              <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest italic opacity-50 uppercase">
                Authority Manual Selection Active.
              </p>
            </div>
          </div>
        </aside>
      ...
      {modalConfig && (
        <AdminConfirmModal
          ...
          onConfirm={(confirmAction: string) => handleSave(confirmAction)}
