...
export default function AdminContent() {
  ...
  const fetchContent = async () => {
    try {
      ...
      const json = (await res.json()) as { data: MediaReport[] };
  ...
  const handleAction = async (reason: string, _targetId: string, _inputValue: string, _evidence: string) => {
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      'download',
      `content_oversight_report_${new Date().toISOString().split('T')[0]}.csv`,
    );
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-24 rounded-xl border border-[#ffffff08] bg-obsidian-surface/50 text-center">
          ...
          <p className="text-[10px] text-zinc-600 mt-3 font-bold uppercase tracking-[0.2em] max-w-sm mx-auto leading-relaxed">
            All community content and media uploads are within safety policy limits. No pending
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((report) => (
            <div
              key={report.id}
              className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] hover:border-white/10 transition-all relative overflow-hidden group shadow-sm"
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-center shadow-inner">
                    {report.type === 'image' ? (
                      <ImageIcon className="h-5 w-5 text-iris" />
                    ) : (
                      <MessageSquare className="h-5 w-5 text-emerald-500" />
                    )}
                  </div>
                  <div>
                    <span className="font-bold uppercase tracking-[0.2em] text-[10px] text-zinc-400">
                      {report.type ?? 'Media Content'}
                    </span>
                ...
                <div
                  className={`px-2 py-1 rounded text-[8px] font-bold uppercase tracking-widest ${report.status === 'reported' ? 'bg-iris/10 text-iris border border-iris/20' : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'}`}
                >
                  {report.status}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-[#ffffff05]">
                <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-600">
                  Reporter: {report.reporterEmail?.split('@')[0] ?? 'Anonymous'}
                </span>
                <button
                  onClick={() => {
            ...
            <button
              onClick={() =>
                setModalConfig({
                  ...
                  message:
                    'Permanently remove this content from the platform. This action is irreversible and will be logged.',
            ...
            <button
              onClick={() =>
                setModalConfig({
                  ...
                  message:
                    'Mark this report as reviewed and dismissed. No action will be taken on the content.',
          ...
          <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[9px] font-bold text-zinc-700 uppercase tracking-widest mb-0.5">
                    Reason
                  </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              ...
              <p className="text-sm font-semibold text-white capitalize">
                {selectedReport?.type ?? 'Media'}
              </p>
            </div>
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-600">Status</p>
              <p
                className={`text-sm font-semibold capitalize ${selectedReport?.status === 'reported' ? 'text-iris' : 'text-emerald-500'}`}
              >
