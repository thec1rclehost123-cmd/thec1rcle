...
export default function AdminSettings() {
  ...
  const [serviceStatus, setServiceStatus] = useState<ServiceStatusMap>({ storage: null, security: null, sync: null });
  ...
  const fetchConfig = async () => {
    try {
      ...
      const json = (await res.json()) as { data: PlatformConfig[] };
      if (json.data && json.data.length > 0) {
        const globalConfig = json.data.find((c: PlatformConfig) => c.id === 'global') || json.data[0];
        setConfig((prev) => ({
          ...prev,
          ...globalConfig,
        }));
  ...
  useEffect(() => {
    async function fetchHealth() {
      try {
        ...
        setServiceStatus({
          storage: normalize(json.services?.database),
          security: normalize(json.services?.auth),
          sync: normalize(json.services?.api || json.services?.cache),
        });
      } catch (err) {
  ...
  const handleSave = async (reason: string) => {
    ...
    try {
      ...
      alert(`Authority Error: ${(err as Error).message}`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] space-y-8 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-500 border border-white/5 px-2.5 py-1 rounded-md bg-white/[0.02]">
                      Standard Fee
                    </span>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-end gap-2">
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setConfig({ ...config, platformFee: Number(e.target.value) })
                        }
                    ...
                    <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest italic">
                      Charged on all standard ticket transactions.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] space-y-8 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest text-iris border border-iris/20 px-2.5 py-1 rounded-md bg-iris/5">
                      Security Policy
                    </span>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-end gap-2">
                      ...
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setConfig({ ...config, dualApprovalThreshold: Number(e.target.value) })
                        }
                        className="bg-transparent text-6xl font-light tracking-tight text-white w-full focus:outline-none"
                      />
                    </div>
                    <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest italic">
                      Refunds above this value require secondary admin approval.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-6">
              ...
              <div className="p-8 rounded-xl bg-obsidian-surface border border-[#ffffff08] shadow-sm space-y-6">
                {[
                  {
                    id: 'discoverV2' as const,
                    label: 'Proximity Discovery',
                    desc: 'Enable advanced location-based event recommendations.',
                  },
                  {
                    id: 'payoutsAuto' as const,
                    label: 'Automated Settlements',
                    desc: 'Allow the system to automatically process host payments.',
                  },
                  {
                    id: 'datingSocial' as const,
                    label: 'Social Experiences',
                    desc: 'Enable beta social features for event attendees.',
                  },
                ].map((flag) => (
                  <div
                    key={flag.id}
                    className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition-colors border border-transparent hover:border-white/5 group"
                  >
                    <div className="space-y-1">
                      <p className="text-sm font-semibold text-white group-hover:text-emerald-500 transition-colors uppercase tracking-tight">
                        {flag.label}
                      </p>
                    ...
                    <button
                      onClick={() =>
                        setConfig({
                          ...
                          featureFlags: {
                            ...config.featureFlags,
                            [flag.id]: !config.featureFlags[flag.id],
                          },
                        })
                      }
                    ...
                    >
                      <div
                        className={`absolute top-1 h-5 w-5 rounded-full bg-white transition-all shadow-md ${config.featureFlags[flag.id] ? 'left-6' : 'left-1'}`}
                      />
                    </button>
                  </div>
                ))}
          ...
          <aside className="lg:col-span-4 space-y-8">
              ...
              <div className="space-y-3">
                {[
                  ...
                  return (
                    <div
                      key={i}
                      className="flex items-center gap-4 p-5 rounded-xl bg-obsidian-surface border border-[#ffffff08] shadow-sm"
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p
                          className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${color}`}
                        >
            ...
            <section className="p-8 rounded-xl bg-iris/5 border border-iris/20 space-y-6 shadow-2xl shadow-iris/5">
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-semibold tracking-tight text-white uppercase">
                  Maintenance
                </h3>
                <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest leading-relaxed italic">
                  Turn off public access to the entire platform. Only admins can log in.
                </p>
                <button
