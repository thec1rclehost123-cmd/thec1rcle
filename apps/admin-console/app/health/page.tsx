...
export default function AdminHealth() {
  ...
  const fetchHealthData = async () => {
    try {
      ...
      const [webhookRes, healthRes] = await Promise.all([
        fetch('/api/list?collection=failed_webhooks', {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch('/api/health', { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      const [webhookJson, healthJson] = await Promise.all([
        webhookRes.json() as Promise<{ data: WebhookEntry[] }>,
        healthRes.json() as Promise<{ services?: Record<string, string> }>,
      ]);
      ...
      if (healthJson.services) {
        ...
        setServiceStatus({
          ...
          api:
            s.audit_pipeline === 'Healthy'
              ? 'Optimal'
              : s.audit_pipeline === 'Empty'
                ? 'No Logs'
                : 'Degraded',
        });
      }
  ...
  const handleRetry = async (hookId: string) => {
    try {
      const token = await user.getIdToken();
        ...
        {[
          {
            ...
            icon: Database,
            color: serviceStatus.database === 'Degraded' ? 'text-amber-500' : 'text-emerald-500',
          },
          {
            label: 'Payment Gateway',
            status: serviceStatus.payment,
            icon: Zap,
            color: serviceStatus.payment === 'Degraded' ? 'text-amber-500' : 'text-emerald-500',
          },
          {
            label: 'Account Services',
            status: serviceStatus.auth,
            icon: ShieldCheck,
            color: serviceStatus.auth === 'Degraded' ? 'text-amber-500' : 'text-emerald-500',
          },
          {
            label: 'Platform API',
            status: serviceStatus.api,
            icon: Globe,
            color: serviceStatus.api === 'Degraded' ? 'text-amber-500' : 'text-emerald-500',
          },
        ].map((node) => (
          <div
            key={node.label}
            className="p-6 rounded-xl bg-obsidian-surface border border-[#ffffff08] flex items-center gap-4 group hover:border-[#ffffff15] transition-all shadow-sm"
          >
          ...
          {loading ? (
            <div className="p-8 space-y-4">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 bg-white/5 animate-pulse rounded-lg border border-white/5"
                />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="py-24 text-center rounded-xl bg-white/[0.01]">
              ...
              <p className="text-[10px] text-zinc-600 mt-2 font-bold uppercase tracking-widest">
                No background task errors detected.
              </p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#ffffff05]">
                {filtered.map((hook: WebhookEntry) => (
                  <tr key={hook.id} className="hover:bg-white/[0.02] transition-all group">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-white truncate">
                            Issue: {hook.event_type || hook.type}
                          </p>
                          <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest mt-1">
                            ID: {hook.id.slice(0, 12)}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">
                        {new Date(
                          hook.timestamp?._seconds
                            ? hook.timestamp._seconds * 1000
                            : (hook.timestamp as string) || Date.now(),
                        ).toLocaleString()}
                      </p>
                    </td>
                    <td className="px-8 py-6">
