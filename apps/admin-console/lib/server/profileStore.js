...
export async function getUserTickets(userId) {
  ...
  const partnerAssignmentsSnapshot = await db
    .collection('couple_assignments')
  ...
  const ownerAssignmentsSnapshot = await db
    .collection('couple_assignments')
  ...
  await Promise.all(
    orderDocs.map(async (order) => {
      ...
      const totalShared = bundles.reduce(
        (sum, b) => (b.status !== 'cancelled' ? sum + b.totalSlots : sum),
        0,
      );
  ...
  if (allTicketIdsForOrders.length > 0) {
    ...
    const scanSnapshots = await Promise.all(
      allTicketIdsForOrders.map((id) => db.collection('ticket_scans').doc(id).get()),
    );
  ...
  orderDocs.forEach((order) => {
    ...
    order.tickets.forEach((ticketGroup) => {
      const isCoupleTicket =
        ticketGroup.name.toLowerCase().includes('couple') ||
        ticketGroup.name.toLowerCase().includes('pair');
      ...
      for (let i = 0; i < ticketGroup.quantity; i++) {
        ...
        const ticket = {
          ...
          coupleAssignment: isCoupleTicket
            ? assignment || { status: 'unassigned', ownerId: userId }
            : null,
        };
        ...
        ticket.status =
          order.status === 'cancelled' ? 'cancelled' : isScanned || isEventPast ? 'used' : 'active';
