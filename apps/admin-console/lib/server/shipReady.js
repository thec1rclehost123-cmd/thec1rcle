...
export const ShipGatekeeper = {
  async runPreflight() {
      ...
      if (!metaDoc.exists) {
        ...
        if (logs.size >= 2) {
          ...
          addCheck(
            ...
            latest.previousHash === previous.hash,
    ...
    addCheck(
      ...
      roles.includes("super") && roles.includes("ops"),
      "Base roles defined",
    );
    ...
    addCheck(
      "Environment Anchor",
      process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID === "thec1rcle-india",
      "Production project match",
    );
