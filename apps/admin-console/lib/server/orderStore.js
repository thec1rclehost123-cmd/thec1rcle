...
export async function getOrderById(orderId) {
  ...
  if (!isFirebaseConfigured()) {
    console.log(`[OrderStore] Firebase not configured, checking fallbackOrders for ID: ${orderId}`);
    return fallbackOrders.find((o) => o.id === orderId) || null;
  }

  const db = getAdminDb();
  console.log(`[OrderStore] Fetching order from Firestore: ${orderId}`);
  try {
    const doc = await db.collection(ORDERS_COLLECTION).doc(orderId).get();

    if (!doc.exists) {
      console.warn(`[OrderStore] Order document does not exist in Firestore: ${orderId}`);
      return null;
    }

    console.log(`[OrderStore] Order document found: ${orderId}`);
    return mapOrderDocument(doc);
  } catch (error) {
    console.error(`[OrderStore] Error fetching order ${orderId}:`, error);
    throw error;
  }
}
