...
export const adminStore = {
  ...
  async adminProvision({ email, name, role }, adminId, adminRole, reason) {
    ...
    try {
      ...
      if (authErr.code === 'auth/user-not-found') {
        const err: any = new Error(`No such user found with email: ${email}`);
        err.statusCode = 404;
        throw err;
      }
      throw authErr;
  ...
  // --- 📝 6. Logging & Audit ---
  async logAdminAction(params: Record<string, any>) {
    const {
      adminId,
      adminRole,
      action,
  ...
  async listCollection(collection: string, opts: Record<string, any> = {}) {
    const { status, limit, adminRole, cursor, sortBy } = opts;
    const db = getAdminDb();
  ...
  async getLedgerEntries(opts: Record<string, any> = {}) {
    const { entityId, actorId, state, limit = 100 } = opts;
    const db = getAdminDb();
  ...
  async searchCollection(
    collection: string,
    queryStr: string,
    { limit = 25, adminRole }: { limit?: number; adminRole?: string } = {},
  ) {
    const db = getAdminDb();
    const lc = queryStr.toLowerCase();
    const end = lc.replace(/.$/, (c) => String.fromCharCode(c.charCodeAt(0) + 1));

    const SEARCHABLE: Record<string, string[]> = {
      users: ['displayName_lower', 'email_lower', 'company_lower'],
      venues: ['name_lower'],
      hosts: ['name_lower', 'email_lower', 'company_lower'],
      promoters: ['name_lower', 'email_lower'],
      events: ['name_lower'],
      orders: ['id'],
      onboarding_requests: ['companyName_lower', 'email_lower'],
      host_applications: ['companyName_lower', 'email_lower'],
    };

    const fields = SEARCHABLE[collection];
    if (!fields) return { items: [], total: 0 };
    ...
    for (const field of fields) {
      ...
      for (const doc of snapshot.docs) {
        if (results.length >= limit) break;
        const existing = results.some((r) => r.id === doc.id);
        if (!existing) {
