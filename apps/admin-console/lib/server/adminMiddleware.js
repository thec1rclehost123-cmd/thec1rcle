  ...
  return async (req, ...args) => {
    ...
    try {
      ...
      if (role !== "admin" && isAdminClaim !== true) {
        console.error(
          `[SECURITY] Unauthorized admin access attempt by UID: ${decodedToken.uid}. Role: ${role}, AdminClaim: ${isAdminClaim}`,
        );
        return genericNotFound();
      }
      ...
      if (userRoleValue < requiredRoleValue) {
        console.error(
          `[SECURITY] Privileged escalation attempt. Admin ${decodedToken.uid} (${admin_role}) requested ${requiredRole} access.`,
        );
        return genericNotFound();
      }
      ...
      if (authTime < threshold) {
        console.error(
          `[SECURITY] Admin session stale (30m+). Re-auth required for UID: ${decodedToken.uid}`,
        );
        return genericNotFound();
      }
