...
async function repairEvents() {
  ...
  snapshot.docs.forEach((doc) => {
    ...
    if (!data.keywords || data.keywords.length === 0) {
      const searchString =
        `${data.title} ${data.category} ${data.host} ${data.location} ${updates.cityKey || data.cityKey}`.toLowerCase();
      updates.keywords = Array.from(
        new Set(searchString.split(/[\s,]+/).filter((k) => k.length > 2)),
      );
      needsUpdate = true;
    }
