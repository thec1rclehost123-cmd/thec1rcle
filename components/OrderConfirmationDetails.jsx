...
export default function OrderConfirmationDetails({ order, event }) {
    ...
    const shareOrder = () => {
        if (navigator.share) {
            navigator.share({
                title: `My Entry to ${event.title}`,
                text: `I'm in! Check out ${event.title} on THE C1RCLE.`,
            ...
            <div className="w-full max-w-[800px] flex flex-col items-center gap-10">
                ...
                <div className="text-center space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            Your Access Coordinates have been established.
                        </motion.p>
                    </div>
                </div>
                    ...
                    <div className="relative glass-panel bg-white/[0.03] border border-white/10 backdrop-blur-2xl rounded-[40px] p-8 md:p-10 shadow-3xl overflow-hidden text-center">
                        <div className="space-y-8">
                            <div className="space-y-2">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-orange">Mission Briefing</h3>
                                <h2 className="text-2xl font-black uppercase tracking-tight text-white leading-tight">{event.title}</h2>
                            </div>

                            <div className="flex justify-center gap-12">
                                <div className="space-y-1">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-white/30">Target Date</p>
                                    <p className="text-xs font-black text-white uppercase">{event.date.split(',')[0]}</p>
                                </div>
                                <div className="w-px h-8 bg-white/10" />
                                <div className="space-y-1">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-white/30">Entry Time</p>
                                    <p className="text-xs font-black text-white uppercase">{event.time}</p>
                                </div>
                                <div className="w-px h-8 bg-white/10" />
                                <div className="space-y-1">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-white/30">Sector</p>
                            ...
                            <div className="pt-8 border-t border-white/5 grid grid-cols-2 gap-4">
                                <div className="text-left">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-white/30">Registry ID</p>
                                    <p className="text-[10px] font-mono font-black text-white/40 truncate mt-1">#ORD-{order.id.slice(-8).toUpperCase()}</p>
                                </div>
                                <div className="text-right flex flex-col items-end">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-white/30">Authorization</p>
