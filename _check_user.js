...
async function main() {
  for (const [name, cfg] of Object.entries(configs)) {
    ...
    if (!creds) { console.log(`${name}: could not parse creds`); continue; }
    try {
      const app = initializeApp({ credential: cert(creds) }, name);
      const user = await getAuth(app).getUserByEmail(email);
      console.log(`${name} (${creds.projectId}): USER EXISTS — uid=${user.uid}`);
    } catch (e) {
      console.log(`${name} (${creds.projectId}): ${e.code === "auth/user-not-found" ? "USER NOT FOUND" : "ERROR: " + e.message.substring(0, 80)}`);
    }
  }
}
main();
